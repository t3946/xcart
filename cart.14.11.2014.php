...
#
# $Id: cart.php,v 1.95.2.20 2006/12/06 14:51:51 svowl Exp $
#
...
//func_print_r($cidev_check_shipping, $cart["all_shippings"]);
...
//func_print_r($k_values_of_shippings, $k_values_of_cart_all_shippings, $sid_values_of_shippings, $sid_values_of_cart_all_shippings, $array_diff_k, $array_diff_sid);
	        }
        	else {
                	$cidev_redirect = true;
	        }  
...
# START: random:17710_17631 [2009 Mar 26 09:25] 
		/*
# END: random:17710_17631 [2009 Mar 26 09:25] 
		if (empty($shipping) && $_current_carrier == 'UPS' && $empty_other_carriers != 'Y') {
			$current_carrier = '';
			$intershipper_recalc = 'Y';
...
//func_print_r($tmp_payment_methods);
...
##
###
	$customer_zone = func_get_customer_zone_ship($userinfo, "","R");
	...
	if (!empty($cart["shipping_groups"])){
		...
		foreach ($cart["shipping_groups"] as $mid => $v){
			$cart["shipping_groups"][$mid]["count_shipping_rates_for_canada"] = func_query_first_cell("SELECT COUNT(*) FROM $sql_tbl[shipping_rates] WHERE manufacturerid='$mid' AND type='R' AND zoneid='12'");
		}
	}

//func_print_r($userinfo, $customer_zone, $zones);
