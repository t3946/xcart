...
##
###
	db_query("INSERT INTO $sql_tbl[product_question] (productid, email, phone, question, date) VALUES ('$productid', '$customer_email', '$phone', '$question', '".time()."')");
	$product_question_id = db_insert_id();

$product_question_id = sprintf('%1$05d', $product_question_id);
...
$product_question_subject_line = str_replace("{{prqnid}}", $product_question_id, $product_question_subject_line);
...
$product_question_message_body_to_customer = str_replace("{{prqnid}}", $product_question_id, $product_question_message_body_to_customer);
...
if ($config["product_question_email"]["product_question_send_to_brand"] == "Y"){
...
$from = $product_question_departments_email;
$to = $customer_email;
$subject = "PRQN-".$product_question_id.": ".$product_question_subject_line;
...
###
	$subject = "PRQN-".$product_question_id.": ".$product_info["product"];
	$body = $question;
...
if ($config['product_queries']['product_queries_enable'] == 'Y' && url_exists($chech_domain)) {
	...
	$question = urlencode(strip_tags($question));

	$params="email=".htmlspecialchars($email);
	$params.="&phone=".$phone;
	$params.="&product_name=".urlencode($product_info["product"]);
	$params.="&product_url=".$product_link;
	$params.="&gtin=".$product_info["upc"];
	$params.="&mpn=".$product_info["mpn"];
	$params.="&brand=".urlencode($brand_name);
