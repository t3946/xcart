...
###
                                        ...
                                        $product_info["shipping_freight"] = "0.01";
                                        $product_info["amount"] = 1;
                                        $product_info["weight"] = $weight;
                                        $products[0] = $product_info;
                                        $Add_Shipping_rate = func_calculate_shippings($products, $shipping["methodid"], $userinfo, "master");
                                        if (!empty($Add_Shipping_rate["shipping_cost"]) && $Add_Shipping_rate["shipping_cost"] > 0){
                                                ...
                                                $found_shippings[$i]["rate"] = $Shipping_charge;
                                        }
...
*/					
		...
		db_query("INSERT INTO $sql_tbl[approximation_shipping_rates] (manufacturerid, state, bw_1, bw_75, bw_150, last_updated_date) VALUES ('$manufacturerid', '$state_info[code]', '".$found_shippings[0]["rate"]."', '".$found_shippings[1]["rate"]."', '".$found_shippings[2]["rate"]."', '".time()."') ON DUPLICATE KEY UPDATE bw_1 = '".$found_shippings[0]["rate"]."', bw_75 = '".$found_shippings[1]["rate"]."', bw_150 = '".$found_shippings[2]["rate"]*2."', last_updated_date = '".time()."'");
