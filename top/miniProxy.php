...
if (stripos($contentType, "text/html") !== false) {
  ...
  //$responseBody = preg_replace('/(?<=href=(\\"|\'))[^\\"\']+(?=(\\"|\'))/', PROXY_PREFIX . '$0', $responseBody);

  //echo $responseBody;
  include_once 'simple_html_dom.php';
  $doc = str_get_html($responseBody);

  $proxifyAttributes = array("href", "src");
  foreach($proxifyAttributes as $attrName) {
    foreach($doc->find("a[" . $attrName . "]") as $element) { //For every element with the given attribute...
      $attrContent = $element->$attrName;
      if ($attrName == "href" && preg_match("/^(about|javascript|magnet|mailto):/i", $attrContent)) continue;
      $attrContent = rel2abs($attrContent, $url);
      $attrContent = PROXY_PREFIX . $attrContent;
      $element->$attrName = $attrContent;
    }
  }
