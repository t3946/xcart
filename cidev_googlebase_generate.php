...
#
# $Id: cidev_googlebase_generate.php,v 1.42.2.29 2008/05/08 06:31:19 max Exp $
#
...
if ($mode == "fcreate_ksdfh89346hedfsduf0w384klsfsdf78236"){
...
if ( (!empty($cidev_storefronts) && is_array($cidev_storefronts)) && ($use_next_store_front == "Y" || $mode == "fcreate_ksdfh89346hedfsduf0w384klsfsdf78236") ){
	foreach ($cidev_storefronts as $k => $v){
...
if (
	...
	&& (
		($REQUEST_METHOD == "GET" && ($mode == "fcreate_ksdfh89346hedfsduf0w384klsfsdf78236" || $mode == 'fcontinue_23i4ywjhdgasdui68wd79w8rhwedf6khjsdf') )
	)
) {

	print($sf_info["domain"] . ": GoogleBase product feed generation. <br />");
...
#
	...
	if (($mode == "fcreate_ksdfh89346hedfsduf0w384klsfsdf78236" || $use_next_store_front == "Y") && is_file($froogle_file)) {
		unlink($froogle_file);
		$froogle_export_step = '';
		$number_of_steps = '';
		$froogle_total = '';
...
//				func_header_location('cidev_googlebase_generate.php?mode=ffinish');
...
#
				...
				func_header_location("cidev_googlebase_generate.php?mode=fcontinue_23i4ywjhdgasdui68wd79w8rhwedf6khjsdf&use_next_store_front=Y&cidev_current_store_front=".$cidev_current_store_front);
			}
...
//                                $post = iconv("UTF-8", "ISO-8859-1//TRANSLIT",func_froogle_convert($product['product_froogle'], 80))."\t".
                                $post = iconv("UTF-8", "ISO-8859-1//TRANSLIT",func_froogle_convert($product['product'], 80))."\t".
...
##
###
				// in future it could be place to http://www.artistsupplysource.com/admin/froogle.php
				$cidev_exclude_product_without_img = "Y";
###
##
#

                            if ( ($tmbn_no_img != "Y") || ($tmbn_no_img == "Y" && $cidev_exclude_product_without_img != "Y" )){
			...
			func_header_location('cidev_googlebase_generate.php?mode=fcontinue_23i4ywjhdgasdui68wd79w8rhwedf6khjsdf&cidev_current_store_front='.$cidev_current_store_front);
		}		
	}
	else {
		die("EEEEEEEEEEEEror");
	...
	func_header_location("cidev_googlebase_generate.php?use_next_store_front=Y&mode=fcontinue_23i4ywjhdgasdui68wd79w8rhwedf6khjsdf&cidev_current_store_front=".$cidev_current_store_front);
}
