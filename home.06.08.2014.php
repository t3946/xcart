...
//print($search_query);
	...
	$is_such_search_phrase = func_query_first_cell($qqq="SELECT id FROM $sql_tbl[search_stats] WHERE search_phrase='".addslashes($e_search_data["substring"])."' AND customer_id='".$$XCART_SESSION_NAME."' AND area_selector='$area_selector'");

//func_print_r($qqq, $is_such_search_phrase);
//die();

	if (empty($is_such_search_phrase)){

//func_print_r($_SERVER, $cat, $current_category);
//die();
		...
		db_query("INSERT INTO $sql_tbl[search_stats] (search_phrase, area_selector, customer_id, date_time, source_url, request_delay, hits, storefrontid) VALUES ('".addslashes($e_search_data["substring"])."', '$area_selector', '".$$XCART_SESSION_NAME."', '".time()."', '".$source_url."', '".$result["took"]."', '".$result["hits"]["total"]."', '$current_storefront')");
	}
	#
