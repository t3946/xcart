...
$similar_cron_generated_flag_arr = func_query("SELECT DISTINCT similar_cron_generated_flag FROM $sql_tbl[products]");
...
$count_products = func_query_first_cell("SELECT COUNT(*) FROM $sql_tbl[products] WHERE similar_cron_generated_flag='N'");
$take_per_cycle = 100;
...
for ($i = 0; $i < $count_cycles; $i++) {
    ...
    $products = db_query("SELECT productid, cost_to_us FROM $sql_tbl[products] WHERE similar_cron_generated_flag='N' LIMIT $take_per_cycle");

    while ($product_info = db_fetch_array($products)) {

	$cat = func_query_first_cell("SELECT categoryid FROM $sql_tbl[products_categories] WHERE main='Y' AND productid='$product_info[productid]'");
#
