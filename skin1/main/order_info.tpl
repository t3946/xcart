{* $Id: order_info.tpl,v 1.37.2.2 2006/11/01 08:28:48 max Exp $ *}
...
<tr>
	<td colspan="2" valign="top" class="ProductTitle" align="center"><b>{$v.group_name} items (delivery: {$v.shipping}, {include file="currency.tpl" value=$order.shipping_costs[$k]})</b><br />
</td>
</tr>
<tr>
<td colspan="2" valign="top" class="ProductTitle">&nbsp;</td>
</tr>

{section name=prod_num loop=$v.products}
{if $v.products[prod_num].deleted eq "" and ($active_modules.Product_Configurator eq "" or $v.products[prod_num].extra_data.pconf.parent eq "")}
<tr> 
	<td colspan="2" valign="top" class="ProductTitle">{$v.products[prod_num].product} #{$v.products[prod_num].productid}</td>
</tr>

{if $active_modules.Product_Configurator ne "" and $v.products[prod_num].extra_data.pconf.cartid ne ""}
<tr> 
...
<tr> 
	...
	<td valign="top">{$v.products[prod_num].productcode|default:"-"}</td>
</tr>

{if $usertype ne "C"}
<tr>
	<td valign="top">{$lng.lbl_provider}</td>
	<td valign="top">{$v.products[prod_num].original_provider}</td>
</tr>
{/if}

<tr> 
	<td valign="top">{$lng.lbl_price}</td>
	<td valign="top">{include file="currency.tpl" value=$v.products[prod_num].display_price}</td>
</tr>

{if $customer.tax_exempt ne "Y" and $order.extra.tax_info.display_cart_products_tax_rates eq "Y" and ($v.products[prod_num].product_type ne "C" or $v.products[prod_num].display_price gt 0)}
<tr>
	<td valign="top" style="padding-left: 15px;">{$lng.lbl_including}</td>
	<td style="white-space: nowrap;">
{foreach from=$v.products[prod_num].extra_data.taxes item=tax}
...
<tr> 
	...
	<td valign="top">{$lng.lbl_n_items|substitute:"items":$v.products[prod_num].amount}</td>
</tr>

{if $v.products[prod_num].product_options ne "" && $active_modules.Product_Options ne ''}
<tr> 
	<td valign="top">{$lng.lbl_selected_options}</td>
	<td valign="top">{include file="modules/Product_Options/display_options.tpl" options=$v.products[prod_num].product_options options_txt=$v.products[prod_num].product_options_txt force_product_options_txt=$v.products[prod_num].force_product_options_txt}</td>
</tr>
{/if}

{if $active_modules.Egoods and $v.products[prod_num].download_key and ($order.status eq "P" or $order.status eq "C")}
<tr>
	<td colspan="2">{$lng.lbl_download_link}:<br /><a href="{$catalogs.customer}/download.php?id={$v.products[prod_num].download_key}" target="_blank">{$catalogs.customer}/download.php?id={$v.products[prod_num].download_key}</a></td>
</tr>

<tr>
	<td>{$lng.lbl_download_link_expires}:</td>
	<td>{$v.products[prod_num].expires|date_format:$config.Appearance.datetime_format}</td>
</tr>
{/if}

{if $active_modules.RMA && $current_membership_flag ne 'FS'}
{include file="modules/RMA/item_returns.tpl" product=$v.products[prod_num]}
{/if}
...
{if $active_modules.Product_Configurator ne "" and $v.products[prod_num].extra_data.pconf.cartid ne ""}
{include file="modules/Product_Configurator/pconf_order_info.tpl" cartid=$v.products[prod_num].extra_data.pconf.cartid}
{/if}
{/section}
<tr>
<td valign="top" class="ProductDetails" height="14">&nbsp;</td>
...
<tr> 
	<td valign="top">{$lng.lbl_delivery}s</td>
	<td valign="top">
{foreach from=$order.shipping item=ordershipping}
{$ordershipping|trademark}<br />
{/foreach}
...
{assign var="shipping_cost" value=$order.coupon_discount}
{/if}
...
<tr> 
	...
	<td valign="top"><a href="{$customer.url}">{$customer.url}</a></td>
</tr>
