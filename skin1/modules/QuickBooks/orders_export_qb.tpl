...
SPL{$delimiter}PAYMENT{$delimiter}{$orders[oid].date|date_format:"%m/%d/%Y"}{$delimiter}Accounts Receivable{$delimiter}{$orders[oid].b_lastname}, {$orders[oid].b_firstname} - {$orders[oid].login}{$delimiter}-{$order_total}{$delimiter}{$orders[oid].orderid}
ENDTRNS
{/if}
{/section}
{/if}{* $config.QuickBooks.qb_exportpayments eq "Y" *}
