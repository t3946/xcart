...
function refreshCustomColor()
    ...
    sHTML="<table cellpadding=0 cellspacing=0 width=100%><tr><td colspan=8 style=\"font-family:verdana;font-size:9px;color:black;line-height:9px;padding:1px\">"+this.txtCustomColors+":</td></tr></table>";
...
function loadHTMLFull(sHTML, firstLoad)//first load full HTML
	...
	if (!isHTML_EditorFF || sHTML.replace(/^\s+$/, '').length > 0) {
    var oDoc=oEditor.document.open("text/html","replace");
    oDoc.write(sHTML);
    oDoc.close();
	}
