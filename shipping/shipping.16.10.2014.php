...
//func_print_r($shipping);
				...
				if ($v["shippingid"] == "1" && $userinfo["s_country"] == "US" && $count_products == "1" && $products[0]["amount"] == "1"){
					# $UPS_Ground_found = true;

					$approximation_shipping_rates = func_query_first("SELECT * FROM $sql_tbl[approximation_shipping_rates] WHERE manufacturerid='".$products[0]["manufacturerid"]."' AND state='$userinfo[s_state]'");
					...
					if (!empty($approximation_shipping_rates)){
						...
						$orig_intershipper_rates = $intershipper_rates;
						unset($intershipper_rates);

						$orig_weight = $products[0]["weight"];
						$products[0]["weight"] = $weight;


//func_print_r($Shipping_charge);
...
//						$product_info = func_query_first("SELECT * FROM $sql_tbl[products] WHERE productid='".$products[0]["productid"]."'");
...
//func_print_r($Add_Shipping_rate);
						...
						unset($intershipper_rates);
						$intershipper_rates = $orig_intershipper_rates;

						$products[0]["weight"] = $orig_weight;
//						$products = $orig_products;


//func_print_r($approximation_shipping_rates, $Shipping_charge, $intershipper_rates);
					}
				}
###
##
			...
			foreach ($shipping as $k => $v){
				if ($v["shippingid"] == "1" && !empty($intershipper_rates) && is_array($intershipper_rates)){
					foreach ($intershipper_rates as $v_i_r){
						if ($v_i_r["methodid"] == "1"){
							$shipping[$k]["original_rate"] = $v_i_r["orig_rate"];

							global $XCART_SESSION_NAME, $$XCART_SESSION_NAME, $mode;

							if ($mode == "checkout" || $mode == "shipping"){
...
//       	                                                                db_query("INSERT INTO $sql_tbl[shipping_quote_log] (quote_id, datetime, manufacturerid, session_id, customer_id, ups_ground_quote, approx_ground_quote, product_cost) VALUES ('$max_quote_id', '".time()."', '$ship_manufacturerid', '".$$XCART_SESSION_NAME."', '$login', '".$shipping[$k]["original_rate"]."', '".$shipping[$k]["rate"]."', '".$manufacturerid_cost."')");
       	                                                                db_query("INSERT INTO $sql_tbl[shipping_quote_log] (quote_id, datetime, manufacturerid, session_id, customer_id, ups_ground_quote, approx_ground_quote, product_cost) VALUES ('$max_quote_id', '".time()."', '$ship_manufacturerid', '".$$XCART_SESSION_NAME."', '$login', '".$shipping[$k]["rate_before_approximation"]."', '".$shipping[$k]["rate"]."', '".$manufacturerid_cost."')");
...
//                                                                                db_query("INSERT INTO $sql_tbl[shipping_quote_log] (quote_id, datetime, manufacturerid, session_id, customer_id, ups_ground_quote, approx_ground_quote, product_cost) VALUES ('$current_quote_id', '".time()."', '$ship_manufacturerid', '".$$XCART_SESSION_NAME."', '$login', '".$shipping[$k]["original_rate"]."', '".$shipping[$k]["rate"]."', '".$manufacturerid_cost."')");
                                                                                db_query("INSERT INTO $sql_tbl[shipping_quote_log] (quote_id, datetime, manufacturerid, session_id, customer_id, ups_ground_quote, approx_ground_quote, product_cost) VALUES ('$current_quote_id', '".time()."', '$ship_manufacturerid', '".$$XCART_SESSION_NAME."', '$login', '".$shipping[$k]["rate_before_approximation"]."', '".$shipping[$k]["rate"]."', '".$manufacturerid_cost."')");
