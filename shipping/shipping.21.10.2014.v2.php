...
//func_print_r($shipping, $intershipper_rates);
...
//				if ($v["shippingid"] == "1" && $userinfo["s_country"] == "US" && $count_products == "1" && $products[0]["amount"] == "1" && $v["rate"] > 0)
...
###
							...
							$real_weight = $v_p["weight"];
	
        		                                $Volume = $v_p["dim_x"]*$v_p["dim_y"]*$v_p["dim_z"];
				...
				if ($v["shippingid"] == "1" && !empty($intershipper_rates) && is_array($intershipper_rates)){
					foreach ($intershipper_rates as $v_i_r){
						if ($v_i_r["methodid"] == "1"){
							...
							if (($mode == "checkout" && empty($_GET["paymentid"])) || $mode == "shipping"){
