...
function func_shipper_FEDEX($weight, $userinfo, $debug, $cart) {
	...
	if (empty($config['Shipping']['FEDEX_account_number']) || empty($config['Shipping']['FEDEX_meter_number']))
		return false;
	...
	if (!empty($carrier_codes) && is_array($carrier_codes)) {
		...
		if (true) {
			...
			if ($debug != "Y" && func_is_shipping_result_in_cache($md5_request)  && false) {
			...
			#
				...
				var_dump($host, $xml_query, $result);
...
OUT;
        ...
        $pack['weight'] = $pack['weight']; // TODO check
...
function func_fedex_get_estimated_time($entry, $prefix)
{
    ...
    if (empty($estimated_time)) {
        ...
        if (!empty($estimated_time))
            $estimated_time = strftime('%a %b %e', strtotime($estimated_time));
    }
