...
$similar_cron_generated_flag_arr = func_query("SELECT SQL_NO_CACHE DISTINCT similar_cron_generated_flag FROM $sql_tbl[products] WHERE generate_similar_products='Y'");
...
if ($similar_cron_generated_flag_arr_count == "1" && $similar_cron_generated_flag_arr[0]["similar_cron_generated_flag"] == "Y"){
	db_query("UPDATE $sql_tbl[products] SET similar_cron_generated_flag='N' WHERE generate_similar_products='Y'");
}

$count_products = func_query_first_cell("SELECT SQL_NO_CACHE COUNT(*) FROM $sql_tbl[products] WHERE similar_cron_generated_flag='N' AND generate_similar_products='Y'");
$take_per_cycle = 100;
...
for ($i = 0; $i < $count_cycles; $i++) {
    ...
    $products = db_query("SELECT SQL_NO_CACHE productid, cost_to_us FROM $sql_tbl[products] WHERE similar_cron_generated_flag='N' AND generate_similar_products='Y' LIMIT $take_per_cycle");
