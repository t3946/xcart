...
if (!empty($cat) && 1!=1){
        ...
        $tmp_products1 = func_query("
                SELECT $sql_tbl[products_categories].productid, $sql_tbl[pricing].price FROM $sql_tbl[products_categories]
                LEFT JOIN $sql_tbl[products]
                ON $sql_tbl[products_categories].productid = $sql_tbl[products].productid
                LEFT JOIN $sql_tbl[pricing]
                ON $sql_tbl[products_categories].productid = $sql_tbl[pricing].productid
                WHERE 
                $sql_tbl[products_categories].categoryid = '$cat' AND $sql_tbl[products_categories].main='Y' AND $sql_tbl[pricing].price > '$product_info[taxed_price]' AND $sql_tbl[pricing].quantity = '1' AND $sql_tbl[pricing].membershipid='$membershipid' AND $sql_tbl[pricing].variantid='0' AND $sql_tbl[products].productid != '$product_info[productid]' AND $sql_tbl[products].forsale='Y'
                ORDER BY $sql_tbl[pricing].price ASC LIMIT 3");
        ...
        $tmp_products2 = func_query("
                ...
                WHERE 
                $sql_tbl[products_categories].categoryid = '$cat' AND $sql_tbl[products_categories].main='Y' AND $sql_tbl[pricing].price <= '$product_info[taxed_price]' AND $sql_tbl[pricing].quantity = '1' AND $sql_tbl[pricing].membershipid='$membershipid' AND $sql_tbl[pricing].variantid='0' AND $sql_tbl[products].productid != '$product_info[productid]' AND $sql_tbl[products].forsale='Y'
                ORDER BY $sql_tbl[pricing].price DESC LIMIT 3");

/*
$tmp_products1 = func_query(" 
  	    SELECT xcart_pricing.price,xcart_pricing.productid FROM xcart_pricing
	    WHERE xcart_pricing.productid IN 
	            (
		    SELECT xcart_products_categories.productid FROM xcart_products_categories
        	    LEFT JOIN xcart_products ON xcart_products.productid != '$product_info[productid]' AND xcart_products.productid = xcart_products_categories.productid AND xcart_products.forsale='Y'
	            WHERE 
        	        xcart_products_categories.main='Y' AND
                	xcart_products_categories.categoryid = '$cat'
		    )
            AND
                xcart_pricing.quantity = '1' 
                AND xcart_pricing.variantid='0' 
                AND xcart_pricing.price > '$product_info[taxed_price]'
...
*/
        ...
        if (!empty($tmp_products1) && is_array($tmp_products1)){
                foreach ($tmp_products1 as $k => $v){
                        ...
                        $tmp_products_arr[$k]["price"] = $v["price"];

                        if ($v["price"] > 0)
                                $kf = $product_info["taxed_price"]/$v["price"];
        ...
        if (!empty($tmp_products2) && is_array($tmp_products2)){
                foreach ($tmp_products2 as $k => $v){
                        ...
                        $tmp_products_arr[$count_tmp_products]["price"] = $v["price"];

                        if ($product_info["taxed_price"] > 0)
                                $kf = $v["price"]/$product_info["taxed_price"];
