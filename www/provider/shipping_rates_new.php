...
if ($distributor_section == "19"){
	$type = "R";
} elseif ($distributor_section == "21") {
	$type = "D";
}

if (empty($type)){
	func_heder_location("manufacturers.php?manufacturerid=".$manufacturerid);
}
...
$provider_condition=($single_mode?"":"AND provider='master'");
			...
			foreach ($posted_data as $rateid=>$v) {
				func_array2update("shipping_rates", 
					array(
                        ...
                        "min_shipping_charge" => isset($v['min_shipping_charge']) ? func_convert_number($v['min_shipping_charge']) : 0,
                        "max_shipping_charge" => isset($v['max_shipping_charge']) ? func_convert_number($v['max_shipping_charge']) : $maxvalue,
		...
		if ($shippingid_new) {
			func_array2insert("shipping_rates", 
				array(
					...
					"minweight" => func_convert_number($minweight_new ?? 0),
					"maxweight" => func_convert_number($maxweight_new ?? 0),
					"maxamount" => func_convert_number($maxamount_new ?? 0),
					"mintotal" => func_convert_number($mintotal_new ?? 0),
					"maxtotal" => func_convert_number($maxtotal_new ?? 0),
					"rate" => func_convert_number($rate_new),
					"item_rate" => func_convert_number($item_rate_new),
					"min_shipping_charge" => isset($min_shipping_charge) ? func_convert_number($min_shipping_charge) : 0,
					"max_shipping_charge" => isset($max_shipping_charge) ? func_convert_number($max_shipping_charge) : $maxvalue,
...
# START: random:1073746882_1073747063 [2008 Dec 24 16:25] 
	func_header_location("manufacturers.php?zoneid=$zoneid&shippingid=$shippingid&type=$type&manufacturerid=$manufacturerid&distributor_section=$distributor_section");
# END: random:1073746882_1073747063 [2008 Dec 24 16:25] 
}
...
/*
	...
	$cidev_marckups_less_than_n = func_query("SELECT COUNT(*) as count, manufacturerid FROM $sql_tbl[shipping_rates] WHERE provider='master' $type_condition GROUP BY manufacturerid HAVING count < '$cidev_marckup_nums' ORDER BY count DESC");
...
//func_print_r($cidev_marckups_less_than_n);
	...
	$all_marckups = func_query("SELECT DISTINCT(manufacturerid) FROM $sql_tbl[shipping_rates] WHERE provider='master' $type_condition");
