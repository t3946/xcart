...
#
# $Id: discounts.php,v 1.31 2006/03/15 13:27:32 max Exp $
#
...
if ($REQUEST_METHOD == "POST") {
	...
	if ($mode == "delete") {
		...
		if (is_array($posted_data)) {
			foreach ($posted_data as $discountid => $v) {
				...
				if (
					...
					func_query_first_cell("SELECT COUNT(*) FROM $sql_tbl[discounts] LEFT JOIN $sql_tbl[discount_memberships] ON $sql_tbl[discount_memberships].discountid = $sql_tbl[discounts].discountid WHERE $sql_tbl[discounts].discountid != '$discountid' AND $sql_tbl[discounts].minprice = '$v[minprice]' AND $membership_where $provider_condition $sf_condition") > 0
					) {
				...
				func_array2update("discounts", 
					...
					"discountid='$discountid' $provider_condition $sf_condition"
				);
		...
		if ($discount_new > 0 && ($discount_type_new != 'percent' || $discount_new <= 100)) {
			$ids = func_query_column("SELECT discountid FROM $sql_tbl[discounts] WHERE minprice = '$minprice_new' $provider_condition $sf_condition");
		...
		if (!$is_err) {
			...
			if (!empty($active_modules['Multiple_Storefronts'])) {
				$d_query['storefrontid'] = $current_storefront;
			}
			$_id = func_array2insert("discounts", $d_query);
			if (!empty($discount_membershipids_new) && !in_array(-1, $discount_membershipids_new)) {
...
$discounts = func_query("SELECT * FROM $sql_tbl[discounts] WHERE 1 $provider_condition $sf_condition ORDER BY minprice");
if (!empty($discounts)) {
