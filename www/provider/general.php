...
#
# $Id: general.php,v 1.16 2006/01/11 06:56:25 mclap Exp $
#
...
# Get the orders info
#
$curtime = time(); #+24*3600*3;

$start_dates[] = func_query_first_cell("SELECT last_login FROM $sql_tbl[customers] WHERE login='".addslashes($login)."'");  # Since last login
...
# owner filter
$tax_provider_condition = ($single_mode) ? "" : " AND $sql_tbl[tax_rates].provider='".addslashes($login)."'";
$order_provider_condition = ($single_mode) ? "" : " AND $sql_tbl[order_details].provider='".addslashes($login)."'";
$shiprate_provider_condition = ($single_mode) ? "" : " AND $sql_tbl[shipping_rates].provider='".addslashes($login)."'";
$product_provider_condition = ($single_mode) ? "" : " AND $sql_tbl[products].provider='".addslashes($login)."'";
...
foreach($start_dates as $start_date) {
    ...
    $orders['P'][] = func_query_first_cell("SELECT COUNT(*)"
        . " FROM $sql_tbl[orders], $sql_tbl[order_details]"
        . " WHERE $sql_tbl[orders].cb_status='P' $date_condition");
    $orders['F'][] = func_query_first_cell("SELECT COUNT(*)"
        . " FROM $sql_tbl[orders], $sql_tbl[order_details]"
        . " WHERE $sql_tbl[orders].cb_status IN ('F','D') $date_condition");
    $orders['I'][] = func_query_first_cell("SELECT COUNT(*)"
        . " FROM $sql_tbl[orders], $sql_tbl[order_details]"
        . " WHERE $sql_tbl[orders].cb_status='I' $date_condition");
    $orders['Q'][] = func_query_first_cell("SELECT COUNT(*)"
        . " FROM $sql_tbl[orders], $sql_tbl[order_details]"
        . " WHERE $sql_tbl[orders].cb_status='Q' $date_condition");
