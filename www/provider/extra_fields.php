...
#
# $Id: extra_fields.php,v 1.29.2.2 2006/04/29 10:12:11 max Exp $
#
...
if ($REQUEST_METHOD=="POST") {
	...
	if ($mode == "delete" && is_array($posted_data) && !empty($posted_data)) {
		...
		foreach ($posted_data as $k => $v) {
			...
			} elseif (($res = func_ef_check_service_name($v['service_name'], $login, $k)) !== true) {
		...
		#
		if (($res = func_ef_check_service_name($new['service_name'], $login)) !== true) {
