...
#
# $Id: mod_ARB.php,v 1.14.2.11 2007/01/15 06:09:30 svowl Exp $
#
...
function func_shipper_ARB($weight, $userinfo, $debug, $cart) {
	...
	$ab_ship_key = ab_get_ship_key($ab_url, $ab_id, $ab_password, $ab_ship_accnum, $config["Company"]["location_zipcode"], $ab_testmode, ($userinfo['s_country'] != 'US'));
...
function ab_int_ratings($allowed_shipping_methods, $userinfo, $weight, $debug, $params) {
	global $config, $sql_tbl, $intershipper_error, $intershipper_rates, $mod_AB_ship_flags, $dhl_ext_country_store;

	if ($config['Company']['location_country'] != 'US' || $userinfo['s_country'] == 'US')
		return array();
	...
	$sh_address = $config['Company']['location_address'];
	$sh_city = $config['Company']['location_city'];
	$sh_state = $config['Company']['location_state'];
	$sh_zipcode = $config['Company']['location_zipcode'];
	$sh_country = $config['Company']['location_country'];

	$rc_address = $userinfo['s_address'];
	$rc_address_2 = $userinfo['s_address_2'];
