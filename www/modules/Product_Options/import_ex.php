...
#
# $Id: import_ex.php,v 1.9.2.1 2006/07/20 08:25:26 max Exp $
#
...
# Export data	
	...
	while (($id = func_export_get_row($data)) !== false) {
		...
		$mrow = func_query("SELECT $sql_tbl[class_options].optionid, $sql_tbl[classes].class,"
			. " $sql_tbl[class_options].option_name as 'option', $sql_tbl[product_options_ex].exceptionid"
			. " FROM $sql_tbl[classes], $sql_tbl[class_options], $sql_tbl[product_options_ex], $sql_tbl[products] $sf_join"
			. " WHERE $sql_tbl[classes].classid = $sql_tbl[class_options].classid AND $sql_tbl[class_options].optionid = $sql_tbl[product_options_ex].optionid"
			. " $sf_condition AND $sql_tbl[classes].productid = $sql_tbl[products].productid AND $sql_tbl[classes].productid = '$id'"
			. (empty($provider_sql) ? "" : " AND $sql_tbl[products].provider = '$provider_sql'") . " ORDER BY $sql_tbl[product_options_ex].exceptionid");
		if (empty($mrow))
			continue;
