...
#
# $Id: import_variants.php,v 1.16.2.5 2006/10/06 05:34:28 max Exp $
#
...
#
	...
	#
		...
		while (func_query_first_cell("SELECT COUNT(*) FROM $sql_tbl[variants] WHERE productcode = '$variantcode'") > 0 && !$productcode_long) {
		...
		if ($productcode_long) {
			...
			while (func_query_first_cell("SELECT COUNT(*) FROM $sql_tbl[variants] WHERE productcode = '$variantcode'") > 0) {
...
# Post-import step
	...
	while (list($pid, $tmp) = func_import_read_cache("VR")) {
		...
		func_rebuild_variants($pid, true, 0);
...
# Export data
	...
	while (($id = func_export_get_row($data)) !== false) {
		...
		$mrow = db_query("SELECT $sql_tbl[variants].*, $sql_tbl[variants].variantid as image, $sql_tbl[pricing].price"
			. " FROM $sql_tbl[variants], $sql_tbl[pricing], $sql_tbl[products] $sf_join"
			. " WHERE $sql_tbl[variants].productid = $sql_tbl[products].productid AND $sql_tbl[pricing].variantid = $sql_tbl[variants].variantid"
			. " AND $sql_tbl[pricing].quantity = 1 AND $sql_tbl[pricing].membershipid = 0 $sf_condition AND $sql_tbl[variants].productid = '$id'"
