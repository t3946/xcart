...
#
# $Id: import.php,v 1.3.2.1 2006/08/11 07:12:34 max Exp $
#
...
#
	...
	if (is_null($_sname)) {
		...
		$_fieldid = func_query_first_cell("SELECT fieldid FROM $sql_tbl[extra_fields] WHERE service_name = '$values[service_name]'".$provider_condition);
		if (!empty($_fieldid))
			func_import_save_cache('EN', $values['service_name'], $_fieldid);
	}

	# Check service name
	if (($res = func_ef_check_service_name($_sname, $import_data_provider ? $import_data_provider : $login, $values['fieldid'])) !== true) {
		switch ($res) {
			case "format":
