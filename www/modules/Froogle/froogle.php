...
x_load('backoffice','files','taxes', 'debug', 'froogle', 'crypt');
...
if ($REQUEST_METHOD == "POST" && $mode == "f_update_options"){

	$id = func_query_first_cell("SELECT id FROM $sql_tbl[froogle_options] WHERE storefrontid='$current_storefront'");

	$ClientID = addslashes(text_crypt($ClientID));

	if (empty($id)){
		db_query("INSERT INTO $sql_tbl[froogle_options] (storefrontid, MerchantID, ClientID, BingMerchantID, BingCatalogID, enable_incremental_feed_updates) VALUES ('$current_storefront', '$MerchantID', '$ClientID', '$BingMerchantID', '$BingCatalogID', '$enable_incremental_feed_updates')");		
	} else {
		db_query("UPDATE $sql_tbl[froogle_options] SET MerchantID='$MerchantID', ClientID='$ClientID', BingMerchantID='$BingMerchantID', BingCatalogID='$BingCatalogID', enable_incremental_feed_updates='$enable_incremental_feed_updates' WHERE id='$id'");
	}
...
//func_print_r($config["Company"]["skip_generating_googlebase_feed"]);

if (

	$config["Company"]["skip_generating_googlebase_feed"] != "Y"
	&&
...
//				$number_of_steps = ceil($froogle_total / FROOGLE_EXPORT_LIMIT);
				$number_of_steps = ceil($froogle_total / $take_products_per_cycle);
			}
			
//			$limit_from = ($froogle_export_step - 1) * FROOGLE_EXPORT_LIMIT;
			$limit_from = ($froogle_export_step - 1) * $take_products_per_cycle;
...
//			$products = db_query("SELECT NOW(), $sql_tbl[products].*, $sql_tbl[categories].categoryid_path, $sql_tbl[pricing].price, $sql_tbl[images_T].image_path $fields FROM ($sql_tbl[categories], $sql_tbl[products_categories], $sql_tbl[pricing], $sql_tbl[products]) LEFT JOIN $sql_tbl[images_T] ON $sql_tbl[products].productid = $sql_tbl[images_T].id $joins WHERE $sql_tbl[products].productid = $sql_tbl[products_categories].productid AND $sql_tbl[products_categories].categoryid = $sql_tbl[categories].categoryid AND $sql_tbl[pricing].priceid = $sql_tbl[quick_prices].priceid AND $sql_tbl[products].forsale = 'Y' AND $sql_tbl[categories].avail = 'Y' $where GROUP BY $sql_tbl[products].productid HAVING (price > '0' OR $sql_tbl[products].product_type = 'C') ORDER BY $sql_tbl[products].product $limit");
			...
			while ($product = db_fetch_array($products)) {
				...
				$post_arr = GetGoogleBaseOneRow($product["productid"]);
				$post = $post_arr["row"];

			    }

			    if ( !empty($post) && (  ($cidev_feed_type == "thefind") || ($tmbn_no_img != "Y") || ($tmbn_no_img == "Y" && $cidev_exclude_product_without_img != "Y" ))  ){
...
##
###
$froogle_options = func_query_first("SELECT * FROM $sql_tbl[froogle_options] WHERE storefrontid='$current_storefront'");
$froogle_options["ClientID"] = text_decrypt($froogle_options["ClientID"]);
$smarty->assign("froogle_options", $froogle_options);
###
##
#
