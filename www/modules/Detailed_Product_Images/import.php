...
# Export data
	...
	while ($id = func_export_get_row($data)) {
		...
		$row = func_query("SELECT $sql_tbl[images_D].imageid, $sql_tbl[images_D].alt, $sql_tbl[images_D].orderby"
			. " FROM $sql_tbl[images_D], $sql_tbl[products] $sf_join"
			. " WHERE $sql_tbl[images_D].id = $sql_tbl[products].productid $sf_condition AND $sql_tbl[images_D].id = '$id'"
