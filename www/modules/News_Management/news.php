			...
			if (!$err) {
				...
				if (!empty($list["listid"])) {
					...
					if (!empty($active_modules['Multiple_Storefronts'])) {
						$list_values['storefrontid'] = $current_storefront;
					}	
...
if (!empty($active_modules['Multiple_Storefronts'])) {
	$lists = func_query("SELECT * FROM $sql_tbl[newslists] WHERE lngcode='$shop_language' AND storefrontid = $current_storefront");
} else {
