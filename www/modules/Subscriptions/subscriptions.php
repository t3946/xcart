...
function get_subscription_info($login) {
	...
	return func_query("SELECT sc.*,p.product,o.date,o.cb_status,o.dc_status,s.*"
        . " FROM $sql_tbl[subscription_customers] AS sc, $sql_tbl[products] AS p,"
        . " $sql_tbl[orders] AS o, $sql_tbl[subscriptions] AS s"
        . " WHERE sc.productid=p.productid AND sc.orderid=o.orderid AND sc.productid=s.productid AND sc.login='$login'");
}
...
if ($current_area != "C") {
	...
	if ($action == "search") {
		...
		if ($products) {
			foreach ($products as $product) {
				...
				$customers = func_query("SELECT sc.*, o.cb_status, o.dc_status, o.date FROM $sql_tbl[subscription_customers] AS sc, $sql_tbl[orders] AS o WHERE sc.orderid=o.orderid AND sc.productid='$product[productid]' ".$customerlogin_condition." ORDER BY sc.login");
				if ($customers) {
					foreach ($customers as $k1=>$customer) {
						$customers[$k1]["last_payed_order_status"] = func_query_first_cell("SELECT cb_status FROM $sql_tbl[orders] WHERE orderid='$customer[last_payed_orderid]'");
	...
	for ($i = 0; $i < count($subscriptions); $i++) {
		...
		$subscriptions[$i]["last_payed_orderstatus"] = func_query_first_cell("SELECT cb_status FROM $sql_tbl[orders] WHERE orderid='".$subscriptions[$i]["last_payed_orderid"]."'");
