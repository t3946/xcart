...
function func_send_periodical_email() {
	...
	if ($config['Maintenance_Agent']['periodic_orders'] == 'Y') {
		...
		$orders['I'] = func_query_first_cell("SELECT COUNT(*) FROM $sql_tbl[orders]"
            . " WHERE cb_status='I' $date_condition");
		$orders['Q'] = func_query_first_cell("SELECT COUNT(*) FROM $sql_tbl[orders]"
            . " WHERE cb_status='Q' $date_condition");
