...
if ($option == "Appearance") {
	if ($config['Appearance']['count_products'] != $_POST['count_products'] && !empty($_POST['count_products'])) {
...
if ($REQUEST_METHOD == 'POST' && !empty($_POST)) {
	...
	if ($config["Fancy_Categories"]["fancy_categories_skin"] != $_POST['fancy_categories_skin']) {

		# Change subskin
		$ini = parse_ini_file($smarty->template_dir.DIRECTORY_SEPARATOR.$fcat_module_path.DIRECTORY_SEPARATOR.$_POST['fancy_categories_skin'].DIRECTORY_SEPARATOR."config.ini", true);
        if ($ini['download'] == 'Y' && !empty($_POST['fancy_cache']) && !empty($_POST['fancy_js']) && !empty($_POST['fancy_download'])) {
			$flag = true;
        }
		unset($ini);
		...
		foreach ($cache_options as $oname) {
            ...
            if ($config["Fancy_Categories"][$oname] != (empty($_POST[$oname]) ? "" : "Y")) {
				$ccount++;
		}
		}
