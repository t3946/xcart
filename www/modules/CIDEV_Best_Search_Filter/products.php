...
if (!empty($active_modules['CIDEV_Best_Search_Filter'])) {
	...
	x_session_register('cidev_previous_categoryid');

	if ( !empty($cat) && ($cidev_previous_categoryid != $cat || ($old_search_data['cidev_main_value'] != "catalog" && !empty($old_search_data['cidev_main_value']))) ){

		$cidev_previous_categoryid = $cat;
	...
	if (!empty($old_search_data) && is_array($old_search_data) && $cidev_copy_selected_filter_value && array_key_exists('cidev_selected_filter_values', $old_search_data)){
		...
		$search_data['products']['cidev_main_value'] = $old_search_data["cidev_main_value"];
    		...
    		if ($config['CIDEV_Best_Search_Filter']['cidev_disable_manufacturers'] != 'Y') {
        		$search_data['products']['manufacturers'] = $old_search_data["manufacturers"];
		}
	}
}
?>
