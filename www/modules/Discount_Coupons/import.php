...
#
# $Id: import.php,v 1.5.2.1 2006/11/15 07:45:13 max Exp $
#
...
# Export data	
	...
	while ($id = func_export_get_row($data)) {
		...
		$row = func_query_first("SELECT * FROM $sql_tbl[discount_coupons] WHERE coupon = '".addslashes($id)."'".(empty($provider_sql) ? "" : " AND provider = '$provider_sql'") .  $sf_condition);
		if (empty($row))
			continue;
