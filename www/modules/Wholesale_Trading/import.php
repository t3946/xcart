...
#
# $Id: import.php,v 1.9.2.1 2006/07/26 07:52:51 max Exp $
#
...
# Export data	
	...
	while ($id = func_export_get_row($data)) {
		...
		if (!empty($active_modules['Multiple_Storefronts'])) {
			$sf_join = "LEFT JOIN $sql_tbl[products_sf] ON $sql_tbl[products_sf].productid=$sql_tbl[products].productid";
		...
		if (empty($active_modules['Product_Options'])) {
			$mrow = func_query("SELECT $sql_tbl[pricing].variantid FROM $sql_tbl[pricing], $sql_tbl[products] " . $sf_join
				. " WHERE $sql_tbl[pricing].productid = $sql_tbl[products].productid $sf_condition"
				. " AND ($sql_tbl[pricing].membershipid > 0 OR $sql_tbl[pricing].quantity > 1) AND $sql_tbl[pricing].productid = '$id'"
				. (empty($provider_sql) ? "" : " AND $sql_tbl[products].provider = '$provider_sql'") . " GROUP BY $sql_tbl[pricing].variantid");
		} else {
			$mrow = func_query("SELECT $sql_tbl[pricing].variantid, IFNULL($sql_tbl[variants].productcode, $sql_tbl[products].productcode) as productcode"
				. " FROM $sql_tbl[products] " . $sf_join
				. " LEFT JOIN $sql_tbl[pricing] ON $sql_tbl[products].productid =  $sql_tbl[pricing].productid"
				. " LEFT JOIN $sql_tbl[variants] ON $sql_tbl[pricing].variantid = $sql_tbl[variants].variantid"
				. " WHERE ($sql_tbl[pricing].membershipid > 0 OR $sql_tbl[pricing].quantity > 1) $sf_condition AND $sql_tbl[pricing].productid = '$id'"
				. (empty($provider_sql) ? "" : " AND $sql_tbl[products].provider = '$provider_sql'") . " GROUP BY $sql_tbl[pricing].variantid");
		}
		if (empty($mrow))
			continue;
