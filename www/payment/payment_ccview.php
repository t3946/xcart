...
//		$url = 'cart.php?'.$XCART_SESSION_NAME.'='.$sessid.'&mode=order_message&orderids='.$_orderids;
		$url = 'cart.php?mode=order_message&orderids='.$_orderids.'&'.$XCART_SESSION_NAME.'='.$sessid;

		$change_order_status = "Q";

#
##
###
		$current_paymentid_in_db = func_query_first_cell("SELECT paymentid FROM $sql_tbl[orders] WHERE orderid='$_orderids.'");
		if (in_array($current_paymentid_in_db, array("100", "17")) && $payment_status == "Completed"){
