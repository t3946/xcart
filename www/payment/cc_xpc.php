 ...
 * @version    $Id: cc_xpc.php,v 1.22 2010/07/21 11:47:21 joy Exp $
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && $_POST['action'] == 'return' && !empty($_POST['refId']) && !empty($_POST['txnId'])) {
    ...
    if ($status) {
        ...
        if (
            ...
            || ($response['authorizeInProgress'] > 0 && $bill_output['code'] == 3)
        ) {
        ...
        if ($bill_output['code'] == 1 && $response['isFraudStatus']) {
...
} elseif ($_SERVER['REQUEST_METHOD'] == 'GET' && $_GET['action'] == 'cancel' && !empty($_GET['refId']) && !empty($_GET['txnId'])) {
...
} elseif ($_SERVER['REQUEST_METHOD'] == 'POST' && $_POST['action'] == 'callback' && !empty($_POST['txnId']) && !empty($_POST['updateData'])) {
...
-->
    ...
    if (!$duplicate) {
        func_array2insert('cc_pp3_data', array('ref' => 'XPC' . $refId, 'sessionid' => $XCARTSESSID), true);
    }
