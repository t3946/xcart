...
#
# $Id: import_orders.php,v 1.12.2.6 2006/11/07 06:47:24 max Exp $
#
...
if ($import_step == "define") {
    ...
    $order_statuses = func_query_hash('SELECT code, type FROM ' . $sql_tbl['order_statuses'], 'type', true, true);
    if (is_array($order_statuses)) {
        foreach ($order_statuses as $type => $statuses) {
            $import_specification['ORDERS']['columns'][strtolower($type) . '_status']['variants'] = $statuses;
        }
    }
}
