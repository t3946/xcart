...
//if (!empty($active_modules['CIDEV_Best_Search_Filter'])) {
    include $xcart_dir.'/modules/CIDEV_Best_Search_Filter/include/product_modify.php';
//}
...
if ($productid != "") {
	...
	$oProduct = \Modules\Goods\Models\ProductModel::objects()->get(['productid' => $productid]);
...
if ($REQUEST_METHOD == "POST") {
	...
	if ($mode == "delete_PQ" && !empty($posted_data) && is_array($posted_data)){
			...
			if ($v["to_delete"] == $id){
				db_query("DELETE FROM $sql_tbl[product_question] WHERE id='$id'");
			}
		}
...
//	if (!empty($active_modules['CIDEV_Best_Search_Filter'])){
		$dialog_tools_data["left"][] = array("link" => $pm_link."&section=cidev_filter", "title" => func_get_langvar_by_name("lbl_cidev_best_search_filter"));
//	}
...
#
	...
	if (empty($cost_to_us) || $cost_to_us == "" || $cost_to_us < 0){
		$check_cost_to_us_result = false;
	} else {
		$check_cost_to_us_result = true;
	}
	...
	#
		...
		#
			...
			$time = time();
			db_query("INSERT INTO $sql_tbl[products] (productcode, provider, original_provider, add_date, mod_date, source_sfid, r_avail,avail, manufacturerid) VALUES ('$productcode', '$provider', '$provider','" . $time . "', '" . $time . "', '$source_sfid',0,0,$manufacturerid)");
...
##
###
	if (strpos($discount_table, ":")){


		$check1_count = substr_count($discount_table, ':');
		$check2_count = substr_count($discount_table, ',')+1;
...
//			"update_search_index" => $update_search_index,
			...
			"seo_product_name" => trim($seo_product_name),
			"seo_h2" => trim($seo_h2),
			"lead_time_message" => $lead_time_message,
			"product_price_multiplier" => $product_price_multiplier,
			"generate_similar_products" => $generate_similar_products,
//			"pc_classify_status" => $pc_classify_status,
...
###
							...
							foreach ($pid as $k_pid => $v_pid){

		                                                $current_product_info = func_query_first("SELECT * FROM $sql_tbl[products] WHERE productid='$v_pid'");

        	                                        	if (
                	                        	                $current_product_info["update_search_index"] == "N" &&
                        	        	                        (
                        		                                 (stripslashes($query_data["product"]) != $current_product_info["product"] && isset($query_data["product"])) ||
                	                	                         ($query_data["productcode"] != $current_product_info["productcode"] && isset($query_data["productcode"])) ||
        	                                	                 ($query_data["brandid"] != $current_product_info["brandid"] && isset($query_data["brandid"])) ||
	                                                	         (stripslashes($query_data["fulldescr"]) != $current_product_info["fulldescr"] && isset($query_data["fulldescr"])) ||
                                                        		 ($query_data["upc"] != $current_product_info["upc"] && isset($query_data["upc"]))
	                                                	        )
        	                                	        ){
									db_query("UPDATE $sql_tbl[products] SET update_search_index='Y' WHERE productid='$v_pid'");
                        		                        }

                	        	                        if (isset($query_data["forsale"]) && $query_data["forsale"] == "N" && $current_product_info["update_search_index"] == "N"){
        	                        	                        $query_data["update_search_index"] = 'D';
	                                        	        }
							}
						}
###
...
//        		    $top_message['clean_url_fill_error'] = true;
                } elseif ($check_cost_to_us_result == false) {
...
/*
$canada_zones = func_query("SELECT zoneid FROM xcart_zone_element WHERE field_type='C' AND field LIKE '%CA%'");
$canada_zones_arr = array();
if (!empty($canada_zones)){
	foreach ($canada_zones as $k => $v){
		$canada_zones_arr[] = $v["zoneid"];
	}
}
$canada_zones = implode("','", $canada_zones_arr);
*/

$product_info["count_shipping_rates_for_canada"] = func_query_first_cell($qqq="SELECT COUNT(*) FROM $sql_tbl[shipping_rates] WHERE manufacturerid='$product_info[manufacturerid]' AND (type='R' OR type='D') AND zoneid='12'");
//$product_info["count_shipping_rates_for_canada"] = func_query_first_cell($qqq="SELECT COUNT(*) FROM $sql_tbl[shipping_rates] WHERE manufacturerid='$product_info[manufacturerid]' AND (type='R' OR type='D') AND zoneid IN ('".$canada_zones."')");
//func_print_r($qqq);
###
##
#
...
if ($config['product_queries']['product_queries_enable'] == 'Y'){
 $server_url = "http://www.productqueries.com/api2/?method=GetQAList";
 $additional_query = "";
...
//func_print_r($productqueries_page_arr);
###
##
#
