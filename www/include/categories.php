...
//i	$alt_path_add_parent_ids = func_query_hash('SELECT categoryid, parentid FROM '.$sql_tbl['categories_parents'].' WHERE categoryid IN ('.$alt_path_ids.')', 'categoryid', true, true);
...
//i		$search_condition[] = "($sql_tbl[category_memberships].membershipid IS NULL OR $sql_tbl[category_memberships].membershipid = '$user_account[membershipid]')";
...
//		$sort_condition = " ORDER BY $sql_tbl[categories].order_by, $sql_tbl[categories].category";
		$sort_condition = " ORDER BY $sql_tbl[categories].avail desc, $sql_tbl[categories].order_by asc, $sql_tbl[categories].category asc";
	}
	...
	$join_tbl = '';
	if ($current_area == "C" || $current_area == "B") {
//i		$join_tbl .= " LEFT JOIN $sql_tbl[categories_lng] USE INDEX (PRIMARY) ON $sql_tbl[categories_lng].code='$shop_language' AND $sql_tbl[categories_lng].categoryid=$sql_tbl[categories].categoryid ";
//i		$join_tbl .= " LEFT JOIN $sql_tbl[category_memberships] ON $sql_tbl[category_memberships].categoryid = $sql_tbl[categories].categoryid ";
...
#	$to_search .= ",$sql_tbl[categories].is_bold ";
	...
	if (defined('NEED_PRODUCT_CATEGORIES')) {
		...
		$inner_join = " LEFT JOIN xcart_categories c2 ON c2.categoryid_path like CONCAT(xcart_categories.categoryid_path,'/%') and c2.avail = 'Y' and c2.pc_ready_to_classify = 'Y' and c2.storefrontid = xcart_categories.storefrontid ";
		$_categories = db_query("SELECT $to_search
									FROM $sql_tbl[categories] $join_tbl " . $inner_join.
				(!empty($search_condition) ? 'WHERE ' . implode(' AND ', $search_condition) : '') . " AND $sql_tbl[categories].avail = 'Y' GROUP BY $sql_tbl[categories].categoryid HAVING ready_to_class > 0 " . $sort_condition);
	}
	else {
		$_categories = db_query("SELECT $to_search FROM $sql_tbl[categories] $join_tbl " . (!empty($search_condition) ? 'WHERE ' . implode(' AND ', $search_condition) : '') . " GROUP BY $sql_tbl[categories].categoryid " . $sort_condition);
	}
	...
	if (!empty($category)) {
		...
		if (!empty($previous_catid) && count($previous_catid) > 1) {
			...
			if ($pclen > 1 && is_numeric($previous_catid[$pclen - 1]) && is_numeric($previous_catid[$pclen - 2])) {
				$is_main_parent = func_query_first_cell('SELECT COUNT(categoryid) FROM '.$sql_tbl['categories'].' as c WHERE c.categoryid='.$previous_catid[$pclen - 1] .' AND c.parentid='.$previous_catid[$pclen - 2].' AND c.storefrontid='.$current_storefront);
				$is_add_parent = func_query_first_cell('SELECT COUNT(categoryid) FROM '.$sql_tbl['categories_parents'].' as cp WHERE cp.categoryid='.$previous_catid[$pclen - 1] . ' AND cp.parentid='. $previous_catid[$pclen - 2]);
			}
		...
		if(!empty($_cat_sequense)) {
			$search_condition_2 = "";
			if ($current_area == "C" || $current_area == "B") {
				$search_condition_2 = " AND $sql_tbl[categories].avail = 'Y' AND $sql_tbl[categories].storefrontid=$current_storefront";
			}
...
//i	        $category['clean_urls_history'] = func_query_hash("SELECT id, clean_url FROM $sql_tbl[clean_urls_history] WHERE resource_type = 'C' AND resource_id = '$cat' ORDER BY mtime DESC", "id", false, true);
...
if (!empty($ch_cat)) {
	if (!is_array($ch_cat)) {
		$ch_cat = explode(',', $ch_cat);
	}
 
	foreach ($ch_cat as $v) {
		$current_category[$v] = func_get_category_data($v);
	}
...
#
	if ($current_category = func_get_category_data($cat)) {
		...
		$cat_path = $cat_path[0];
//i    		$orderby = func_query_first_cell("SELECT order_by FROM $sql_tbl[categories] WHERE categoryid = '$cat_path'");
//i		$current_category["main_order_by"] = $orderby;
...
//		if ($current_area != "C"){
		    ...
		    if (!empty($linked_out_category_indexes) && is_array($linked_out_category_indexes)){
			foreach ($linked_out_category_indexes as $k => $v){

	                        $linked_out_category_id = "linked_out_category_id_".$v;
        	                $linked_out_category_id = $current_category[$linked_out_category_id];

				if (!empty($linked_out_category_id)){

					$categoryid_path = func_query_first_cell("SELECT categoryid_path FROM $sql_tbl[categories] WHERE categoryid='$linked_out_category_id'");

					if (!empty($categoryid_path)){
						$categoryid_path_data = func_categoryid_path2category_path($categoryid_path);
						$current_category["linked_out_category_name_$v"] = implode("/", $categoryid_path_data);

						if (!empty($current_category["linked_out_category_keyphrase_id_$v"])){

/*
							$linked_out_category_keyphrase_id_arr = explode(",", $current_category["linked_out_category_keyphrase_id_$v"]);

							if (!empty($linked_out_category_keyphrase_id_arr) && is_array($linked_out_category_keyphrase_id_arr)){
								foreach ($linked_out_category_keyphrase_id_arr as $kk => $vv){
									if (!empty($vv)){
										$linked_out_category_keyphrase_selected_arr[$vv] = func_query_first_cell("SELECT keyphrase FROM $sql_tbl[seo_categories_keyphrases] WHERE id='$vv'");
									}
								}

								$current_category["linked_out_category_keyphrase_selected_arr_$v"] = $linked_out_category_keyphrase_selected_arr;
								unset($linked_out_category_keyphrase_selected_arr);
							}
*/
								 $current_category["linked_out_category_keyphrase_selected_$v"] = func_query_first_cell("SELECT keyphrase FROM $sql_tbl[seo_categories_keyphrases] WHERE id='".$current_category["linked_out_category_keyphrase_id_$v"]."'");
						} else {
							$current_category["linked_out_category_keyphrase_$v"] = func_query("SELECT * FROM $sql_tbl[seo_categories_keyphrases] WHERE categoryid='$linked_out_category_id' AND used!='Y'");
						}
					}
				}
			}
