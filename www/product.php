...
$distributor_model = $oProduct->distributor;

Profiler::getInstance()->addPoint();

$reverse_sku = $distributor_model->reverse_sku;
$remove_dashes = $distributor_model->remove_dashes;

if ($remove_dashes == "Y"){
    $oProduct->productcode = str_replace("-", ".", $oProduct->productcode);
}

if ($reverse_sku == "Y"){

    $cidev_strlen = strlen($oProduct->productcode) - 1;

    $new_sku = "";
    for($i=0;$i<strlen($oProduct->productcode);$i++){
        $new_sku .= substr($oProduct->productcode,$cidev_strlen,1);
        $cidev_strlen--;
    }
    $oProduct->productcode = $new_sku;
}
###
##
#
