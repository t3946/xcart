{* $Id: tax_formula.tpl,v 1.6.2.2 2006/07/11 08:39:27 svowl Exp $ *}
...
<tr> 
	...
	<input type="button" size="8" value="{$lng.lbl_add|strip_tags:false|escape}" onclick="javascript: document.getElementById('value_{$name}').value = (isNaN(parseFloat(document.getElementById('value_{$name}').value)) ? '' : parseFloat(document.getElementById('value_{$name}').value)); if (document.getElementById('value_{$name}').value != '') addElm('{$name}', document.getElementById('value_{$name}').value, 'V');" />
	</td> 
</tr> 
