{* $Id: snapshots.tpl,v 1.6.2.2 2006/07/11 08:39:27 svowl Exp $ *}
{*
...
{if $snapshots[cwi].no_file ne "Y"}
	<option value="{$snapshots[cwi].time}"{if ($search_prefilled.snapshot eq "" and %cwi.last%) or $search_prefilled.snapshot eq $snapshots[cwi].time} selected="selected"{/if}>{$snapshots[cwi].time|date_format:$config.Appearance.datetime_format} - {$snapshots[cwi].descr|default:$lng.lbl_noname_snapshot}</option>
{/if}
{/section}
	</select>
	</td>
...
{section name=cwi loop=$snapshots}
{if $snapshots[cwi].no_file ne "Y"}
	<option value="{$snapshots[cwi].time}"{if ($search_prefilled.snapshot1 eq "" and %cwi.last%) or $search_prefilled.snapshot1 eq $snapshots[cwi].time} selected="selected"{/if}>{$snapshots[cwi].time|date_format:$config.Appearance.datetime_format} - {$snapshots[cwi].descr|default:$lng.lbl_noname_snapshot}</option>
{/if}
{/section}
	</select>
	</td>
...
<tr>
	<td><img src="{$ImagesDir}/spacer.gif" width="25" height="1" /></td>
...
{section name=cwi loop=$snapshots}
{if $snapshots[cwi].no_file ne "Y"}
	<option value="{$snapshots[cwi].time}"{if ($search_prefilled.snapshot2 eq "" and %cwi.last%) or $search_prefilled.snapshot2 eq $snapshots[cwi].time} selected="selected"{/if}>{$snapshots[cwi].time|date_format:$config.Appearance.datetime_format} - {$snapshots[cwi].descr|default:$lng.lbl_noname_snapshot}</option>
{/if}
{/section}
	</select>
	</td>
...
<tr{cycle values=", class='TableSubHead'"}>
	<td>
{if $fileinfo.file}
<a href="file_edit.php?dir={$fileinfo.dir}&amp;file={$fileinfo.file}">{$fileinfo.display_filename}</a>
{else}
