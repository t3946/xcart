{* $Id: html_catalog.tpl,v 1.17.2.6 2007/05/14 12:27:19 max Exp $ *}
<script type="text/javascript" language="JavaScript">
{literal}
var previous_type = "hyphen_4";
function change_catalog_examples() {
	if (!document.htmlcatalogform || !document.htmlcatalogform.namestyle)
		return false;

	var catalog_type = document.htmlcatalogform.namestyle.value;
	if (catalog_type == "old")
		catalog_type = "default";

	var example_div = document.getElementById(catalog_type);
	if (example_div)
		example_div.style.display = "";
	if (previous_type != "") {
		var previous_div = document.getElementById(previous_type);
		if (previous_div)
			previous_div.style.display = "none";
	}
	previous_type = catalog_type;
}
{/literal}	
var txt_drop_old_catalog_warning = "{$lng.txt_drop_old_catalog_warning|escape:javascript}";
</script>
...
<td>
<select name="namestyle" onchange="javascript:change_catalog_examples();">
...
<br />
<b>{$lng.lbl_filemask_example}</b>:
{foreach from=$name_styles item="style" key="name"}
<div id="{$name}" {if $name ne 'hyphen_4'}style="display:none"{/if}>{$style.category}</div>
{/foreach}
</td>
</tr>
