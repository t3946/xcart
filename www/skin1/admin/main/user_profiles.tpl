{* $Id: user_profiles.tpl,v 1.17.2.2 2006/07/11 08:39:27 svowl Exp $ *}
...
{foreach from=$usertypes_array item=to_disable key=utype}
	<td align="center">
	<input type="checkbox"{if $to_disable eq ''} onclick="javascript: document.getElementById('dr_{$item.field}_{$utype}').disabled = !this.checked;" name="default_data[{$item.field}][avail][{$utype}]"{/if}{if $item.avail.$utype eq "Y"} checked="checked"{/if}{if $to_disable} disabled="disabled"{/if} />
	&nbsp;/&nbsp;
	<input type="checkbox"{if $to_disable eq ''} id="dr_{$item.field}_{$utype}" name="default_data[{$item.field}][required][{$utype}]"{/if}{if $item.required.$utype eq "Y"} checked="checked"{/if}{if $to_disable || $item.avail.$utype ne "Y"} disabled="disabled"{/if} />
{if $to_disable}
...
{if ($item.field eq "ssn" && $v.section eq 'P') || ($item.field eq "b_zipcode" && $v.section eq 'B') || ($item.field eq "s_zipcode" && $v.section eq 'S') || ($item.field eq "url" && $v.section eq 'C')}
<tr{cycle values=", class='TableSubHead'"}>
