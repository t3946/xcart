...
if (!empty($department_info) && is_array($department_info) && !empty($mnfs) && is_array($mnfs) && !empty($products) && is_array($products)) {
    ...
    $body = str_replace(['{{instock}}', '{{outofstock}}', '{{outofstock_eta}}', '{{shipfrom}}'], [
        $cidev_instock_items_table,
        $cidev_outofstock_items_table,
        $cidev_outofstock_items_eta_table,
        $shipfrom
    ], $body);

    $subject = str_replace(['{{instock}}', '{{outofstock}}', '{{outofstock_eta}}', '{{shipfrom}}'], [
        $cidev_instock_items_table,
        $cidev_outofstock_items_table,
        $cidev_outofstock_items_eta_table,
        $shipfrom
...
$body = str_replace(['{{userfirstname}}', '{{userfullname}}'], [$userfirstname, $userfullname], $body);
...
if (strtolower($order['po_details']['name_of_purchaser']) === 'unknown' ||
    $order['po_details']['purchase_manager_phone'] === '(000) 000-0000' ||
    $order['po_details']['po_fax'] === '(000) 000-0000' ||
    strtolower($order['po_details']['purchase_manager_email']) === 'unknown@unknown.com') {
...
if (strtolower($order['po_details']['accounts_payable_full_name']) === 'unknown' ||
    $order['po_details']['accounts_payable_phone'] === '(000) 000-0000' ||
    $order['po_details']['accounts_payable_fax'] === '(000) 000-0000' ||
    strtolower($order['po_details']['accounts_payable_email']) === 'unknown@unknown.com') {
    if (!empty($missing_information_replace_text)) {
        $missing_information_replace_text .= '<br />';
    }
...
$body = str_replace(['{{missing_information}}', '{{po_number}}'], [$missing_information_replace_text, $order['po_number']], $body);

$subject = str_replace(['{{signature}}', '{{userfirstname}}', '{{userfullname}}', '{{po_number}}'], [
    $signature,
    $userfirstname,
    $userfullname,
    $order['po_number']
...
$subject = str_replace(['{{shipping_original}}', '{{shipping_on_po}}', '{{shipping_difference}}'], [
    $shipping_original,
    $shipping_on_po,
    $shipping_difference
], $subject);
$body = str_replace(['{{shipping_original}}', '{{shipping_on_po}}', '{{shipping_difference}}'], [
    $shipping_original,
    $shipping_on_po,
    $shipping_difference
], $body);
...
if (($group_model = $order['shipping_groups'][$manufacturerid]['oOrderGroup']) && $invoice = $group_model->invoices->limit(1)->get()) {
    ...
    $_credit_diff = $_credit_diff < 0 ? '(' . $currency . abs($currency->getCurrencyFormat($_credit_diff)) . ')' : $currency . $currency->getCurrencyFormat($_credit_diff);

    $subject = str_replace(['{{first_dx_shipping_quoted}}', '{{first_dx_first_inv_shipping_charged}}', '{{credit_memo_amount_requested}}'], [
        $currency . $currency->getCurrencyFormat($group_model->actual_shipping_net),
        $currency . $currency->getCurrencyFormat($invoice->shipping_charged),
        $_credit_diff
    ], $subject);
    $body = str_replace(['{{first_dx_shipping_quoted}}', '{{first_dx_first_inv_shipping_charged}}', '{{credit_memo_amount_requested}}'], [
        $currency . $currency->getCurrencyFormat($group_model->actual_shipping_net),
        $currency . $currency->getCurrencyFormat($invoice->shipping_charged),
        $_credit_diff
    ], $body);
