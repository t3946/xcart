...
if ($REQUEST_METHOD === 'POST') {
    ...
    if ($mode === 'unfreeze_order') {
        ...
        if (!empty($unfreeze_orderid)) {
            db_query("UPDATE xcart_orders SET unfreeze_cb_status='Y' WHERE orderid={$unfreeze_orderid}");
        ...
        if (!empty($posted_data) && is_array($posted_data)) {
            ...
            foreach ($posted_data as $k => $v) {
                ...
                if ($v['del'] === 'Y') {
                    db_query("DELETE FROM xcart_checks_deposited_orders WHERE id='{$k}'");
                    continue;
                }
                ...
                $current_time = time();
                db_query("UPDATE xcart_checks_deposited_orders SET check_number='{$check_number}', amount='{$amount}', notes='{$notes}' WHERE id='{$k}'");

                $total_deposit_amount += $amount;

            }

            db_query("UPDATE xcart_checks_deposited SET total_deposit_amount='{$total_deposit_amount}' WHERE checks_deposited_id='{$checks_deposited_id}'");
        }
        ...
        if (!empty($checks_deposited_id) && !empty($add_orderids) && is_array($add_orderids)) {
            ...
            foreach ($add_orderids as $key_number => $add_orderid) {
                ...
                $add_orderid = func_query_first_cell("SELECT orderid FROM xcart_orders WHERE orderid={$add_orderid}");
                ...
                $cb_statuses = func_query("SELECT cb_status, manufacturerid FROM xcart_order_groups WHERE orderid={$add_orderid}");

                if (!empty($cb_statuses)) {
                    foreach ($cb_statuses as $ks => $vs) {
                        if (!in_array($vs['cb_status'], $allowed_statuses, true)) {
                ...
                db_query("UPDATE xcart_checks_deposited SET total_deposit_amount=total_deposit_amount+{$add_amount} WHERE checks_deposited_id='{$checks_deposited_id}'");
                $current_time = time();
                db_query("INSERT INTO xcart_checks_deposited_orders (checks_deposited_id, orderid, check_number, amount, notes, date_added) VALUES ('$checks_deposited_id', '$add_orderid', '" . trim($add_check_number) . "', '$add_amount', '" . trim($add_note) . "', '$current_time')");
            ...
            foreach ($checks_deposited_orders as $k => $v) {
                ...
                if (!empty($cb_statuses)) {
                    ...
                    if ($allowed_status_found && $grand_total == $v['amount'] && $currency_in_db == $currency) {
                        ...
                        db_query("UPDATE xcart_order_groups SET cb_status='P' WHERE orderid={$v['orderid']}");

                        db_query("UPDATE xcart_checks_deposited SET currency_locked='Y' WHERE checks_deposited_id={$checks_deposited_id}");
                        ...
                        if (!empty($config['Purchase_Order']['Checks_deposited_Attention_tag'])) {
                            ...
                            $is_such_tag_in_db = func_query_first_cell("SELECT status_id FROM xcart_orders_additional_tags WHERE orderid={$v['orderid']} AND status_id='{$config['Purchase_Order']['Checks_deposited_Attention_tag']}'");
...
if (isset($_GET['checks_deposited_id'])) {
    ...
    if (!empty($checks_deposited_id)) {
        $checks_deposited = func_query_first("SELECT * FROM xcart_checks_deposited WHERE checks_deposited_id='{$checks_deposited_id}'");
        $smarty->assign('checks_deposited', $checks_deposited);

        $checks_deposited_orders = func_query("SELECT * FROM xcart_checks_deposited_orders WHERE checks_deposited_id='{$checks_deposited_id}' ORDER BY orderid");
        if (!empty($checks_deposited_orders)) {

            foreach ($checks_deposited_orders as $k => $v) {
                $order_prefix = func_query_first_cell("SELECT order_prefix FROM xcart_orders WHERE orderid='{$v['orderid']}'");
...
#
    $checks_deposited = func_query('SELECT * FROM xcart_checks_deposited ORDER BY date DESC');
    ...
    $content = Xcart::app()->template->render('order/check/deposited.tpl', $params);
