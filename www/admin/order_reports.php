...
##
###
	$search_condition .= " AND o.cb_status NOT IN ('A','I','D', 'S1', 'S2', 'S3', 'S4') ";
	...
	if (!empty($data['storefront_ids'])){
                        if (!is_array($data['storefront_ids'])) {
                                $search_condition .= " AND o.storefrontid='$data[storefront_ids]'";
                        } else {
                              $search_condition .= " AND o.storefrontid IN ('" . implode('\',\'', $data['storefront_ids']) . "')";
                        }
	}
###
##
...
###
						$group["accounting"][5]["net"] = $group["accounting"][0]["net"] - $group["accounting"][1]["net"] - $group["accounting"][2]["net"] - $group["accounting"][3]["net"] + $group["accounting"][4]["net"];
						$group["accounting"][5]["gross"] = $group["accounting"][0]["gross"] - $group["accounting"][1]["gross"] - $group["accounting"][2]["gross"] - $group["accounting"][3]["gross"] + $group["accounting"][4]["gross"];

						$orders[$k]["shipping_groups"][$mid]["accounting"][5]["net"] = $group["accounting"][5]["net"];
						$orders[$k]["shipping_groups"][$mid]["accounting"][5]["gross"] = $group["accounting"][5]["gross"];
