...
##
###
if (empty($ch_cat) && $mode == "delete" && $confirmed != "Y" && !empty($cat)){
        $ch_cat[0] = $cat;
}
###
##
...
if ($REQUEST_METHOD == "POST") {
	...
	if ($mode == "apply") {
			...
			foreach ($posted_data as $k => $v) {
				...
				if (is_numeric($v['parentid']) && $v['parentid'] != 0) {
					...
					if (!empty($active_modules['Multiple_Storefronts'])) {
						$parent_sf = func_get_category_sf($v['parentid']);
						if ($parent_sf != func_get_category_sf($k) || $parent_sf != $current_storefront) {
							$sf_error = true;
						}
					}
					
					if ($test_parentid > 0 && !$sf_error) {
						$query_data['parentid'] = $v['parentid'];
						$old_path = func_query_first_cell('SELECT categoryid_path FROM '.$sql_tbl['categories'].' WHERE categoryid='.intval($k));
						$query_data['categoryid_path'] = func_query_first_cell("SELECT categoryid_path FROM $sql_tbl[categories] WHERE categoryid='$v[parentid]'")."/".intval($k);
						// Change categoryid path for all subcategories
						$all_subcats = func_query_column("SELECT subcat.categoryid FROM $sql_tbl[categories] USE INDEX (PRIMARY) LEFT JOIN $sql_tbl[categories] as subcat ON subcat.categoryid_path LIKE CONCAT($sql_tbl[categories].categoryid_path, '/%') WHERE $sql_tbl[categories].categoryid = ".intval($k));
						if (is_array($all_subcats)) {
							$all_subcats_str = implode(', ', $all_subcats);
						} 
						if (!empty($all_subcats_str)) {
							$old_path_len = strlen($old_path) + 1;
							db_query('UPDATE '.$sql_tbl['categories'].' SET categoryid_path=CONCAT("'.$query_data['categoryid_path'].'", SUBSTRING(categoryid_path, '.$old_path_len.')) WHERE categoryid IN ('.$all_subcats_str.')');
						}
					} else {
					...
					if ($v['parentid'] == 0) {
						...
						$old_path = func_query_first_cell('SELECT categoryid_path FROM '.$sql_tbl['categories'].' WHERE categoryid='.intval($k));
						$query_data['categoryid_path'] = intval($k);
						// Change categoryid path for all subcategories
						$all_subcats = func_query_column("SELECT subcat.categoryid FROM $sql_tbl[categories] USE INDEX (PRIMARY) LEFT JOIN $sql_tbl[categories] as subcat ON subcat.categoryid_path LIKE CONCAT($sql_tbl[categories].categoryid_path, '/%') WHERE $sql_tbl[categories].categoryid = ".intval($k));
						if (is_array($all_subcats)) {
							$all_subcats_str = implode(', ', $all_subcats);
				...
				if (!empty($v['additional_parentid'])) {
					...
					if ($parids_changed) {
						...
						if (!empty($active_modules['Multiple_Storefronts'])) {
							foreach ($test_parentids as $tp) {
								$parent_sf = func_get_category_sf($tp);
								if ($parent_sf != func_get_category_sf($k) || $parent_sf != $current_storefront) {
									$sf_add_error = true;
									$err_add_parentid[] = $tp;
									break;
						...
						if ($not_tested_col != $tested_col) {
							...
							if (is_array($test_parentids)) {
								foreach ($test_parentids as $id) {
									$add_parameters = '';
									if (isset($additional_parentid[$k][$id]['is_bold'])) {
										$add_parameters .= ', is_bold="' . $additional_parentid[$k][$id]['is_bold'].'"';
									} 
									...
									if (($id == $cat_org) && ($v['parentid'] != $cat_org)) {
										$add_parameters = ', is_bold="'.$v['is_bold'].'", order_by='.intval($v['order_by']);
									}
		...
		if ($sf_add_error || $sf_error) {
			$top_message = array(
				'content'	=> $top_message['content'] . '<br />' . func_get_langvar_by_name('err_adm_category_upd_sf'),
				'type'		=> 'W'
			);
		}
