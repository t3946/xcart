...
if ($REQUEST_METHOD == "POST") {
	...
	if ($mode == "apply") {
		...
		if ($posted_data) {
			...
			foreach ($posted_data as $k => $v) {
				...
				func_array2update("info_pages_categories", $query_data, "categoryid='".intval($k)."'");
			}
		...
		if (!empty($k)) {
			$path = explode("/", func_query_first_cell("SELECT categoryid_path FROM $sql_tbl[info_pages_categories] WHERE categoryid = '$k'"));
			if (!empty($path)) {
				array_shift($path);
				if (!empty($path))
		...
		$top_message["type"] = "I";
		func_header_location("info_pages.php?cat=$cat_org");
	...
	elseif ($mode == "delete") {
		...
		if ($confirmed == "Y") {
			...
			$top_message["type"] = "I";
			func_header_location("info_pages.php?cat=$parent_categoryid");
		}
		else {
			...
			#
			func_header_location("process_info_category.php?cat=$cat&mode=delete");
		}
	}
}
...
if ($mode == "add") {
	...
	# Add new category
	#
	func_header_location("info_category_modify.php?$QUERY_STRING");
}
...
if ($mode == "delete" && $confirmed != "Y") {
	...
	$subcats = func_query("SELECT categoryid, category FROM $sql_tbl[info_pages_categories] WHERE categoryid_path LIKE '".$current_category["categoryid_path"]."/%'");
	if (!is_array($subcats))
		$subcats = array();
