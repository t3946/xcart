...
$pbx_options = func_query("SELECT * FROM $sql_tbl[pbx_options] ORDER BY extension");

if (!empty($pbx_options)){
	foreach ($pbx_options as $kp => $vp){
		$pbx_options[$kp]["linked_xcart_accounts"] = func_query("SELECT login, activity, usertype FROM $sql_tbl[customers] WHERE pbx_extension='$vp[extension]'");
	}
}
