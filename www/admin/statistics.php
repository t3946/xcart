...
//	$date_range["end_date"] = mktime() + $config["Appearance"]["timezone_offset"];
	$date_range["end_date"] = time() + $config["Appearance"]["timezone_offset"];
...
if ($mode == "general") {
	...
	$statistics['products'] = func_query_first_cell("select count(productid) from $sql_tbl[products]" . ((empty($sf_codition)) ? '' : ' WHERE ' . $sf_condition));
	$statistics['categories'] = func_query_first_cell("select count(categoryid) from $sql_tbl[categories] where parentid='0'" . ((empty($sf_codition)) ? '' : ' AND ' . $sf_condition));
	$statistics['subcategories'] = func_query_first_cell("select count(categoryid) from $sql_tbl[categories] where parentid!='0'" . ((empty($sf_codition)) ? '' : ' AND ' . $sf_condition));
	...
	$statistics["products_last_month"] = func_query_first_cell("select count(productid) from $sql_tbl[products] WHERE ($add_date>='$start_date' AND $add_date<='$end_date')" . ((empty($sf_codition)) ? '' : ' AND ' . $sf_condition));
