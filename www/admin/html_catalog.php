...
# $Id: html_catalog.php,v 1.82.2.12 2007/01/11 11:18:42 max Exp $

# This script generates search engine friendly HTML catalog for X-cart

require "./auth.php";
if($secret_key == $config["Appearance"]["html_catalog_key"]){
	@set_time_limit(0);
	$REQUEST_METHOD = "POST";
	$mode = "catalog_gen";
	$start_category = "";
...
$max_name_length = 64;
$php_scripts = array("search.php","giftcert.php","help.php", "cart.php", "product.php","register.php", "home.php", "pconf.php", "giftregs.php", "manufacturers.php","news.php","orders.php", "giftreg_manage.php","returns.php","survey.php","antibot_image.php");
...
$name_styles = array (
	"hyphen" => array (
		"category" => "{category_name}-c-{categoryid}/{sort}-p-{page}.html",
		"product" => "{category_name}-c-{categoryid}/{product_name}-p-{productid}.html",
		"staticpage" => "{page_name}-sp-{pageid}.html",
		"manufacturer" => "{name}-{sort}-p-{page}-mf-{id}.html",
		"name_delim" => "_"
	),
	"hyphen_4" => array (
		"category" => "{category_name}-c-{categoryid}/{sort}-p-{page}.html",
	...
	"new" => array (
		"category" => "{category_name}_c_{categoryid}/{sort}_page_{page}.html",
		"product" => "{category_name}_c_{categoryid}/{product_name}_p_{productid}.html",
		"staticpage" => "{page_name}_sp_{pageid}.html",
		"manufacturer" => "{name}_{sort}_p_{page}_mf_{id}.html",
		"name_delim" => "_"
	),
	# Old scheme (before 3.5.0)
	"default" => array (
		"category" => "category_{categoryid}_{category_name}/{sort}_page_{page}.html",
		"product" => "category_{categoryid}_{category_name}/product_{productid}_{product_name}.html",
		"staticpage" => "page_{pageid}_{page_name}.html",
		"manufacturer" => "manufacturer_{id}_{sort}_p_{page}.html",
		"name_delim" => "_"
	)
);
...
function func_save_catalogs($cat_arr = false) {
	global $sql_tbl;
	
	db_query("REPLACE INTO $sql_tbl[config] VALUES ('html_catalog_dirs','','".addslashes(serialize($cat_arr))."','','','text','','')");
}
...
function func_generate_catalog_arr($cat_arr = array()) {
	...
	foreach ($all_languages as $language) {
		$catalogs[] = array (
						...
						"cat_url" => $http_location.$cat_arr[$language['code']]."/index.html"
						);
	}
...
function my_save_data($filename, $data) {
	global $hc_state, $email_test;

	$filename = func_normalize_path($filename);

if (strpos($data, "home.php?cat=") !== false) {
    echo("ZZZZZZZZZZZ=$filename");
};
	...
	if ($fp === false) {
		...
		$email_test .= func_get_langvar_by_name("lbl_cannot_save_file_N", array("file" => func_relative_path($filename)),false,true)."\n";
		x_session_save();
		exit;
	}
	...
	$email_test .= func_relative_path($filename)."\n"; 
...
function category_filename($cat, $cat_name, $page = 1, $sort_field, $sort_direction){
	...
	if ($cat_name=='index') $cat_name = '';

	$cat_name = normalize_name($cat_name);
	if($sort_field == "orderby" && $sort_direction == 0 && $page==1){
		$cat_name = str_replace(array("{category_name}", "{categoryid}"),array($cat_name, $cat),substr($hc_state["namestyle"]["category"],0,(strpos($hc_state["namestyle"]["category"],"/"))))."/index.html";
	} else {
...
function product_filename($productid, $prod_name=false){
	...
	$cat=func_query_first("SELECT $sql_tbl[categories].category, $sql_tbl[categories].categoryid FROM $sql_tbl[categories], $sql_tbl[products_categories] WHERE $sql_tbl[categories].categoryid=$sql_tbl[products_categories].categoryid AND $sql_tbl[products_categories].productid = '$productid' AND $sql_tbl[products_categories].main='Y'");
	$cat_name = normalize_name($cat["category"]);
	$prod_name = normalize_name($prod_name);
	$prod_name = preg_replace(array("!{category_name}!S", "!{categoryid}!S", "!{product_name}!S", "!{productid}!S"),array($cat_name, $cat["categoryid"], $prod_name, $productid),$hc_state["namestyle"]["product"]);
...
function manufacturer_filename($id, $page, $sort_field, $sort_direction) {
	global $config;
	global $max_name_length, $sql_tbl;
	global $hc_state, $current_lng;
	...
	$name = str_replace(array("{name}", "{page}", "{id}","{sort}"),array($name, $page, $id, $sort_field.$sort_direction), $hc_state["namestyle"]["manufacturer"]);
...
function is_current_shop($href) {
	...
	if (preg_match("/^http:\/\//S", $href)) {
	...
	} elseif (preg_match("/^https:\/\//S", $href)) {
		...
		return (substr($href, 0, strlen($http_parsed['path'])) == $http_parsed['path'] || substr($href, 0, strlen($https_parsed['path'])) == $https_parsed['path']);
	}
...
function manufacturer_callback($found) {
	...
	if (preg_match("/manufacturerid=(\d+)/S", $found[3], $m)) {
		...
		return $found[1].$hc_state["catalog"]["webpath"].manufacturer_filename($id, $page, $sort_field, $sort_direction).$found[4];
	}
...
function process_page($page_src, $page_name, $name_func, $name_params) {
	...
	$page_src = preg_replace_callback('/((?:window|self).location[ ]*=[ ]*["\'])([^"\']*home.php)\?(cat=[^"\'>]+)(["\'])/iS', "category_callback", $page_src);
...
if ($REQUEST_METHOD=="POST" && $mode=="catalog_gen" || $REQUEST_METHOD=="GET" && $mode=="continue") {
	...
	$email_test .= func_get_langvar_by_name("lbl_generating_catalog",false,false,true)."\n\n";
	func_flush();
	...
	if (empty($hc_state) || $REQUEST_METHOD=="POST") {
		...
		$email_test .= func_get_langvar_by_name("lbl_converting_pages_to_html",false,false,true)."\n"; 
			...
			if (preg_match_all("/home.php\?cat=0?&(?:amp;)?sort=&(?:amp;)?sort_direction=&(?:amp;)?page=(\d)/Ss", $page_src, $match)) {
				...
				for ($i = 2; $i < $max_index_page; $i++) {
					list($http_headers, $subpage_src) = func_fetch_page(
						...
						$robot_cookies);
					convert_page($catdir["path"], $subpage_src, 'category', array(0, 'index', $i, $config['Appearance']['products_order'], 0));
				}
			}
		}
...
");
		...
		while ($page_data = db_fetch_array($pages_data)) {
			...
			convert_page('',$page_src,'manufacturer',array($page_data["manufacturerid"], 1, $config['Appearance']['products_order'], 0));
			...
			if (preg_match_all("/manufacturers.php\?manufacturerid=".$page_data["manufacturerid"]."&[^\"']*page=(\d+)[&\"']/S", $page_src, $mpages)) {
				...
				foreach ($sorts as $s) {
					for ($pn = 1; $pn < $max_pages; $pn++) {
						for ($sd = 0; $sd < 2; $sd++) {
							if ($sd ==0 && $s == $config['Appearance']['products_order'] && $pn == 1)
								continue;
							...
							convert_page('', $page_src, 'manufacturer', array($page_data["manufacturerid"], $pn, $s, $sd));
						}
					}
				}
	...
	$email_test .= "\n".func_get_langvar_by_name("lbl_html_catalog_created_successfully",false,false,true)."\n";
	echo func_get_langvar_by_name("lbl_time_elapsed_n_secs", array("sec" => round($time_end-$hc_state["time_start"],2)),false,true);
	$email_test .= func_get_langvar_by_name("lbl_time_elapsed_n_secs", array("sec" => round($time_end-$hc_state["time_start"],2)),false,true);
	x_session_unregister("hc_state");
	if($secret_key == $config["Appearance"]["html_catalog_key"]){
		@mail($config["Appearance"]["html_catalog_email"],'HTML Catalog',$email_test);
		die;
	} else {
