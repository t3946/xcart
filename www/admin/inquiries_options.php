	...
	if ($mode == "update_inquiry_type" && !empty($post_data) && is_array($post_data)){
		...
		foreach ($post_data as $inq_type_id => $v){
			...
			elseif (!empty($v["inquiry_type"])) {
				db_query("UPDATE $sql_tbl[inquiry_types] SET inquiry_type='".$v["inquiry_type"]."', active='$v[active]', type_position='$v[type_position]' WHERE inq_type_id='$inq_type_id'");
			}
		}
	}
        ...
        if ($mode == "update_inquiry_attn_tag" && !empty($post_data) && is_array($post_data)){
                ...
                foreach ($post_data as $inq_tag_id => $v){
                        ...
                        elseif (!empty($v["inquiry_attn_tag"])) {
                                db_query("UPDATE $sql_tbl[inquiries_attention_tags] SET inquiry_attn_tag='".$v["inquiry_attn_tag"]."', active='$v[active]', tag_position='$v[tag_position]' WHERE inq_tag_id='$inq_tag_id'");
                        }
                }
        }
