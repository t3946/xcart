...
{
    $.post('ajax_add_to_cart.php', {}, set_cart_info, 'json');
}



function set_cart_info(data)
{
    if (data.redirect) {
...
function ajax_add_to_cart(id, add_date, source) {
    var waiting_html = added_html = error_html = '';
    var button_html = $('#add2cart_' + id).html();

    $('#add2cart_' + id).addClass('ajax_add2cart_wait_container');
    if (source == "product") {
         waiting_html = '<span class="btn_atcart_big_wait" /></span>';
         added_html = '<span class="btn_atcart_big_added"></span>';
         error_html = '<span class="btn_atcart_big_error"></span>';
    } else {
         waiting_html = '<span class="btn_atcart_small_wait" /></span>';
         added_html = '<span class="btn_atcart_small_added"></span>';
         error_html = '<span class="btn_atcart_small_error"></span>';
    }
    ...
    $.post('ajax_add_to_cart.php', info, function (data) {
