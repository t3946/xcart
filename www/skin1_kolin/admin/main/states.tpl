{* $Id: states.tpl,v 1.29.2.3 2006/07/11 08:39:27 svowl Exp $ *}
<script type="text/javascript">
<!--
var addStateRF = [
	['new_state_code', "{$lng.lbl_code|escape:javascript}"],
	['new_state_name', "{$lng.lbl_state|escape:javascript}"]
];
-->
</script>
...
{include file="main/check_all_row.tpl" style="line-height: 170%; text-align: right;" form="statesform" prefix="selected"}
	</td>
</tr>
...
<tr class="TableHead">
	...
	<td width="10%">BASE STATE ZIP CODE</td>
	<td width="10%">Phone</td>
	<td width="40%">{$lng.lbl_state}</td>
{if $config.General.use_counties eq "Y"}
	<td width="20%" align="center">{$lng.lbl_counties}</td>
{/if}
	<td width="10%" align="center">{$lng.lbl_delete}</td>
</tr>
...
<tr class="TableSubHead">
	<th colspan="{$colspan}">{$states[state].country|default:$lng.lbl_unknown_country}</th>
</tr>
{/if}
...
<tr{cycle values=", class='TableSubHead'"}>
	...
	<td align="center"><input type="text" size="6" name="posted_data[{$states[state].stateid}][base_state_zipcode]" value="{$states[state].base_state_zipcode|escape}" /></td>
	<td align="center"><input type="text" size="16" name="posted_data[{$states[state].stateid}][phone]" value="{$states[state].phone|escape}" /></td>
