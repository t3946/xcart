...
{capture name=dialog}
 ...
 {if $inquiries ne ""}
  ...
  {foreach from=$inquiries item=v key=k}
    ...
    <tr>
	<td>{$v.datetime|date_format:'%d-%b-%Y<br />at %H:%M'}</td>
        <td>{$v.createdby_name}</td>
        <td><a href="{if $v.otrs_ticket_link ne ""}{$v.otrs_ticket_link}{else}#{/if}" target="_blank">{$v.inq_email_subject}</a></td>
