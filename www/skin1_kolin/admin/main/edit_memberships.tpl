{* $Id: edit_memberships.tpl,v 1.11.2.3 2006/07/11 08:39:26 svowl Exp $ *}
{capture name=dialog}
...
<tr class="TableHead">
	...
	{if $type eq 'A' || $type eq 'P'}<td>{$lng.lbl_m_type}</td>{/if}
...
{if $type eq 'A' || ($type eq 'P' && $active_modules.Simple_Mode)}
		<option value="FS"{if $v.flag eq 'FS'} selected="selected"{/if}>{$lng.lbl_subtype_FS}</option>
{/if}
{if $type eq 'P' && !$active_modules.Simple_Mode}
		<option value="RP"{if $v.flag eq 'RP'} selected="selected"{/if}>{$lng.lbl_subtype_RP}</option>
{/if}
{if $type eq 'P' && !$active_modules.Simple_Mode}
        <option value="CM"{if $v.flag eq 'CM'} selected="selected"{/if}>{$lng.lbl_subtype_CM}</option>
{/if}
	</select>
	</td>
	{/if}
...
<tr>
	<td colspan="{if $type eq 'A' || $type eq 'P'}6{else}5{/if}" class="SubmitBox">
	<input type="button" value="{$lng.lbl_delete_selected|strip_tags:false|escape}" onclick='javascript: document.form{$type}.mode.value="delete"; document.form{$type}.submit();' />
...
<tr>
	<td colspan="{if $type eq 'A' || $type eq 'P'}6{else}5{/if}"><br /><br />{include file="main/subheader.tpl" title=$lng.lbl_add_new}</td>
</tr>
...
{/if}
{if $type eq 'P' && !$active_modules.Simple_Mode}
        <option value="CM">{$lng.lbl_subtype_CM}</option>
{/if}
