{* $Id: zones.tpl,v 1.9.2.3 2006/08/11 10:08:44 svowl Exp $ *}
...
checkboxes = new Array({foreach from=$zones item=v key=k}{if $k > 0},{/if}'to_delete[{$v.zoneid}]'{/foreach});
...
{if  $zones[zone].elements}
	{section name=el loop=$zones[zone].elements}
		{strip}
			...
			{/if}
			{if not $smarty.section.el.last},{/if}
		{/strip}
	{/section}
{else}
...
<tr>
	...
	<input type="button" value="{$lng.lbl_delete_selected|strip_tags:false|escape}" onclick="javascript: document.zonesform.submit();" />
	</td>
</tr>
...
{/capture}
{include file="dialog.tpl" title=$lng.lbl_destination_zones content=$smarty.capture.dialog extra='width="100%"'}
