...
{foreach from=$speed_bar item=sb key=k}
<tr>
	<td>
	<input type="hidden" name="posted_data[{$k}][id]" value="{$sb.id}" />
	<input type="text" size="3" maxlength="5" name="posted_data[{$k}][orderby]" value="{$sb.orderby}" />
	</td>
	<td><input type="text" size="25" name="posted_data[{$k}][title]" value="{$sb.title|escape}" /></td>
	<td><input type="text" size="45" name="posted_data[{$k}][link]" value="{$sb.link|escape}" /></td>
	<td align="center"><input type="checkbox" name="posted_data[{$k}][active]" value="Y"{if $sb.active eq "Y"} checked="checked"{/if} /></td>
	<td><input type="button" value="{$lng.lbl_delete|strip_tags:false|escape}" onclick='javascript: document.speedbarform.id.value="{$sb.id}"; submitForm(this, "delete");' /></td>
</tr>
{/foreach}
