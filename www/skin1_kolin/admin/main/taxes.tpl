{* $Id: taxes.tpl,v 1.15.2.4 2006/08/11 07:23:19 svowl Exp $ *}
...
checkboxes = new Array({foreach from=$taxes item=v key=k}{if $k > 0},{/if}'to_delete[{$v.taxid}]'{/foreach});
...
<tr{cycle values=", class='TableSubHead'"}>
	...
	<td class="FormButton"><a href="taxes.php?taxid={$taxes[tax].taxid}">{$taxes[tax].tax_name}</a>
{if $active_modules.Simple_Mode ne "" or $usertype eq "P"}
