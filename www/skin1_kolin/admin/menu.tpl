...
{capture name=menu}
    ...
    {if !($membership_code eq "ADMIN_CUSTOMER_SERVICE") && !$xcartApp->user->hasRoles(['vrs','vrv'])}
    ...
    {if $active_modules.Simple_Mode eq ""}
        {if (!$xcartApp->user->hasRoles(['vrs','vrv']))}
            <a href="{$catalogs.admin}/search.php" class="VertMenuItems">{$lng.lbl_admin_menu_products}{if $active_modules.Multiple_Storefronts} {$lng.lbl_sf}{/if}</a>
        {/if}
    {/if}
    {if (!$xcartApp->user->hasRoles(['vrs','vrv']))}
    ...
    {if !($membership_code eq "ADMIN_CUSTOMER_SERVICE" || $membership_code eq "ADMIN_PRODUCT_MANAGER" || $membership_code eq "ADMIN_CUSTOMER_SERVICE_AND_PRODUCT_MANAGER")}
        {if (!$xcartApp->user->hasRoles(['vrs','vrv']))}
        <a href="{$catalogs.admin}/taxes.php" class="VertMenuItems">Taxes</a>
        {/if}
    {/if}
