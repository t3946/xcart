{* $Id: category_del_confirmation.tpl,v 1.20 2005/11/30 13:29:35 max Exp $ *}
...
<dd><a href="product_modify.php?productid={$subcats[subcat].products[product].productid}" target="_blank">{$subcats[subcat].products[product].productcode}</a> <a href="{$catalogs.customer}/product.php?productid={$subcats[subcat].products[product].productid}" target="_blank">{$subcats[subcat].products[product].product}</a></dd>
{/section}
</dl>
{/if}
</li>
...
<input type="hidden" name="ch_cat" value="{$ch_cat}" />
