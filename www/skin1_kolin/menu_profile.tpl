...
{capture name=menu}
<a href="register.php?mode=update" class="VertMenuItems">{$lng.lbl_modify}</a>
{if $usertype eq "C"}
<a href="register.php?mode=delete" class="VertMenuItems">{$lng.lbl_delete}</a>
{/if}
{if ($usertype eq 'A' || ($usertype eq 'P' && $active_modules.Simple_Mode)) && $is_merchant_password eq 'Y'}
<a class="VertMenuItems" href="{$catalogs.admin}/change_mpassword.php">{$lng.lbl_change_mpassword}</a>
{/if}
{if $usertype eq "C"}
<a href="orders.php" class="VertMenuItems">{$lng.lbl_orders_history}</a>
