...
{if $default_fields.position.avail eq 'Y'}
<tr>
<td align="right">Position</td>
<td>{if $default_fields.position.required eq 'Y'}<font class="Star">*</font>{else}&nbsp;{/if}</td>
<td nowrap="nowrap">
<input type="text" id="position" name="position" size="32" maxlength="32" value="{$userinfo.position}" />
{if $reg_error ne "" and $userinfo.position eq "" && $default_fields.position.required eq 'Y'}<font class="Star">&lt;&lt;</font>{/if}
</td>
</tr>
{/if}
