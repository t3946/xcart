...
<input type="text" name="additional_values[{$v.fieldid}]" id="additional_values_{$v.fieldid}" size="32" value="{if $v.value eq "" && ($new_login_type eq "P" || $new_login_type eq "A") && $main eq "user_add"}{$config.Company.operating_company_name}{else}{$v.value|escape|replace:"&amp;#039;":"'"}{/if}" {if $v.title eq "Company"} placeholder="{$lng.lbl_fill_in_examples_Company_name}" {/if} onkeyup="cidev_check_field_if_empty('additional_values_{$v.fieldid}')" />
