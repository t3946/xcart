{* $Id: popup_users.tpl,v 1.1.2.7 2006/10/25 06:39:34 max Exp $ *}
...
searchform_def[8] = ['StartDay', '{$search_prefilled.start_date|default:$smarty.now|date_format:"%d"}'];
searchform_def[9] = ['StartMonth', '{$search_prefilled.start_date|default:$smarty.now|date_format:"%m"}'];
searchform_def[10] = ['StartYear', '{$search_prefilled.start_date|default:$smarty.now|date_format:"%Y"}'];
searchform_def[11] = ['EndDay', '{$search_prefilled.end_date|default:$smarty.now|date_format:"%d"}'];
searchform_def[12] = ['EndMonth', '{$search_prefilled.end_date|default:$smarty.now|date_format:"%m"}'];
searchform_def[13] = ['EndYear', '{$search_prefilled.end_date|default:$smarty.now|date_format:"%Y"}'];
-->
</script>
...
<input type="hidden" name="force_submit" value="{$force_submit}" />
...
<tr{cycle values=', class="TableSubHead"'}>
	...
	<td><a href="user_modify.php?user={$users[cat_num].login|escape:"url"}&amp;usertype={$users[cat_num].usertype}{$pagestr|amp}" title="{$lng.lbl_modify_profile|escape}" target="_blank">{$users[cat_num].login}</a></td>
	<td><a href="user_modify.php?user={$users[cat_num].login|escape:"url"}&amp;usertype={$users[cat_num].usertype}{$pagestr|amp}" title="{$lng.lbl_modify_profile|escape}" target="_blank"><font class="ItemsList">{$users[cat_num].firstname} {$users[cat_num].lastname}</font></a> / {$users[cat_num].email}</td>
	<td>
