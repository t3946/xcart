...
function cidev_checkFormFields(){
	var productcode = document.getElementById('cidev_productcode').value;

	if (productcode != ''){
		return true;
	}
	else {
		alert("Please enter new product SKU");
		return false;
	}
}
...
//]]>
		...
		<form action="process_product.php" method="post" name="cloneproductform" onsubmit="javascript: return cidev_checkFormFields();">
		...
		<ul class="checkboxes_list">
			...
			<li>
			<label>
			<br />&nbsp;&nbsp;New product {$lng.lbl_sku} (required)<br />
			&nbsp;<input type="text" name="productcode" id="cidev_productcode" value="" size="32" /><br />
			</label>
			</li>
		</ul>
