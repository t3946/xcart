...
<textarea id="{$id}" name="{$name}"{if $cols} cols="{$cols}"{/if}{if $rows} rows="{$rows}"{/if}{if $class} class="{$class}"{/if} style="width: 700px;">{$data|escape:"html"}</textarea>
<div id="{$id}Box" style="width: 700px; display: none;">
<textarea id="{$id}Adv"{if $cols} cols="{$cols}"{/if}{if $rows} rows="{$rows}"{/if}{if $class} class="{$class}"{/if} style="width: 700px;{if $no_links eq 'Y'}display:none;{/if}">{$data|escape:"html"}</textarea>
