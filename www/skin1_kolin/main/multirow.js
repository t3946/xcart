...
function add_inputset_row(name, buttonTD, lastTR) {
	...
	for (var x = 0; x < origTR.cells.length; x++) {
		...
		cloneTDContent = true;
		if (multirowInputSets[name] && multirowInputSets[name].noCloneHTMLId) {
		   if (origTR.cells[x].id == multirowInputSets[name].noCloneHTMLId)
				cloneTDContent = false;
		}

		/* Clone cell */
		var curTD = lastTR.appendChild(origTR.cells[x].cloneNode(cloneTDContent));
...
/* --- */
			if (tName == 'SCRIPT' && elm.textContent.search("datepicker") != -1 && elm.textContent.search("_box_") != -1){
				elm.textContent = elm.textContent.replace(/_box_[0-9]/g, '_box_'+(lastTR.inheritedRowIndex+1)+'');
			}
/* --- */
			...
			if (tName == 'INPUT' || tName == 'SELECT' || tName == 'TEXTAREA') {
				...
				if (elm.name.search("add_orderids") != -1){
					Deposit_add_orderids = "Y";
				}
				...
				if (name == "add_to_order"){
					...
					if (elm.id != ""){
						if (
							elm.id.search("tracking_ship_date_") != -1 ||
							elm.id.search("tracking_shipping_") != -1 ||
							elm.id.search("tracking_carrier_") != -1
						){
	                                	        elm.id = elm.id.replace(/[0-9]+$/, ''+(lastTR.inheritedRowIndex+1)+'');
						}
					}
	...
	if (window.inputset_plus_img && window.inputset_minus_img){
		...
		if (Deposit_add_orderids == "Y"){
			curTD.innerHTML = '<a href="javascript: void(0);" onclick="javascript: add_inputset_subrow(this);"><img src="'+inputset_plus_img+'" alt="'+lbl_add_row+'" /></a><a href="javascript: void(0);" onclick="javascript: remove_inputset(this); func_calc_total_deposit_amount();"><img src="'+inputset_minus_img+'" alt="'+lbl_remove_row+'" /></a>';
		} else {
		...
		if (Deposit_add_orderids == "Y"){
			curTD.innerHTML = '<a href="javascript: void(0);" onclick="javascript: add_inputset_subrow(this);">'+lbl_add_row+'</a>&nbsp;&nbsp;<a href="javascript: void(0);" onclick="javascript: remove_inputset(this); func_calc_total_deposit_amount();">'+lbl_remove_row+'</a>';
		} else {
