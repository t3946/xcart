                ...
                <table width="100%" style="border-collapse: collapse;" class="order_log_table">
                    ...
                    {foreach from=$order_logs item=item key=key name=log}
                        ...
                        <tr class="bordered{if $item.type=='S'} highlight{/if}">
                            ...
                            <td valign="top">
                                {if $item.login === $login}<b>{/if}
                                    {if !($previous_key gte "0" && $order_logs[$previous_key].type eq $item.type && $order_logs[$previous_key].date eq $item.date && $order_logs[$previous_key].login eq $item.login && ($item.type eq "C" || $item.type eq "S"))}{if $item.firstname ne ""}{$item.firstname}
                                        <br/>
                                    {/if}
                                    {if $item.login ne ""}
                                        {if $item.firstname ne ""}({/if}{$item.login}{if $item.firstname ne ""}){/if}{/if}
