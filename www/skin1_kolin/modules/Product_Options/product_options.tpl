{* $Id: product_options.tpl,v 1.51.2.3 2006/10/25 06:39:35 max Exp $ *}
...
var v_alert = "{$lng.txt_variant_alert|replace:'"':'\"'|replace:"\n":""}";
var v_del_alert = "{$lng.txt_delete_variant_alert|replace:'"':'\"'|replace:"\n":""}";
var del_variants = [];
...
{if $geid ne ''}<td width="15" class="TableSubHead" valign="top"><input type="checkbox" value="Y" name="fields[classes][{$v.classid}]" /></td>{/if}
	...
	<td valign="top" class="DataTable">{if $v.is_modifier eq 'Y'}{$lng.lbl_modificator}{elseif $v.is_modifier eq 'T'}{$lng.lbl_text_field}{else}{$lng.lbl_variant}{/if}</td>
...
<table width="100%">
<tr>
	<td align="left"><input type="submit" value="{$lng.lbl_update|strip_tags:false|escape}" />&nbsp;&nbsp;&nbsp;<input type="button" value="{$lng.lbl_delete_selected|strip_tags:false|escape}" onclick="javascript: if(variant_del_alert()) {ldelim} document.productoptionsform.mode.value='product_options_delete'; document.productoptionsform.submit(); {rdelim}" /></td>
	<td align="right"><input type="button" value="{$lng.lbl_add_new_|strip_tags:false|escape}" onclick="javascript: self.location='{$script_name}?submode=product_options_add&amp;productid={$product.productid}&amp;section=options{$redirect_geid}#add_option_section';" /></td>
</tr>
</table>
<br />
{/if}
...
<tr{cycle name="exceptions" values=', class="TableSubHead"'}>
    {if $geid ne ''}<td width="15" class="TableSubHead" rowspan=""><input type="checkbox" value="Y" name="fields[exceptions][{$o.exceptionid}]" /></td>{/if}
...
<tr>
    {if $geid ne ''}<td width="15" class="TableSubHead">&nbsp;</td>{/if}
	<td>&nbsp;</td>
    <td><input type="button" value="{$lng.lbl_delete_selected|strip_tags:false|escape}" onclick="javascript: document.exceptionform.mode.value='product_options_ex_delete'; document.exceptionform.submit();" /></td>
</tr>
{/if}
<tr>
    {if $geid ne ''}<td width="15" class="TableSubHead">&nbsp;</td>{/if}
...
{/capture}
{if $config.Product_Page.show_js_valid_script eq 'Y'}
