...
(function ($) {
  function Scroller(elm, settings) {
    ...
    function read() {
      that.temp =
        (input && that.val !== null && that.val != elm.val()) ||
        that.values === null
          ? s.parseValue(elm.val() ? elm.val() : "", that)
          : that.values.slice(0);
      that.setValue(true);
    }
    function scrollToPos(time, orig, index, manual, dir) {
      // сall validation event
    ...
    that.enable = function () {
      s.disabled = false;
      if (input) elm.prop("disabled", false);
    };
    /**
    ...
    that.disable = function () {
      s.disabled = true;
      if (input) elm.prop("disabled", true);
    };
    /**
    ...
    that.scroll = function (t, val, time, orig, index) {
      ...
      t.attr(
        ...
        (time
          ...
          : "") +
          (has3d
            ? prefix + "-transform:translate3d(0," + px + "px,0);"
            : "top:" + px + "px;")
      );
    ...
    that.setValue = function (sc, fill, time) {
      ...
      if (fill) {
        ...
        that.val = v;
        if (input) elm.val(v).trigger("change");
      }
    };
    /**
    ...
    that.change = function (manual) {
      var v = s.formatResult(that.temp);
      if (s.display == "inline") that.setValue(false, manual);
      else $(".dwv", dw).html(formatHeader(v));
      if (manual) s.onChange.call(e, v, that);
    };
    /**
    ...
    that.show = function () {
      ...
      var html =
        ...
        (s.display == "inline"
          ...
          : '<div class="dwo"></div><div class="dw dwbg"><div class="dwwr">' +
            (s.headerText ? '<div class="dwv"></div>' : ""));
      for (var i = 0; i < s.wheels.length; i++) {
        html +=
          '<div class="dwc' +
          (s.mode != "scroller" ? " dwpm" : " dwsc") +
          (s.showLabel ? "" : " dwhl") +
        ...
        for (var label in s.wheels[i]) {
          html +=
            ...
            '<div class="dwl">' +
            label +
            '</div><div class="dww dwrc" style="height:' +
            s.rows * hi +
            "px;min-width:" +
            s.width +
            'px;"><ul>';
          ...
          html +=
            '</ul><div class="dwwo"></div></div><div class="dwwol"></div></div></td>';
          l++;
        }
      ...
      html +=
        (s.display != "inline"
          ? '<div class="dwbc"><span class="dwbw dwb-s"><a href="#" class="dwb">' +
            s.setText +
            '</a></span><span class="dwbw dwb-c"><a href="#" class="dwb">' +
            s.cancelText +
            "</a></span></div>"
      ...
      if (s.display != "inline") {
        ...
        $("input,select").each(function () {
          if (!$(this).prop("disabled")) $(this).addClass("dwtd");
        });
    ...
    that.init = function (ss) {
      ...
      if (elm.data("dwro") !== undefined) e.readOnly = bool(elm.data("dwro"));
    ...
    methods = {
      ...
      option: function (option, value) {
        return this.each(function () {
          ...
          if (inst) {
            var obj = {};
            if (typeof option === "object") obj = option;
      ...
      destroy: function () {
        return this.each(function () {
          ...
          if (inst) {
            ...
            if ($(this).is("input")) this.readOnly = bool($(this).data("dwro"));
          }
        });
      },
    };
