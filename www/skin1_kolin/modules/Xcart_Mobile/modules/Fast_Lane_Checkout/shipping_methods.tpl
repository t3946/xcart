...
{if $login ne ""}
<br /><br />
{include file="customer/buttons/button.tpl" href="register.php?mode=update&action=cart&amp;paymentid=`$cart.paymentid`" button_title="Modify"}
{/if}
...
<input {* onclick="cidev_save_shippingid(document.cartform, 'shippingids');" *} type="radio" id="shippingid{$s.shippingid}" name="shippingids[{$k}]" value="{$s.shippingid}"{if $s.shippingid eq $cart.shippingids[$k] || ($cart.shippingids[$k] eq "" && $selected_any eq "N")}{assign var="selected_any" value="Y"} checked="checked"{/if}{if $allow_cod} onclick="javascript: display_cod({if $s.is_cod eq 'Y'}true{else}false{/if});"{/if} />
		{*</td>*}
	  {/if}
		{*<td>*}
