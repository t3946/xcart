{*
$Id: cart_totals.tpl 63 2012-10-30 11:56:13Z skot $
vim: set ts=2 sw=2 sts=2 et:
*}
<div class="cart-total-row">
  {if $show_shipping_section eq "Y"} {* show shippings section *}
      {if $config.Shipping.enable_shipping eq "Y"}
        <div data-role="collapsible" data-theme="c" data-content-theme="b">
          <h3>{$lng.lbl_shipping_details}</h3>
          <div>
            {if $link_shipping eq "Y" and $cart.shippingid}
              <div class="shipping-method">
                ...
                {include file="customer/buttons/button.tpl" style="link" button_title=$lng.lbl_modify href="cart.php?mode=checkout&section=shipping_method" data_theme="b"}
              </div>
            {else}
...
{if $cart.applied_giftcerts}
  <br />
  <br />
  <h3>{$lng.lbl_applied_giftcerts}</h3>
...
{/if}
{/if} {* show shippings section *}
</div>
{if $active_modules.Special_Offers and $cart.bonuses ne "" and $show_shipping_section ne "Y"}
