{* $Id: reasons.tpl,v 1.4.2.2 2006/07/11 08:39:33 svowl Exp $ *}
{capture name=dialog}
<form action="returns.php" method="post">
<input type="hidden" name="mode" value="reasons" />
<table>
{foreach from=$reasons item=v key=k}
<tr>
	<td><input type="text" name="posted_data[{$k}]" value="{$v}" size="32" /></td>
	<td><a href="returns.php?mode=reasons_delete&amp;idx={$k}">{$lng.lbl_delete}</a></td>
</tr>
{/foreach}
<tr>
    <td>&nbsp;</td>
...
<tr> 
	<td class="TopLabel" colspan="2">{include file="main/subheader.tpl" title=$lng.lbl_new_reason}</td>
</tr>
<tr>
