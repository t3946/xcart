...
<tr>
	<td>
{if $c.parentid ne 0 or ($c.order_by ge $cat_start && $c.order_by le $cat_end)}
...
{/if}
	<td width="100%" align="left"><a href="home.php?cat={$catid}" class="CatMenu{if $c.main_order_by gt 500}Info{/if}Item">{$c.category}</a></td>
...
{/if}
{if $config.Fancy_Categories.icons_icons_in_categories >= $level+1}
    <td class="FCIconCell"><a href="home.php?cat={$catid}&amp;catexp={$catid}" class="CatMenu{if $c.main_order_by gt 500}Info{/if}Item"><img src="{if $c.is_icon ne 'Y'}{$ImagesDir}/spacer.gif{elseif $c.icon_url ne ''}{$c.icon_url}{else}{$xcart_web_dir}/image.php?type=C&amp;id={$catid}{/if}" class="FCIcon" alt="" /></a></td>
{/if}
    <td width="100%" align="left"><a href="home.php?cat={$catid}&amp;catexp={$catid}" id="nojs" class="CatMenu{if $c.main_order_by gt 500}Info{/if}Item" style="display: block; width: 100%;">{$c.category}</a></td>
{if $config.Fancy_Categories.icons_disable_subcat_triangle ne 'Y'}
    <td class="FCTriangleCell"><a href="home.php?cat={$cat}&amp;catexp={$catid}" class="CatMenu{if $c.order_by gt 500 || $orderby gt 500}Info{/if}Item"><img src="{if $c.subcategory_count eq 0}{$ImagesDir}/spacer.gif{elseif $c.expanded}{$fc_skin_web_path}/tree_subdir_minus.gif{else}{$fc_skin_web_path}/tree_subdir_plus.gif{/if}" class="FCTriangle" alt="" /></a></td>
{/if}
</tr>
</table>
</div>
