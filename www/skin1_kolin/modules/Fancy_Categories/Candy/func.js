// $Id: func.js,v 1.7.2.4 2006/11/28 09:35:37 max Exp $

var OnLoadDelay = 100;
var OnLoadTimes = 300;

var categories = new Array();
var rootcats = new Array();
var timeOuts = new Array();
var currentCat = false;
var span_content = new Array();
...
function fcShowMenu(obj, cat) {
  if (window.fancy_body_is_load === false) return;
  ...
  if (obj.submenu.object.innerHTML.length == 0) obj.submenu.loading();
  ...
  if (fcConfig.preload && fcConfig.download) fcPreLoad(obj.submenu);
}
...
function fcHideMenu(cat) {
  ...
  if (!obj || !obj.submenu) return false;
  ...
  while (
    obj.parentCat &&
    !obj.selected &&
    !obj.parentCat.parentLayer.selected &&
    obj.parentCat.parentLayer.parentCat
  ) {
    obj = obj.parentCat.parentLayer;
  }

  // Hide if parent menu item isn't selected
  if (obj.parentCat && !obj.parentCat.selected) obj.hide();
...
function fcCancelHideMenu(obj) {
  if (obj && obj.tm) clearTimeout(obj.tm);
}
...
function fcPreLoad(obj) {
  if (!fcConfig.preload || obj.preloaded) return false;
...
function fcHTTPLoad(obj) {
  ...
  if (isOpera || isIE || isSafari) {
    setTimeout(
      new Function(
        ...
        "var s = document.getElementById('" +
          span.id +
          "').getElementsByTagName('script')[0]; s.language = 'JavaScript'; if (s.setAttribute) s.setAttribute('src', '" +
          href +
          "'); else s.src = '" +
          href +
          "';"
    ...
    setTimeout(
      new Function(
        "",
        "fcHTTPCheckLoaded(" + obj.parentCat.cat + ', "' + span.id + '");'
      ),
      100
    );
    ...
    setTimeout(
      new Function(
        ...
        "var s = document.getElementById('" +
          span.id +
          "').getElementsByTagName('script')[0]; s.language = 'JavaScript'; s.onload = new Function('', 'fcHTTPLoaded(" +
          obj.parentCat.cat +
          ',"' +
          span.id +
          "\");'); if (s.setAttribute) s.setAttribute('src', '" +
          href +
          "'); else s.src = '" +
          href +
          "';"
...
function fcHTTPLoaded(cat, span) {
  ...
  if (span_content[cat]) {
    if (obj.submenu.loadingTO) clearTimeout(obj.submenu.loadingTO);
...
function fcHTTPCheckLoaded(cat, id) {
  if (span_content[cat]) {
    if (timeOuts[cat]) timeOuts[cat] = null;
    return fcHTTPLoaded(cat, id);
  }

  // Start first delayed onload event check
  if (!timeOuts[cat]) {
    timeOuts[cat] = new Array(
      setTimeout(
        new Function("", "fcHTTPCheckLoaded(" + cat + ', "' + id + '");'),
        OnLoadDelay
      ),
      1
    );
...
function fcSelect(obj) {
  if (!obj.bLeft)
    obj.bLeft = document.getElementById(catPrefix + obj.cat + "l");
  if (!obj.bCenter)
    obj.bCenter = document.getElementById(catPrefix + obj.cat + "m");
  if (!obj.bRight)
    obj.bRight = document.getElementById(catPrefix + obj.cat + "r");

  if (obj.bLeft) obj.bLeft.src = bLeftOn.src;
  if (obj.bCenter) setBGImage(obj.bCenter, bCenterOn.src);
  if (obj.bRight) obj.bRight.src = bRightOn.src;

  obj.selected = true;
  if (obj.parentLayer) obj.parentLayer.selected = true;
}
...
function fcUnSelect(obj) {
  ...
  if (obj.bLeft) obj.bLeft.src = bLeftOff.src;
  if (obj.bCenter) setBGImage(obj.bCenter, bCenterOff.src);
  if (obj.bRight) obj.bRight.src = bRightOff.src;
...
function fcCreateLayer(obj) {
  ...
  // Define layer events
  d.onmouseover = new Function(
    "",
    "this.layer.selected = true; fcCancelHideMenu(this.layer); fcCancelHideMenu(this.layer.parentCat.parentLayer);"
  );
  d.onmouseout = new Function(
    "",
    "this.layer.selected = false; fcInitHideMenu(this.layer);"
  );

  // сall load procedure for this sublayer
  obj.submenu.load();
...
function fcInitItem(obj, cat) {
  if (obj.cat) return false;

  // Define events for this menu item
  obj.onmouseout = new Function(
    "",
    "fcUnSelect(this); fcInitHideMenu(this.submenu);"
  );
...
function fcInitSubmenu(obj) {
  if (!categories[obj.parentCat.cat]) return false;
  ...
  for (var c in categories[obj.parentCat.cat]) {
    ...
    if (subobj) {
      ...
      subobj.child = obj.parentCat.subcats[c] =
        categories[obj.parentCat.cat][c];
...
function fcLoadingAnim(cat) {
  if (cat == undefined && this) cat = this.parentCat.cat;

  if (!fcConfig.download) return;
  ...
  var prefix = "";
  if (!obj.ticks || obj.ticks >= 5) obj.ticks = 0;

  obj.ticks++;

  for (var x = 0; x < obj.ticks; x++) prefix += ".";
