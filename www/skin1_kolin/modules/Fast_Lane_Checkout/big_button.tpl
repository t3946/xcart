...
{if $js_link eq $href}
 {if $js_onclick_to_href ne ""}
{assign var="js_link" value="javascript: $js_onclick_to_href self.location='`$href`';"}
{assign var="onclick" value="javascript: $js_onclick_to_href self.location='`$href`';"}
 {else}
{assign var="js_link" value="javascript: self.location='`$href`'"}
 {/if}
...
{if $button_title eq "Checkout"}
{include file="buttons/button.tpl" btn_to_checkout="Y" js_link=$js_link button_type="checkout" js_onclick_to_href=$js_onclick_to_href}
{else}
{* {include file="buttons/button.tpl" btn_other="Y" js_link=$js_link} *}
{include file="buttons/button.tpl" js_link=$js_link}
{/if}
...
<td class="{$bg_title_class}"{$reading_direction_tag}><font class="ProductTitle{if $bold eq "N"} ProductTitleRelated{/if}">&nbsp;{$button_title}&nbsp;</font>{if $arrow eq "Y"}<img src="{$ImagesDir}/rarrow_flc.gif" class="BBCorner" alt="" />{/if}</td>
