{* $Id: news_messages_list.tpl,v 1.11.2.2 2006/07/11 08:39:32 svowl Exp $ *}
...
checkboxes = new Array({foreach from=$messages item=v key=k}{if $k > 0},{/if}'to_delete[{$v.newsid}]'{/foreach});
...
<tr>
	...
	<td align="center">{$messages[idx].send_date|date_format:$config.Appearance.datetime_format}</td>
	<td align="center">
...
<tr>
	<td colspan="5"><br />
{if $messages ne ""}
<input type="button" value="{$lng.lbl_delete_selected|strip_tags:false|escape}" onclick="javascript: if (confirm('{$lng.txt_delete_messages|strip_tags}')) {ldelim} document.messagesform.action.value='delete'; document.messagesform.submit(); {rdelim}" />
<br /><br />
{/if}
