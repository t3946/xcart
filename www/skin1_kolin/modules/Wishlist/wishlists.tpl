{* $Id: wishlists.tpl,v 1.7.2.2 2006/07/11 08:39:36 svowl Exp $ *}
...
<tr>
	...
	<td height="10" width="100%"><input type="text" name="search_data[login]" value="{$search_data.login}" /></td>
</tr>
<tr>
...
<tr>
	...
	<td width="10" height="10">&nbsp;</td>
	<td height="10" width="100%"><input type="text" name="search_data[productid]" value="{$search_data.productid}" /></td>
</tr>
<tr>
...
<tr{cycle name=c1 values=', class="TableSubHead"'}>
	<td>{if $usertype eq "A" or ($usertype eq "P" and $active_modules.Simple_Mode ne "")}<a href="user_modify.php?user={$v.login|escape:"url"}&amp;usertype={$v.usertype}">{$v.firstname}&nbsp;{$v.lastname}&nbsp;({$v.login})</a>{else}{$v.firstname}&nbsp;{$v.lastname}&nbsp;({$v.login}){/if}</td>
	<td><a href="wishlists.php?mode=wishlist&amp;customer={$v.login}">{$lng.lbl_n_items_in_wishlist|substitute:"items":$v.products_count}</a></td>
</tr>
{/foreach}
</table>
