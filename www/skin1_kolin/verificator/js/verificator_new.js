...
function getFirstAvailStep() {
    return $('.amazon-search button').first().attr('data-step-id')
}
function initButtons() {
    $('.amazon-search button').removeClass('active');
}
...
function loadOriginalProduct(frame_Name) {
    var sOriginalProduct = $('#original_product').attr('data-url');
...
function loadRemoteFrame(frame_Name, url, idx_force, appendto) {
    ...
    document.title = $('.amazon-search button[data-step-id=' + idx + ']').attr('data-title');

    return false;
}

function submitConclusion(verify_status_id, batch_id, product_id, note_text, asin, conclusion_buttons) {
    $.post('/admin/amazon/verification/submit', {
...
$(document).ready(function () {
    ...
    $('.amazon-search button').not('#original_product').click(function () {
        if (!$(this).hasClass('active')) {
            ...
            loadRemoteFrame('ifrm', url, currstep);
            var check = $('#split-screen-checkbox').prop("checked");
            if (!check) {
                $('.iframediv.left').css('width', '100%');
                $('.iframediv.right').css('width', '0');
    ...
    var trans = $('.amazon-search button');
    if (trans) {
        if (trans.length == 0) {
            $('#no_products').show();
        } else {
            $('.amazon-search button[data-step-id=' + Math.max(getLocationId(), getFirstAvailStep()) + ']').click();
            if (isShowOriginal() == 1) {
                clickOriginalProduct($('#original_product'));
            }
            else if (isSplitScreen() == 1) {
    ...
    $('#conclusion_buttons').find('input[type=radio]').change(function () {
        ...
        var step = $(this).closest('div.step').addClass('completed').removeClass('not');
        if ($(this).val()=='contradict' || $(this).val()=='different') {
            step.addClass('not');
        }
