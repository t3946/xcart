...
function popupOpen(src, title) {
  if (!_popup) _popup = new popupLayer();

  if (!_popup.valid) return false;
...
function popupLayer(options) {
  ...
  if (typeof options != "undefined" && options.constructor == Object) {
    for (var i in options) {
      if (
        hasOwnProperty(options, i) &&
        typeof this[i] != "undefined" &&
        this[i].contructor == options[i].constructor &&
        (this[i].contructor == Number || this[i].contructor == Boolean)
      )
        this[i] = options[i];
    }
  }
  ...
  if (!this.isModal) this._bg.style.display = "none";
  ...
  if (this._msie6) {
    ...
    this._msie6FramePaddingH =
      parseInt(this._frame$.css("paddingTop")) +
      parseInt(this._frame$.css("paddingBottom")) +
      parseInt(this._frame$.css("marginTop")) +
      parseInt(this._frame$.css("marginBottom")) +
      parseInt(this._base$.css("paddingTop")) +
      parseInt(this._base$.css("paddingBottom")) +
      2;
  ...
  jQuery.event.add(document.body, "mouseup", function (e) {
    return o._borderMUp(e);
  });

  jQuery.event.add(document.body, "mousemove", function (e) {
    return o._borderMMove(e);
  });

  jQuery.event.add(document.body, "resize", function (e) {
    return o._wndResized(e);
  });

  if (this.saveScroll) {
    jQuery.event.add(window, "scroll", function (e) {
...
popupLayer.prototype.defaultWidth = 200;
popupLayer.prototype.defaultHeight = 200;
...
popupLayer.prototype.load = function (src, title) {
  ...
  src +=
    (src.search(/\?/) === -1 ? "?" : "&") +
    "open_in_layer=Y&is_ajax_request=Y&keep_https=Y";

  var o = this;
  try {
    this._ajax = $.get(src, function (data, s) {
      o._onload(data, s);
    });
  } catch (e) {
    this.close();
...
popupLayer.prototype.hide = function () {
  ...
  if (this.isModal) {
    ...
    if (this._msie6) this._switchSelects(true);
  }

  this._base.style.display = "none";

  if (this._bg_iframe) this._bg_iframe.style.display = "none";
...
popupLayer.prototype.show = function () {
  ...
  if (this.isModal) {
    ...
    if (this._msie6) this._switchSelects(false);
  }

  this._base.style.display = "";

  if (this._bg_iframe) this._bg_iframe.style.display = "";
...
popupLayer.prototype.setDim = function (x, y, w, h) {
  ...
  var sl =
    localIsStrict && localBrowser != "Chrome" && localBrowser != "Safari"
  ...
  if (x === false) x = this._lastDim.left;

  if (y === false) y = this._lastDim.top;

  if (w === false) w = this._lastDim.width;

  if (h === false) h = this._lastDim.height;
  ...
  if (w != this._lastDim.width) {
    ...
    if (this._msie6) {
      this._content.style.width =
        this._footer.style.width =
        this._title.style.width =
          w - (this._borderWidth + 1) * 2 + "px";
      this._rSide.style.left = w - this._borderWidth - 1 + "px";
    } else {
      this._content.style.width =
        this._footer.style.width =
        this._title.style.width =
          w - this._borderWidth * 2 + "px";
    }
...
popupLayer.prototype.autoResize = function (reposition) {
  ...
  if (hs > h) {
    h = hs + 20;
    if ($.browser.msie && parseInt($.browser.version) == 7) h += 20;
  }
  ...
  if (this._lastDim.width >= w) w = false;

  if (this._lastDim.height >= h) h = false;

  return reposition
    ? this.setDim("center", "center", w, h)
    : this.setDim(false, false, w, h);
};
...
popupLayer.prototype._borderMDown = function (e) {
  ...
  if ($.browser.msie) {
    ...
    this._borderOwner._content.setAttribute(
      "style",
      this._borderOwner._content.getAttribute("style") +
...
popupLayer.prototype._borderMMove = function (e) {
  if (!this._currentDrag) return true;
  ...
  switch (this._currentDrag._border) {
    case "t":
      if (
        y < this._lastDim.height + this._lastDim.top - this.minHeight &&
        y > this.parentWindowBorderV
      ) {
        this._dndBorder.style.top = y + "px";
        this._dndBorder.style.height =
          this._lastDim.height +
          this._lastDim.top -
          y +
          this._dndBorderAddHeight +
          "px";
      }
      break;
    ...
    case "tr":
      ...
      if (
        x > this._lastDim.left + this.minWidth &&
        x < this._lastWndDim.width - this.parentWindowBorderH
      ) {
        this._dndBorder.style.width =
          x - this._lastDim.left + this._dndBorderAddWidth + "px";
      }
      break;
    ...
    case "br":
      if (
        y > this._lastDim.top + this.minHeight &&
        y - this._lastDim.top <
          this._lastWndDim.height - this.parentWindowBorderV * 2
      ) {
        this._dndBorder.style.height =
          y - this._lastDim.top + this._dndBorderAddHeight + "px";
      }
      if (
        x > this._lastDim.left + this.minWidth &&
        x < this._lastWndDim.width - this.parentWindowBorderH
    ...
    case "bl":
      ...
      if (
        x < this._lastDim.width + this._lastDim.left - this.minWidth &&
        x > this.parentWindowBorderH
      ) {
    ...
    case "title":
      ...
      if (
        x > this.parentWindowBorderH &&
        x <
          this._lastWndDim.width -
            this._lastDim.width -
            this.parentWindowBorderH
      ) {
        this._dndBorder.style.left = x + "px";
      }
      if (
        y > this.parentWindowBorderV &&
        y <
          this._lastWndDim.height -
            this._lastDim.height -
            this.parentWindowBorderV
      ) {
        this._dndBorder.style.top = y + "px";
      }
      break;
...
popupLayer.prototype._borderMUp = function (e) {
  ...
  if ($.browser.msie) {
    ...
    this._content.setAttribute(
      "style",
      (this._content.getAttribute("style") + "").replace(
        /-moz-user-select: none;/,
        ""
      )
    );
...
popupLayer.prototype._onload = function (data, s, obj) {
  if (!this.valid || s != "success") return false;
...
popupLayer.prototype._processResponse = function (data) {
  ...
  try {
    if (
      ...
      (this._ajax && (l = this._ajax.getResponseHeader("Location")))
    ) {
      // Redirect
      if (m) l = m[1];
...
popupLayer.prototype._insertData = function (data) {
  ...
  if (!m) m = data.match(new RegExp("<body[^>]*>\n*((?:.*\n)+.*)</body>", "i"));
...
popupLayer.prototype._processInsertedData = function () {
  ...
  if (this.useAutoResize) this.autoResize(true);
...
popupLayer.prototype._submitFormHnd = function () {
  ...
  $.each($(this).serializeArray(), function (i, elm) {
    if (data[elm.name]) {
      ...
      else data[elm.name][data[elm.name].length] = elm.value;
    } else {
  ...
  if (this.method == "post") {
    this.ajax = $.post(url, data, function (data, s) {
      pc._onload(data, s, o);
    });
  } else {
    this.ajax = $.get(url, data, function (data, s) {
      pc._onload(data, s, o);
    });
  }
...
popupLayer.prototype._clickAHnd = function () {
  ...
  this.ajax = $.post(url, function (data, s) {
    pc._onload(data, s, o);
  });
...
popupLayer.prototype._switchSelects = function (switchon) {
  ...
  for (var i = 0; i < elms.length; i++) {
    ...
    if (p && p.className == "popup-content") continue;
