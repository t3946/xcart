...
{if $suggests_arr ne ""}
    ...
    {foreach from=$suggests_arr item=v_s key=k_s}
        <br/>
        <a href="/keyword/{$v_s.clean_suggest}/">{$v_s.twotabsearchtextbox}</a>
    {/foreach}
    <br/>
    <br/>
{/if}
                ...
                <tr class="google_impression_object" data-product-id="{$products[product].productid}"
                    ...
                    <td class="PListImgBox">
                        <div class="PListImgBox" {if $products[product].oProduct && $products[product].oProduct->isGroupRoot()}style="width:150px; height:150px;"{/if}">
                            <a href="{$products[product].oProduct->getAbsoluteUrl(true)}" {if $search_all_website eq 'Y'}target="_blank"{/if}>
                        ...
                        <a {include file="on_product_click.tpl"}
                                href="{if $search_all_website eq 'Y'}{if $products[product].clean_url ne ""}{$products[product].clean_url}{else}//{$products[product].domain}/product.php?productid={$products[product].productid}{/if}{else}/product.php?productid={$products[product].productid}{/if}"
                                class="SeeDetails" {if $search_all_website eq 'Y'}target="_blank"{/if}>{$lng.lbl_see_details}</a>
                        ...
                        {else}
                            ...
                            {if $usertype eq "C" and $config.Appearance.buynow_button_enabled eq "Y"}
                                ...
                                {if ($products[product].new_notify_in_stock_price ne "" && $notify_when_in_stock[$tmp_productid] ne "Y")}
                                    <br/>
                                    <span class="BuyNowQuantity">{$lng.lbl_quantity}:</span>
                                    <b>{$lng.txt_out_of_stock}</b>
                                    <br/>
                                    <div id="notify_tr1_{$products[product].productid}">
                                        <I><a href="javascript: void(0);"
                                              onclick="javascript: $('#notify_tr1_{$products[product].productid}').hide(); $('#notify_tr2_{$products[product].productid}').show();">Notify
                                    ...
                                    <div id="notify_tr2_{$products[product].productid}" style="display: none;">
                                        ...
                                        <form name="notifyform_{$products[product].productid}" method="post"
                                            ...
                                            <B>Your email address:</B> <input type="text" name="notify_email"
                                                                              value="{if $notify_email ne ""}{$notify_email}{/if}"/>

                                            {if $main eq "catalog"}
                                                <input type="hidden" name="cat" value="{$cat}"/>
                                                {if $action_notify_url ne ""}
                                                    <input type="hidden" name="redirect_to_notify_url"
                                                           value="{$action_notify_url}"/>
                                                {/if}
