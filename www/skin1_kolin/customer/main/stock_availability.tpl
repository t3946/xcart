<div class="survey_form">
	...
	{if $order.shipping_groups.$m.all_distributor_info.d_sec14_show_shipto eq "Y" || $admin_area_uses eq "Y"}
		{if ($admin_area_uses eq "Y" && $order.dc_status ne "M") || $admin_area_uses ne "Y"}
...
{if $admin_area_uses eq "Y"}
	<input type="submit" value="Update the order" name="Update the order" {* {if $order.amazonorderid ne ""}disabled="disabled" {/if} *} />
{else}
{*
