...
export default (function () {
  ...
  const Constructor = function () {
    ...
    $(".warehouse_products").each((i, elem) => {
      ...
      const $shipping = $distributorCart.find(
        ".shipping-methods-group input:checked"
      );
      ...
      success: function (res) {
        ...
        if (isCheckoutPage()) {
          ...
          for (let manufacturer_id in res.distributor_carts) {
            ...
            const deliveryCost =
              whTotal
                .closest(".warehouse_products")
                .find(".shipping-methods-group input:checked")
            ...
            whTotal
              ...
              .text(
                self.formatNumber(
                  parseFloat(manufacturer["subtotal"]) + deliveryCost
                )
              );
          }
        }
  ...
  Constructor.prototype.fieldUpdate = function (data, callback = null) {
    const form = Forms.getValidationForms().find(
      (item) => item.form.id === "CheckoutForm9"
    );
