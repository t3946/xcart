...
export const PaymentMethods = (function () {
  ...
  function updateSubmitHint() {
    const text = $('[name="CheckoutForm[paymentid]"]:checked').data(
      "submit-hint"
    );
  ...
  function init() {
    ...
    const $radioInputMethods = $root.find(
      'input[name="CheckoutForm[paymentid]"]'
    );
    const $allLongDescriptions = $paymentMethods.find(
      ".payment-method-description-long"
    );
    ...
    $paymentMethods.click(function (e) {
      ...
      const $input = $paymentMethodItem.find(
        '[name="CheckoutForm[paymentid]"]'
      );
    ...
    let defaultInput =
      $radioInputMethods.filter(`[value="${defaultValue}"]`) ||
      $radioInputMethods.eq(0);
