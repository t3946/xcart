...
(() => {
  ...
  window.toLocaleCurrency = (number = 0) => formatter.format(number);
  ...
  $(document).on("component.quantity.change", (e, data) => {
    ...
    if (data.target || (data.product && data.product.dataset.product)) {
      ...
      let extended = quantity * price.toFixed(app.options.currency.decimal);
      /* HTML вывод Увеличения/уменьшения кол-ва товаров на странице продукта */
      toHtml(
        product,
        "[var-price]",
        number_format(
          price,
          app.options.currency.decimal,
          app.options.currency.decimals_separator,
          app.options.currency.thousands_separator
        )
      );
      toHtml(
        product,
      ...
      if (quantity) {
        ...
        if (list_price && list_price !== price) {
          ...
          let safe_percentage = Math.floor(safe_price / (extended * 0.01));
          ...
          toHtml(
            product,
            "[var-price-perunit-safe]",
            toLocaleCurrency(per_unit)
          );
        }
      }
