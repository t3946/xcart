...
(function ($) {
  ...
  ConfirmActionModal.prototype = {
    ...
    html: {
      ...
      close:
        '<span class="confirm-action-modal-close" data-confirm-action-close>&times;</span>',
    ...
    init: function () {
      ...
      this.components.$title.css(
        this.styles.titles[this.options.title.style || "danger"]
      );
            ...
            return this.components.$label
              ...
              .append([
                this.components.$checkbox
                  .clone()
                  .attr("data-confirm-condition-id", key),
                this.components.$conditionText
                  .clone()
                  .text(condition.text || "Are you sure?"),
              ]);
          }
        },
        this
    ...
    setContent: function (options, container) {
      if (options.html) {
        var html =
          typeof options.html === "function" ? options.html() : options.html;
        if ($.isArray(html)) {
          html = html.join("\n");
        }
        container.html(html);
    ...
    listen: function (sourceEvent) {
      ...
      this.$element.find("[data-confirm-action-id]").each(function () {
        $(this).on("click.confirm.action", function (e) {
          ...
          var callback =
            that.options.actions[$(this).data("confirm-action-id")].callback;
      ...
      each(this.options.actions, function (actionKey, action) {
        ...
        each(action.conditions || [], function (key, condition) {
          ...
          if (that.options.conditions[condition].type === "checkbox") {
            if (
              !that.$element
                .find("[data-confirm-condition-id=" + condition + "]")
                .is(":checked")
            ) {
              disabled = true;
            }
        ...
        that.$element
          .find("[data-confirm-action-id=" + actionKey + "]")
          .prop("disabled", disabled)
          .css("cursor", disabled ? "not-allowed" : "pointer")
          .fadeTo(50, disabled ? 0.5 : 1);
      });
    },
  ...
  const ConfirmAction = function (element, options) {
