...
gulp.task(
  "frontend:css",
  gulp.series("frontend:scss", "frontend:css:raw", () => {
    let pipe = gulp.src(frontend.src.css).pipe(
      autoprefixer({
...
gulp.task("frontend:fonts", function () {
  return gulp.src(frontend.src.fonts).pipe(gulp.dest(frontend.dst.fonts));
});
...
gulp.task("backend:jsx", function () {
  ...
  if (backend.config && backend.config.babel) {
    pipe = pipe.pipe(babel(backend.config.babel)).on("error", function (err) {
...
gulp.task("backend:fonts", function () {
  return gulp.src(backend.src.fonts).pipe(gulp.dest(backend.dst.fonts));
});

gulp.task("backend:raw", function () {
  return gulp.src(backend.src.raw).pipe(gulp.dest(backend.dst.raw));
});
...
gulp.task("clear:backend", function () {
  return gulp
    .src(
      ["backend/dist/*", "backend/temp/*", backend.dst.jsx, backend.dst.scss],
      { allowEmpty: true }
    )
    .pipe(rimraf());
});
