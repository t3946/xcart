...
(function ($) {
  ...
  mmodal.prototype = {
    ...
    renderContainer: function () {
      this.$content = $("<div />").addClass(this.classes.content);

      this.$close = $('<a href="#">&times;</a>').addClass(
        this.classes.closeButton
      );

      this.$container = $("<div />").addClass(
        this.classes.container + " " + this.options.skin
      );

      if (this.options.animation) {
        this.$container.addClass(
          this.classes.animation + " " + this.options.animation + "In"
        );
      }

      this.$container.append(this.$close).append(this.$content);
    ...
    open: function () {
      ...
      if (
        this.options.setWidth !== "undefined" &&
        this.options.setWidth == false
      ) {
        this.$container.show();
      } else {
        this.$container
          .css("width", this.options.width || this.$container.width())
          .show();
      }
    ...
    close: function () {
      ...
      if (this.inline) {
        ...
        var $target = this.$inlineParent.children(
          ":eq(" + this.inlineIndex + ")"
        );
      ...
      if (this.options.animation) {
        this.$container.addClass(
          this.classes.animation + " " + this.options.animation + "Out"
        );
    ...
    bindEvents: function () {
      ...
      this.$bg.on("mousewheel", function (e) {
        var $this = $(this);
      ...
      if (this.options.touchEvents) {
        this.$bg
          ...
          .on("touchmove", function (e) {
            ...
            var deltaY =
              e.originalEvent.touches[0].pageY - self.touches.startingY;
            var deltaX =
              e.originalEvent.touches[0].pageX - self.touches.startingX;
