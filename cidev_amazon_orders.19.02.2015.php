...
$now = gmdate('Y-m-d\TH:i:s\Z', time()-24*60*60*100);
$request->setCreatedAfter($now);
 // object or array of parameters
$dom_xml = invokeListOrders($service, $request);

if ($dom_xml["Caught_Exception"] == "Request is throttled" && $dom_xml["Response_Status_Code"] == "503"){
	func_flush("sleeping...");
	func_flush();
	sleep('123');
...
if (!empty($dom_xml_arr) && !empty($dom_xml_arr["ListOrdersResponse"]["ListOrdersResult"]["Orders"]) && is_array($dom_xml_arr["ListOrdersResponse"]["ListOrdersResult"]["Orders"])){
...
### Do not need it. all this info already present in $v
#
...
###
			...
			if ($dom_OrderItems_xml["Caught_Exception"] == "Request is throttled" && $dom_OrderItems_xml["Response_Status_Code"] == "503"){
		                func_flush("sleeping...");
                		func_flush();
				sleep('123');
			        func_flush("Unsleeped.");
			...
			if (!empty($OrderItems_arr) && is_array($OrderItems_arr["ListOrderItemsResponse"]["ListOrderItemsResult"]["OrderItems"])){

				$PurchaseDate = strtotime($v['PurchaseDate']);

				$StateOrRegion = $v['ShippingAddress']['StateOrRegion'];

				$StateOrRegion_code = func_query_first_cell("SELECT code FROM $sql_tbl[states] WHERE country_code = '".$v['ShippingAddress']['CountryCode']."' AND state = '".$StateOrRegion."'");
				...
				if ($v['ShippingAddress']['CountryCode'] == "US" && strpos($PostalCode, "-")!==false){
					$PostalCode_arr = explode("-",$PostalCode);
					$PostalCode = $PostalCode_arr[0];
				}

				$Address = $v['ShippingAddress']['AddressLine1'] .(!empty($v['ShippingAddress']['AddressLine2'])?' '.$v['ShippingAddress']['AddressLine2'] :'').(!empty($v['ShippingAddress']['AddressLine3'])?' '.$v['ShippingAddress']['AddressLine3'] :'');

        		        $insert_data = array (
		                        'order_prefix' => 'AZ-',
                		        'login' => 'amazon',
		                        'amazonorderid' => $v["AmazonOrderId"],
                		        'total' => $v['OrderTotal']['Amount'],
		                        'date' => $PurchaseDate,
                		        'cb_status' => 'P',
		                        'dc_status' => ($v['OrderStatus']=='Unshipped' ? 'T' : 'S'),
                		        'bd_status' => 'W',
		                        'payment_method' => 'Amazon Seller',
                		        'firstname' => $v['BuyerName'],
                		        's_firstname' => $v['ShippingAddress']['Name'],
		                        's_address' => $Address,
		                        's_city' => $v['ShippingAddress']['City'],
		                        's_state' => $StateOrRegion,
                		        's_country' => $v['ShippingAddress']['CountryCode'],
		                        's_zipcode' => $PostalCode,
		                        'b_firstname' => $v['BuyerName'],
                                        'b_address' => $Address,
                                        'b_city' => $v['ShippingAddress']['City'],
                                        'b_state' => $StateOrRegion,
                                        'b_country' => $v['ShippingAddress']['CountryCode'],
		                        'b_zipcode' => $PostalCode,
                		        'phone' => $v['ShippingAddress']['Phone'],
		                        'email' => $v['BuyerEmail'],
                		        'language' => 'US',
		                        'paymentid' => '1',
                		        'storefrontid' => '0',
...
//func_print_r($v_item, $insert_data);
				    ...
				    if (!in_array($prod_info["manufacturerid"], $manufacturerid_arr)){
	                                    $insert_data2 = array (
                        	                ...
                        	                'shipping' => $v['ShipmentServiceLevelCategory'],
                                	        'cb_status' => 'P',
                                        	'dc_status' => ($v['OrderStatus']=='Unshipped' ? 'T' : 'S'),
	                                        'bd_status' => 'W',
        	                                'total_net' => $v['OrderTotal']['Amount'],
                	                        'total_gross' => $v['OrderTotal']['Amount'],
                        	                'acc_paymentid' => '1'
                                	    );
				    ...
				    $extra_data["display"]["price"] = $v_item['ItemPrice']['Amount'];
				    $extra_data["display"]["discounted_price"] = $v_item['ItemPrice']['Amount']*$v_item['QuantityOrdered'];
...
//func_print_r($v_item, $insert_data, $insert_data2, $insert_data3);
				...
				db_query("INSERT INTO $sql_tbl[cidev_amazon_order_raw] (orderid, order_info, orderitems_info) VALUES ('$new_orderid', '".addslashes(serialize($v))."', '".addslashes(serialize($OrderItems_arr))."')");

		                $log = '<a style="color: #1411FF;" href="https://sellercentral.amazon.com/gp/orders-v2/details/ref=ag_orddet_cont_myo?ie=UTF8&orderID='.$v["AmazonOrderId"].'" target="_blank">Amazon order # '.$v["AmazonOrderId"].'</a><br />Grand total $'.$product_total;
...
### 2s
 ...
 $request->setSellerId(MERCHANT_ID);
 $request->setNextToken('2');
 // object or array of parameters
 $dom_xml2 = invokeListOrdersByNextToken($service, $request);
