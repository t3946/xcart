...
if (!(strstr($referer, "http://$xcart_http_host/") || strstr($referer, "https://$xcart_https_host/") || $referer == '') && !isset($_COOKIE['RefererCookie'])) {
        $curr_time = time();
		$referer_result = func_query_first_cell("SELECT COUNT(visits) FROM $sql_tbl[referers] WHERE referer='".addslashes($referer)."'");
		if ($referer_result)
...
if (!isset($_COOKIE['RefererCookie']) || empty($referer_session)) {
	if (empty($referer_session)) {
		$referer_session = (isset($_COOKIE['RefererCookie'])) ? $_COOKIE['RefererCookie'] : $referer;
	}
