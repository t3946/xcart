...
###
	if (is_array($_FILES)) {
		foreach ($_FILES as $k => $v) {
			foreach ($v as $kk => $vv){
				if ($kk == "type"){
					$tmp_type_pos = strpos($vv, "image");

					if ($cidev_form_name == "product_files"){
						if ($tmp_type_pos !== false){
							unset($_FILES[$k]);
						}
					} else {
						if ($tmp_type_pos === false){
...
#
    ...
    if (is_array($_FILES)) {
        ...
        $id = $productid;
        $notimage = 'avail';
        foreach ($_FILES as $file_key => $ufile) {
...
//				if ($geid && $ge_d_images == 'Y') {
				if ($geid && ($ge_d_images == 'Y' || $ge_p_files == 'Y') ) {
...
#cidev
if (empty($alt[$multi_id])){
	$alt[$multi_id] = $product_info["product"];

	$orderby = func_query_first_cell('SELECT MAX(orderby) FROM '. $sql_tbl['images_D']
            . ' WHERE id="' . $productid . '"') + 10;
}
#cidev

				$image_id = func_save_image($_file_upload_data, "D", $productid, array("alt" => $alt[$multi_id], "orderby" => $orderby));


# cidev
if (!empty($additional_operation) && is_array($additional_operation)){
        foreach ($additional_operation as $k => $v){
	  if ($k == $multi_id){
                if ($v == "gt"){
	                ...
	                $image_x_y = func_query_first("SELECT image_x, image_y FROM $sql_tbl[images_D] WHERE imageid='$image_id'");
        	        if (!empty($image_x_y) && $image_x_y["image_x"] <= 300 && $image_x_y["image_y"] <= 300){
                	        func_delete_images("D", "imageid='$image_id'");
	                }

                }
                else if ($v == "gt_del_img"){
                        if (func_generate_image($productid, 'D', 'T', false, false, $image_id)) {
                                func_save_product_thumb_image($productid, 'T');
                        }
...
//				func_array2insert("images_D", $data);
				$new_image_id = func_array2insert("images_D", $data);
...
if (!empty($additional_operation) && is_array($additional_operation)){
	  ...
	  if ($k == $multi_id){
                if ($v == "gt"){
                        if (func_generate_image($pid, 'D', 'T', false, false, $new_image_id)) {
                                func_save_product_thumb_image($pid, 'T');
                        }
                        if (func_generate_image($pid, 'D', 'P', false, false, $new_image_id)) {
                                func_save_product_thumb_image($pid, 'P');
                        }

                        $image_x_y = func_query_first("SELECT image_x, image_y FROM $sql_tbl[images_D] WHERE imageid='$new_image_id'");
                        if (!empty($image_x_y) && $image_x_y["image_x"] <= 300 && $image_x_y["image_y"] <= 300){
                                func_delete_images("D", "imageid='$new_image_id'");
                        }
...
#cidev
	if (!empty($cidev_del_ids) && is_array($cidev_del_ids)){
		foreach ($cidev_del_ids as $k => $v){
			func_delete_image($v, "D", true);
		}
	}
#cidev
	...
	func_refresh('product_files');
		...
		if (func_generate_image($productid, 'D', 'T', false, false, $thumbid)) {
			...
			$top_message['type'] = 'I';
                ...
                if (func_generate_image($productid, 'D', 'P', false, false, $thumbid)) {
                        func_save_product_thumb_image($productid, 'P');
                        $top_message['content'] = func_get_langvar_by_name('lbl_thumb_success_generated', null, false, true);
                        $top_message['type'] = 'I';
                }
...
##
###
		$image_x_y = func_query_first("SELECT image_x, image_y FROM $sql_tbl[images_D] WHERE imageid='$thumbid'");
		if (!empty($image_x_y) && $image_x_y["image_x"] <= 300 && $image_x_y["image_y"] <= 300){
			func_delete_images("D", "imageid='$thumbid'");
		}
###
##
#
...
} elseif ($mode == 're_gen_det_img' && !empty($thumbid)) {

/*
                $auto_thumb_error = '';
                if (func_generate_image($productid, 'D', 'D', false, false, $thumbid)) {
                        $top_message['content'] = "Regenerated";
                        $top_message['type'] = 'I';
                }
*/
		func_refresh();
