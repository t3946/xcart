	...
	db_query("UPDATE $sql_tbl[config] SET value='$Feeds_storage_password' WHERE name='Feeds_storage_password'");
        ...
        if (!empty($Supplier_feeds) && is_array($Supplier_feeds)){
			...
			if ($v["delete"] == "Y"){
				...
				db_query("UPDATE $sql_tbl[supplier_feeds] SET feed_name='$v[feed_name]', feed_type='$v[feed_type]', manufacturerid='$v[manufacturerid]', storefront_id='$v[storefront_id]', base_category_id='$v[base_category_id]', feed_file_name='$v[feed_file_name]', last_update_time='$v[last_update_time]', average_update_period='$v[average_update_period]', last_update_items_count='$v[last_update_items_count]', threshold='$v[threshold]', add_new_only='$v[add_new_only]', last_md5='$v[last_md5]', enabled='$v[enabled]' WHERE feed_id='$v[feed_id]'");
			}
                }
        }
    }
