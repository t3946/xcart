...
#
# $Id: func.php,v 1.11.2.1 2006/10/30 06:50:47 max Exp $
#
...
function func_is_allowed_trans() {
	...
	if ($config['Stop_List']['slist_failed_transaction_limit'] > 0) {
		$counter = func_query_first_cell ("SELECT COUNT(*) FROM $sql_tbl[orders], $sql_tbl[order_extras] WHERE $sql_tbl[order_extras].orderid = $sql_tbl[orders].orderid AND $sql_tbl[orders].cb_status = 'F' AND $sql_tbl[order_extras].khash = 'ip' AND $sql_tbl[order_extras].value = '".$REMOTE_ADDR."'");
