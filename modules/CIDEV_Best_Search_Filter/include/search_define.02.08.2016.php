...
//func_print_r($search_data, $all_filter_name_id);
//die("search_define.php");
...
//	                        $fields[] = "$sql_tbl[cidev_filter_products].fv_id";
/*
        	                $inner_joins['cidev_filter_products'] = array(
                	            'on' => "$sql_tbl[cidev_filter_products].productid = $sql_tbl[products].productid"
	                        );

	                        $inner_joins['cidev_filter_values'] = array(
					'on' => "$sql_tbl[cidev_filter_values].fv_id = $sql_tbl[cidev_filter_products].fv_id"
				);
                                $inner_joins['cidev_filters'] = array(
                                    'on' => "$sql_tbl[cidev_filters].f_id = $sql_tbl[cidev_filter_values].f_id"
                                );
*/
				$fv_id_where_condition = array();
				$f_id_where_condition = array();

func_print_r($sorted_filter_values_id);

$ccc = count($sorted_filter_values_id);
				foreach ($sorted_filter_values_id as $f_id => $fv_ids){
					$aaa = implode(', ', $fv_ids);
					$fv_id_where_condition[$f_id] = "$sql_tbl[cidev_filters].f_id = $f_id AND $sql_tbl[cidev_filter_values].fv_id IN ($aaa)";
//					if (!empty($fv_ids) && is_array($fv_ids)){
//						$fv_id_where_condition[$f_id] = "$sql_tbl[cidev_filter_products].fv_id IN (" . implode(",", $fv_ids) . ")";
		
//						$f_id_where_condition[$f_id] = "$sql_tbl[cidev_filters].f_id='$f_id'";

/*
						$cidev_filters_table = 'xcart_cidev_filters_'.$f_id;
			                        $left_joins[$cidev_filters_table] = array(
							"tblname" => "cidev_filters",
//							'on' => "$cidev_filters_table.f_id = $sql_tbl[cidev_filters].f_id"
							'on' => "$cidev_filters_table.f_id = $sql_tbl[cidev_filter_values].f_id"
			                        );

						$where[] = "$cidev_filters_table.f_id = '$f_id'";
*/
//
//					}
...
//				$where[] = " ( " . implode(" AND ", $fv_id_where_condition) . " ) ";
				$fff1 = implode(" OR ", $fv_id_where_condition);
//				$fff = "(SELECT p.productid FROM $sql_tbl[cidev_filters] JOIN $sql_tbl[cidev_filter_values] USING(f_id) JOIN $sql_tbl[cidev_filter_products] p USING(fv_id) WHERE ( $fff1 ) GROUP BY p.productid HAVING COUNT(1) = $ccc)";
				$fff = "SELECT productid FROM ( SELECT * FROM xcart_cidev_filters JOIN xcart_cidev_filter_values USING(f_id) JOIN xcart_cidev_filter_products p USING(fv_id) WHERE $fff1 GROUP BY productid, f_id ORDER BY f_id ) aaa GROUP BY productid HAVING COUNT(1) = $ccc";


				$where[] = "$sql_tbl[products].productid IN ($fff)";

			}


//func_print_r($left_joins, $where);
//die();

//func_print_r($search_data, $all_filter_name_id, $sorted_filter_values_id, $fv_id_where_condition, $where);
//die("search_define.php");


//func_print_r($search_data['products']['filter_value_id'], $full_filter_values_id);
//die();

/*
		        if (!empty($full_filter_values_id)){
				$where[] = "$sql_tbl[cidev_filter_products].fv_id IN (" . implode(",", $full_filter_values_id) . ")";
	        	}
*/
...
/*
                        ...
                        $fields[] = "IF($sql_tbl[cidev_filters].f_id IS NULL,'','Y') as is_filter";

                        $left_joins['cidev_filter_values'] = array(
                            'on' => "$sql_tbl[cidev_filter_products].fv_id = $sql_tbl[cidev_filter_values].fv_id"
                        );
