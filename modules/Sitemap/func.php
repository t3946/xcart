...
{
    global $sql_tbl, $config, $current_storefront, $active_modules;
    ...
    if ($config['Sitemap']['sitemap_display_extra'] == 'Y') {
        ...
        if (!empty($active_modules['Multiple_Storefronts']) && isset($current_storefront)) {
            $sf_condition = 'AND storefront = "' . $current_storefront . '"';
        } else {
            $sf_condition = '';
        }

        $query = "SELECT $sql_tbl[sitemap_extra].name as name, $sql_tbl[sitemap_extra].url as url,"
            . " $sql_tbl[sitemap_extra].id as id"
            . " FROM $sql_tbl[sitemap_extra]"
            . " WHERE $sql_tbl[sitemap_extra].active = 'Y' $sf_condition ORDER BY $order_by";
        $items = func_query($query);
        return $items;
    } else {
        return false;
...
{
    global $current_storefront, $active_modules;
    ...
    if (!empty($active_modules['Multiple_Storefronts']) && isset($current_storefront)) {
        $insert['storefront'] = $current_storefront;
    }
...
/**
 * Get all exrta URLs from db for the current SF
 *
 * @return array
 */
function sitemap_extra_geturls()
{
    ...
    if (!empty($active_modules['Multiple_Storefronts']) && isset($current_storefront)) {
        $sf_condition = 'WHERE storefront = "' . $current_storefront . '"';
    } else {
        $sf_condition = '';
    }
    $query = "SELECT $sql_tbl[sitemap_extra].* FROM $sql_tbl[sitemap_extra]"
        . " $sf_condition ORDER BY $sql_tbl[sitemap_extra].orderby";
