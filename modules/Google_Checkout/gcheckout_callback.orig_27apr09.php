...
#
# $Id: gcheckout_callback.php,v 1.1.2.17 2007/01/23 13:11:11 svowl Exp $
#
...
if ($PHP_AUTH_USER != $config['Google_Checkout']['gcheckout_mid'] || $PHP_AUTH_PW != $config['Google_Checkout']['gcheckout_mkey']) {
		...
		foreach ($pcodes as $_pcode) {
			...
			if (!empty($active_modules['Discount_Coupons']) && $coupon_result == 0) {
				...
				if ($coupons_counter == 1) {
					...
					$discount_result = func_calculate_discounts(0, $products, $_promo_code);
					...
					if ($discount_result['coupon_type'] == 'free_ship')
						$_pcode_message = func_google_encode(func_get_langvar_by_name('lbl_gcheckout_coupon_freeship', array('coupon_code'=>$_promo_code), 'US', true, true));
					else
						$_pcode_message = func_google_encode(func_get_langvar_by_name('lbl_gcheckout_coupon_applied', array('coupon_code'=>$_promo_code), 'US', true, true));
...
OUT;
			...
			elseif (!empty($active_modules['Gift_Certificates']) && ($gc_error_code = func_giftcert_check($_promo_code)) == 0) {
				...
				if (!empty($gc)) {
					...
					$_pcode_message = func_google_encode(func_get_langvar_by_name('lbl_gcheckout_giftcert_applied', false, 'US', true, true));
					$result_code[] =<<<OUT
...
OUT;
	...
	if (!empty($addresses) && is_array($addresses)) {
		...
		for ($i = 0; $i < count($addresses); $i++) {
			...
			$_tmp['b_state'] = $_tmp['s_state'] = func_array_path($addresses[$i], "#/REGION/0/#");
			...
			if (!empty($shipping_requested)) {
				...
				$_allowed_shipping_methods = func_get_shipping_methods_list($cart, $cart['products'], $cart['userinfo']);
				...
				for ($j = 0; $j < count($shipping_requested); $j++) {
					if (!empty($_allowed_shipping_methods)) {
						foreach ($_allowed_shipping_methods as $_ship_method) {
							if (func_insert_trademark($_ship_method['shipping'], @$insert_trademark, true) == $shipping_requested[$j]['shipping']) {
				...
				# Calculate tax cost
				$tmp_cart = $cart;
				$calc_result = func_calculate($tmp_cart, $tmp_cart['products'], '', 'C');
...
###############################################################################
	...
	if ($financial_os != 'REVIEWING' || !preg_match("/^GO|/", $_pp3_data['trstat'])) {
		...
		$profile_values['firstname'] = func_array_path($parsed, $type."/BUYER-BILLING-ADDRESS/CONTACT-NAME/0/#");
		...
		$profile_values['b_firstname'] = $profile_values['firstname'];
		$profile_values['b_address'] = func_array_path($parsed, $type."/BUYER-BILLING-ADDRESS/ADDRESS1/0/#") .
			($_addr2 = func_array_path($parsed, $type."/BUYER-BILLING-ADDRESS/ADDRESS2/0/#") ? "\n" . $_addr2 : "");
		$profile_values['b_city'] = func_array_path($parsed, $type."/BUYER-BILLING-ADDRESS/CITY/0/#");
		$profile_values['b_state'] = func_array_path($parsed, $type."/BUYER-BILLING-ADDRESS/REGION/0/#");
		$profile_values['b_country'] = func_array_path($parsed, $type."/BUYER-BILLING-ADDRESS/COUNTRY-CODE/0/#");
		$profile_values['b_zipcode'] = func_array_path($parsed, $type."/BUYER-BILLING-ADDRESS/POSTAL-CODE/0/#");

		$profile_values['s_firstname'] = func_array_path($parsed, $type."/BUYER-BILLING-ADDRESS/CONTACT-NAME/0/#");
		$profile_values['s_address'] = func_array_path($parsed, $type."/BUYER-SHIPPING-ADDRESS/ADDRESS1/0/#") .
			($_addr2 = func_array_path($parsed, $type."/BUYER-SHIPPING-ADDRESS/ADDRESS2/0/#") ? "\n" . $_addr2 : "");
		$profile_values['s_city'] = func_array_path($parsed, $type."/BUYER-SHIPPING-ADDRESS/CITY/0/#");
		$profile_values['s_state'] = func_array_path($parsed, $type."/BUYER-SHIPPING-ADDRESS/REGION/0/#");
		$profile_values['s_country'] = func_array_path($parsed, $type."/BUYER-SHIPPING-ADDRESS/COUNTRY-CODE/0/#");
		$profile_values['s_zipcode'] = func_array_path($parsed, $type."/BUYER-SHIPPING-ADDRESS/POSTAL-CODE/0/#");
		
		$ship_to_details = array();
		$ship_to_details['s_company'] = func_array_path($parsed, $type."/BUYER-SHIPPING-ADDRESS/COMPANY-NAME/0/#");
		$ship_to_details['s_email'] = func_array_path($parsed, $type."/BUYER-SHIPPING-ADDRESS/EMAIL/0/#");
		$ship_to_details['s_phone'] = func_array_path($parsed, $type."/BUYER-SHIPPING-ADDRESS/PHONE/0/#");
		$ship_to_details['s_fax'] = func_array_path($parsed, $type."/BUYER-SHIPPING-ADDRESS/FAX/0/#");
		...
		if (empty($login) || $login_type != 'C') {
			...
			# Log in a user with new profile
			...
			func_gcheckout_debug("\t+ Customer created - login: '$login'");
		...
		$shipping_name = func_array_path($parsed, $type."/ORDER-ADJUSTMENT/SHIPPING/MERCHANT-CALCULATED-SHIPPING-ADJUSTMENT/SHIPPING-NAME/0/#");

		if (!empty($shipping_name)) {

			# Get the shipping methods list with rates
			$_allowed_shipping_methods = func_get_shipping_methods_list($cart, $cart['products'], $cart['userinfo'], ($merchant_calculation_successful == 'false' ? true : false));

			# Generate the full list of shipping methods for current address
			$_shippingid = 0;
			if (!empty($_allowed_shipping_methods)) {
				foreach ($_allowed_shipping_methods as $_ship_method) {
					if (func_insert_trademark($_ship_method['shipping'], @$insert_trademark, true) == $shipping_name) {
			...
			if ($merchant_calculation_successful == 'false') {
				...
				$cart['shipping_cost'] = func_array_path($parsed, $type."/ORDER-ADJUSTMENT/SHIPPING/MERCHANT-CALCULATED-SHIPPING-ADJUSTMENT/SHIPPING-COST/0/#");
			}
		...
		$cart = func_calculate($cart, $cart['products'], $login, 'C');
		...
		if ($email_allowed == 'true') {
			...
			if ($_emails != $ship_to_details['s_email'])
				$_emails .= ', ' . $ship_to_details['s_email'];
...
###############################################################################
	...
	if (!empty($_message4log)) {
		$gcheckout_orders_update_fields[] = "state_log=CONCAT(state_log,'-" . implode('-', $_message4log) . (!empty($reason) ? "-Reason: " . str_replace('-', '_', $reason) : "") . "')";
	}
	...
	if (!empty($_pp3_data)) {
		...
		func_array2update(
			"cc_pp3_data",
			array(
				"trstat=CONCAT(trstat,'-".$new_financial_os."|".$_curtime."')"
			),
			"ref = '$goid'"
		);
...
###############################################################################
	...
	func_array2update(
		...
		array(
			"param3" => $for_param3,
			"param4" => "RISK-INFO-RECEIVED",
			"trstat=CONCAT(trstat,'-RISK INFO|".$_curtime."')"
		),
		"ref = '$goid'"
	);
	...
	func_array2update(
		"gcheckout_orders",
		array(
			"state_log=CONCAT(state_log,'-RISK INFO received|".$_curtime."')"
		),
		"goid = '$goid'"
	);
...
###############################################################################
	...
	func_array2update(
		"cc_pp3_data",
		array(
			"trstat=CONCAT(trstat,'-CONFIRMED|".$_curtime."')"
		),
		"ref = '$goid'"
	);
	...
	func_array2update(
		"gcheckout_orders",
		array(
			"financial_state" => "CHARGED (CONFIRMED)|" . $_curtime,
			"state_log=CONCAT(state_log,'-Confirmation of CHARGED state is received|".$_curtime."')"
		),
		"goid = '$goid'"
	);
...
###############################################################################
	...
	func_array2update(
		...
		array(
			...
			"financial_state" => "REFUNDED ({$config['Google_Checkout']['gcheckout_currency']} $refund_amount)|" . $_curtime,
			"state_log=CONCAT(state_log,'-REFUNDED ({$config['Google_Checkout']['gcheckout_currency']} $refund_amount)|".$_curtime."')"
		),
		"goid = '$goid'"
	);
...
###############################################################################
	...
	func_array2update(
		"gcheckout_orders",
		array(
			"refunded_amount=refunded_amount+$chargeback_amount", 
			"financial_state" => "CHARGEBACK ({$config['Google_Checkout']['gcheckout_currency']} $chargeback_amount)|" . $_curtime,
			"state_log=CONCAT(state_log,'-CHARGEBACK ({$config['Google_Checkout']['gcheckout_currency']} $chargeback_amount)|".$_curtime."')"
		),
		"goid = '$goid'"
	);
...
if ($is_exit) {
	...
	if (empty($orderids)) {
		...
		if (!empty($goid)) {
			# Send 'cacel-order' request for Google Checkout order
