...
if ($mode == "upselling_links") {
    ...
    if (!$selected_productid && $selected_sku) {
        $prod_id = func_query_first_cell("SELECT productid FROM $sql_tbl[products] WHERE productcode = '$selected_sku'");
        if (!$prod_id) 
            $prod_id = func_query_first_cell("SELECT productid FROM $sql_tbl[variants] WHERE productcode = '$selected_sku'");
        if ($prod_id)
            $selected_productid = $prod_id;
    }
