...
#
# $Id: import_lng.php,v 1.5.2.1 2006/07/20 08:25:26 max Exp $
#
...
# Export data	
	...
	while (($id = func_export_get_row($data)) !== false) {
		...
		$mrow = func_query("SELECT $sql_tbl[class_lng].classid, $sql_tbl[class_lng].code, $sql_tbl[class_lng].class as class_name,"
			. " $sql_tbl[class_lng].classtext as descr, $sql_tbl[classes].class"
			. " FROM $sql_tbl[classes], $sql_tbl[class_lng], $sql_tbl[products] $sf_join"
			. " WHERE $sql_tbl[classes].classid = $sql_tbl[class_lng].classid AND $sql_tbl[class_lng].code = '$current_code' $sf_condition"
			. " AND $sql_tbl[classes].productid = $sql_tbl[products].productid AND $sql_tbl[classes].productid = '$id'"
