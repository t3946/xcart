...
#
# $Id: import_option_lng.php,v 1.6.2.1 2006/07/20 08:25:26 max Exp $
#
...
# Export data	
	...
	while (($id = func_export_get_row($data)) !== false) {
		...
		$mrow = func_query("SELECT $sql_tbl[product_options_lng].*, $sql_tbl[class_options].option_name as 'option', $sql_tbl[classes].class"
			. " FROM $sql_tbl[classes], $sql_tbl[product_options_lng], $sql_tbl[class_options], $sql_tbl[products] $sf_join"
			. " WHERE $sql_tbl[class_options].optionid = $sql_tbl[product_options_lng].optionid AND $sql_tbl[classes].classid = $sql_tbl[class_options].classid"
			. " AND $sql_tbl[product_options_lng].code = '$current_code' AND $sql_tbl[classes].productid = $sql_tbl[products].productid $sf_condition"
			. " AND $sql_tbl[classes].productid = '$id'" . (empty($provider_sql) ? "" : " AND $sql_tbl[products].provider = '$provider_sql'"));
		if (empty($mrow))
			continue;
