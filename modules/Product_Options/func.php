...
#
# $Id: func.php,v 1.88.2.14 2007/01/05 07:22:05 max Exp $
#
...
function func_get_product_variants($productid, $membershipid = 0, $area = false) {
	...
	if (empty($membershipid) || $area != 'C' || empty($active_modules['Wholesale_Trading'])) {
...
function func_check_product_options ($productid, $options, $trusted_options = false) {
	...
	$textids = func_query_column("SELECT classid FROM $sql_tbl[classes] WHERE classid IN ('".implode("','", func_array_map("intval", array_keys($options)))."') AND is_modifier = 'T'");
