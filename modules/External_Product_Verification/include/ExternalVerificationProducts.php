...
class ExternalVerificationProducts extends Data
{
    ...
    public function getAsin()
    {
        if (is_null($this->sAsin)) {
            $this->sAsin = '';
            if ($this->getBatchId()) {
                if (in_array($this->getAction(), ExternalVerificationBatch::$aProductStatuses['processed'])) {
                    $oAsin = ExternalVerificationProducts::model(['productid' => $this->getProductId(), 'batch_id' => $this->getBatchId(), 'action' => 'asin_on_amazon']);
    ...
    public function getComment()
    {
        if (is_null($this->sComment)) {
            $this->sComment = '';
            if (in_array($this->getAction(), ExternalVerificationBatch::$aProductStatuses['processed'])) {
                $oAsin = ExternalVerificationProducts::model(['productid' => $this->getProductId(), 'batch_id' => $this->getBatchId(), 'action' => 'comments_if_not']);
                if ($oAsin->getProductId()) {
    ...
    {
        switch ($sAction) {
            case ExternalVerificationProductsQueue::PRODUCT_QUEUE_STATUS_IN_ETALON_MATCH :
                $sAction = 'match';
                break;
            case ExternalVerificationProductsQueue::PRODUCT_QUEUE_STATUS_IN_ETALON_NOT_MATCH :
                $sAction = 'not_match';
                break;
            case ExternalVerificationProductsQueue::PRODUCT_QUEUE_STATUS_IN_ETALON_NOT_FOUND :
                $sAction = 'not_found';
                break;
        }
    ...
    public function getProductImage()
    {
        if (is_null($this->ProductImage)) {
            if ($this->getBatchId()) {
                $oImage = ExternalVerificationProducts::model(['productid' => $this->getProductId(), 'batch_id' => $this->getBatchId(), 'action' => 'product_image']);
    ...
    public function getProductName()
    {
        if (is_null($this->ProductName)) {
            if ($this->getBatchId()) {
                $oImage = ExternalVerificationProducts::model(['productid' => $this->getProductId(), 'batch_id' => $this->getBatchId(), 'action' => 'product_names']);
                if ($oImage->getProductId()) {
    ...
    public function getProductDescription()
    {
        if (is_null($this->ProductDescription)) {
            if ($this->getBatchId()) {
                $oImage = ExternalVerificationProducts::model(['productid' => $this->getProductId(), 'batch_id' => $this->getBatchId(), 'action' => 'product_description']);
                if ($oImage->getProductId()) {
    ...
    public function getQtyOnAmazon()
    {
        if (is_null($this->QtyOnAmazon)) {
            if ($this->getBatchId()) {
                $oImage = ExternalVerificationProducts::model(['productid' => $this->getProductId(), 'batch_id' => $this->getBatchId(), 'action' => 'qty_on_amazon']);
                if ($oImage->getProductId()) {
    ...
    public function getQtyOnOurWebSite()
    {
        if (is_null($this->QtyOnOurWebSite)) {
            if ($this->getBatchId()) {
                $oImage = ExternalVerificationProducts::model(['productid' => $this->getProductId(), 'batch_id' => $this->getBatchId(), 'action' => 'qty_on_our_website']);
                if ($oImage->getProductId()) {
