...
if (
    ...
    && $_SERVER['SERVER_NAME'] != MAIN_SF_DOMAIN
    && (empty($test_storefronts) || !in_array($_SERVER['SERVER_NAME'], $test_storefronts)) // Set the test storefronts in the config.local.php
) {
...
if (!empty($current_storefront) && $current_storefront > 0) {
    ...
    if (!empty($current_storefront_info["storefrontid"]) && $current_storefront_info["storefrontid"] > 0) {
        // First page openning
        $site_domain = func_query_first_cell("SELECT domain FROM $sql_tbl[storefronts] WHERE storefrontid='" . $current_storefront_info["storefrontid"] . "'");
    } else {
        $site_domain = MAIN_SF_DOMAIN;
    }
}
