...
$storefronts = func_query_hash('SELECT s.storefrontid, s.storefrontid as id, s.domain, s.prefix, s.status, s.orderby, i.filename, i.image_x, c.value as storefront_name FROM ' . $sql_tbl['storefronts'] . ' as s LEFT JOIN ' . $sql_tbl['images_S'] . ' as i ON s.storefrontid=i.id LEFT JOIN ' . $sql_tbl['storefronts_config'] . ' as c ON s.storefrontid=c.storefrontid WHERE c.name="company_name" ORDER BY s.orderby, s.domain', 'id', false, false);
