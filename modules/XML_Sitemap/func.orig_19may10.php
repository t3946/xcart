...
function xmlmap_prepare_items($spec, $items)
{
	...
	foreach ($items as $item) {
		if (isset($item['id'])) {
			...
			$url = $http_location . constant('DIR_CUSTOMER') . '/' . func_get_resource_url($spec['type'], $item['id'], '', false);
		} else {
			// Add domain and web path to url if necessary
			global $https_location;
			if (!strstr($item['url'], $http_location) && !strstr($item['url'], $https_location)) {
				$url = $http_location . constant('DIR_CUSTOMER') . '/' . $item['url'];
			} else {
				$url = $item['url'];
			}
		}
...
function xmlmap_addurl($url)
{
	...
	if (!empty($url)) {
		global $http_location, $sql_tbl;
		if (strpos(strtolower($url), strtolower($http_location)) === false) {
			$url = $http_location . '/' . $url;
		}
		
		if (0 < func_query_first_cell("SELECT count(id) FROM $sql_tbl[xmlmap_extra] WHERE url = '$url'")) {
			return func_get_langvar_by_name('xmlmap_error_extraurlexists');
		} else {
			$result = db_query("INSERT INTO $sql_tbl[xmlmap_extra] SET url='$url'");
			db_free_result($result);
		}
	} else {
...
{
	global $sql_tbl;
	return func_query("SELECT * FROM $sql_tbl[xmlmap_extra]");
}
