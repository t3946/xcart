...
*/
	2 => array(
		...
		'multilanguage' => false,
//                'items_query'   => "SELECT CONCAT('%s', $sql_tbl[brands].brandid) as url, $sql_tbl[brands].brandid as id, IFNULL($sql_tbl[xmlmap_lastmod].date, '%s') as date FROM $sql_tbl[brands] LEFT JOIN $sql_tbl[xmlmap_lastmod] ON $sql_tbl[xmlmap_lastmod].id = $sql_tbl[brands].brandid AND $sql_tbl[xmlmap_lastmod].type = 'B' WHERE $sql_tbl[brands].avail='Y'" . ((empty($sf_condition)) ? '' : " AND $sql_tbl[brands].$sf_condition"),
                'multilanguage' => false,
