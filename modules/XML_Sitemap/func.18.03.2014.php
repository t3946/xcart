		...
		if (!empty($items) && is_array($items)) {
			foreach ($items as $k => $v){
				if (!empty($v["id"]) && ($spec["type"] == "C" || $spec["type"] == "P" || $spec["type"] == "M" || $spec["type"] == "S" || $spec["type"] == "B")){
					...
					$clean_url_link = func_query_first_cell("SELECT clean_url FROM $sql_tbl[clean_urls] WHERE resource_type='$resource_type' AND resource_id='$v[id]'");
...
//			db_query("REPLACE INTO $sql_tbl[xmlmap_lastmod] (id, type, date) VALUES ('$item[id]', '$spec[type]', CONCAT(CURDATE(), 'T', CURTIME(), '+00:00'))");
			...
			$lastmod = func_query_first_cell("SELECT date FROM $sql_tbl[xmlmap_lastmod] WHERE id='$item[id]' AND type='$spec[type]'");
		}
###
##
#
