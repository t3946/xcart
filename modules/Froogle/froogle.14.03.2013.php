...
if (
	...
	if ($fp !== false) {
		...
		if (empty($froogle_export_step)) {
			...
			if ($froogle_iso) {
				fputs($fp, "title\tdescription\tlink\tadwords_redirect\tadwords_grouping\timage link\tadditional image link\tid\tprice\tpayment accepted\tpayment notes\tquantity\tweight\texpiration date\tbrand\tcondition\tproduct type\tmpn\tmodel number\tgtin\tcompatible with\tonline only\tshipping\tavailability\n");
...
//			$product['link'] = $product['froogle_location'] . constant('DIR_CUSTOMER') . '/product.php?productid=' . $product['productid'] . '&utm_source=' . $sf_info['prefix'] . 'froogle&utm_medium=cpc&utm_campaign='.$product['productcode'];
			$product['link'] = $product['froogle_location'] . constant('DIR_CUSTOMER') . '/product.php?productid=' . $product['productid'] . '&utm_source=' . $sf_info['prefix'] . 'froogle_Google-Shopping&utm_medium='.func_froogle_convert($product['brand'], 256).'&utm_campaign='.$product['productcode'];


		$product['adwords_redirect'] = $product['froogle_location'] . constant('DIR_CUSTOMER') . '/product.php?productid=' . $product['productid'] . '&utm_source=' . $sf_info['prefix'] . 'froogle_Product-Ads&utm_medium='.func_froogle_convert($product['brand'], 256).'&utm_campaign='.$product['productcode'];
...
//func_print_r($tmp, $tmp_all, $tmbn, $tmbn_no_img, $additional_image_link);
//die("123");
...
/*
if ($product['productid'] == "74356"){
print"I am testing.....";
x_load("debug");
func_print_r($product['descr']);

$product['descr'] = iconv("UTF-8", "ISO-8859-1//TRANSLIT", $product['descr']);
func_print_r($product['descr']);

die("Will be enabled soon.");
}
*/


				$post = iconv("UTF-8", "ISO-8859-1//TRANSLIT",func_froogle_convert($product['product']))."\t".
				iconv("UTF-8", "ISO-8859-1//TRANSLIT",func_froogle_convert($product['descr']))."\t".
				$tmbn."\t".
				$product['page_url']."\t".
