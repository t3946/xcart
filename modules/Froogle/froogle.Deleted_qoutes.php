...
if (
	...
	if ($fp !== false) {
		...
		if (empty($froogle_export_step)) {
			...
			fwrite($fp, iconv("UTF-8", "UTF-8//IGNORE", utf8_encode('"Title"'."\t".'"Description"'."\t".'"Image_Link"'."\t".'"Page_URL"'."\t".'"Direct_URL"'."\t".'"Price"'."\t".'"SKU"'."\t".'"UPC-EAN"'."\t".'"MPN"'."\t".'"ISBN"'."\t".'"Unique_ID"'."\t".'"Free Shipping"'."\t".'"Online_Only"'."\t".'"Stock_Quantity"'."\t".'"Brand"'."\t".'"Categories"'."\t".'"Condition"'."\t".'"Hot or Not"'."\t".'"Compatible_With"'."\t".'"Similar_To"'."\t".'"Weight"'."\n")));

		    }
		    else {

			if ($froogle_iso) {
				fputs($fp, "title\tdescription\tlink\timage link\tadditional image link\tid\tprice\tpayment accepted\tpayment notes\tquantity\tweight\texpiration date\tbrand\tcondition\tproduct type\tmpn\tmodel number\tgtin\tcompatible with\tonline only\tshipping\tavailability\n");
