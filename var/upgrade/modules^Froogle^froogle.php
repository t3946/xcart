...
if (
	...
	if ($fp !== false) {
		...
		if (empty($froogle_export_step)) {
			if ($froogle_iso) {
				fputs($fp, "title\tdescription\tlink\timage link\tid\tprice\tpayment accepted\tpayment notes\tquantity\tweight\texpiration date\tbrand\tcondition\tproduct type\tmpn\tmodel number\tgtin\tcompatible with\tonline only\tshipping\n");
		...
		if (!empty($active_modules['Multiple_Storefronts'])) {
			...
			$joins .= " LEFT JOIN $sql_tbl[products_sf] ON  $sql_tbl[products].productid= $sql_tbl[products_sf].productid";
