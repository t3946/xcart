			...
			<div class="b_scroll_div">
				...
				{foreach from=$new item="nproduct"}
					<tr{cycle values=', class="dark"' name="ncycle"}>
						{foreach from=$colnames item="col"}
							<td class="p_new_style"{if $col eq 'productcode'} nowrap="nowrap"{/if}>{$nproduct[$col]}</td>
						{/foreach}
					</tr>
				{/foreach}
				</table>
			...
			<div class="b_scroll_div">
				...
				{foreach from=$existing item="eproduct" name="ecolumns"}
					<tr{cycle values=', class="dark"' name="ecycle"}>
						{foreach from=$colnames item="col"}
							{if $col eq 'productcode'}
								<td nowrap="nowrap">{$eproduct.dbsr[$col]}</td>
							{elseif $col eq 'add_date'}
								<td class="b_empty_value">{$eproduct.dbsr[$col]|date_format:"%A %d %B %Y %T %p"}
								<td>{$eproduct.csv[$col]}</td>
							{else}
								<td class="b_empty_value">{$eproduct.dbsr[$col]}</td>
								<td>{$eproduct.csv[$col]}</td>
							{/if}
						{/foreach}
					</tr>
			...
			<div class="b_scroll_div">
				...
				{foreach from=$discontinued item="dproduct"}
					<tr{cycle values=', class="dark"' name="dcycle"}>
						{foreach from=$colnames item="col"}
							{if $col eq 'add_date'}
								<td class="p_discont_style">{$dproduct[$col]|date_format:"%A %d %B %Y %T %p"}
							{else}
								<td class="p_discont_style"{if $col eq 'productcode'} nowrap="nowrap"{/if}>{$dproduct[$col]}</td>
							{/if}
						{/foreach}
					</tr>
				{/foreach}
