...
#
# $Id: atracking.php,v 1.24 2006/04/07 11:28:00 svowl Exp $
#
...
elseif ($mode == "cartfunnel" && $cart_funnel > 0) {
#
...
#
    ...
    $statistics[0][visits] = func_query_first_cell("SELECT COUNT(pv.pageid) FROM $sql_tbl[stats_pages_views] as pv, $sql_tbl[stats_pages] as p WHERE p.pageid=pv.pageid AND p.page LIKE '%product.php?productid=%' AND $date_condition");
    ...
    $statistics[3][percent_parent] = sprintf("%2.02f", ( $statistics[3][visits] / ( $statistics[1][visits] + $statistics[2][visits] ) ) * 100);
...
#
	...
	$date = "date_time+'".$config["Appearance"]["timezone_offset"]."'";
	$date_condition = "($date>='$start_date' AND $date<='$end_date')";
	...
	$statistics = func_query("SELECT * FROM $sql_tbl[login_history] WHERE ".$date_condition);
}
else {
