...
#
# $Id: import.php,v 1.15 2006/04/11 10:33:11 max Exp $
#
...
# Export data	
	...
	while (($id = func_export_get_row($data)) !== false) {
		...
		if (!empty($active_modules['Multiple_Storefronts'])) {
			$mrow = db_query("SELECT $sql_tbl[classes].* FROM $sql_tbl[classes], $sql_tbl[products] WHERE $sql_tbl[classes].productid = $sql_tbl[products].productid AND $sql_tbl[products].storefrontid=$current_storefront AND $sql_tbl[classes].productid = '$id'".(empty($provider_sql) ? "" : " AND $sql_tbl[products].provider = '$provider_sql'"));
		} else {
		$mrow = db_query("SELECT $sql_tbl[classes].* FROM $sql_tbl[classes], $sql_tbl[products] WHERE $sql_tbl[classes].productid = $sql_tbl[products].productid AND $sql_tbl[classes].productid = '$id'".(empty($provider_sql) ? "" : " AND $sql_tbl[products].provider = '$provider_sql'"));
		}
		if (!$mrow)
			continue;
		...
		while ($row = db_fetch_array($mrow)) {
			...
			# Export options
			if ($row['type'] != "T") {
				$options = func_query("SELECT * FROM $sql_tbl[class_options] WHERE classid = '$row[classid]'");
				if (!empty($options)) {
