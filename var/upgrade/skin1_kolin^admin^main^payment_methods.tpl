...
<tr class="TableHead">
	<td>&nbsp;</td>
	<td width="25%">{$lng.lbl_methods}</td>
	<td width="15%" nowrap="nowrap">{$lng.lbl_special_instructions}</td>
	<td width="10%">{$lng.lbl_protocol}</td>
	<td width="5%">{$lng.lbl_membership}</td>
{if $active_modules.Anti_Fraud}
	<td width="10%" nowrap="nowrap">{$lng.lbl_check}*</td>
{/if}
	<td width="5%">{$lng.lbl_pos}</td>
	<td width="10%">{$lng.lbl_acc_proc}</td>
	<td width="10%">{$lng.lbl_acc_percent}</td>
	<td width="10%">{$lng.lbl_acc_per_trans}</td>
</tr>
...
{/if}
	...
	<textarea name="posted_data[{$payment_methods[method].paymentid}][payment_details]" cols="30" rows="3">{$payment_methods[method].payment_details|escape:"html"}</textarea>
	</td>
	<td valign="top">
...
<tr>
	<td align="center" colspan="{if $active_modules.Anti_Fraud}10{else}9{/if}" class="SubmitBox"><input type="submit" value="{$lng.lbl_update|strip_tags:false|escape}" /></td>
</tr>
