...
# Enable built-in benchmark
    define('BENCH', true);
    define('BENCH_SIMPLE', false);
    define('BENCH_DISPLAY_TYPE', "A");
    define('BENCH_DISABLE_LOGGING_TO_FILE', true);
# ------------------
...
if ($mode == "search") {
	foreach ($search_fields_in_get as $v) {
		...
		} elseif (!empty($HTTP_GET_VARS[$v])) {
				$search_args_str .= '&'.$v.'='.urlencode(stripslashes($HTTP_GET_VARS[$v]));
		}
	}
...
include $xcart_dir."/include/search_test.php";
