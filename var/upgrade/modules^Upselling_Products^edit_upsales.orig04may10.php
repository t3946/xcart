...
echo "<!-- ".var_export($HTTP_POST_VARS,true)."-->\n";	
...
//			echo "<!-- $productid - $pid BYLO: ".var_export($bl_link,true)." STALO: ".var_export(isset($blids[$pid]),true)."-->\n";
			if (isset($blids[$pid]) && !$bl_link) {
//				echo "<!-- New link: $pid - $productid -->\n";
				db_query("INSERT INTO $sql_tbl[product_links] SET productid1 = '$pid', productid2 = '$productid'");
			} elseif (!isset($blids[$pid]) && $bl_link) { 
//				echo "<!-- Remove BL link: $pid - $productid -->\n";
    ...
    if (!$selected_productid && $selected_sku) {
		if (strpos($selected_sku,",") !== false)
			$selected_skus = explode (", ", $selected_sku);
		else 
...
//		echo "<!--".var_export($selected_productids,true)."-->";
    }
