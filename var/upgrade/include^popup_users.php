...
#
# $Id: popup_users.php,v 1.1.2.4 2006/11/03 06:54:39 max Exp $
#
...
if (isset($HTTP_GET_VARS['format']))
	$popup_search_users_formats[$form] = $HTTP_GET_VARS['format'];

$format = empty($popup_search_users_formats[$form]) ? "~~firstname~~ ~~lastname~~ (~~email~~)" : $popup_search_users_formats[$form];

$advanced_options = array("usertype", "membershipid", "registration_type", "address_type", "phone", "url", "registration_date", "last_login_date");
...
function func_set_saved_users($users) {
	...
	$useids = array();
	foreach ($users as $u) {
		$useids[] = strtr($u['login'], array('\\'=>'\\\\',"'"=>"\\'",'"'=>'\\"',"\r"=>'\\r',"\n"=>'\\n','</'=>'<\/'));
	}
...
if ($mode == "save") {
	...
	# Save selected users
	if (!empty($user)) {
		$users = func_query("SELECT * FROM $sql_tbl[customers] WHERE login IN ('".implode("','", array_keys($user))."')");
