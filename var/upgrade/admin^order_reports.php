	...
	if (!empty($orders)) {
		...
		foreach ($orders as $k => $v) {
			...
			foreach ($orders[$k]["shipping_groups"] as $mid => $group) {
				if ((!empty($data["manufacturers"]) && !in_array($mid, $data["manufacturers"])) || (empty($data["include_margin_100"]) && $group["profit_margin"] == 100) || !in_array($group["status"], array('P','C','S','R'))) {
					unset($orders[$k]["shipping_groups"][$mid]);
				} else {
					$manufacturers[$mid] = $group["code"];
					$accounting_enabled = in_array($group["status"], array('P','C','S','R'));
					foreach ($price_details_names as $dn) {
