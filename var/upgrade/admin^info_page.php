...
$location[] = array(func_get_langvar_by_name("lbl_info_pages"), "");
...
#
	...
	#
		...
		if (is_array($posted_data)) {
			...
			foreach($posted_data as $pageid=>$v) {
				$page_data = func_query_first("SELECT * FROM $sql_tbl[info_pages] WHERE pageid='$pageid' AND categoryid = '$cat'");
				if (!empty($page_data) && !empty($v["to_delete"])) {
					@unlink(func_pages_dir($page_data["level"]).$page_data["filename"]);
					db_query("DELETE FROM $sql_tbl[info_pages] WHERE pageid='$pageid'");
					$deleted = true;
				}
			}
			$top_message["content"] = func_get_langvar_by_name("msg_adm_pages_del");
		}
		func_header_location("info_pages.php");
	}
	...
	#
		if (is_array($posted_data)) {
			foreach($posted_data as $pageid=>$v) {
				db_query("UPDATE $sql_tbl[info_pages] SET orderby='".intval($v["orderby"])."', active='$v[active]' WHERE pageid='$pageid' AND level = '$sec'");
			}
		}
	...
	#
		...
		if (!$fillerr) {
			...
			if (empty($pageid)) {
				...
				$filename = func_query_first_cell("SELECT filename FROM $sql_tbl[info_pages] WHERE pageid='$pageid'");
				if (func_query_first_cell("SELECT COUNT(*) FROM $sql_tbl[info_pages] WHERE pageid='$pageid' AND language = '$shop_language'") == 0) {
					$pageid = func_query_first_cell("SELECT pageid FROM $sql_tbl[info_pages] WHERE filename = '".addslashes($filename)."' AND language = '$shop_language'");
				}
			}
			...
			if (empty($pageid)) {
				db_query("INSERT INTO $sql_tbl[info_pages] (filename, title, level, orderby, active, language) VALUES ('$filename', '$pagetitle', '$level', '$orderby', '$active', '$current_language')");
				$pageid = db_insert_id();
				$top_message["content"] = func_get_langvar_by_name("msg_adm_pages_add");
			}
			else {
				db_query("UPDATE $sql_tbl[info_pages] SET title='$pagetitle', orderby='$orderby', active='$active' WHERE pageid='$pageid'");
		...
		func_header_location("info_page.php?pageid=$pageid");
	}
	...
	#
		...
		if (is_array($root_pages)) {
			$orderby = func_query_first_cell("SELECT MAX(orderby) FROM $sql_tbl[info_pages] WHERE level='R'");
			foreach ($root_pages as $k=>$file) {
				if (!preg_match("/^(.+)\/(.*)$/S", $file, $found))
					continue;
				$file = $found[2];

				if (!func_query_first("SELECT filename FROM $sql_tbl[info_pages] WHERE filename='$file' AND level='R'")) {
					$orderby += 10;
					db_query("INSERT INTO $sql_tbl[info_pages] (filename, title, orderby, active, language) VALUES ('$file', '".basename($file)."', '$orderby', 'Y', '$current_language')");
				}
			}
		}

		if (is_array($embedded_pages)) {
			$orderby = func_query_first_cell("SELECT MAX(orderby) FROM $sql_tbl[info_pages] WHERE level='E'");
			foreach ($embedded_pages as $k=>$file) {
				...
				if (!func_query_first("SELECT filename FROM $sql_tbl[info_pages] WHERE filename='$file' AND categoryid='#cat' AND language='$lang'")) {
					$orderby += 10;
					db_query("INSERT INTO $sql_tbl[info_pages] (filename, title, orderby, active, language) VALUES ('$file', '$file', '$orderby', 'Y', '$lang')");
				}
			}
		}
	}
