...
# Get brands list
	...
	if (!empty($active_modules['Multiple_Storefronts'])) {
		$sf_condition = 'WHERE ' . $sql_tbl['brands'] . '.storefrontid = ' . $current_storefront;
	} else {
		$sf_condition = '';
	}
	$brands = func_query("SELECT $sql_tbl[brands].brandid, IFNULL($sql_tbl[brands_lng].brand, $sql_tbl[brands].brand) as brand FROM $sql_tbl[brands] LEFT JOIN $sql_tbl[brands_lng] ON $sql_tbl[brands].brandid = $sql_tbl[brands_lng].brandid AND $sql_tbl[brands_lng].code = '$shop_language' $sf_condition ORDER BY $sql_tbl[brands].orderby, $sql_tbl[brands].brand");
