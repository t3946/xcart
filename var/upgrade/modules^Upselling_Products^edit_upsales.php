...
if ($mode == "upselling_links") {
    ...
    if (!$selected_productid && $selected_sku) {
		if (strpos($selected_sku,",") !== false) 
       	...
       	$product_sf = func_query_first_cell('SELECT storefrontid FROM ' . $sql_tbl['products'] . ' WHERE productid=' . $productid);

		if (!empty($active_modules['Multiple_Storefronts']) && is_numeric($product_sf)) {
			$prod_ids = func_query_column("SELECT productid FROM $sql_tbl[products] WHERE productcode IN ('".implode("', '", $selected_skus)."') AND storefrontid=$product_sf");
			$prod_ids_var = func_query_column("SELECT v.productid FROM $sql_tbl[variants] as v LEFT JOIN $sql_tbl[products] as p ON p.productid=v.productid WHERE v.productcode IN ('".implode("', '", $selected_skus)."') AND p.storefrontid=$product_sf");
