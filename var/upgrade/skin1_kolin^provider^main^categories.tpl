...
<tr{cycle values=', class="TableSubHead"'}>
	<td><input type="checkbox" name="ch_cat[]" value="{$catid}" /></td>
	<td width="1%"><input type="text" size="3" name="posted_data[{$catid}][order_by]" maxlength="3" value="{if $c.parentid neq $cat && $c.add_order_by}{$c.add_order_by}{else}{$c.order_by}{/if}" /></td>
	...
	<td align="center"><a href="javascript: var el = document.getElementById('rcat_{$catid}'); el.setAttribute('checked', 'checked'); submitForm(document.processcategoryform, 'update');" title="{$lng.lbl_categories_more}">{$lng.lbl_categories_more}</a></td>
