...
if (isset($_GET['phpinfo'])) {
	phpinfo();
	echo "<center><input type='button' value='Back' onclick='window.history.go(-1);'></center>";
	exit;
}
...
class CompatibilityChecker
{
	...
	function CompatibilityChecker()
	{
		...
		if ($shoppingCart = $this->getShoppingCartType()) {
			$this->shoppingCart = new $shoppingCart();
			
			$this->mysqlHost	= $this->shoppingCart->mysqlHost;
			$this->mysqlUser	= $this->shoppingCart->mysqlUser;
			$this->mysqlPass	= $this->shoppingCart->mysqlPass;
			$this->mysqlDb		= $this->shoppingCart->mysqlDb;
			
			$this->sslServer	= $this->shoppingCart->sslServer;
			$this->nosslServer	= $this->shoppingCart->nosslServer;
			
			$this->storeOwnerEmail = $this->shoppingCart->getAdminEmail();
	...
	function getShoppingCartType() {
		...
		{
			if (eregi("oscommerce",PROJECT_VERSION)) return "oscommerce";
			if (eregi("cre loaded",PROJECT_VERSION)) return "creloaded";
		}

		//For ZenCart
		if (defined('PROJECT_VERSION_NAME'))
		{
			if (eregi("zen cart", PROJECT_VERSION_NAME)) return "zencart";
		}
	...
	function getCheckMethods()
	{
		...
		$checkMethods['product_errors'][] = array(
				...
				"check_field"	=> $this->shoppingCart->productDescription
		);
		...
		$checkMethods['category_errors'][] = array(
				...
				"check_field"	=> $this->shoppingCart->categoryName
		);
		...
		$checkMethods['category_errors'][] = array(
				...
				"check_field"	=> $this->shoppingCart->categoryParentId
		);
 ...
 # End of tsergiy's modification
	...
	function check_MySQLVersion()
	{
		...
		if (is_object($this->shoppingCart)) {
			...
			if ($this->shoppingCart->mysqlError) {
	...
	function check_IonCubeLoader()
	{
		...
		if (!function_exists('_il_exec'))	{
			...
			$result['check_result_status'] = "BAD";
			$result['check_issue_solution'] = "Required only if Zend Optimizer can't be used. Please configure/check <a href='http://www.ioncube.com/faqs/loaders.php' target='_blank'>all settings/conditions</a> and request the ionCube encoded versions from <a href='mailto:".TECHNICAL_EMAIL."?subject=Shopping Cart Diagnostic Utility Results'>MagneticOne</a>." . $dl_check;
		}
	...
	{
		if (is_object($this->shoppingCart)) {
			$version = $this->shoppingCart->getShoppingCartVersion();
			...
			$result = array ('check_result'			=> 'Unknown',
							'check_result_status'	=> 'BAD',
							'check_issue_solution'	=> 'Can\'t find shopping cart');
		}
	...
	function check_ProductCount()
	{
		...
		$sql = "SELECT COUNT(" . $this->shoppingCart->fields['product_id'] . ") AS total " .
			"FROM " . $this->shoppingCart->tables['products'];
	...
	function check_CategoryCount()
	{
		...
		$sql = "SELECT COUNT(" . $this->shoppingCart->fields['category_id'] . ") AS total " .
			"FROM " . $this->shoppingCart->tables['categories'];
	...
	function check_ManufacturerCount()
	{
		...
		$sql = "SELECT COUNT(" . $this->shoppingCart->fields['manufacturer_id'] . ") AS total " .
			"FROM " . $this->shoppingCart->tables['manufacturers'];
	...
	{
		return $this->check_Field($this->shoppingCart->productName, " LIKE '%<%>%'", 'Remove HTML tags from product names');
	}
	
	function check_HtmlInProductModels()
	{
		return $this->check_Field($this->shoppingCart->productModel, " LIKE '%<%>%'", 'Remove HTML tags from product models');
	}
	
	function check_HtmlInProductPrices()
	{
		return $this->check_Field($this->shoppingCart->productPrice, " LIKE '%<%>%'", 'Remove HTML tags from product prices');
	}
	
	function check_ProductPricesByNull()
	{
		return $this->check_Field($this->shoppingCart->productPrice, ' IS null', 'Set valid prices for products');
	}
	
	function check_ProductNamesByEmpty()
	{
		return $this->check_Field($this->shoppingCart->productName, ' IS null OR ' . $this->shoppingCart->productName . " = ''", 'Set valid names for products');
	}
	
	function check_NonPrintableDescriptionChars()
	{
		$errProducts = array();
		$sql = $this->shoppingCart->selectProducts($this->shoppingCart->productDescription);
		$query = $this->sqlQuery($sql);

		while ($dataArray = mysql_fetch_assoc($query)) {
			preg_match_all('/.{8}[^[:print:]\r\n\t].{8}/u', $dataArray[$this->shoppingCart->productDescription], $matches);

			if (sizeof($matches[0])) {
				
				foreach ($matches[0] as $badChar) {
					$dataArray[$this->shoppingCart->productDescription] = htmlspecialchars($dataArray[$this->shoppingCart->productDescription]);
					$badChar = htmlspecialchars($badChar);
					$dataArray[$this->shoppingCart->productDescription] = str_replace($badChar, '<font color="red">' . $badChar . '</font>',
						$dataArray[$this->shoppingCart->productDescription]);
				}
	...
	{
		return $this->check_Field($this->shoppingCart->categoryName, " LIKE '%<%>%'", 'Remove HTML tags from category names');
	}
	
	function check_CategoryNamesByEmpty()
	{
		return $this->check_Field($this->shoppingCart->categoryName, ' IS null OR ' . $this->shoppingCart->categoryName . " = ''", 'Set names for categories');
	}
	
	function check_ParentCategoriesByNull()
	{
		return $this->check_Field($this->shoppingCart->categoryParentId, ' IS null', 'Set parent ID for categories, parent category is null now');
	}
	
	function check_ParentCategoriesByExistance()
	{
		return $this->check_Field('c2.' . $this->shoppingCart->categoryId,
			' IS null AND c.' . $this->shoppingCart->categoryParentId . ' > 0',
			'Set correct parent ID for categories, parent category doesn\'t exist');
	}
	
	function check_ParentCategoriesBySelf()
	{
		return $this->check_Field($this->shoppingCart->categoryParentId, ' = ' . $this->shoppingCart->categoryId,
			'Change parent ID for categories, this and parent category id\'s are equal now');
	}
	
	function check_ParentCategoriesByCycling()
	{
		return $this->check_Field('c2.' . $this->shoppingCart->categoryParentId,
			' = c.' . $this->shoppingCart->categoryId .
			' AND c.' . $this->shoppingCart->categoryParentId . ' <> c.' . $this->shoppingCart->categoryId,
	...
	function check_ParentCategoriesByInteger()
	{
		...
		//return $this->check_Field($this->shoppingCart->categoryParentId, ' <> CONVERT(CAST(' . $this->shoppingCart->categoryParentId . ' AS UNSIGNED), CHAR)',
		
		//This query works on MySQL version >= 3.23
		return $this->check_Field($this->shoppingCart->categoryParentId, ' <> CONCAT(' . $this->shoppingCart->categoryParentId . ' + 0, \'\')' ,
	...
	function check_PhysicalPaths()
	{
		...
		if (defined('DIR_FS_ADMIN')) {
			...
			$pathOption = $this->shoppingCart->getAdminConfigurationPaths();
		
		} else {
			
			//this is store configuration
			$pathOption = $this->shoppingCart->getStoreConfigurationPaths();
		}
	...
	function check_ConfigurationOptions()
	{
		...
		if (defined('DIR_FS_ADMIN')) {
			...
			//this is osCommerce administration area
			$keys = $this->shoppingCart->getAdminConfigurationKeys();
		
		} else {
			
			//this is store configuration
			$keys = $this->shoppingCart->getStoreConfigurationKeys();
		}
	...
	function check_AdminEmail()
	{
		...
		} elseif (eregi('localhost\.([a-z]>?)*$', $adminEmail)) {
	...
	function check_CatalogDir()
	{
		...
		if (defined('DIR_FS_ADMIN')) {
			...
			if (strpos($this->shoppingCart->catalogDir, $rootDir) === 0) {

				if (file_exists($documentRoot . $this->shoppingCart->catalogDir . 'includes/application_top.php')) {
					...
					foreach ($checkFiles as $val) {
						...
						if (file_exists($documentRoot . $this->shoppingCart->catalogDir . $val)) {
							$flagResult = true;
							break;
						}
					}
			...
			if ($this->shoppingCart->catalogDir == $catalogDir) {
				$flagResult = true;
			}
		}
		
		if ($flagResult) {
			$result['check_result'] = $this->shoppingCart->catalogDir;
			$result['check_result_status'] = "OK";
			$result['check_issue_solution'] = "";
			...
			$result['check_result'] = $this->shoppingCart->catalogDir;
			$result['check_result_status'] = "BAD";
			$result['check_issue_solution'] = "Check correctness of {$this->shoppingCart->configCatalogDir} configuration value";
		}
	...
	function check_WritableFolders()
	{
		...
		$writableFolders = $this->shoppingCart->getWritableFolders();
		$problems = array();
	...
	function check_KeyFiles()
	{
		...
		if (is_array($this->shoppingCart->foreignFiles)) {
			
			foreach ($this->shoppingCart->foreignFiles as $file) {
		...
		foreach ($keys as $key => $value) {
			...
			if (defined($key)) {
				...
				if ($value['mask']) {
					...
					if (!eregi($value['mask'], $value['value'])) {
	...
	function check_Field($field, $condition, $solution)
	{
		...
		switch ($field) {
			...
			case $this->shoppingCart->productName:
			case $this->shoppingCart->productModel:
			case $this->shoppingCart->productPrice:
				$sql = $this->shoppingCart->selectProducts($field, $condition);
				break;
				
			case $this->shoppingCart->categoryName:
			case $this->shoppingCart->categoryParentId:
				$sql = $this->shoppingCart->selectCategories($field, $condition);
				break;
				
			case 'c2.' . $this->shoppingCart->categoryId:
			case 'c2.' . $this->shoppingCart->categoryParentId:
				$sql = $this->shoppingCart->selectCategories($field, $condition, $this->shoppingCart->categoryParentId);
				break;
    ...
    function outputProblems($products, $testField)
    {
    	...
    	if (is_object($this->shoppingCart)) {
    		$codeFieldConditionArray = array($this->shoppingCart->productId, $this->shoppingCart->categoryId, 'Option', 'Folder');
    		$testFieldCondition = ($testField == $this->shoppingCart->productDescription);
	...
	{
		$matches = array();
		preg_match('(\\d+)', '$Revision: 44 $', $matches);
		return $matches[0];
	}
	...
	{
		if (is_object($this->shoppingCart)) {
			$this->shoppingCart->sessionSave($variable);
	...
	{
		if (is_object($this->shoppingCart)) {
			$this->shoppingCart->sessionLoad($variable);
...
class oscommerce
{
	...
	function selectProducts($field = '', $condition = '')
	{
		...
		switch ($field) {
			...
			default:
				$selectFields = 'products_id, products_model, products_quantity, products_image, products_price, products_weight, products_status';
				$table = TABLE_PRODUCTS;
				break;
		}
		
		return "SELECT $selectFields FROM " . $table . $where;
	}
...
class creloaded extends oscommerce
{
	
	function creloaded(){
		return $this->oscommerce();
	}
	...
	function getStoreConfigurationKeys()
	{
		$cfgKeys = oscommerce::getStoreConfigurationKeys();
	...
	function getAdminConfigurationKeys()
	{
		$cfgKeys = oscommerce::getAdminConfigurationKeys();
...
class zencart extends oscommerce
{
	...
	function getAdminConfigurationPaths()
	{
		$cfgKeys = oscommerce::getAdminConfigurationPaths();
