...
<tr>
	<td><span style="font-size: 13px; color: #000000;">{if $product.fulldescr ne ""}{$product.fulldescr}{else}{$product.descr}{/if}</span></td>
{*
{if $current_price gt 0 and $product.list_price gt 0}
	<td align="right" valign="top" width="60" id="save_percent_box"{if $current_price >= $product.list_price} style="display: none;"{/if}>
...
{math equation="100-(price/lprice)*100" price=$current_price lprice=$product.list_price format="%3.0f" assign=discount}  
...
<tr>
	<td width="30%" class="BlackT">{$product.upc_ean_isbn.type}:</td>
	<td nowrap="nowrap">{$product.upc_ean_isbn.value}</td>
</tr>
{/if}
{if $config.Appearance.show_in_stock eq "Y" and $config.General.unlimited_products ne "Y" and $product.distribution eq "" && $product.avail <= $config.Appearance.quantity_threshold}
<tr>
	<td width="30%" class="BlackT">{$lng.lbl_in_stock}:</td>
...
{if $show_dimensions}
<tr>
	<td width="30%">{$lng.lbl_shipping_dimensions}:</td>
...
<font class="ProductDetailsTitle"><span id="product_price" style="white-space: nowrap;">{include file="currency.tpl" value=$current_price plain_text_message=true}</span></font>
...
{/if}
{if $product.taxes}<br />{include file="customer/main/taxed_price.tpl" taxes=$product.taxes}
{/if}
{else}
...
{/if}
<tr><td height="25" width="30%" class="BlackT">{$lng.lbl_quantity}:</td>
...
{*<br />{$lng.lbl_shipping_charge_per_item}: <font class="ProductDetailsTitle">{include file="currency.tpl" value=$product.shipping_freight}</font><br /><br />*}
{else}
{*<br />{$lng.txt_message_for_zero_charge|replace:"XX":"`$config.Company.location_city`, `$company_state`, `$company_country`"}<br /><br />*}
{/if}
<br />{$product.cart_manufact_text_displayed}<br /><br />
...
<tr>
	...
	<td id="add2cart_{$product.productid}">{include file="buttons/buy_now.tpl" style="button" href="javascript: if ('`$config.General.opt_ajax_cart`' == 'Y') ajax_add_to_cart(`$product.productid`, `$product.add_date`, 'product'); else document.orderform.submit();" b=1 class="ajax_button"}
	{*
...
{/capture}
{include file="dialog.tpl" title=$product.producttitle content=$smarty.capture.dialog extra='width="100%"' product=$product save_label="true" product_sku=$product.productcode product_free_ship=$product.free_ship_text}
{/if}
