...
if ($REQUEST_METHOD == "POST" || ($mode == "delete_image" && $brandid)) {
	...
	#
        ...
        if (is_array($HTTP_POST_FILES) && isset($HTTP_POST_FILES['edit_image'])) {
            $id = $brandid;
...
#
	...
	if (!empty($brandid)) {
		$brand_data = func_query_first("SELECT $sql_tbl[brands].*, IF($sql_tbl[images_B].id IS NULL, '', 'Y') as is_image, IFNULL($sql_tbl[brands_lng].brand, $sql_tbl[brands].brand) as brand, IFNULL($sql_tbl[brands_lng].descr, $sql_tbl[brands].descr) as descr FROM $sql_tbl[brands] LEFT JOIN $sql_tbl[brands_lng] ON $sql_tbl[brands_lng].brandid = $sql_tbl[brands].brandid AND $sql_tbl[brands_lng].code = '$shop_language' LEFT JOIN $sql_tbl[images_B] ON $sql_tbl[images_B].id = $sql_tbl[brands].brandid WHERE $sql_tbl[brands].brandid = '$brandid'");
