...
foreach($start_dates as $start_date) {
	...
	if (!empty($active_modules['Multiple_Storefronts'])) {
		$ordered_products = func_query("SELECT $sql_tbl[order_details].productid, $sql_tbl[products].productcode,"
			...
			. " AND $sql_tbl[orders].status NOT IN ('F','D') AND $sql_tbl[order_details].productid = $sql_tbl[products].productid"
...
#
$last_order = func_query_first("SELECT orderid, order_prefix, status, total, title, firstname, lastname, date FROM $sql_tbl[orders] ORDER BY date DESC LIMIT 1");
