...
# Export data	
	...
	while (($id = func_export_get_row($data)) !== false) {
		...
		$row = func_query_first("SELECT javascript_code as jscript FROM $sql_tbl[product_options_js], $sql_tbl[products] WHERE $sql_tbl[product_options_js].productid = $sql_tbl[products].productid $sf_condition AND $sql_tbl[product_options_js].productid = '$id'".(empty($provider_sql) ? "" : " AND $sql_tbl[products].provider = '$provider_sql'"));
		if (empty($row))
			continue;
