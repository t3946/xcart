...
if ($REQUEST_METHOD=="POST") {
	if ($mode == "add_gc" || $mode == "modify_gc" || $mode == "preview") {
		...
		if (empty($gc_template) || !func_allowed_path($smarty->template_dir."/modules/Gift_Certificates/", $smarty->template_dir."/modules/Gift_Certificates/".$gc_template)) {
...
elseif ($mode == 'print') {
	...
	$expired_condition=($config["Gift_Certificates"]["gc_show_expired"]=="Y"?"":" and g.status!='E'");

	$giftcerts=func_query("SELECT g.*,  o.order_prefix, g.add_date+'".$config["Appearance"]["timezone_offset"]."' as add_date FROM $sql_tbl[giftcerts] as g LEFT JOIN $sql_tbl[orders] as o ON g.orderid=o.orderid where 1 $expired_condition");
