...
#
# $Id: anti_fraud.php,v 1.31.2.1 2006/05/11 06:47:26 max Exp $
#

if ( !defined('XCART_START') ) { header("Location: ../../"); die("Access denied"); }

x_load('http','mail','tests');
...
elseif (empty($config['Anti_Fraud']['anti_fraud_license'])) {
	...
	# Big order
	if ($config['Anti_Fraud']['anti_fraud_order_limit'] > 0 && $order_total > 0 && $order_total > $config['Anti_Fraud']['anti_fraud_order_limit']) {
		$multi = $multi * 2;
		$added_data['order_limit_excess'] = '1';
	}

	# Has processed orders
	if (func_query_first_cell("SELECT COUNT(*) FROM $sql_tbl[orders] WHERE status IN ('P','C') AND login = '$userinfo[login]'") > 0) {
		$multi = $multi / 2;
		$added_data['completed_orders'] = '1';
	}

	# Has cancelled orders
	if (func_query_first_cell("SELECT COUNT(*) FROM $sql_tbl[orders] WHERE status IN ('D','F') AND login = '$userinfo[login]'") > 0) {
		$multi = $multi * 1.5;
		$added_data['declined_orders'] = '1';
	}

	# Another name from this IP
	if (func_query_first_cell("SELECT COUNT(*) FROM $sql_tbl[orders], $sql_tbl[order_extras] WHERE $sql_tbl[order_extras].orderid = $sql_tbl[orders].orderid AND $sql_tbl[order_extras].khash = 'ip' AND $sql_tbl[order_extras].value = '$REMOTE_ADDR' AND $sql_tbl[orders].login <> '$userinfo[login]'") > 0) {
		$multi = $multi * 2;
	...
	if (strpos($header, "200 OK") !== FALSE) {
		...
		if ($af_result['available_request'] == $af_result['used_request'] && $af_result['available_request'] == 0 && $af_result['error'] == "NOT_AVAILABLE_SERVICE") {
			$extra['Anti_Fraud']['result'] = "sk_invalid";
			if ($config['Anti_Fraud']['eml_af_sk_invalid'] == 'Y')
				func_send_mail($config["Company"]["orders_department"], "mail/anti_fraud_sk_invalid_subj.tpl", "mail/anti_fraud_sk_invalid.tpl", $config["Company"]["orders_department"], false);
		}
		elseif ($af_result['available_request'] <= $af_result['used_request']) {
			...
			if ($config['Anti_Fraud']['eml_af_sk_expire'] == 'Y')
				func_send_mail($config["Company"]["orders_department"], "mail/anti_fraud_sk_expire_subj.tpl", "mail/anti_fraud_sk_expire.tpl", $config["Company"]["orders_department"], false);
		}
