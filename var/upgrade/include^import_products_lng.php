...
#
# $Id: import_products_lng.php,v 1.10.2.2 2006/11/15 09:25:33 max Exp $
#
...
#
	...
	while ($productid = func_export_get_row($data)) {
		...
		if (!empty($active_modules['Multiple_Storefronts'])) {
			$row = func_query_first("SELECT pl.* FROM $sql_tbl[products_lng] as pl LEFT JOIN $sql_tbl[products] as p ON p.productid=pl.productid  WHERE pl.productid = '$productid' AND pl.code = '$current_code' AND p.storefrontid=$current_storefront");
		} else {
		...
		$row['fulldescr'] = $row['fulldescr'];

		func_unset($row, "product", "fulldescr");
