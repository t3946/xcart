...
if (!empty($active_modules['Multiple_Storefronts'])) {
	$sf_condition = " AND $sql_tbl[brands].storefrontid=$current_storefront";
} else {
	$sf_condition = '';
}
...
if ($brandid) {
	...
	$brand = func_query_first("SELECT $sql_tbl[brands].*, IF($sql_tbl[images_B].id IS NULL, '', 'Y') as is_image, $sql_tbl[images_B].image_path, IFNULL($sql_tbl[brands_lng].brand, $sql_tbl[brands].brand) as brand, IFNULL($sql_tbl[brands_lng].descr, $sql_tbl[brands].descr) as descr FROM $sql_tbl[brands] LEFT JOIN $sql_tbl[brands_lng] ON $sql_tbl[brands].brandid = $sql_tbl[brands_lng].brandid AND $sql_tbl[brands_lng].code = '$shop_language' LEFT JOIN $sql_tbl[images_B] ON $sql_tbl[brands].brandid = $sql_tbl[images_B].id WHERE $sql_tbl[brands].brandid = '$brandid'$sf_condition ORDER BY $sql_tbl[brands].orderby");
	...
	$total_items = func_query_first_cell("SELECT COUNT(*) FROM $sql_tbl[brands] WHERE avail = 'Y'$sf_condition");
	if ($total_items > 0) {
		$objects_per_page = $config["Brands"]["brands_per_page"];
		$total_nav_pages = ceil($total_items/$objects_per_page)+1;
		include $xcart_dir."/include/navigation.php";
		$brands = func_query("SELECT $sql_tbl[brands].*, IFNULL($sql_tbl[brands_lng].brand, $sql_tbl[brands].brand) as brand, IFNULL($sql_tbl[brands_lng].descr, $sql_tbl[brands].descr) as descr FROM $sql_tbl[brands] LEFT JOIN $sql_tbl[brands_lng] ON $sql_tbl[brands].brandid = $sql_tbl[brands_lng].brandid AND $sql_tbl[brands_lng].code = '$shop_language' WHERE avail = 'Y'$sf_condition ORDER BY $sql_tbl[brands].orderby, brand LIMIT $first_page, $objects_per_page");
