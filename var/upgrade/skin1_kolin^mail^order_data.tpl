...
{if $products}{$v.group_name} {$lng.lbl_items} ({$lng.lbl_delivery_by} {$v.shipping|trademark:''}, {include file="currency.tpl" value=$v.shipping_cost.gross|default:0}):{/if}
...
{if $tr.tracknum ne ""}
{$lng.eml_order_shipped|substitute:"shipper":$tracking_links[$tr.linkid].shipping|substitute:"distributor":$v.group_name}
{$lng.lbl_tracking_number_is} {$tr.tracknum}
{$tracking_links[$tr.linkid].link|substitute:"tracknum":$tr.tracknum}
{else}
{$lng.eml_order_shipped_nolink|substitute:"shipper":$tracking_links[$tr.linkid].shipping}
...
{$v.group_name} {$lng.lbl_items} ({$lng.lbl_delivery_by} {if $giftcerts[giftcert].send_via eq "E"}{$lng.lbl_email}{else}{$lng.lbl_gc_postal_mail}{/if}, {include file="currency.tpl" value=$v.shipping_cost.gross|default: 0}):
