...
#
# $Id: cc_authorizenet_sim.php,v 1.29.2.2 2007/01/05 13:20:34 max Exp $
#

if (!isset($REQUEST_METHOD))
	$REQUEST_METHOD = $HTTP_SERVER_VARS["REQUEST_METHOD"];

if ($REQUEST_METHOD == "POST" && $HTTP_POST_VARS["x_response_code"]) {

	require "./auth.php";

	$bill_output["sessid"] = func_query_first_cell("SELECT sessionid FROM $sql_tbl[cc_pp3_data] WHERE ref='".$HTTP_POST_VARS["x_invoice_num"]."'");
...
#x_trans_id 29378-1382296
	...
	$url = $current_location."/payment/cc_authorizenet_sim.php";

	$sequence = rand(1, 1000);
	$tstamp = (time()-(int)$module_params["param06"]*3600);
	$fingerprint = hmac ($txnkey, $loginid . "^".$sequence."^".$tstamp."^".$amount."^".$currencycode);
...
<form action="https://secure.authorize.net/gateway/transact.dll" method="POST" name=process>
<input type=hidden name=x_fp_sequence value="<?php echo htmlspecialchars($sequence); ?>">
<input type=hidden name=x_fp_timestamp value="<?php echo htmlspecialchars($tstamp); ?>">
<input type=hidden name=x_fp_hash value="<?php echo htmlspecialchars($fingerprint); ?>">
<input type=hidden name=x_Show_Form value="PAYMENT_FORM">
<input type=hidden name=x_Test_Request value="<?php echo htmlspecialchars($module_params["testmode"] == "N" ? "FALSE" : "TRUE"); ?>"> 
<input type=hidden name=x_Login value="<?php echo htmlspecialchars($loginid); ?>">
<input type=hidden name=x_Amount value="<?php echo htmlspecialchars($cart["total_cost"]); ?>">
<input type=hidden name=x_Currency_Code value="<?php echo htmlspecialchars($currencycode); ?>">
<input type=hidden name=x_Card_Num value="<?php echo htmlspecialchars($userinfo["card_number"]); ?>">
<input type=hidden name=x_Exp_Date value="<?php echo htmlspecialchars($userinfo["card_expire"]); ?>">
<input type=hidden name=x_Method value="<?php echo "CC"; ?>">
<input type=hidden name=x_Type value="<?php echo "AUTH_CAPTURE"; ?>">
<input type=hidden name=x_First_Name value="<?php echo htmlspecialchars($bill_firstname); ?>">
<input type=hidden name=x_Last_Name value="<?php echo htmlspecialchars($bill_lastname); ?>">
<input type=hidden name=x_Phone value="<?php echo htmlspecialchars($userinfo["phone"]); ?>">
<input type=hidden name=x_Company value="<?php echo htmlspecialchars($userinfo["company"]); ?>">
...
<input type=hidden name=x_ship_to_First_Name value="<?php echo htmlspecialchars($userinfo["s_firstname"]); ?>">
<input type=hidden name=x_ship_to_Last_Name value="<?php echo htmlspecialchars($userinfo["s_lastname"]); ?>">
<input type=hidden name=x_ship_to_Address value="<?php echo htmlspecialchars($userinfo["s_address"]); ?>">
<input type=hidden name=x_ship_to_City value="<?php echo htmlspecialchars($userinfo["s_city"]); ?>">
<input type=hidden name=x_ship_to_State value="<?php echo htmlspecialchars($userinfo["s_state"]?$userinfo["s_state"]:"n/a"); ?>">
<input type=hidden name=x_ship_to_Zip value="<?php echo htmlspecialchars($userinfo["s_zipcode"]); ?>">
<input type=hidden name=x_ship_to_Country value="<?php echo htmlspecialchars($userinfo["s_country"]); ?>">
<input type=hidden name=x_Invoice_num value="<?php echo htmlspecialchars($_orderids); ?>">
<input TYPE=HIDDEN NAME="x_Relay_Response" VALUE="True">
<input TYPE=HIDDEN NAME="x_Relay_URL" VALUE="<?php echo htmlspecialchars($url); ?>">
</form>
<table width=100% height=100%>
         <tr><td align=center valign=middle>Please wait while connecting to <b>Authorize.Net</b> payment gateway...</td></tr>
</table>
</body>
</html>
<?php 
