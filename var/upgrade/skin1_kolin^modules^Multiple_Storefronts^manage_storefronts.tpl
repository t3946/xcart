...
<tr class="TableHead">
	...
	<td>{$lng.lbl_mf_main_domain}</td>
</tr>

{if $storefronts}
	{foreach from=$storefronts item=storefront}
	<tr{cycle values=", class='TableSubHead'"}>
		<td><input type="checkbox" name="delete[{$storefront.storefrontid}]" value="Y" /></td>
		<td><input type="text" size="4" name="update[{$storefront.storefrontid}][orderby]" value="{$storefront.orderby}" /></td>
		<td align="center">{$storefront.config.opt_order_prefix}</td>
		<td><a href="configuration.php?option=Multiple_Storefronts&amp;sf={$storefront.storefrontid}">{$storefront.config.company_name|escape}</a></td>
		<td><a href="http://{$storefront.domain}" target="_blank" title="">{$storefront.domain}</a></td>
	</tr>
	{/foreach}
	<tr>
		<td colspan="5">
