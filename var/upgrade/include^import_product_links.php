...
#
# $Id: import_product_links.php,v 1.5.2.1 2006/06/15 07:01:23 max Exp $
#
...
#
	...
	#
		...
		foreach ($row['productid2'] as $k => $v) {
			...
			# Update product link
			if (func_query_first_cell("SELECT COUNT(*) FROM $sql_tbl[product_links] WHERE productid1 = '$row[productid1]' AND productid2 = '$k'")) {
...
# Export data	
	...
	while ($id = func_export_get_row($data)) {
		...
		if (!empty($active_modules['Multiple_Storefronts'])) {
			$mrow = func_query("SELECT $sql_tbl[product_links].* FROM $sql_tbl[product_links], $sql_tbl[products] WHERE $sql_tbl[products].storefrontid=$current_storefront AND $sql_tbl[product_links].productid1 = $sql_tbl[products].productid AND $sql_tbl[product_links].productid1 = '$id'".(empty($provider_sql) ? "" : " AND $sql_tbl[products].provider = '$provider_sql'"));
		} else {
		$mrow = func_query("SELECT $sql_tbl[product_links].* FROM $sql_tbl[product_links], $sql_tbl[products] WHERE $sql_tbl[product_links].productid1 = $sql_tbl[products].productid AND $sql_tbl[product_links].productid1 = '$id'".(empty($provider_sql) ? "" : " AND $sql_tbl[products].provider = '$provider_sql'"));
		}
		if (empty($mrow))
			continue;
