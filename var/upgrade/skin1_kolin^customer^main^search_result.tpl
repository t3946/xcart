...
<input type="text" name="posted_data[substring]" size="30" style="width:70%" value="{$search_prefilled.substring|stripslashes|escape}" />
&nbsp;
...
{foreach from=$search_categories item=v}
		<option value="{$v.categoryid}" {if $search_prefilled.categoryid eq $v.categoryid}selected="selected"{/if}>{$v.category_path}</option>
{/foreach}
	</select>
	</td>
</tr>
