...
class QueueImagesActiveCommand extends Command
{
    ...
    public function consume(AMQPMessage $message): void
    {
        ...
        if ($message->body && $data = json_decode($message->body, true, 512, JSON_THROW_ON_ERROR)) {
            ...
            [$image] = ProductImageModel::objects()->getOrCreate(
                ['hash' => $data['image_hash']]
            );

            [$link] = ProductImageLinkModel::objects()->getOrNew(
                [
            ...
            if ($link->getChangedAttributes()) {
                echo "Image link has changed\n";
                echo "==============Queue data=============\n";
                print_r($data);
                echo "==============Changed image data=============\n";
                print_r($link->getChangedAttributes());
            }
    ...
    {
        try {
            [$model, $is_new] = ProductImagesModel::objects()->getOrNew([
