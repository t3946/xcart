...
class GuzzleDownloader implements DownloaderInterface
{
    ...
    public function __construct($params = [], $headers = [])
    {
        ...
        $this->client->timeout = $params['timeout'] ?? $this->client->timeout;

        $this->initProxy();

        $this->client->headers['User-Agent'] = $this->getUserAgent();
        foreach ($headers as $key => $header) {
            $this->client->headers[$key] = $header;
        }
        $this->client->setServerParameter('HTTP_USER_AGENT', $this->client->headers['User-Agent']);
        $this->auth = $this->processAuth();
    ...
    public function getProxy($is_new = false): string
    {
        $proxy_invalid = null;
        $res = null;

        if (!$is_new && $this->current_proxy) {
            return $this->current_proxy;
        }

        if (!$this->proxies) {
            $this->proxies = Parser::getProxies();
        }
        if ($this->proxies) {
            while (!$proxy_invalid) {
                $this->current_proxy = $this->proxies[$proxy_index = random_int(0, count($this->proxies) - 1)];

                try {
                    print "Check proxy {$this->current_proxy} ";

                    //$proxy_checker = new ProxyChecker($url = 'https://getbootstrap.com/');
                    //$proxy_checker->checkProxy($this->current_proxy);

                    print "Success. Use {$this->current_proxy}" . "\n";
                    $proxy_invalid = true;
                } catch (Exception $e) {
                    print "Error: {$e->getMessage()}\n";
                    unset($this->proxies[$proxy_index]);
                }
            }
            return $this->current_proxy ?? '';
        }
    ...
    {
        if (($this->params['use_proxy'] ?? false) === true) {
            $this->client->proxy = $this->getProxy(true);
        }
    }

    public function fetch($url, $type = 'GET', $params = [], $files = [])
    {
        if (($crawler = $this->get($url, $type, $params, $files)) && !$this->auth && !$this->checkLogin($crawler)) {
            if ($this->processAuth($crawler)) {
                $crawler = $this->get($url, $type, $params);
            } else {
                return null;
            }
        }
    ...
    {
        try {
            $crawler = $this->client->request($type, $url, array_merge($params, ['http_errors' => false]), $files);
            if ($crawler->count()) {
                //Storage::disk('local')->put('logs/response.html', $crawler->html());
            }
            $status = $this->client->getResponse()->getStatus();
            if (!in_array($status, [200, 404], true )) {
                //Storage::disk('local')->put('errors/response.html', $crawler->html());
    ...
    public function massFetch($urls)
    {
        $client = $this->client->getClient();
        $requests = function () use ($client, $urls) {
            foreach ($urls as $uri) {
                $cookies = CookieJar::fromArray(
                    $this->client->getCookieJar()->allRawValues($uri),
                    parse_url($uri, PHP_URL_HOST)
                );
                yield function () use ($client, $uri, $cookies) {
                    return $client->getAsync($uri, ['allow_redirects' => true, 'cookies' => $cookies]);
                };
            }
        };

        return $this->client->pool($requests());

    }

    public function direct_request($url, $type = 'GET', $params = [])
    {
        return $this->client->getClient()->request($type, $url, $params);
    }
    ...
    protected function processAuth(Crawler $crawler = null): bool
    {
        if ($this->getAuthUrl() && $this->getAuthInfo()) {
            if ($this->params['csrf'] ?? '') {
                if (!$crawler) {
                    $crawler = $this->get($this->getAuthUrl());
                }
                $p = $this->params['csrf'];
                if ($crawler && ($v = $crawler->filter("input[name='{$p}']")->first()) && $v->count()) {
                    $csrf = [$v->attr('name') => $v->attr('value')];
                }
            }

            $crawler_n = $this->client->request('POST', $this->getAuthUrl(), array_merge($this->getAuthInfo(), $csrf ?? []));
            return $this->checkLogin($crawler_n);
        }
        return false;
    }

    private function getCheckLoginText()
    {
        return $this->params['checkLoginText'] ?? null;
    }

    public function getAuthUrl()
    {
        return $this->params['authUrl'] ?? null;
    }

    public function getAuthInfo()
    {
        return $this->params['authInfo'] ?? [];
    }
    ...
    public function checkLogin(Crawler $crawler): bool
    {
        ...
        if ($this->getCheckLoginText() && $crawler->count()) {
            if (strpos($crawler->text(), $this->getCheckLoginText()) !== false) {
                return true;
            }
        } else {
            return true;
