<span class='select-holder'>
        <select name="{$name}" id="{$id}" {raw $html}>
            {if $field->empty}
                <option {if !$field->getSelected()}selected="selected"{/if} style="font-weight: bold" value="{$field->empty}">{$field->empty}</option>
            {/if}
            {foreach $field->getChoices() as $key => $field_name}
                <option {if strpos($field_name, '..') === false} style="font-weight: bold" {/if} value="{$key}"
