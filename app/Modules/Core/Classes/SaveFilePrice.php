...
class SaveFilePrice
{
    /** @var int Время выполнения запроса */
    public int $time_exec = 0;
    /** @var int Количество отправленных записей */
    ...
    public function sendStats(): void
    {
        if (filter_var($this->need_send, FILTER_VALIDATE_BOOLEAN)) {
            $ar_active = [
                ...
                'feed_source' => 'manual',
                'products_in_feed' => $this->count_send,
    ...
    public function collectField(array $select, $ar_save): void
    {
        ...
        foreach ($select as $table_index => $ar_key) {
            ...
            foreach ($ar_key as $key => $field) {
                ...
                if ($field === 'images') {
                    $this->ar_update_field[$table_index]['images'][] = $value;
//                    $this->fields_image[$table_index][] = "{$field}_$this->count_image_field";
    ...
    private function collectProductData(?ProductModel $product_model, int $t_index, int $num_row): ?array
    {
        ...
        if (!empty($this->ar_update_field[$t_index]['images'])) {
    ...
    {
        $ar_images = array_column($this->ar_update_field[$table_index]['images'], $num_row);

        if ($ar_images = array_filter($ar_images)) {
            ...
            Xcart::app()->queue->send('images', json_encode($ar_images_rabbit, JSON_THROW_ON_ERROR), true);
        }
    }
}