...
<head>
    ...
    <script src="{$SkinDir}/verificator/js/verificator_new.js" type="text/javascript"></script>
...
<header id="header">
    <div class="buttons-wrap-left">
        ...
        <div class="wrap-left">
            <div class="three ui buttons amazon-search">
                <span style="font-family:Verdana; line-height: 50px; margin-right: 15px; font-size: 18px;  position: relative; ">Compare</span>
                {if $links['asin']}
                    <button data-step-id="0" id="search_amazon_by_asin" style="display: block" class="ui button attached left"
                            data-url="{$links['asin'][0]}"
                            data-title="{$links['asin'][1]|escape}">
                        Open product by ASIN
                    </button>
                {/if}
                {if $links['upc']}
                    <button data-step-id="1" id="search_amazon_by_upc" style="display: block" class="ui attached button {if !$links['asin']}left{/if}" data-url="{$links['upc'][0]}" data-title="{$links['upc'][1]|escape}">
                        Search product by UPC
                    </button>
                {/if}
                {if $links['name']}
                    <button data-step-id="2" id="search_amazon_by_name" style="display: block" class="ui right attached button" data-url="{$links['name'][0]}" data-title="{$links['name'][1]|escape}">
                        Search product by Product Name
                    </button>
                {/if}
                <span style="font-size: 18px; margin-left: 25px; margin-top:14px; font-family:Verdana;">AND</span>
                <span style="margin-left: 25px;">
                <button data-step-id="3" id="original_product" class="ui left attached button" data-url="{$model->getAbsoluteUrl(true)}">Original product</button>
                </span>
            </div>
        </div>
    </div>
    <div class="buttons-wrap-right">
        ...
        <div class="buttons-right">
            ...
            <div data-asin="" data-batch-id="{$order->orderid}"
                 data-product-id="{$model->productid}" class="ui buttons select"
