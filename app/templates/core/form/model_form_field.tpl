    ...
    <div class="row">
        ...
        <div class="columns large-6">
            ...
            {elseif $type == 'checkbox'}
                ...
                <input type="{if $type}{$type}{else}text{/if}" name="{$model->classNameShort()}[{$field}]" id="m_{$field}" value="1" class="{$class}" {if $model->getField($field)->isRequired()}required{/if} {if $model.$field}checked{/if}>
            {else}
