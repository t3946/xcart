...
class Order extends Data
{
    ...
    {
        $oldValue = $this->{$sFieldName};
        $newValue = $sNewValue;

        $result = parent::updateField($sFieldName, $sNewValue); // TODO: Change the autogenerated stub

        OrderEventHelper::registerAfterSaveEvent($this->getOrderId(), $sFieldName, $newValue, $oldValue);
        ...
        if ($this->referer_id)
        {
            if ($oRefererDomain = ReferrerModel::objects()->filter(['referer_id' => $this->referer_id])->get()) {
                $sRefererDomain = $oRefererDomain->referer;
                 if (strpos($sRefererDomain, '//') === false) {
                    $sRefererDomain = "//{$sRefererDomain}";
                 }
            }
        }
