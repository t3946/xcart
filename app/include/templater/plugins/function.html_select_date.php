...
#
# $Id: function.html_select_date.php,v 1.4 2006/01/11 06:56:01 mclap Exp $
#
# Templater plugin
# -------------------------------------------------------------
# Type:     function
...
function smarty_function_html_select_date($params, &$smarty) {
	...
	if ($display_months) {
		...
		for ($i = 1; $i <= 12; $i++) {
			if (!$is_native) {
				if ($is_fullname) {
					$month_names[$i] = str_replace("%B", strftime(func_get_langvar_by_name("lbl_month_fullname_".$i), mktime(0, 0, 0, $i, 1, 2000)), $month_format_orig);
				}
				if ($is_abbr) {
					$month_names[$i] = str_replace("%b", strftime(func_get_langvar_by_name("lbl_month_abbr_".$i), mktime(0, 0, 0, $i, 1, 2000)), $month_format_orig);;
				}
			}
	...
	if ($display_years) {
		...
		if ($year_as_text) {
			...
			if (!in_array($time[0], $years)) $years[] = $time[0];
			if ($reverse_years) {
