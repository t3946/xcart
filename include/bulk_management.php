...
###
                                                if ($colnames[$i] == 'avail'){
                                                        $top_message['content'] = '!AVAIL field is found. You need to use r_avail.';
                                                        $top_message['type'] = 'E';
                                                        func_header_location($xcart_web_dir . '/admin/search.php?mode=search');
                                                }
###
		...
		if (isset($qis_zero)) {
			...
			if (is_array($colnames) && !in_array('avail', $colnames)) {
				$additional_colnames[] = 'avail';
                        ...
                        $changes['discontinued']['r_avail'] = $qis_zero;
                        if (is_array($colnames) && !in_array('r_avail', $colnames)) {
                                $additional_colnames[] = 'r_avail';
                        }

		} else {
			$changes['discontinued']['avail'] = 'N';
			$changes['discontinued']['r_avail'] = 'N';
		}
...
/*
                                        if (
						$eproduct["dbsr"]["update_search_index"] == "N" && 
						(
		 				 ($eproduct["csv"]["product"] != $eproduct["dbsr"]["product"] && isset($eproduct["csv"]["product"])) ||
						 ($eproduct["csv"]["productcode"] != $eproduct["dbsr"]["productcode"] && isset($eproduct["csv"]["productcode"])) ||
						 ($eproduct["csv"]["brandid"] != $eproduct["dbsr"]["brandid"] && isset($eproduct["csv"]["brandid"])) ||
						 ($eproduct["csv"]["fulldescr"] != $eproduct["dbsr"]["fulldescr"] && isset($eproduct["csv"]["fulldescr"])) ||
						 ($eproduct["csv"]["upc"] != $eproduct["dbsr"]["upc"] && isset($eproduct["csv"]["upc"]))
						)
					){
						$equery["update_search_index"] = "Y";
                                        }

					if (isset($eproduct["csv"]["forsale"]) && $eproduct["csv"]["forsale"] == "N" && ($eproduct["dbsr"]["update_search_index"] == "N" || $eproduct["csv"]["update_search_index"] == "N")){
						$equery["update_search_index"] = "D";
					}
*/
###
##
#
		...
		if (!empty($discontinued_db) && !empty($changes['discontinued'])) {
			...
			while ($row = db_fetch_array($discontinued_db)) {
				...
				$dp_skus_arr[] = $dproduct['productcode'];
			}
...
//				$set_str .= 'forsale = "N", update_search_index="D"';
				$set_str .= 'forsale = "N"';
...
##
###
	                        if (!empty($dp_skus_arr)){
        	                        foreach ($dp_skus_arr as $k_dp_skus_arr => $v_dp_skus_arr){
                                        
                	                        $current_update_search_index = func_query_first_cell("SELECT update_search_index FROM $sql_tbl[products] WHERE productcode='$v_dp_skus_arr'");
                        	                if ($current_update_search_index == "N"){
                                	                db_query("UPDATE $sql_tbl[products] SET update_search_index='D' WHERE productcode='$v_dp_skus_arr'");
                                        	}
	                                }
        	                }
###
