...
#
	...
	# Check parent category storefront
	if (!empty($active_modules['Multiple_Storefronts'])) {
		$parent_sf = func_query_first_cell('SELECT storefrontid FROM ' . $sql_tbl['categories'] . ' WHERE categoryid="' . $values['parentid'] . '"');
		if ($parent_sf != $values['storefrontid']) {
			func_import_error('err_adm_import_wrong_category_storefront', array('section' => $section));
			func_header_location('import.php?error=1');
		}
	}
