...
$advanced_options = array("productcode", "productid", "provider", "price_max", "avail_max", "weight_max", "forsale", "flag_free_ship", "flag_ship_freight", "flag_global_disc", "flag_free_tax", "flag_min_amount", "flag_low_avail_limit", "flag_list_price", "flag_vat", "flag_gstpst", "manufacturers", "empty_discount_slope", "discount_table", "brands","extra_sku");
	...
	if (!empty($posted_data)) {
        ...
        if (is_array($posted_data["extra_sku"]) || $current_area != "C") {
            ...
            if (count($posted_data["extra_sku"]) == 1) {
                $posted_data["productcode"] = $posted_data["extra_sku"][0];
	        } else {
...
# END: random:18298_18304_18324 [2009 Jun 08 09:50] 
    ...
    } elseif (is_array($data["extra_sku"])) {
        $search_by_variants = true;
        $productcode_cond_string = empty($active_modules['Product_Options']) ? "$sql_tbl[products].productcode" : "IFNULL(search_variants.productcode, $sql_tbl[products].productcode)";
