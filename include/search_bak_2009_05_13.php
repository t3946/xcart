...
$advanced_options = array("productcode", "productid", "provider", "price_max", "avail_max", "weight_max", "forsale", "flag_free_ship", "flag_ship_freight", "flag_global_disc", "flag_free_tax", "flag_min_amount", "flag_low_avail_limit", "flag_list_price", "flag_vat", "flag_gstpst", "manufacturers", "empty_discount_slope", "discount_table");
				...
				if ($login_type == 'P') {
					...
					if (is_array($selected_manufacturers)) {
						if (!empty($data["manufacturers"])) {	
						    ...
						    $where[] = "$sql_tbl[products].manufacturerid IN ('".implode("','", $selected_manufacturers)."')";
						}
					} else {
...
# END: random:1073746882_1073747063 [2008 Dec 24 16:25] 
	...
	if (!empty($data["empty_discount_slope"])) {
		...
		if (!empty($data["discount_slope"])) {
			$where[] = "$sql_tbl[products].discount_slope = '".$data["discount_slope"]."'";
		}
