...
#
# $Id: product_page_options.php,v 1.0 2011/01/18 15:58:24 kate Exp $
#
...
if ($REQUEST_METHOD == 'POST') {
	if (is_array($rep)) {
		db_query('DELETE FROM ' . $sql_tbl['replacements']);
		foreach ($rep as $r) {
			if (!empty($r['what']) || !empty($r['by'])) {
				$rquery = array(
					'what' => $r['what'],
					'by' => $r['by'],
				);
				func_array2insert('replacements', $rquery);
			}
		}
	}
}
...
if (!empty($replacements)) {
	foreach ($replacements as $r) {
		$r['what'] = stripslashes($r['what']);
		$r['by'] = stripslashes($r['by']);
	}
