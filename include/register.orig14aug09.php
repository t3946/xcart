...
# END: random:17710_17631 [2009 Mar 26 09:25] 
	...
	if (!empty($active_modules['Image_Verification']) && isset($antibot_input_str) && $show_antibot_arr[$page] == 'Y') {
		...
		#Message id:  537841186 
        $spam_check = (
                            ($default_fields["s_country"]["avail"] == "Y" && $s_country == "AF") ||
                            ($default_fields["b_country"]["avail"] == "Y" && $b_country == "AF") ||
                            ($default_fields["s_firstname"]["avail"] == "Y" && $default_fields["s_lastname"]["avail"] == "Y" && $s_firstname == $s_lastname) ||
                            ($default_fields["b_firstname"]["avail"] == "Y" && $default_fields["b_lastname"]["avail"] == "Y" && $b_firstname == $b_lastname) ||
                            ($default_fields["firstname"]["avail"] == "Y" && $default_fields["lastname"]["avail"] == "Y" && $firstname == $lastname) 
                      );	

		if (isset($antibot_input_str) && !empty($antibot_input_str) && $spam_check) {
...
# END: random:1073746882_1073747063 [2008 Dec 24 16:25] 
			...
			if (defined('AREA_TYPE') && in_array(constant('AREA_TYPE'), array('C', 'B')) && isset($HTTP_COOKIE_VARS["RefererCookie"]))
				$profile_values['referer'] = $HTTP_COOKIE_VARS["RefererCookie"];
