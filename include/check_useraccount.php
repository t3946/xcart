...
if (!empty($_GET['operate_as_user']) && (!empty($identifiers['A']) || !empty($identifiers['P']) && !empty($active_modules['Simple_Mode']))) {
	# operate as user
...
if ($remember_user && $remember_user_days > 0) {
	...
	} elseif (zerolen($remember_login) && !empty($_COOKIE[$remember_key])) {

		# Check remember key
		$remember_login = $_COOKIE[$remember_key];
		if (empty($remember_login) || !is_string($remember_login) || !func_query_first_cell("SELECT COUNT(login) FROM $sql_tbl[customers] WHERE login = '$remember_login' AND usertype = '$current_area'")) {
