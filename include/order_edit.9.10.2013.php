...
##
###
				db_query("UPDATE $sql_tbl[order_groups] SET actual_shipping_net='$v[actual_shipping_cost_net]', actual_shipping_gross='$v[actual_shipping_cost_net]' WHERE orderid='$orderid' AND manufacturerid='$m_id'");				
...
*/
		...
		if ($order["paymentid"] == PAYMENT_PURCHASE_ID) {
			...
			$data = explode("\n",$order["details"]);
			if ($data) {
				$po_fields = array("PO Number" => $po_number, "Company name" => $po_company_name, "Name of purchaser" => $name_of_purchaser, "Position" => $po_position);
				...
				if ($po_update) {
		                ...
		                db_query("UPDATE $sql_tbl[orders] SET details='" . addslashes(text_crypt($order['details'])) . "' WHERE orderid=$order[orderid]");
				}
			}
		}
