...
# END: random:19885 [2010 Jan 11 11:55] 
	...
	if ($short_list) {
		$to_search = "$sql_tbl[categories].categoryid,$sql_tbl[categories].parentid,$sql_tbl[categories].categoryid_path,$sql_tbl[categories].category,$sql_tbl[categories].avail,$sql_tbl[categories].order_by,$sql_tbl[categories].pc_ready_to_classify";
	} else {
...
#
		...
		elseif($main != "product"){
			...
			if (!empty($cat)){
				...
				$parentid = func_query_first_cell("SELECT $sql_tbl[categories].parentid FROM $sql_tbl[categories] WHERE $sql_tbl[categories].categoryid='$cat'");
				if (empty($parentid)){
					func_header_location($xcart_web_dir."/", true, 301);
				}

	                        $avail = func_query_first_cell("SELECT avail FROM $sql_tbl[categories] WHERE categoryid='$parentid'");
              			if ($avail == "Y"){
