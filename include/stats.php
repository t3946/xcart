...
if ( !defined('XCART_SESSION_START') ) { header("Location: ../"); die("Access denied"); }
	...
	$result = func_query_first_cell ("SELECT COUNT(*) FROM $sql_tbl[orders], $sql_tbl[partner_payment]"
        . " WHERE $sql_tbl[orders].orderid=$sql_tbl[partner_payment].orderid"
        . " AND $sql_tbl[partner_payment].login='$login' AND $sql_tbl[orders].cb_status <> 'P'"
        . " AND $sql_tbl[orders].dc_status <> 'C'");
	$stats_info ["unapproved_sales"] = $result;

	$result = func_query_first ("SELECT SUM($sql_tbl[partner_payment].commissions) AS numba"
        . " FROM $sql_tbl[partner_payment], $sql_tbl[orders]"
        . " WHERE $sql_tbl[partner_payment].orderid=$sql_tbl[orders].orderid"
        . " AND $sql_tbl[partner_payment].login='$login' AND $sql_tbl[orders].cb_status <> 'P'"
        . " AND $sql_tbl[orders].dc_status <> 'C' AND $sql_tbl[partner_payment].paid!='Y'");
