...
if ($REQUEST_METHOD == "POST" && $mode == 'search' && empty($e_mode)) {
...
###
        ...
        if ($current_area == "C" && $total_items > 0 && !empty($active_modules['CIDEV_Best_Search_Filter']) && !empty($cat_ids_str)) {
...
//                                $product_feed_enabled = func_query_first_cell("SELECT d_enable_feed FROM $sql_tbl[manufacturers] WHERE manufacturerid='$v[manufacturerid]'");
                                ...
                                if ($manufacturers_in_found_products[$v["manufacturerid"]]["d_enable_feed"] == "Y" && empty($v["is_variants"]) && $products[$k]["avail"] <= 0){
