...
#
# $Id: adaptives.php,v 1.22 2006/03/24 06:33:11 max Exp $
#
# Adaptives module
#
...
if (preg_match("/^([\w\d_]+)[\/ ]+([^(]+)\s+\((.+)\)(?:\s+(.+))?$/USs", $HTTP_USER_AGENT, $preg)) {
	array_shift($preg);
	...
	if($ua['browser'] == 'Mozilla' && ($ua['platform'] == 'MacPPC' || $ua['platform'] == 'Win32') && empty($preg[3])) {
		...
		$browsers = array("Opera" => "Opera", "Firefox" => "Firefox", "Netscape" => "Netscape", "FireBird" => "Firefox", "Phoenix" => "Firefox", "Safari" => "Safari");
...
if(defined('IS_ROBOT')) {
	...
	# Start request
	if((!isset($adaptives['is_first_start']) || ($adaptives['is_first_start'] == 'Y' && $is_location == 'Y')) && $REQUEST_METHOD == 'GET') {
		$is_location = '';
		$adaptives = array('is_first_start' => 'Y');
		x_session_save();
	# Else ...
	} else {
