...
#
# $Id: import_featured_products.php,v 1.5.2.2 2006/09/08 13:47:08 max Exp $
#
...
if ($import_step == "define") {
	...
	if (!empty($active_modules['Multiple_Storefronts'])) {
		$import_specification['FEATURED_PRODUCTS']['columns']['storefrontid'] = array(
			'type'		=> 'N',
			'default'	=> 0);
	}
...
#
	...
	if ($import_file["drop"][strtolower($section)] == "Y") {
		...
		if (!empty($active_modules['Multiple_Storefronts'])) {
			db_query("DELETE FROM $sql_tbl[featured_products] WHERE storefrontid=$current_storefront");
		} else {
	...
	#
		...
		if (!empty($active_modules['Multiple_Storefronts'])) {
			$data['storefrontid'] = $row['storefrontid'];
		}

		# Update featured product
		if (func_query_first_cell("SELECT COUNT(*) FROM $sql_tbl[featured_products] WHERE productid = '$data[productid]' AND categoryid = '$data[categoryid]'$sf_condition")) {
...
# Export data	
	...
	while ($id = func_export_get_row($data)) {
		...
		if (!empty($active_modules['Multiple_Storefronts'])) {
			$rows = func_query("SELECT * FROM $sql_tbl[featured_products] WHERE productid = '$id' AND storefrontid=$current_storefront");
		} else {
