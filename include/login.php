...
if ($REQUEST_METHOD == "POST") {
	...
	if ($mode == "login") {
		...
		$antibot_input_str = $_POST['antibot_input_str'];

		if (isset($_POST['login_antibot_on'])) {
			$login_antibot_on = $_POST['login_antibot_on'];
		}
...
##
###
			$redirect_to_page = "home.php";

			if ($usertype != "C"){
				$membershipid = func_query_first_cell("SELECT membershipid FROM $sql_tbl[customers] WHERE login='$login'");
/*
https://basecamp.com/2070980/projects/1577907/messages/26942964
mysql> SELECT * FROM xcart_memberships WHERE membershipid='1' OR membershipid='10';
+--------------+------+--------------------------------+--------+---------+------+
| membershipid | area | membership                     | active | orderby | flag |
+--------------+------+--------------------------------+--------+---------+------+
|            1 | A    | Tracking Number Entry Operator | Y      |      10 | FS   |
|           10 | A    | Customer Service               | Y      |      20 |      |
+--------------+------+--------------------------------+--------+---------+------+
*/
				if ($membershipid == "1" || $membershipid == "10"){
					$redirect_to_page = "orders.php";
				}
			}
###
##
#
			...
			func_header_location($redirect_to."/".$redirect_to_page);
