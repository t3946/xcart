					...
					$add_tracking_log = false;
        	                        ...
        	                        foreach ($v_tracking_shipper as $_k => $sh) {
                	                        if (!empty($sh) && func_check_tracking_number($sh, trim($v["tracking_number"][$invoice_number][$_k]))) {
                        	                        $tracknums[$m_id][$tmp_tracknums_counter] = array('linkid' => $sh, 'tracknum' => trim($v["tracking_number"][$invoice_number][$_k]), 'invoice_number' => $invoice_number);
							$tmp_tracknums_counter++;
//                                	                $order['shipping_groups'][$m_id]['dc_status'] = 'S';
//                                        	        define('TRACKING_ADDED', 1);

	                                                $shipping_link = func_query_first_cell("SELECT shipping FROM $sql_tbl[tracking_links] WHERE linkid='$sh'");
        	                                        $log .= "invoice_number_".$invoice_number. ": " . $shipping_link.": ".trim($v["tracking_number"][$invoice_number][$_k])."<br />";
                	                                $add_tracking_log = true;
...
#
##
                                                                        if ($shipping_charged > $order['shipping_groups'][$m_id]["actual_shipping_cost"]["net"] && !empty($config["Attention_tags_invoices"]["tag_for_Shipping_charged_GT_Shipping_quoted_by_distr"])){
