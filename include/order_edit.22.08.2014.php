...
//func_print_r($_POST, $items);
//die();

//func_print_r($distributors_to_delete);
//die();
		...
		$tmp_mnfs = func_get_order_manufacturers($orderid);
                ...
                if ($po_update == "1"){
                        ...
                        $new_po_fields["po_number"] = $po_number;
                        $new_po_fields["po_company_name"] = $po_company_name;
                        $new_po_fields["name_of_purchaser"] = $name_of_purchaser;
                        $new_po_fields["purchase_manager_phone"] = $purchase_manager_phone;
                        $new_po_fields["purchase_manager_phone_ext"] = $purchase_manager_phone_ext;
                        $new_po_fields["po_fax"] = $po_fax;
                        $new_po_fields["purchase_manager_email"] = $purchase_manager_email;
                        $new_po_fields["accounts_payable_full_name"] = $accounts_payable_full_name;
                        $new_po_fields["accounts_payable_phone"] = $accounts_payable_phone;
                        $new_po_fields["accounts_payable_phone_ext"] = $accounts_payable_phone_ext;
                        $new_po_fields["accounts_payable_fax"] = $accounts_payable_fax;
                        $new_po_fields["accounts_payable_email"] = $accounts_payable_email;

			func_check_po_fields($order_data["order"]["po_details"], $new_po_fields, $order_data["order"]["order_prefix"], $order_data["order"]["orderid"]);
                }
###
##
#
...
###
					...
					if ($current_dc_status != $v['dc_status']){
						$time_to_dispatch = time() - $order["date"];
	                                        db_query("UPDATE $sql_tbl[order_groups] SET time_to_dispatch='$time_to_dispatch' WHERE manufacturerid='$m_id' AND orderid='$orderid'");
					}
                                }
###
