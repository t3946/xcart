...
#
# $Id: bench.php,v 1.20.2.5 2006/09/12 11:55:40 max Exp $
#
...
function __add_mark($data = '', $type = 'PHP', $is_force = false) {
	return false; /*speed optimization*/
	...
	if (defined("BENCH_SIMPLE") && !constant("BENCH_SIMPLE")) {
		...
		if (
			...
			function_exists("x_log_flag") && !defined('BENCH_DISABLE_LOGGING_TO_FILE')
		) {
...
function __debug() {
	...
	global $_GET, $PHP_SELF, $REQUEST_METHOD, $sql_tbl, $var_dirs, $xcart_dir, $xcart_web_dir, $QUERY_STRING, $_POST;

	define("BENCH_DISPLAY", true);

	# Add benchmark result to file
	if (function_exists("db_query") && !empty($sql_tbl['benchmark_pages']) && (!defined("BENCH_BLOCK_SAVE_BIN") || !constant("BENCH_BLOCK_SAVE_BIN"))) {
		...
		if ($fp) {
			...
			if ($method == 'P' && !empty($_POST['mode']) && strpos($QUERY_STRING, 'mode=') === false) {
