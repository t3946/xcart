...
#
# $Id: taxes.php,v 1.29.2.3 2007/01/22 07:52:07 max Exp $
#
...
if ($REQUEST_METHOD == "POST") {
	...
	if ($mode == "tax_options" && (!empty($active_modules['Simple_Mode']) || $current_area == 'A')) {
	...
	elseif ($mode == "delete" && (!empty($active_modules['Simple_Mode']) || $current_area == 'A')) {
		#
	...
	elseif ($mode == "update" && (!empty($active_modules['Simple_Mode']) || $current_area == 'A')) {
		#
		# Update taxes list
		#
	...
	elseif ($mode == "details" && (!empty($active_modules['Simple_Mode']) || $current_area == 'A')) {
		#
		...
		$error = false;
		if (empty($tax_service_name) || empty($tax_formula)) {
			#
		...
		if ($error) {
			...
			$tax_details["tax_regnumber"] = $tax_regnumber;
			$tax_details["tax_priority"] = $tax_priority;
	...
	elseif ($mode == "delete_rates" && !empty($taxid)) {
		...
		#
		if (!empty($to_delete) && is_array($to_delete)) {
			db_query("DELETE FROM $sql_tbl[tax_rates] WHERE rateid IN ('".implode("','", array_keys($to_delete))."')".$provider_condition);
	...
	elseif ($mode == "apply" && !empty($to_delete) && is_array($to_delete)) {
		...
		if (empty($active_modules['Simple_Mode'])) {
			$res = db_query("SELECT productid FROM $sql_tbl[products] WHERE provider = '$login'");
		}
		else {
