	...
	if ($mode == "order_edit_apply") {
		...
		if (!empty($items)) {
			foreach ($items as $itemid => $v) {
				...
				$count_product_in_stock = func_oe_get_quantity_in_stock($productid, $order_data["order"]["status"], $v['product_options'], @$order_data["products"][$k]);
				if ($v["amount"] > 0) {
...
#				$order["shipping_groups"][$m_id]["shippingid"] = $v["shippingid"];
				...
				if (!in_array($v["status"], array('C','S')) && $user_account["flag"] == "FS") {
					$v["status"] = 'C';
				}
				$order["shipping_groups"][$m_id]["status"] = $v["status"];
				...
				foreach ($v["tracking_shipper"] as $_k => $sh) {
					if (!empty($v["tracking_shipper"][$_k])) {
						$tracking[] = array('linkid' => $sh, 'tracknum' => trim($v["tracking_number"][$_k]));
						$order["shipping_groups"][$m_id]["status"] = 'S';
	...
	elseif ($mode == "accounting_apply" && $user_account["flag"] != "FS") {
		...
		if (!empty($groups)) {
			...
			foreach ($groups as $m_id => $v) {
				...
				if (in_array($order['shipping_groups'][$m_id]['status'], array('P','C','S','R'))) {
					for ($ak=1; $ak<=4; $ak++) {
