...
#
# $Id: import_tax_rates.php,v 1.14.2.3 2006/10/12 07:04:17 max Exp $
#
...
#
	...
	if (!empty($values["rateid"])) {
		...
		if (!$simple_mode) {
			$values["rateid"] = (func_query_first_cell("SELECT provider FROM $sql_tbl[tax_rates] WHERE rateid='$values[rateid]' AND provider<>'".addslashes($values["provider"])."' LIMIT 1") == $values["provider"] ? "" : $values["rateid"]);
		}
	}
	...
	if (!empty($values["zone"])) {
		...
		if (is_null($_zoneid)) {
			$_zoneid = func_query_first_cell("SELECT zoneid FROM $sql_tbl[zones] WHERE zone_name='".addslashes($values["zone"])."' $provider_condition LIMIT 1");
			if (!empty($_zoneid)) {
