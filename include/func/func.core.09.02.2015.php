...
*/
		...
		if ((!empty($loadproduct["product"]["options"]) && is_array($loadproduct["product"]["options"])) && ($loadproduct["product"]["options"]["0"]["qty"] == "0" || $loadproduct["product"]["options"]["0"]["qty_unit"] == "0" || empty($unit_price) || $unit_price == "0.00")){
			$instock = "N";
		}
		else {
			$instock = "Y";
			...
			if (!empty($loadproduct["product"]["options"]) && is_array($loadproduct["product"]["options"]) && $loadproduct["product"]["options"]["0"]["qty"]>0 && $unit_price>0){
			...
			if (!empty($loadproduct["product"]["discount"]) && is_array($loadproduct["product"]["discount"])){
				...
				$new_discount_table_arr = array();
	                        ...
	                        foreach ($loadproduct["product"]["discount"] as $k_d => $v_d){
        	                        if ($v_d["min_qty"] > 1 && !empty($v_d["discount_pct"]) && strtolower($v_d["discount_pct"]) != "null"){
//						$v_d["discount_pct"] = str_replace(".", "", $v_d["discount_pct"]);
                	                        $new_discount_table_arr[] = ceil($v_d["min_qty"]).":".(round($v_d["discount_pct"]/100,4));
                        	        }
	                        }
