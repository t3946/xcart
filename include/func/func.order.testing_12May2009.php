...
#
# $Id: func.order.php,v 1.34.2.70 2009/02/05 10:39:29 avg Exp $
#
...
# END: random:1073746882_1073747063 [2008 Dec 24 16:25]
	...
	if ($current_area != "C" && !empty($active_modules["Stop_List"])) {
		if (func_ip_exist_slist($order['extra']['proxy_ip'] ? $order['extra']['proxy_ip'] : $order["extra"]["ip"]))
			$order["blocked"] = "Y";
	}
	...
	if (!empty($order['applied_taxes']) && is_array($order['applied_taxes'])) {
		foreach ($order['applied_taxes'] as $k => $v) {
			$intl_tax_name = func_get_languages_alt('tax_'.$v['taxid']);
			$order['applied_taxes'][$k]['tax_display_name'] = empty($intl_tax_name) ? $v['tax_name'] : $intl_tax_name;
		}
	}
...
function func_order_data($orderid) {
	...
	if (isset($order['extra']['paypal_txnid']) || isset($order['extra']['pnref'])) {
		x_load("paypal");
	...
	$_userinfo = func_userinfo($userinfo["login"], "C", false, false, array("C","H"));
	$_userinfo['existing_login'] = isset($_userinfo['login']);
	...
	foreach ($products as $k=>$v) {
		...
		if ($v["extra_data"]) {
			...
			if (is_array($v["extra_data"]["taxes"])) {
				foreach ($v["extra_data"]["taxes"] as $i=>$j) {
					if ($j["tax_value"] > 0) {
						$intl_tax_name = func_get_languages_alt('tax_'.$j['taxid']);
						$v['extra_data']['taxes'][$i]['tax_display_name'] = empty($intl_tax_name) ? $j['tax_name'] : $intl_tax_name;
						$_product_taxes[$i] = $v['extra_data']['taxes'][$i]['tax_display_name'];
					}
				}
			}
		}
		...
		if ($active_modules['Product_Options']) 
			$v['product_options_txt'] = $v["extra_data"]["product_options_alt"][$shop_language] ? $v["extra_data"]["product_options_alt"][$shop_language] : $v['product_options'];
...
# START: random:17710_17631 [2009 Mar 26 09:25] 
			'shipping_groups' => addslashes(serialize($current_order['shipping_groups'])),
			'shipping_costs' => addslashes(serialize($current_order['display_shipping_costs'])),
# END: random:17710_17631 [2009 Mar 26 09:25] 
...
# END: random:1073746882_1073747063 [2008 Dec 24 16:25] 
		...
		#
		if ($discount_coupon) {
			$_per_user = func_query_first_cell("SELECT per_user FROM $sql_tbl[discount_coupons] WHERE coupon = '".addslashes($discount_coupon)."' LIMIT 1");
			if ($_per_user == "Y") {
				$_need_to_update = func_query_first_cell("SELECT COUNT(*) FROM $sql_tbl[discount_coupons_login] WHERE coupon = '".addslashes($discount_coupon)."' AND login = '".addslashes($userinfo["login"])."' LIMIT 1");
		...
		$customer_notify = (($order_status == "Q" && $config["Email_Note"]["eml_order_q_notif_customer"] == "Y") || ($order_status == "I" && $config["Email_Note"]["enable_init_order_notif_customer"] == "Y"));
...
function func_delete_order($orders, $restore_data = true) {
	...
	if ($restore_data) {
		foreach($_orders as $orderid) {
			...
			if ($discount_coupon) {
				$_per_user = func_query_first_cell("SELECT per_user FROM $sql_tbl[discount_coupons] WHERE coupon='$discount_coupon' LIMIT 1");
				if ($_per_user == "Y") {
					db_query("UPDATE $sql_tbl[discount_coupons_login] SET times_used = IF(times_used>'0', times_used-1, '0') WHERE coupon='$discount_coupon' AND login='".$order_data['userinfo']['login']."'");
				} else {
					db_query("UPDATE $sql_tbl[discount_coupons] SET status='A' WHERE coupon='$discount_coupon' and times_used=times");
					db_query("UPDATE $sql_tbl[discount_coupons] SET times_used=times_used-1 WHERE coupon='$discount_coupon'");
				}
	...
	db_query("LOCK TABLES $sql_tbl[orders] WRITE, $sql_tbl[order_details] WRITE, $sql_tbl[order_extras] WRITE, $sql_tbl[giftcerts] WRITE, $sql_tbl[subscription_customers] WRITE, $sql_tbl[shipping_labels] WRITE".(@$xaff?", $sql_tbl[partner_payment] WRITE, $sql_tbl[partner_product_commissions] WRITE, $sql_tbl[partner_adv_orders] WRITE":"").(@$xrma?", $sql_tbl[returns] WRITE":""));
