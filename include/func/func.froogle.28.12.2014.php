...
function AddProductToGoogleBaseBatch($productid, $MerchantID, $update_type, $service, $forsale, $google_products_batch_count, $gproducts, $google_inventory_batch_count, $ginventory){
	...
	if ($update_type == "1"){
	        if ($forsale == "N"){
        	        try {
				print("tried to delete item with productid = $productid \r\n\r\n");

	                        $log_text = "tried to delete item with productid = $productid";
...
function SubmitGoogleInventoryBatch($ginventory, $service, $MerchantID){
	...
	foreach ($ginventory as $k => $v){
                ...
                $product = func_query_first("SELECT SQL_NO_CACHE $sql_tbl[products].product_type, $sql_tbl[pricing].price $fields FROM ($sql_tbl[categories], $sql_tbl[products_categories], $sql_tbl[pricing], $sql_tbl[products]) $joins WHERE $sql_tbl[products].productid = $sql_tbl[products_categories].productid AND $sql_tbl[products_categories].categoryid = $sql_tbl[categories].categoryid AND $sql_tbl[pricing].priceid = $sql_tbl[quick_prices].priceid $where GROUP BY $sql_tbl[products].productid HAVING (price > '0' OR $sql_tbl[products].product_type = 'C')");
...
//else
	...
	try {
                ...
                $k++;
                print("\ntried to submit $k items as inventory feed \n");

		$log_text = "tried to submit $k items as inventory feed";
		func_backprocess_log("incremental feeds", $log_text);
...
//	func_print_r($gproducts);
	...
	foreach ($gproducts as $k => $v){
		...
		$product_info = GetGoogleBaseOneRow($v["productid"], "main_google");
                ...
                if (empty($product_info["product"]) || !is_array($product_info["product"]) || empty($product_info["product"]["shippings_google_arr"])){
			print("\nProduct skipped - $v[productid] \n");

	                $log_text = "Product skipped - ".$v["productid"];
        	        func_backprocess_log("incremental feeds", $log_text);
...
//		$k_counter -= $count_skipped;
		print("\ntried to submit $k_counter items as product feed \n");

                $log_text = "tried to submit $k_counter items as product feed";
                func_backprocess_log("incremental feeds", $log_text);
...
###
		...
		$a_query = "Select cidev_get_amazon_price('$v[productid]') As 'aprice', M.amazon_leadtimetoship As 'aleadtime' from xcart_products P left join xcart_manufacturers M ON M.manufacturerid = P.manufacturerid where P.productid = '$v[productid]'";
...
EOD;
		...
		$a_query = "Select cidev_get_amazon_price('$productid') As 'aprice', M.amazon_leadtimetoship As 'aleadtime' from xcart_products P left join xcart_manufacturers M ON M.manufacturerid = P.manufacturerid where P.productid = '$productid'";
		$a_result = func_query_first($a_query);
