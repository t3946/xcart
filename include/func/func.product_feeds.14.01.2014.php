...
*/
			...
			if ($line_number > 1){
				...
				$productcode = func_query_first_cell("SELECT productcode FROM $sql_tbl[products] WHERE productcode='".addslashes($feed_productcode)."'");
				...
				if (!empty($productcode)){
					...
					if ($DropShip == "N"){
						db_query("UPDATE $sql_tbl[products] SET avail='$Stock', eta_date_mm_dd_yyyy='$CorrectED_mm_dd_yyyy' WHERE productcode='".addslashes($productcode)."'");
						$product_is_updated = true;
						...
						if (!empty($CorrectED_mm_dd_yyyy)){
							db_query("UPDATE $sql_tbl[products] SET avail='0', eta_date_mm_dd_yyyy='$CorrectED_mm_dd_yyyy' WHERE productcode='".addslashes($productcode)."'");
							$product_is_updated = true;
							$marked_as_out_of_stock_products = true;
						} else {
							db_query("UPDATE $sql_tbl[products] SET avail='1000000', eta_date_mm_dd_yyyy='' WHERE productcode='".addslashes($productcode)."'");
							$product_is_updated = true;
							$marked_as_in_stock_products = true;
						}
					}
...
//die();
			...
			$ALV_products = db_query("SELECT productid, productcode FROM $sql_tbl[products] WHERE productcode LIKE 'ALV-%'");
			...
			while ($product = db_fetch_array($ALV_products)) {
				...
				if (!in_array($product["productcode"], $all_feed_productcodes)) {
					$file_is_found_and_uploaded = true;
...
//		func_print_r($NEW_PRODUCTS);
	...
	$body .= "updated products (in first iteration): ".$count_updated_products."\n";
	$body .= "marked as `out of stock` products (in first iteration): ".$count_marked_as_out_of_stock_products."\n";
		    ...
		    while (($buffer = fgetcsv($handle, 1000, ",")) !== FALSE) {
				...
				$On_Order_Qt = trim($buffer[5]);
					...
					if (!empty($current_eta_date_mm_dd_yyyy)){
						...
						db_query("UPDATE $sql_tbl[products] SET list_price='$Base_Price', avail='0', eta_date_mm_dd_yyyy='$new_eta_date_mm_dd_yyyy' WHERE productcode='".addslashes($productcode)."'");
                    ...
                    if (!empty($all_feed_productcodes) && is_array($all_feed_productcodes)){
                        ...
                        $EDR_products = db_query("SELECT productid, productcode FROM $sql_tbl[products] WHERE productcode LIKE 'EDR-%'");
