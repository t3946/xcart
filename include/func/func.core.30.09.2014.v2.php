...
function func_define_approximate_shippings($productid, $product_info=''){
	...
	if (!empty($approximation_shipping_rates)){
		...
		foreach ($approximation_shipping_rates as $k => $v){
			...
			$product_info["weight"] = ceil($product_info["weight"]);
...
//func_print_r($intershipper_rates);
			...
			$Add_Shipping_rate = func_calculate_shippings($products, $shipping_id, $customer_info, "master");
...
//func_print_r($text);
	...
	$categories = db_query($query="SELECT categoryid, pc_category_weight FROM $sql_tbl[categories] WHERE pc_ready_to_classify='Y' AND avail='Y' AND storefrontid='$sfid'");
	...
	while ($category = db_fetch_array($categories)) {
		...
		$current_category_terms = func_query("SELECT $sql_tbl[pc_terms].term FROM $sql_tbl[pc_terms] LEFT JOIN $sql_tbl[pc_category_terms] ON $sql_tbl[pc_terms].termid=$sql_tbl[pc_category_terms].termid WHERE $sql_tbl[pc_category_terms].categoryid='$categoryid' AND $sql_tbl[pc_category_terms].bayes_weight<>0");
		...
		foreach ($text_arr as $word){
			if (in_array($word, $current_category_terms_arr)){
				...
				$termid = func_query_first_cell("SELECT termid FROM $sql_tbl[pc_terms] WHERE term='$word'");
