...
function func_CHECK_B_S($order_data){
	...
	$full_address_s = func_correct_field($full_address_s);
        ...
        $full_addresb_b = $order_data["userinfo"]["b_address"] . "-". $order_data["userinfo"]["b_addresb_2"] . "-". $order_data["userinfo"]["b_city"] . "-". $order_data["userinfo"]["b_state"] . "-". $order_data["userinfo"]["b_country"] . "-". $order_data["userinfo"]["b_zipcode"];
...
function func_CHECK_EMAIL_VS_NAME($order_data){
	...
	$b_firstname_arr = explode(" ", $b_firstname);

	if (!empty($b_firstname_arr) && is_array($b_firstname_arr) && !empty($email_1)){
		foreach ($b_firstname_arr as $k => $v){
			$name = trim($v);
...
function func_CHECK_STATES($order_data){
	...
	if (!empty($customer_ip)){
	        ...
	        if (!empty($customer_ip_INTEGER)){
	                ...
	                if (!empty($locId)){
	                        ...
	                        if (!empty($geo_litecity_location)){
                	                ...
                	                $geoip_areaCode = $geo_litecity_location["areaCode"];
	                        }
        	        }
	        }
	}
		...
		$Telephone_area_codes_state = func_correct_field($Telephone_area_codes["parent_area_code"]);
        }

	if ($s_state == $b_state && $b_state == $geoip_state && $Telephone_area_codes_state == $geoip_areaCode){
		$fraud_score = "1";
	}
...
function func_CHECK_OK_ORDERS_FOR_EMAIL($order_data){
	...
	$_res = db_query("SELECT $sql_tbl[orders].orderid FROM $sql_tbl[orders] LEFT JOIN $sql_tbl[order_groups] ON $sql_tbl[order_groups].orderid=$sql_tbl[orders].orderid WHERE email='$email' AND ($sql_tbl[order_groups].dc_status='S' || $sql_tbl[order_groups].dc_status='G') GROUP BY $sql_tbl[orders].orderid");
	$total_items = db_num_rows($_res);

	$time_condition = time() - 60*60*24*1;
	...
	if ($total_items_min_day > 0){
		$fraud_score = $total_items/$total_items_min_day;
	}
...
function func_CHECK_FULLNAMES_FOR_EMAIL($order_data){
	...
	$_res = db_query("SELECT DISTINCT firstname FROM $sql_tbl[orders] WHERE email='$email'");
        $total_items = db_num_rows($_res);
...
function func_CHECK_DIFFERENT_SHIPPINGS_FOR_IP($order_data){
	...
	$customer_ip = $order_data["order"]["extra"]["ip"];
        ...
        $time_condition = time() - 60*60*24*7;
        $extras = func_query("SELECT extra, orderid,  s_address, s_city, s_state, s_country, s_zipcode FROM $sql_tbl[orders] WHERE date >= '$time_condition'");
...
function func_CHECK_DIFFERENT_BILLINGS_FOR_IP($order_data){
        ...
        $time_condition = time() - 60*60*24*7;
        $extras = func_query("SELECT extra, orderid,  b_address, b_city, b_state, b_country, b_zipcode FROM $sql_tbl[orders] WHERE date >= '$time_condition'");
...
function func_CHECK_DIFFERENT_SHIPPINGS_FOR_PHONE($order_data){
	...
	$phone = preg_replace("/[^0-9]/", "", $phone);
        ...
        $time_condition = time() - 60*60*24*60;
        $orders = func_query("SELECT orderid, phone, s_address, s_city, s_state, s_country, s_zipcode FROM $sql_tbl[orders] WHERE date >= '$time_condition'");
...
function func_CHECK_DIFFERENT_BILLINGSS_FOR_PHONE($order_data){
        ...
        $time_condition = time() - 60*60*24*60;
        $orders = func_query("SELECT orderid, phone, b_address, b_city, b_state, b_country, b_zipcode FROM $sql_tbl[orders] WHERE date >= '$time_condition'");
...
function func_CHECK_DIFFERENT_SHIPPINGS_FOR_EMAIL($order_data){
        ...
        $time_condition = time() - 60*60*24*60;
        $orders = func_query("SELECT orderid, email, s_address, s_city, s_state, s_country, s_zipcode FROM $sql_tbl[orders] WHERE date >= '$time_condition' AND email='$email'");
...
function func_CHECK_DIFFERENT_BILLINGS_FOR_EMAIL($order_data){
        ...
        $time_condition = time() - 60*60*24*60;
        $orders = func_query("SELECT orderid, email, b_address, b_city, b_state, b_country, b_zipcode FROM $sql_tbl[orders] WHERE date >= '$time_condition' AND email='$email'");
...
function func_CHECK_DIFFERENT_SHIPPINGS_FOR_CARD($order_data){
	...
	if ($credit_card_found && $credit_card_type_found){
		...
		$credit_card_val = func_correct_field($credit_card_val);
	        ...
	        $time_condition = time() - 60*60*24*60;
        	$all_details = func_query("SELECT details, orderid,  s_address, s_city, s_state, s_country, s_zipcode FROM $sql_tbl[orders] WHERE date >= '$time_condition'");
...
function func_CHECK_DIFFERENT_BILLING_FOR_SHIPPING($order_data){
        ...
        $time_condition = time() - 60*60*24*60;
        $orders = func_query("SELECT orderid, phone, s_address, s_city, s_state, s_country, s_zipcode, b_address, b_city, b_state, b_country, b_zipcode FROM $sql_tbl[orders] WHERE date >= '$time_condition'");

        if (!empty($orders) && is_array($orders)){

                $count_different_addresses = 0;
				...
				if ($tmp_full_address_s != $tmp_full_address_b){
					$count_different_addresses++;
				}
                        }
                }

                if ($count_different_addresses > 0){
                        $fraud_score = 1/$count_different_addresses;
                }
        }
...
function func_CHECK_TOTAL($order_data){
	...
	$order_total_div = 50/$order_data["order"]["total"];
        ...
        $fraud_score = min($order_total_div, "1");
