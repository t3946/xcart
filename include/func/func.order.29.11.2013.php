...
# START: random:20341 [2010 Jul 29 14:46] 
		if (!empty($current_order['shipping_groups'])) {
			...
			foreach ($current_order['shipping_groups'] as $mid => $v) {
				...
				$insert_data['shipping'] = @$cart['groups_delivery'][$mid];
...
function func_check_and_send_request_availability_email($orderid){
	...
	if (!empty($mnfs) && is_array($mnfs)) {
		foreach ($mnfs as $m_id => $mv) {
			...
			if ( 
				...
				$order_notes .= "\r\n" . date('l jS \of F Y h:i:s A') . ": Request availability email was sent automatically to '". $mv["manufacturer"] . "' distributor, when order was placed by customer. ";
					...
					$cidev_outofstock_items_table .= '<tr><td width="150px" style="text-align: left; font-weight: bold;">Item number</td><td width="250px" style="text-align: left; font-weight: bold;">Item name</td><td style="text-align: right; font-weight: bold;">"Out of stock" quantity</td></tr>';
                                        ...
                                        foreach ($products as $k => $v){
                                                if ($v["manufacturerid"] == $m_id){
                                                        ...
                                                        $cidev_items_table .= '<tr><td width="150px" style="text-align: left;">'.$tmp_sku.'</td><td width="250px" style="text-align: left;"><a href="'.$v["links"]["customer"].'">'.$v["product"].'</a></td><td style="text-align: right;">'.$v["amount"].'</td></tr>';
...
### !!! ###
                                ...
                                if (!empty($mv["d_shipping_options"])){
                                        $d_shipping_options_arr = explode(",", $mv["d_shipping_options"]);
                                        $d_shipping_options_count = count($d_shipping_options_arr);
                                        $mnfs[$m_id]["d_shipping_options_arr"] = $d_shipping_options_arr;
                                        $mnfs[$m_id]["d_shipping_options_count"] = $d_shipping_options_count;
				
					if ($d_shipping_options_count == "1"){
						$mess_body = str_replace("{{shipping_method}}", $mv["d_shipping_options"], $mess_body);
					}
                                } else {
					$mess_body = str_replace("{{shipping_method}}", "the least expensive shipping method", $mess_body);
				}
				...
				if ($mv["d_availability_must_be_checked"] == "Y"){
					...
					if ($tmp_cur_time_date_format >= "8.30" && $tmp_cur_time_date_format <= "16.30"){
...
function func_instock_and_outofstock_items_table($products) {
        ...
        foreach ($products as $k => $v){
                ...
                if ($v["back"] > 0){
                        ...
                        if (!empty($tmp_eta_date_mm_dd_yyyy)){
                                ...
                                $tmp_eta_date_mm_dd_yyyy = date("j-M-Y", mktime(0, 0, 0, $tmp_eta_date_mm_dd_yyyy_arr[0], $tmp_eta_date_mm_dd_yyyy_arr[1], $tmp_eta_date_mm_dd_yyyy_arr[2]));
                        }

                        $cidev_outofstock_items_table .= '<tr><td width="150px" style="text-align: left;">'.$tmp_sku.'</td><td width="250px" style="text-align: left;"><a href="'.$v["links"]["customer"].'">'.$v["product"].'</a></td><td style="text-align: right;">'.$v["back"].'</td><td style="text-align: right;" nowrap="nowrap">'.$tmp_eta_date_mm_dd_yyyy.'</td></tr>';
                }
        }
