                ...
                if ($handle) {
                    ...
                    while (($buffer = fgetcsv($handle, 4096, "\t")) !== FALSE) {
                        ...
                        if ($line_number > 0){  //<-------------------
                                ...
                                $product_info_arr = func_query_first("SELECT productid, productcode, forsale, avail, eta_date_mm_dd_yyyy, list_price, update_search_index FROM $sql_tbl[products] WHERE productcode='".addslashes($feed_productcode)."'");
...
*/
					...
					if ($QUALIFIER == "D"){
						...
						db_query("UPDATE $sql_tbl[products] SET avail='$new_avail', forsale='$new_forsale', update_search_index='Y' WHERE productid='$productid'");
						...
						if ($current_eta_date_mm_dd_yyyy != $new_eta_date_mm_dd_yyyy){
							$product_is_updated = true;
                                                ...
                                                db_query("UPDATE $sql_tbl[products] SET avail='$new_avail', forsale='$new_forsale', update_search_index='Y', eta_date_mm_dd_yyyy='$new_eta_date_mm_dd_yyyy' WHERE productid='$productid'");
					}
		...
		foreach ($needed_lines as $k => $v){
			if ($k % 2 != 0){
				...
				$Inventory_Status = strtolower($Inventory_Status);
                                ...
                                $product_info_arr = func_query_first("SELECT productid, productcode, forsale, avail, eta_date_mm_dd_yyyy, list_price, cost_to_us, new_map_price, update_search_index FROM $sql_tbl[products] WHERE productcode='".addslashes($feed_productcode)."'");
							...
							if ($new_forsale == 'N'){
								if ($product_info_arr["update_search_index"] == "N"){
									$update_search_index = 'D';
                                        		...
                                        		db_query("UPDATE $sql_tbl[products] SET avail='$new_avail', eta_date_mm_dd_yyyy='$new_eta_date_mm_dd_yyyy', forsale='$new_forsale', list_price='$new_list_price', cost_to_us='$new_cost_to_us', new_map_price='$new_new_map_price', update_search_index='$update_search_index' WHERE productid='$productid'");
                                                        $product_is_updated = true;
                                        }
		...
		if (!empty($all_feed_productcodes) && is_array($all_feed_productcodes)){
						...
						if ($update_search_index == "N"){
							$update_search_index = "D";
                                                ...
                                                db_query("UPDATE $sql_tbl[products] SET avail='0', forsale='N', update_search_index='$update_search_index' WHERE productid='".$product["productid"]."'");
                                        }
                                }
                        }
		}
...
#
				...
				$PRICE = price_format($PRICE);
                                ...
                                $product_info_arr = func_query_first("SELECT productid, productcode, forsale, avail, eta_date_mm_dd_yyyy, cost_to_us, weight, update_search_index FROM $sql_tbl[products] WHERE productcode='".addslashes($feed_productcode)."'");
						...
						if ($AVAIL == 0){
							...
							$new_eta_date_mm_dd_yyyy = "";
                                        ...
                                        if ($new_forsale != $current_forsale || $new_eta_date_mm_dd_yyyy != $current_eta_date_mm_dd_yyyy || $current_avail != $new_avail || $new_weight != $current_weight || $new_cost_to_us != $current_cost_to_us){
                                                        ...
                                                        db_query("UPDATE $sql_tbl[products] SET avail='$new_avail', eta_date_mm_dd_yyyy='$new_eta_date_mm_dd_yyyy', forsale='$new_forsale', cost_to_us='$new_cost_to_us', weight='$new_weight', update_search_index='$update_search_index' WHERE productid='$productid'");
                                                        $product_is_updated = true;
                                        }
        ...
        if (!empty($needed_lines)){
			...
			if (!empty($tmp_SKU) && strpos($tmp_SKU, '-') !== true){
				...
				$cost_to_us = price_format($v[10]);
                                ...
                                $product_info_arr = func_query_first("SELECT productid, productcode, update_search_index, forsale, avail, eta_date_mm_dd_yyyy, cost_to_us FROM $sql_tbl[products] WHERE productcode='".addslashes($feed_productcode)."'");
                                ...
                                if (!empty($product_info_arr)){
                                        ...
                                        if ($new_forsale != $current_forsale || $new_eta_date_mm_dd_yyyy != $current_eta_date_mm_dd_yyyy || $current_avail != $new_avail || $new_cost_to_us != $current_cost_to_us){
                                                        ...
                                                        db_query("UPDATE $sql_tbl[products] SET avail='$new_avail', eta_date_mm_dd_yyyy='$new_eta_date_mm_dd_yyyy', forsale='$new_forsale', cost_to_us='$new_cost_to_us', update_search_index='$update_search_index' WHERE productid='$productid'");
                                                        $product_is_updated = true;
                                        }
...
//func_print_r($buffer);
				...
				$feed_productcode = $manufacturer_code . "-" . $SKU;
                                ...
                                $product_info_arr = func_query_first("SELECT productid, productcode, forsale, avail, update_search_index, eta_date_mm_dd_yyyy, lead_time_message FROM $sql_tbl[products] WHERE productcode='".addslashes($feed_productcode)."'");
						...
						if (strtolower($FEED_ETA) == "special order item"){
							...
							$new_eta_date_mm_dd_yyyy = $FEED_ETA;
                                        ...
                                        if ($new_forsale != $current_forsale || $new_eta_date_mm_dd_yyyy != $current_eta_date_mm_dd_yyyy || $current_avail != $new_avail || $new_lead_time_message != $current_lead_time_message){
                                                        ...
                                                        db_query("UPDATE $sql_tbl[products] SET avail='$new_avail', eta_date_mm_dd_yyyy='$new_eta_date_mm_dd_yyyy', forsale='$new_forsale', lead_time_message='$new_lead_time_message', update_search_index='$update_search_index' WHERE productid='$productid'");
                                                        $product_is_updated = true;
                                        }
			...
			$tmp_SKU = strtoupper($tmp_SKU);
				...
				if (empty($NEW_PRODUCTS)){
					$NEW_PRODUCTS[] = "SKU | UPC | COST | MSRP | AVAILABILITY | Quantity";
                                ...
                                $product_info_arr = func_query_first("SELECT productid, productcode, forsale, avail, update_search_index, eta_date_mm_dd_yyyy, cost_to_us, list_price FROM $sql_tbl[products] WHERE productcode='".addslashes($feed_productcode)."'");
						...
						if ($current_forsale != "N"){
							$discontinued_products[] = $product_info_arr;
                                        ...
                                        if ($new_forsale != $current_forsale || $new_eta_date_mm_dd_yyyy != $current_eta_date_mm_dd_yyyy || $current_avail != $new_avail || $new_cost_to_us != $current_cost_to_us || $new_list_price != $current_list_price){
                                                        ...
                                                        db_query("UPDATE $sql_tbl[products] SET avail='$new_avail', eta_date_mm_dd_yyyy='$new_eta_date_mm_dd_yyyy', forsale='$new_forsale', cost_to_us='$new_cost_to_us', list_price='$new_list_price', update_search_index='$update_search_index' WHERE productid='$productid'");
                                                        $product_is_updated = true;
                                        }
...
###
		    ...
		    $handle = @fopen($local_file_name_full_path, "r");
				...
				$FEED_STATUS = strtolower($FEED_STATUS);
                                ...
                                $product_info_arr = func_query_first("SELECT productid, productcode, update_search_index, forsale, avail, eta_date_mm_dd_yyyy FROM $sql_tbl[products] WHERE productcode='".addslashes($feed_productcode)."'");
						...
						$new_forsale = "Y";
                                        ...
                                        if ($new_forsale != $current_forsale || $new_eta_date_mm_dd_yyyy != $current_eta_date_mm_dd_yyyy || $current_avail != $new_avail){
                                                        ...
                                                        db_query("UPDATE $sql_tbl[products] SET avail='$new_avail', eta_date_mm_dd_yyyy='$new_eta_date_mm_dd_yyyy', forsale='$new_forsale', update_search_index='$update_search_index' WHERE productid='$productid'");
                                                        $product_is_updated = true;
                                        }
...
#
				...
				$product_info = func_query_first("SELECT productid, forsale, free_ship_zone, update_search_index, eta_date_mm_dd_yyyy, manufacturerid, shipping_freight, low_avail_limit, free_tax, discount_slope, discount_table, discount_avail, avail FROM $sql_tbl[products] WHERE productcode='".addslashes($SKU)."'");
...
//                                        $shipping_freight = $default_product_info["shipping_freight"];
                                        ...
                                        $discount_table = $default_product_info["discount_table"];
                                        $discount_avail = $default_product_info["discount_avail"];
                                        $avail = $default_product_info["avail"];
                                }
