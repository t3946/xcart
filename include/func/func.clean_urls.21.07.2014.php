 ...
 * @version    $Id: func.clean_urls.php,v 1.23.2.8 2012/01/10 12:27:54 ferz Exp $
...
function func_clean_url_has_url($clean_url, $excluding_resource = NULL)
{
    ...
    $count = func_query_first_cell("SELECT SQL_NO_CACHE COUNT(*) FROM " . $sql_tbl['clean_urls'] . " WHERE clean_url='" . func_addslashes($clean_url) . "'" . $exclusion_condition);
...
function func_clean_url_resource_has_record($resource_type, $resource_id, $filename = '')
{
    ...
    $count = func_query_first_cell("SELECT SQL_NO_CACHE COUNT(*) FROM " . $sql_tbl['clean_urls'] . " WHERE resource_type = '" . func_addslashes($resource_type) . "' AND resource_id = '" . func_addslashes($resource_id) . "'");

    if (
        $count == 0
        && $resource_type == 'S'
    ) {
        $count = func_query_first_cell("SELECT SQL_NO_CACHE COUNT(*) FROM " . $sql_tbl['clean_urls'] . ", " . $sql_tbl['pages'] . " WHERE " . $sql_tbl['clean_urls'] . ".resource_id = " . $sql_tbl['pages'] . ".pageid AND " . $sql_tbl['clean_urls'] . ".resource_type = 'S' AND " . $sql_tbl['pages'] . ".filename = '" . func_addslashes($filename) . "'");
    }
...
function func_clean_url_autogen_page_url($pageid, $params)
{
    ...
    if (isset($params['filename']) && !zerolen($params['filename'])) {
        ...
        $count = func_query_first_cell("SELECT SQL_NO_CACHE COUNT(*) FROM $sql_tbl[clean_urls], $sql_tbl[pages] WHERE $sql_tbl[clean_urls].resource_id = $sql_tbl[pages].pageid AND $sql_tbl[pages].filename = '".func_addslashes($params['filename'])."' AND $sql_tbl[pages].pageid != '$pageid'");
...
function func_clean_url_get_missing_urls_stats()
{
    ...
    foreach ($resources_data as $resource_type => $resource) {
        ...
        $missing_count = func_query_first_cell("SELECT SQL_NO_CACHE COUNT(*) FROM " . $resource['resource_table'] . " LEFT JOIN " . $sql_tbl['clean_urls'] . " ON " . $sql_tbl['clean_urls'] . ".resource_type = '" . $resource_type . "' AND " . $sql_tbl['clean_urls'] . ".resource_id = " . $resource['resource_table'] . "." . $resource['resource_id_column'] . " WHERE " . $sql_tbl['clean_urls'] . ".resource_id IS NULL");

        if ($missing_count > 0) {

            $report[$resource_type]['total_count']         = func_query_first_cell('SELECT SQL_NO_CACHE COUNT(*) FROM ' . $resource['resource_table']);
