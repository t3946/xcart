		...
		$manufacturer_code = trim($general_info["code"]);
			...
			if (!empty($contents) && is_array($contents)){
				...
				if (!empty($all_files)){
					...
					$all_files = array_values($all_files);
					$server_file = $general_info["d_ftp_folder"].$all_files[0]["file"];
					$local_file = $xcart_dir . "/files/product_feeds/" .$manufacturerid."_".$all_files[0]["file"];
				}
			}
...
/*		
			...
			$new_server_file = $general_info["d_ftp_folder"]."backup/".$all_files[0]["file"];

			$command = "rename ".$server_file." ".$new_server_file;
