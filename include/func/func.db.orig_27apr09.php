...
function func_array2insert ($tbl, $arr, $is_replace = false) {
	...
	if (!empty($arr) && !func_check_field_names(array_keys($arr), $query))
		return false;
...
function func_array2update ($tbl, $arr, $where = '') {
	...
	if (!func_check_field_names(array_keys($arr), $query))
		return false;
...
#
# Function for validating SQL table field names
#
function func_check_field_names($names, $query) {

	$valid_names = preg_grep("/^[\w\d_]+$/S", $names);
	if (count($valid_names) != count($names)) {
		global $current_location, $REMOTE_ADDR, $login;

		$mysql_error = "10002 : Incorrect format of field names";
		$msg = <<<MSG
...
SQL query           : $query
Error code          : 10002
Description         : Incorrect format of field names
MSG;
