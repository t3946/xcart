...
#
				...
				if ($update_product_flag || $insert_product_flag){
					...
					if (!empty($COLOR)) $fulldescr .= '<br /><br /> Color: /n/n'.$COLOR;
					if (!empty($SCENT)) $fulldescr .= '<br /><br /> Scent: /n/n'.$SCENT;
...
//                                $NEW_PRODUCTS_header = implode(", ", $buffer);
		    ...
		    $last_imported_updated_products_count = $count_inserted_products."/".$count_updated_products;
                    db_query("UPDATE $sql_tbl[supplier_product_feeds] SET last_import_date='".$launch_time."', last_imported_updated_products_count='$last_imported_updated_products_count' WHERE manufacturerid='$manufacturerid'");
