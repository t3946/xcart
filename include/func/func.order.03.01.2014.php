...
function func_get_shipping_groups($orderid) {
	...
	$return = func_query_hash("SELECT og.*, m.manufacturer as group_name, m.code, s.shipping AS shippingid_name FROM $sql_tbl[order_groups] AS og LEFT JOIN $sql_tbl[manufacturers] AS m ON m.manufacturerid=og.manufacturerid LEFT JOIN $sql_tbl[shipping] AS s ON s.shippingid=og.shippingid WHERE og.orderid='$orderid'","manufacturerid",false);
...
# END: random:20341 [2010 Jul 29 14:46] 
	...
	if (!in_array($order_status, array('I', 'Q', 'O'))) {
		func_unlock("place_order");
		return false;
	}
...
# END: random:20341 [2010 Jul 29 14:46] 
		...
		$mes .= "STEP L ".date("H:i:s")."\n";
	        ...
	        $order_notification = func_get_order_notification($order_status);
...
//				$order_notes = func_query_first_cell("SELECT notes FROM $sql_tbl[orders] WHERE orderid = '$orderid'");
				$order_notes = "";
...
//				db_query("UPDATE $sql_tbl[orders] SET notes='".addslashes($order_notes)."' WHERE orderid = '$orderid'");
...
# END: random:20341 [2010 Jul 29 14:46] 
					...
					$order_products = '<table width="100%" cellspacing="0" cellpadding="0" border="0"><tr><td valign="top"><img alt="" src="http://www.artistsupplysource.com/skin1_kolin/images/S3-Stores-Logo-M.png"></td><td width="100%" valign="top"><table width="100%" cellspacing="0" cellpadding="2"><tr><td width="30">&nbsp;</td><td valign="top"><h1 style="margin-bottom: 0px; margin-top: 0px; text-transform: uppercase;">Invoice</h1><b>Order date:</b> '.date("j-M-Y", $order["date"]).'<br /><b>Order #:</b> '.$order["order_prefix"].$order["orderid"].'<br /></td><td valign="bottom" align="right"><b>S3 Stores, Inc.</b><br />27 Joseph St.,<br>Chatham, Ontario<br />N7L 3G4, Canada<br />Toll Free: 1-800-929-2431<br />Tel: (201) 299-7928<br />Fax: (813) 944-4516<br>Email: orders@s3stores.com</td></tr></table></td></tr></table><hr style="width:100%;margin: 0px 0 5px 0; border: 0 none; border-bottom: 1px solid #999999;">';

					$order_products .= '<table width="45%" cellspacing="0" cellpadding="0" border="0"><tr><td nowrap="nowrap"><b>Full Name:</b></td><td>'.$userinfo["firstname"].'</td></tr><tr><td><b>Phone:</b></td><td>'.$userinfo["phone"].(!empty($userinfo["phone_ext"])? '(Ext: '.$userinfo["phone_ext"].')' : '' ).'</td></tr><tr><td><b>Email:</b></td><td>'.$userinfo["email"].'</td></tr><tr><td colspan="2"><br /><b>Shipping Address</b><hr style="width:100%;margin: 0px; border: 0 none; border-bottom: 1px solid #999999;"></td></tr><tr><td><b>Full Name:</b></td><td>'.$userinfo["s_firstname"].'</td></tr><tr><td><b>Company:</b></td><td>'.$userinfo["additional_fields"]["1"]["value"].'</td></tr><tr><td><b>Address:</b></td><td>'.$userinfo["s_address"].' '.$userinfo["s_address_2"]. '</td></tr><tr><td><b>City:</b> </td><td>'.$userinfo["s_city"].'</td></tr><tr><td><b>State/Province:</b> </td><td>'.$userinfo["s_statename"].'</td></tr><tr><td><b>Country:</b></td><td>'.$userinfo["s_countryname"].'</td></tr><tr><td><b>Zip/Postal code:</b></td><td>'.$userinfo["s_zipcode"].'</td></tr></table>';
					...
					$order_products .= '<table style="margin-top: 5px;" width="100%" cellspacing="0" cellpadding="3" border="1"><tr><th width="60" bgcolor="#cccccc" align="center">SKU</th><th width="*" bgcolor="#cccccc" align="center">Product</th><th width="50" nowrap="nowrap" bgcolor="#cccccc" align="center">Qty</th></tr>';
							...
							$order_products .= '<tr><td align="center">'.$tmp_sku.'</td><td><font style="FONT-SIZE: 11px"><a href="'.$v["links"]["customer"].'">'.$v["product"].'</a>'.$selected_product_options.'</font></td><td align="center">'.$v["amount"].'</td></tr>';
                                                }
...
## 30.12.2013
###
				$order_products .= '<hr style="width:100%; margin: 5px 0 -5px 0; border: 0 none; border-bottom: 1px solid #999999;">S3 Stores, Inc.<br />Phone: 1-800-929-2431<br />Fax (813) 944-4516<br />URL: <a href="http://www.s3stores.com">www.s3stores.com</a>';
...
#
				...
				if ($grand_total > 0)
                                $estimated_profit_margin_after_additional_payment = $estimated_profit_after_additional_payment/((1 - $config["Additional_shipping_charge"]["credit_card_processing_fees"]/100) * $grand_total);
					...
					if ($tmp_cur_time_date_format >= "8.30" && $tmp_cur_time_date_format <= "16.30" && ($tmp_number_of_day_of_week != "0" && $tmp_number_of_day_of_week != "6")){
						...
						if (!empty($request_availability_options) && is_array($request_availability_options)){
							foreach ($request_availability_options as $k_r => $v_r){
								if ($v_r["date_mm_dd_yyyy"] == $tmp_date_mm_dd_yyyy){
...
function func_process_order($orderids) {
	...
	foreach($orderids as $orderid) {
		...
		#
        $order_notification = func_get_order_notification('P');
...
function func_complete_order($orderid) {
	...
	# Send mail notifications
	#
    $order_notification = func_get_order_notification('C');
...
function func_not_paid_order($orderid) {
	...
	# Send mail notifications
	#
    $order_notification = func_get_order_notification('N');
...
function func_decline_order($orderids, $status = "D") {
	...
	foreach($orderids as $orderid) {
		...
		if ($status == 'D' || $status == 'A') {
            ...
            $order_notification = func_get_order_notification($status);
...
function func_send_order_status_notification($orderid, $status) {
    ...
    if ($order_notification && $order_notification['enabled'] == 'Y') {
        ...
        $mail_smarty->assign('type', 'C');
        func_send_mail($order_data['userinfo']['email'], 'mail/order_notification_subj.tpl', 'mail/order_notification.tpl', $config['Company']['orders_department'], false);
        $mail_smarty->assign('type', 'A');
...
function func_get_order_notification($status) {
    global $sql_tbl;
...
function func_get_refund_groups($orderid) {
    ...
    if (!empty($groups)) {
        ...
        $fields = array(
            ...
            'r.productid AS pid', 'r.manufacturerid AS mid'
        );
        ...
        $products = func_query_hash('SELECT ' . implode(', ', $fields) . ' FROM ' . $sql_tbl['refunded_products'] . ' AS r' 
            ...
            . ' AND r.orderid = "' . $orderid . '"', array('mid', 'pid'), false, false);
        ...
        foreach ($products as $k => $v){
				...
				if ($type_of_message == 'backorder_decision_request'){
					$tmp_eta_date_mm_dd_yyyy = "unknown";
                        ...
                        $cidev_outofstock_items_table .= '<tr><td width="150px" style="text-align: left;">'.$tmp_sku.'</td><td width="250px" style="text-align: left;"><a href="'.$v["links"]["customer"].'">'.$v["product"].'</a>'.$selected_product_options.'</td><td style="text-align: right;">'.$v["back"].'</td><td style="text-align: right;" nowrap="nowrap">'.$tmp_eta_date_mm_dd_yyyy.'</td></tr>';
                }
        }
