...
#
	...
	if ($group["amazon_fulfillment_channel"] == "AFN" && $group["acc_paymentid"] == "101"){
#
##
		$acc[0]['gross'] = $group["accounting_gross_0"];
##
#
	}
	else {
...
#
## https://basecamp.com/2070980/projects/1577907/messages/28304328
###
...
#
## https://basecamp.com/2070980/projects/1577907/messages/30564336
###
...
//func_print_r($product, $ref_values);
//die("1");
...
//func_print_r($product, $query_data);
//die("2");
        }
    }
}
...
//-----------------------------
//func_print_r($refund_status, $group, $_POST["mode"], $ref_notify_mode, $order);
//die();
...
#
## https://basecamp.com/2070980/projects/1577907/messages/34072491
###
...
//                foreach ($mproducts as $pid => $product) {
		    ...
		    $pid = $product["productid"];
                    ...
                    $where = 'manufacturerid = "' . $mid . '" AND orderid = "' . $orderid . '"'
                        . ' AND productid = "' . $pid . '" AND itemid = "'.$itemid.'"';
                    ...
                    $max_values = func_query_first('SELECT price, amount FROM ' . $sql_tbl['order_details']
                        . ' WHERE orderid = "' . $orderid . '" AND productid = "' . $pid . '" AND itemid = "'.$itemid.'"');
...
//        . ' WHERE productid = "' . $pid . '" AND manufacturerid = "' . $mid . '" AND orderid = "' . $orderid. '"');
        . ' WHERE itemid = "' . $pid . '" AND manufacturerid = "' . $mid . '" AND orderid = "' . $orderid. '"');
}
