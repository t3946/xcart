...
#
function func_select_product($id, $membershipid, $redirect_if_error=true, $clear_price=false, $always_select=false, $skip_cat_checking=false) {
...
# END: random:1073746882_1073747063 [2008 Dec 24 16:25] 
	...
	if (!empty($active_modules["Manufacturers"])) {
		...
		$add_fields .= ", $sql_tbl[manufacturers].manufacturer, $sql_tbl[manufacturers].cost_to_us_coef_x, $sql_tbl[manufacturers].price_coef_x, $sql_tbl[manufacturers].price_coef_y, $sql_tbl[manufacturers].price_coef_z, $sql_tbl[manufacturers].map_price_coef_x, $sql_tbl[manufacturers].new_map_price_coef_x ";
	}
...
#
    ...
    if ($current_area == 'C' && !empty($product['upc'])) {
...
##
###
        if ($current_area == 'C' && $product["new_map_price"]>0){
...
###
	if (!empty($product["eta_date_mm_dd_yyyy"])){
		...
		$product["eta_date_dd_month_yyyy"] = date("d F Y", $product["eta_date_mktime"]);
	...
	if (!empty($product["manufacturerid"]) && !empty($mpn)){
