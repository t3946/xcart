...
##
###
                if (!empty($v["eta_date_mm_dd_yyyy"]) && $config["backorder_decision_request"]["do_not_offer_backorder_if_eta_more_than_days"] != "" && $config["backorder_decision_request"]["do_not_offer_backorder_if_eta_more_than_days"] > 0){
...
###
	if (!empty($order['shipping_groups']) && is_array($order['shipping_groups'])){
		foreach ($order['shipping_groups'] as $m_id => $v){
			...
			if (!empty($v["reconciliation_id"])){
				$order['shipping_groups'][$m_id]["full_reconciliation_info"] = func_query_first("SELECT * FROM $sql_tbl[reconciliations] WHERE id='$v[reconciliation_id]'");
				$order['shipping_groups'][$m_id]["full_reconciliation_info"]["amount_csv_abs"] = abs($order['shipping_groups'][$m_id]["full_reconciliation_info"]["amount_csv"]);
			}

                        if (!empty($v["ref_reconciliation_id"])){
                                $order['shipping_groups'][$m_id]["ref_full_reconciliation_info"] = func_query_first("SELECT * FROM $sql_tbl[reconciliations] WHERE id='$v[ref_reconciliation_id]'");
                                $order['shipping_groups'][$m_id]["ref_full_reconciliation_info"]["amount_csv_abs"] = abs($order['shipping_groups'][$m_id]["ref_full_reconciliation_info"]["amount_csv"]);
                        }
...
# END: random:20341 [2010 Jul 29 14:46] 
					...
					$order_products .= '<table width="45%" cellspacing="0" cellpadding="0" border="0"><tr><td nowrap="nowrap"><b>Full Name:</b></td><td>'.$userinfo["firstname"].'</td></tr><tr><td><b>Phone:</b></td><td>'.$userinfo["phone"].(!empty($userinfo["phone_ext"])? ' <B>ext</B> '.$userinfo["phone_ext"].'' : '' ).'</td></tr><tr><td colspan="2"><br /><b>Shipping Address</b><hr style="width:100%;margin: 0px; border: 0 none; border-bottom: 1px solid #999999;"></td></tr><tr><td><b>Full Name:</b></td><td>'.$userinfo["s_firstname"].'</td></tr><tr><td><b>Company:</b></td><td>'.$userinfo["additional_fields"]["1"]["value"].'</td></tr><tr><td><b>Address:</b></td><td>'.$userinfo["s_address"].' '.$userinfo["s_address_2"]. '</td></tr><tr><td><b>City:</b> </td><td>'.$userinfo["s_city"].'</td></tr><tr><td><b>State/Province:</b> </td><td>'.$userinfo["s_statename"].'</td></tr><tr><td><b>Country:</b></td><td>'.$userinfo["s_countryname"].'</td></tr><tr><td><b>Zip/Postal code:</b></td><td>'.$userinfo["s_zipcode"].'</td></tr></table>';
...
#
		...
		if (!empty($manufacturerid)){
		        db_query("UPDATE $sql_tbl[order_groups] SET $status_column='$status' WHERE orderid='$orderid' AND manufacturerid='$manufacturerid'");
		} else {
