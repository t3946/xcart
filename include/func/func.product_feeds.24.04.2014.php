...
x_load('cart','mail','order','product','taxes', 'files', 'backoffice', "image", "gd");
...
#
				...
				$PRICE = price_format($PRICE);
                                ...
                                $product_info_arr = func_query_first("SELECT productid, productcode, forsale, avail, eta_date_mm_dd_yyyy, cost_to_us, weight FROM $sql_tbl[products] WHERE productcode='".addslashes($feed_productcode)."'");
						...
						if ($AVAIL == 0){
							...
							$new_eta_date_mm_dd_yyyy = "";
                                        ...
                                        if ($new_forsale != $current_forsale || $new_eta_date_mm_dd_yyyy != $current_eta_date_mm_dd_yyyy || $current_avail != $new_avail || $new_weight != $current_weight || $new_cost_to_us != $current_cost_to_us){
                                                        db_query("UPDATE $sql_tbl[products] SET avail='$new_avail', eta_date_mm_dd_yyyy='$new_eta_date_mm_dd_yyyy', forsale='$new_forsale', cost_to_us='$new_cost_to_us', weight='$new_weight' WHERE productid='$productid'");
                                                        $product_is_updated = true;
                                        }
                    ...
                    if (!empty($all_feed_productcodes) && is_array($all_feed_productcodes)){
                        ...
                        if ($count_products > 0){
                                $PPF_products = db_query("SELECT productid, productcode, forsale FROM $sql_tbl[products] WHERE productcode LIKE 'PPF-%' AND forsale='Y'");
        ...
        if (!empty($needed_lines)){
			...
			if (!empty($tmp_SKU) && strpos($tmp_SKU, '-') !== true){
				...
				$cost_to_us = price_format($v[10]);
                                ...
                                $product_info_arr = func_query_first("SELECT productid, productcode, forsale, avail, eta_date_mm_dd_yyyy, cost_to_us FROM $sql_tbl[products] WHERE productcode='".addslashes($feed_productcode)."'");
                                ...
                                if (!empty($product_info_arr)){
                                        ...
                                        if ($new_forsale != $current_forsale || $new_eta_date_mm_dd_yyyy != $current_eta_date_mm_dd_yyyy || $current_avail != $new_avail || $new_cost_to_us != $current_cost_to_us){
                                                        db_query("UPDATE $sql_tbl[products] SET avail='$new_avail', eta_date_mm_dd_yyyy='$new_eta_date_mm_dd_yyyy', forsale='$new_forsale', cost_to_us='$new_cost_to_us' WHERE productid='$productid'");
                                                        $product_is_updated = true;
                                        }
...
//func_print_r($SKU, $Item_Name, $MSRP, $MAP, $Retail_Price, $Your_Price, $Inventory_Status);
                ...
                if (!empty($all_feed_productcodes) && is_array($all_feed_productcodes)){
                        ...
                        if ($count_products > 0){
                                ...
                                $manufacturer_code_products = db_query("SELECT productid, productcode, forsale FROM $sql_tbl[products] WHERE productcode LIKE '".$manufacturer_code."-%' AND forsale='Y'");
...
//func_print_r($buffer);
				...
				$feed_productcode = $manufacturer_code . "-" . $SKU;
                                ...
                                $product_info_arr = func_query_first("SELECT productid, productcode, forsale, avail, eta_date_mm_dd_yyyy, lead_time_message FROM $sql_tbl[products] WHERE productcode='".addslashes($feed_productcode)."'");
						...
						if (strtolower($FEED_ETA) == "special order item"){
							...
							$new_eta_date_mm_dd_yyyy = $FEED_ETA;
                                        ...
                                        if ($new_forsale != $current_forsale || $new_eta_date_mm_dd_yyyy != $current_eta_date_mm_dd_yyyy || $current_avail != $new_avail || $new_lead_time_message != $current_lead_time_message){
                                                        db_query("UPDATE $sql_tbl[products] SET avail='$new_avail', eta_date_mm_dd_yyyy='$new_eta_date_mm_dd_yyyy', forsale='$new_forsale', lead_time_message='$new_lead_time_message' WHERE productid='$productid'");
                                                        $product_is_updated = true;
                                        }
			...
			$tmp_SKU = strtoupper($tmp_SKU);
				...
				if (empty($NEW_PRODUCTS)){
					$NEW_PRODUCTS[] = "SKU | UPC | COST | MSRP | AVAILABILITY | Quantity";
                                ...
                                $product_info_arr = func_query_first("SELECT productid, productcode, forsale, avail, eta_date_mm_dd_yyyy, cost_to_us, list_price FROM $sql_tbl[products] WHERE productcode='".addslashes($feed_productcode)."'");
						...
						$new_list_price = $list_price;
                                        } elseif ($Low_in_Stock == "OUT OF STOCK"){
                                                $new_avail = 0;
						$new_eta_date_mm_dd_yyyy_time = time() + 60*60*24*35;
                                                $new_cost_to_us = $cost_to_us;
						$new_list_price = $list_price;
                                        } elseif ($Out_of_Stock == "DISCONTINUED"){
                                                $new_avail = 0;
						$new_forsale = "N";
						$discontinued_products[] = $product_info_arr;
                                        ...
                                        if ($new_forsale != $current_forsale || $new_eta_date_mm_dd_yyyy != $current_eta_date_mm_dd_yyyy || $current_avail != $new_avail || $new_cost_to_us != $current_cost_to_us || $new_list_price != $current_list_price){
                                                        db_query("UPDATE $sql_tbl[products] SET avail='$new_avail', eta_date_mm_dd_yyyy='$new_eta_date_mm_dd_yyyy', forsale='$new_forsale', cost_to_us='$new_cost_to_us', list_price='$new_list_price' WHERE productid='$productid'");
                                                        $product_is_updated = true;
                                        }
				...
				$FEED_STATUS = strtolower($FEED_STATUS);
						...
						$new_forsale = "Y";
                                        ...
                                        if ($new_forsale != $current_forsale || $new_eta_date_mm_dd_yyyy != $current_eta_date_mm_dd_yyyy || $current_avail != $new_avail){
                                                        db_query("UPDATE $sql_tbl[products] SET avail='$new_avail', eta_date_mm_dd_yyyy='$new_eta_date_mm_dd_yyyy', forsale='$new_forsale' WHERE productid='$productid'");
                                                        $product_is_updated = true;
                                        }
...
/*
        ...
        if ($file_is_found){
                ...
                while (($buffer = fgetcsv($handle, 8192, ",")) !== FALSE) {
...
#                if ($product_feed_info["last_imported_updated_products_count"] > 0){
#                        $lines_in_file_DIV_last_feed_rows_processed = $lines_in_file/$product_feed_info["last_imported_updated_products_count"];
#
...
#                                $body .= "Last feed rows processed: ".$product_feed_info["last_imported_updated_products_count"]."\n";
#                                $body .= "processing stopped";
...
#                }
                ...
                db_query("UPDATE $sql_tbl[supplier_product_feeds] SET last_imported_updated_products_count='$lines_in_file' WHERE manufacturerid='$product_feed_info[manufacturerid]'");
	}
*/
...
#
	if ($file_is_found){
		    ...
		    $categoryid_path = func_query_first_cell("SELECT categoryid_path FROM $sql_tbl[categories] WHERE categoryid='$default_product_info[categoryid]'");
		    $default_product_info["categoryid_path"] = explode("/", $categoryid_path);
		    $default_product_info["categoryid_path2category_path"] = func_categoryid_path2category_path($categoryid_path);
		    $default_product_info["count_categoryid_path"] = count($default_product_info["categoryid_path"]);

		    foreach ($default_product_info["categoryid_path"] as $k => $v){
			$default_product_info["categoryid_path_categories"][$k] = func_query("SELECT categoryid, category FROM $sql_tbl[categories] WHERE parentid='$v'");
		    }
...
//func_print_r($cost_to_us, $price);
                                ...
                                $product_info = func_query_first("SELECT productid, forsale FROM $sql_tbl[products] WHERE productcode='".addslashes($SKU)."'");
				...
				if ($update_product_flag || $insert_product_flag){
					...
					$cat = "";
					foreach ($default_product_info["categoryid_path_categories"] as $k => $v){
						if ($k != ($default_product_info["count_categoryid_path"] - 1)){
							if (!empty($v) && is_array($v)){
								foreach ($v as $kk => $vv){
									if (trim(strtoupper($vv["category"])) == strtoupper($CATEGORY)){
										$cat = $vv["categoryid"];
										break;
									}
								}
					...
					$tmp_k = $default_product_info["count_categoryid_path"] - 2;
					$previous_cat = $default_product_info["categoryid_path"][$tmp_k];

					$main_cat_was_added = false;
					if ($cat == "" && $previous_cat != ""){
						db_query("INSERT INTO $sql_tbl[categories] (parentid, storefrontid, category, is_bold, order_by) VALUES ('$previous_cat', '$product_feed_info[storefrontid]', '".addslashes($CATEGORY)."', 'N', '10')");
						$cat = db_insert_id();
						$parent_categoryid_path = func_query_first_cell("SELECT categoryid_path FROM $sql_tbl[categories] WHERE categoryid='$previous_cat'")."/".$cat;
						func_array2update("categories", array("categoryid_path" => $parent_categoryid_path), "categoryid = '$cat'");
						$main_cat_was_added = true;

				                    // Autogenerate clean URL.
        				        $clean_url = func_clean_url_autogenerate('C', $cat, array('category' => $CATEGORY));
				                $clean_url_save_in_history = false;
...
//func_print_r($v, $SUBCATEGORY);
//die("v");
                	                                                foreach ($v as $kk => $vv){
                        	                                                if (trim(strtoupper($vv["category"])) == strtoupper($SUBCATEGORY)){
                                	                                                $sub_cat = $vv["categoryid"];
											break;
                                                	                        }
                                                        	        }
	                                                        }
						...
						if (empty($sub_cat)){
							...
							if (!$main_cat_was_added){
								$cat = $previous_cat;
							}

							db_query("INSERT INTO $sql_tbl[categories] (parentid, storefrontid, category, is_bold, order_by) VALUES ('$cat', '$product_feed_info[storefrontid]', '".addslashes($SUBCATEGORY)."', 'N', '10')");
							$categoryid = db_insert_id();
	                                                $parent_categoryid_path = func_query_first_cell("SELECT categoryid_path FROM $sql_tbl[categories] WHERE categoryid='$cat'")."/".$categoryid;
...
//func_print_r($categoryid, $previous_cat);
//die();
	

					$fulldescr = $SHORT_DESCRIPTION;
					if (!empty($PRODUCT_FEATURES)) $fulldescr .= '/n/n Product Features: /n/n'.$PRODUCT_FEATURES;
					if (!empty($INGREDIENTS)) $fulldescr .= '/n/n Ingredients: /n/n'.$INGREDIENTS;
					if (!empty($COUNTRY_OF_ORIGIN)) $fulldescr .= '/n/n Country of origin: '.$COUNTRY_OF_ORIGIN;
					if (!empty($SIZE)) $fulldescr .= '/n/n Size: '.$SIZE;
					if (!empty($COLOR)) $fulldescr .= '/n/n Color: /n/n'.$COLOR;
					if (!empty($SCENT)) $fulldescr .= '/n/n Scent: /n/n'.$SCENT;
					if (!empty($ORGANIC)) $fulldescr .= '/n/n'.$ORGANIC;
					if (!empty($GLUTEN_FREE)) $fulldescr .= '/n'.$GLUTEN_FREE;
					if (!empty($DAIRY_FREE)) $fulldescr .= '/n'.$DAIRY_FREE;
					if (!empty($YEAST_FREE)) $fulldescr .= '/n'.$YEAST_FREE;
					if (!empty($WHEAT_FREE)) $fulldescr .= '/n'.$WHEAT_FREE;
					if (!empty($VEGAN)) $fulldescr .= '/n'.$VEGAN;
					if (!empty($KOSHER)) $fulldescr .= '/n'.$KOSHER;
					if (!empty($FAIR_TRADE)) $fulldescr .= '/n'.$FAIR_TRADE;


//func_print_r($categoryid, $previous_cat, $fulldescr, $SHORT_DESCRIPTION, $buffer[17], $buffer);
//die("asd");
					...
					if (strtolower($STATUS) == "available"){
						$query_data["avail"] = $QTY_ON_HAND;
						$query_data["eta_date_mm_dd_yyyy"] = "";
					} elseif (strtolower($STATUS) == "out of stock"){
                                                $query_data["avail"] = 0;
						$eta = time() + 60*60*24*25;
                                                $query_data["eta_date_mm_dd_yyyy"] = date("m/d/Y", $eta);
					}
