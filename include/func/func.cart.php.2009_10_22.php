...
#
# $Id: func.cart.php,v 1.31.2.42 2007/01/16 08:43:19 svowl Exp $
#
...
# START: random:17710_17631 [2009 Mar 26 09:25] 
function func_real_shipping($delivery, $for_manufacturerid = 0) {
# END: random:17710_17631 [2009 Mar 26 09:25] 
...
function func_calculate_shippings($products, $shipping_id, $customer_info, $provider="") {
	...
	if (!empty($products)) {
		foreach($products as $k=>$product) {
			...
			if ($product["free_shipping"] == "Y" || ($active_modules["Egoods"] && $product["distribution"] != "")) {
				continue;
			}
			else {
...
# END: random:17710_17631 [2009 Mar 26 09:25] 
			$for_manufacturerid = $product['manufacturerid'];
		}
	}
...
# START: random:1073746882_1073747063 [2008 Dec 24 16:25] 
		$shipping_cost = func_real_shipping($shipping_id, $for_manufacturerid);
# END: random:1073746882_1073747063 [2008 Dec 24 16:25] 
...
# START: random:1073746882_1073747063 [2008 Dec 24 16:25] 
			$shipping_cost += $shipping_rt[0]["rate"]+$total_weight_shipping*$shipping_rt[0]["weight_rate"]+$total_ship_items*$shipping_rt[0]["item_rate"]+$total_shipping*$shipping_rt[0]["rate_p"]/100 + $shipping_cost*$shipping_rt[0]["cost_marcup"]/100;
...
# START: random:17710_17631 [2009 Mar 26 09:25] 
						foreach ($products as $kp => $vp) {
							if (($vp['shipping_freight'] != 0 && $vp['manufacturerid'] == $ks) || ($ks == 0 && $vp['shipping_freight'] == 0))
								$_products_[] = $vp;
# END: random:17710_17631 [2009 Mar 26 09:25] 
						}
						...
						if (empty($whole_taxes)) {
							...
							if ($manuf_taxes["taxes"]) {
								foreach ($manuf_taxes["taxes"] as $__tk => $__tv ) {
									if (array_key_exists($whole_taxes["taxes"][$__tk])) {
...
# START: random:17710_17631 [2009 Mar 26 09:25] 
			$m_id = (($product['shipping_freight'] != 0)?$product['manufacturerid']:'0');
			if (!isset($shipping_groups[$m_id])) {
				$manufact_data = func_query_first("SELECT * FROM $sql_tbl[manufacturers] WHERE manufacturerid = '$product[manufacturerid]'");
				...
				$shipping_groups[$m_id]['products'] = array();
# END: random:17710_17631 [2009 Mar 26 09:25] 
			}
# START: random:17710_17631 [2009 Mar 26 09:25] 
			$shipping_groups[$m_id]['products'][] = $product['productid'];
# END: random:17710_17631 [2009 Mar 26 09:25] 
		}
