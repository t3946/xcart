...
function func_define_approximate_shippings($productid){
	global $sql_tbl;

	$product_info = func_query_first("SELECT weight, manufacturerid FROM $sql_tbl[products] WHERE productid='$productid'");
...
//	if (!empty($approximation_shipping_rates) && $count_approximation_shipping_rates == $count_all_states){
	if (!empty($approximation_shipping_rates)){
		...
		foreach ($approximation_shipping_rates as $k => $v){
			...
			$shippings_str_arr[$k] = "US:".$v["state"].":Ground:".$Shipping_charge." USD";
		}
