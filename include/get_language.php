...
if (!empty($edit_lng) && !empty($_GET['old_lng'])) {
	$asl = $edit_lng;
	$_POST['asl'] = $asl;
...
if (empty($current_area) || @$current_area == "C" || @$current_area == "B") {
	...
	if (@$_POST['asl'] && $login) {
		$res = func_query_first ("SELECT charset FROM $sql_tbl[countries] WHERE code='" . $_POST["asl"] . "'");
		if ($res) {
...
if (@$current_area == "C" || @$current_area == "B") {
	...
	# Set cookies
	#
	if ($store_language != @$_COOKIE['store_language'] && !defined('NOCOOKIE')) {
