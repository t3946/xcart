...
#
# $Id: import_shipping_rates.php,v 1.8.2.1 2006/11/24 07:28:29 max Exp $
#
...
#
	...
	if (!empty($values['shipping']) && (is_null($_shippingid) || ($acttion == 'do' && empty($_shippingid)))) {
	...
	if (!empty($values['zone'])) {
		...
		if (is_null($_zoneid)) {
			$_zoneid = func_query_first_cell("SELECT zoneid FROM $sql_tbl[zones] WHERE zone_name='".addslashes($values["zone"])."' AND provider = '".addslashes($import_data_provider)."' LIMIT 1");
			if (empty($_zoneid)) {
				$_zoneid = NULL;
			} else {
...
#
	...
	if ($import_file["drop"][strtolower($section)] == "Y") {
		...
		if ($provider_condition) {
			db_query("DELETE FROM $sql_tbl[shipping_rates] WHERE provider = '".addslashes($import_data_provider)."'");
	...
	#
		...
		# Detect rateid
		$_rateid = func_query_first_cell("SELECT rateid FROM $sql_tbl[shipping_rates] WHERE zoneid = '$row[zoneid]' AND shippingid = '$row[shippingid]' AND provider = '".addslashes($import_data_provider)."' AND maxamount = '$row[maxamount]' AND mintotal = '$row[mintotal]' AND maxtotal = '$row[maxtotal]' AND minweight = '$row[minweight]' AND maxweight = '$row[maxweight]' AND type = '$row[type]'");
