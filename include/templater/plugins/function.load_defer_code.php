 ...
 * @version    $Id: function.load_defer_code.php 188 2011-06-04 16:36:19Z max $
...
function smarty_function_load_defer_code($params, &$smarty)
{
    ...
    if (
        !isset($deferRegistry[$type])
        || empty($deferRegistry[$type])
    ) {
                ...
                if (
                    ...
                    foreach ($deferRegistry[$type][$elem] as $web => $file) {
                        ...
                        if ('css' == $type) {
                            ...
                            $fileSource = preg_replace('/(url\()(?!http)(.*)/', '\1' . $dir . '\2', $fileSource);
