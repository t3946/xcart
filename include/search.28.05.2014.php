...
## Search_Filter
###
//        $search_query_fv_ids .= " WHERE ".implode(" AND ", $where);
//	$search_query_fv_ids .= " AND $sql_tbl[cidev_filter_values].fv_id > 0 ";
//	$search_query_fv_ids .= " GROUP BY $sql_tbl[cidev_filter_values].fv_id ";
###
##
# 
...
//print_r($search_query_count_NEW);
...
## Search_Filter
###
        if ($current_area == "C" && $total_items > 0 && !empty($active_modules['CIDEV_Best_Search_Filter'])) {
...
//func_print_r($filter_found_brands);

		$filter_found_fv_ids = array();
		$cidev_productids = array();
		$cidev_counter = 0;
		$limit_for_implode = 300;
		$count_of_limit_steps = 0;

//$start_time = time();
//print("START search Attribute values. <br /> START TIME: ". $start_time);

		while ($cidev_product = db_fetch_array($_res)) {
			
			$cidev_productids[$count_of_limit_steps][] = $cidev_product["productid"];
			$cidev_counter++;

			if ($cidev_counter > $limit_for_implode){
				$count_of_limit_steps++;
			}
		}

//$next_time = time();
//$diff_time = $next_time - $start_time;
//print("<br /> DIFF time (START TIME - CURRENT TIME): ". $diff_time);

		foreach ($cidev_productids as $k => $v){

	               $cidev_imploded_productids = implode(",", $v);

/*
			# MAIN
                       $cidev_fv_id_query = "SELECT DISTINCT fv_id FROM $sql_tbl[cidev_filter_products] WHERE productid IN ($cidev_imploded_productids)";

                       if (!empty($filter_found_fv_ids)){
	                      $all_imploded_fv_ids_in_products = implode(",", $filter_found_fv_ids);
                              $cidev_fv_id_query .= " AND fv_id NOT IN ($all_imploded_fv_ids_in_products)";
                       }
                        
                       $fv_ids = func_query($cidev_fv_id_query);
*/

                       $cidev_fv_id_query = "SELECT fv_id FROM $sql_tbl[cidev_filter_products] WHERE productid IN ($cidev_imploded_productids)";
...
//print("<br /> DIFF time (START TIME - CURRENT TIME): ". $diff_time);
                       ...
                       if (!empty($fv_ids) && is_array($fv_ids)){
        	              foreach ($fv_ids as $k_fv => $v_fv){
                	             $filter_found_fv_ids[] = $v_fv["fv_id"];
	                      }
                       }
		}
//func_print_r($filter_found_fv_ids);

//$next_time = time();
//$diff_time = $next_time - $start_time;
//print("<br />TOTAL spent time (START TIME - CURRENT TIME): ". $diff_time);
//print("<br />End time (CURRENT TIME): ". $next_time);


		if (!empty($filter_found_fv_ids)){

			$filter_found_fv_ids_count = array_count_values($filter_found_fv_ids);
//func_print_r($filter_found_fv_ids_count);
			$smarty->assign("filter_found_fv_ids_count", $filter_found_fv_ids_count);

			$filter_found_fv_ids_unique = array_unique($filter_found_fv_ids);
			$filter_found_fv_ids_unique = array_values($filter_found_fv_ids_unique);

//func_print_r($filter_found_fv_ids_unique);
			$smarty->assign("filter_found_fv_ids", $filter_found_fv_ids_unique);
		}
	}
###
##
...
###
		if ($current_area == "C") {
			$current_fv_ids = func_query($search_query_fv_ids);

			if (!empty($current_fv_ids)){
				$smarty->assign("current_fv_ids", $current_fv_ids);

				$current_f_ids = array();
				foreach ($current_fv_ids as $k => $v){
					$current_f_ids[] = $v["f_id"];
				}
				$current_f_ids = array_unique($current_f_ids);
				$current_f_ids = array_values($current_f_ids);

				if (!empty($current_f_ids)){
					$current_f_id__fv_ids = array();
					foreach ($current_f_ids as $k => $v){
						foreach ($current_fv_ids as $kk => $vv){
							if ($v == $vv["f_id"]){
								$current_f_id__fv_ids[$v][] = $vv["fv_id"];
							}
						}
					}

					$smarty->assign("current_f_ids", $current_f_ids);
					$smarty->assign("current_f_id__fv_ids", $current_f_id__fv_ids);

//					$cidev_filters_tree = func_cidev_filters_tree(true);
//					$smarty->assign("cidev_filters_tree", $cidev_filters_tree);
				}
			}
//print($search_query_fv_ids);
//func_print_r($current_fv_ids);
//func_print_r($current_f_id__fv_ids);
		}
*/
###
##
...
###
                                ...
                                if ($product_feed_enabled == "Y" && empty($v["is_variants"]) && $products[$k]["avail"] <= 0){
