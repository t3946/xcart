...
//func_print_r($order["shipping_groups"]);
//die();
...
###
		...
		if (!empty($acc_items) && is_array($acc_items)){
			foreach ($acc_items as $itemid => $v){
...
//func_print_r($all_product_total_extended1_arr);
//die();
...
###
				...
				if (!empty($v["tracking_shipper"]) && is_array($v["tracking_shipper"])){
					...
					$add_tracking_log = false;
        	                        ...
        	                        foreach ($v["tracking_shipper"] as $_k => $sh) {
                	                        if (!empty($v["tracking_shipper"][$_k]) && func_check_tracking_number($sh, trim($v["tracking_number"][$_k]))) {
                        	                        $tracknums[$m_id][$tmp_tracknums_counter] = array('linkid' => $sh, 'tracknum' => trim($v["tracking_number"][$_k]));
...
//die();
				...
				if ($mode == "accounting_apply" && $user_account["flag"] != "FS"){
					...
					$update['HST_charged'] = $v["HST_charged"];
		                        ...
		                        if ($order['shipping_groups'][$m_id]['accounting'][1]["gst"] != $v["HST_charged"]){
						$order['shipping_groups'][$m_id]['accounting'][1]["gst"] = $v["HST_charged"];
                		        }

					$all_product_total_extended1_tax = $all_product_total_extended1_arr[$m_id] + $v["tax_charged_except_HST"]; //!!!!

		                        if ($order['shipping_groups'][$m_id]['accounting'][1]["gross"] != ($v["HST_charged"] + $all_product_total_extended1_tax)){
                		                $order['shipping_groups'][$m_id]['accounting'][1]["gross"] = $v["HST_charged"] + $all_product_total_extended1_tax;
		                        }

                		        if ($order['shipping_groups'][$m_id]['accounting'][2]["gross"] != ($v["shipping_charged"] + $v["drop_ship_fee_charged"])){
        		                        $order['shipping_groups'][$m_id]['accounting'][2]["gross"] = $v["shipping_charged"] + $v["drop_ship_fee_charged"];
		                        }
				}
...
//				$order["shipping_groups"][$k]["shipping_charged"] = $order["shipping_groups"][$k]["actual_shipping_cost"]["net"];
				$order["shipping_groups"][$k]["shipping_charged"] = 0;
			}
