...
#
	...
	while ($productid = func_export_get_row($data)) {
		...
		if (!empty($active_modules['Multiple_Storefronts'])) {
			$row = func_query_first("SELECT pl.* FROM $sql_tbl[products_lng] as pl"
				. " LEFT JOIN $sql_tbl[products] as p ON p.productid=pl.productid"
				. " LEFT JOIN $sql_tbl[products_sf] as psf ON p.productid=psf.productid"
				. " WHERE pl.productid = '$productid' AND pl.code = '$current_code'"
				. " AND psf.sfid=$current_storefront");
		} else {
