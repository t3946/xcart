...
if (!empty($cart_manufact_text_displayed_tabs) && is_array($cart_manufact_text_displayed_tabs)){
        foreach ($cart_manufact_text_displayed_tabs as $k => $v){
                $product_tabs[$k+1]["title"] = $v[0];
                $product_tabs[$k+1]["tpl"] = $v[1];
                $product_tabs[$k+1]["anchor"] = $k+1;
        }
}
...
###
$product_info["product_questions"] = func_query("SELECT * FROM $sql_tbl[product_question] WHERE question_published_on_page='Y' AND productid='$productid'");
###
