...
if ($product_feed_enabled == "Y" && empty($product_info["is_variants"]) && $product_info["avail"] <= 0){
...
if (!empty($product_tabs) && is_array($product_tabs)) {
	...
	$count_shipping_rates_for_canada = func_query_first_cell("SELECT COUNT(*) FROM $sql_tbl[shipping_rates] WHERE manufacturerid='$product_info[manufacturerid]' AND (type='R' OR type='D') AND zoneid='12'");
...
if ($product_info["manufacturerid"] == "32" && !empty($product_info["supplier_internal_id"]) && $count_days > 10){
...
###
  		...
  		$add_to_cart_button = func_parse_add_to_cart_button($output);
	        ...
	        if ($new_eta_date_mm_dd_yyyy != $product_info["eta_date_mm_dd_yyyy"] || $product_info["r_avail"] != $new_r_avail || $product_info["cost_to_us"] != $new_cost_to_us){
	                db_query("UPDATE $sql_tbl[products] SET r_avail='$new_r_avail', eta_date_mm_dd_yyyy='$new_eta_date_mm_dd_yyyy', supplier_internal_id_last_parsed_update='".time()."', supplier_internal_id_last_parsed='".time()."', cost_to_us='$new_cost_to_us' WHERE productid='$productid'");
			$make_redirect = true;
	        }
	}
...
//func_print_r($product_info, $count_days);
