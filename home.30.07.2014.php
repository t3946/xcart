...
/*
print($new_search_query_productids);


                $all_productids = func_query("SELECT xcart_products.productid FROM xcart_products LEFT JOIN xcart_products_categories ON xcart_products.productid=xcart_products_categories.productid WHERE xcart_products_categories.categoryid  IN (55264,55268,55278,55279,55280,55281,55282,55283,55294,55719,55720,55721,55722,55723,55724,55725,55726,55727,55728,55263)");

                if (!empty($all_productids) && is_array($all_productids)){
                        $all_productids_arr = array();
                        foreach ($all_productids as $k => $v){
                                $all_productids_arr[] = $v["productid"];
                        }
                }
*/
        ...
        $data_arr["query"]["query_string"]["query"] = $e_search_data["substring"];
        $data_arr["query"]["query_string"]["analyzer"] = "snowball";
        $data_arr["query"]["query_string"]["fields"] = array("productname.productname","sku","upc","brand.brand","description.description");
