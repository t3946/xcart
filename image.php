...
if (zerolen($image) && zerolen($image_path) && isset($config['available_images'][$type]) && !empty($sql_tbl['images_'.$type]) && (!empty($id) || $type == "S" || $type == "F") ) {
	$hash_types = array();
	$i = 0;
	$orig_type = $type;
...
##
###
			if (!$HTTPS && !empty($image_path) && !empty($config["Appearance"]["CDN_domain"]) && $config["Appearance"]["Enable_CDN"] == "Y" && strpos($image_path, "./")!== false && AREA_TYPE == "C"){
