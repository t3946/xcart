...
if ($mode == 'send_asjkdfhawiue4ygefcasjkdhfiweudfc12eqwedf') {

	    $end_date = time();
            $start_date = mktime(0, 0, 0, date("m", $end_date), date("d", $end_date)-1, date("Y", $end_date));  
...
//            $end_date = mktime(23, 59, 59, $Month, $Day, $Year);
		...
		foreach ($providers as $k => $v) {
	        ...
	        if (!empty($config['Email_Note']['eml_send_product_reports_to'])) {
                ...
                $error = false;
                foreach ($operators as $cidev_k => $operator) {
