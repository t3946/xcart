...
include_once "MWSRecommendationsSectionService/Samples/.config.inc.php";
require_once "MWSRecommendationsSectionService/Client.php";
require_once "MWSRecommendationsSectionService/Exception.php";
require_once "MWSRecommendationsSectionService/Model/ListRecommendationsRequest.php";
require_once "MWSRecommendationsSectionService/Model/ListRecommendationsByNextTokenRequest.php";

### ###

#################################
if ($config["cidev_amazon_info"] == "Y"){
        die("Already launched"); // ################################
}
db_query("UPDATE $sql_tbl[config] SET value='Y' WHERE name='cidev_amazon_info'");
...
// print("<br />\r\n<B>" . $Fulfillment_Inventory_Pricing . ":</B> ");
...
######################################################
 ...
 while (!empty($NextToken)){
  ...
  if ($NextToken == "start"){
    ...
    $request->setRecommendationCategory($Fulfillment_Inventory_Pricing);
//    $request->setCategoryQueryList(array("ListingStatus"=>"Active"));

    // object or array of parameters
    $dom_xml = invokeListRecommendations($service, $request);
...
//	$request->setCategoryQueryList(array("ListingStatus"=>"Active"));
	...
	$dom_xml_member_arr = $dom_xml_arr["ListRecommendationsResponse"]["ListRecommendationsResult"][$Fulfillment_Inventory_Pricing."Recommendations"];
    }


    if (!empty($dom_xml_arr) && !empty($dom_xml_arr["ListRecommendationsResponse"]["ListRecommendationsResult"]["NextToken"])){
	$NextToken = $dom_xml_arr["ListRecommendationsResponse"]["ListRecommendationsResult"]["NextToken"];
    } 
    else {
	$NextToken = "";
    }
...
//    $request->setRecommendationCategory($Fulfillment_Inventory_Pricing);    // <------------- uncomment it
//    $request->setCategoryQueryList(array("ListingStatus"=>"Active"));

    // object or array of parameters
    $dom_xml = invokeListRecommendationsByNextToken($service, $request);
...
//func_print_r($dom_xml);
    ...
    if (!empty($dom_xml)){
        ...
        $dom_xml_arr = func_xml2hash($dom_xml, "UTF-8");
        $dom_xml_member_arr = $dom_xml_arr["ListRecommendationsByNextTokenResponse"]["ListRecommendationsByNextTokenResult"][$Fulfillment_Inventory_Pricing."Recommendations"];
    }


    if (!empty($dom_xml_arr["ListRecommendationsByNextTokenResponse"]["ListRecommendationsByNextTokenResult"]["NextToken"])){
	    $NextToken = $dom_xml_arr["ListRecommendationsByNextTokenResponse"]["ListRecommendationsByNextTokenResult"]["NextToken"];
    }
    else {
	    $NextToken = "";
    }
...
//	print("Empty dom_xml_member_arr <br />\r\n");
	continue;
  }
...
// func_print_r($dom_xml);
// func_print_r($dom_xml_member_arr);

  foreach ($dom_xml_member_arr as $k_member => $v_member_arr){

	$productcode = $v_member_arr["ItemIdentifier"]["Sku"];
	...
	##
	###
//	if (!isset($amazon_fba_products[$productid]["report_date"])){
//		$amazon_fba_products[$productid]["report_date"] = time();
//	}

	$LastUpdated = strtotime($v_member_arr["LastUpdated"]);
	$amazon_fba_products[$productid][$k_prefix."LastUpdated"] = $LastUpdated;

	if ($k_prefix == "ffr_"){
		$amazon_fba_products[$productid]["ffr_SalesRank"] = $v_member_arr["SalesRank"];
		$amazon_fba_products[$productid]["ffr_BuyboxPrice"] = $v_member_arr["BuyboxPrice"]["Amount"];
		$amazon_fba_products[$productid]["ffr_NumberOfOffers"] = $v_member_arr["NumberOfOffers"];
		$amazon_fba_products[$productid]["ffr_NumberOfOffersFulfilledByAmazon"] = $v_member_arr["NumberOfOffersFulfilledByAmazon"];
		$amazon_fba_products[$productid]["ffr_AverageCustomerReview"] = $v_member_arr["AverageCustomerReview"];
		$amazon_fba_products[$productid]["ffr_NumberOfCustomerReviews"] = $v_member_arr["NumberOfCustomerReviews"];
	} 
	elseif ($k_prefix == "ir_"){
		$amazon_fba_products[$productid]["ir_SalesForTheLast14Days"] = $v_member_arr["SalesForTheLast14Days"];
		$amazon_fba_products[$productid]["ir_SalesForTheLast30Days"] = $v_member_arr["SalesForTheLast30Days"];
		$amazon_fba_products[$productid]["ir_AvailableQuantity"] = $v_member_arr["AvailableQuantity"];
		$amazon_fba_products[$productid]["ir_DaysUntilStockRunsOut"] = $v_member_arr["DaysUntilStockRunsOut"];
		$amazon_fba_products[$productid]["ir_InboundQuantity"] = $v_member_arr["InboundQuantity"];
		$amazon_fba_products[$productid]["ir_RecommendedInboundQuantity"] = $v_member_arr["RecommendedInboundQuantity"];
		$amazon_fba_products[$productid]["ir_DaysOutOfStockLast30Days"] = $v_member_arr["DaysOutOfStockLast30Days"];
		$amazon_fba_products[$productid]["ir_LostSalesInLast30Days"] = $v_member_arr["LostSalesInLast30Days"];
	}
	elseif ($k_prefix == "pr_"){
		$amazon_fba_products[$productid]["pr_YourPricePlusShipping"] = $v_member_arr["YourPricePlusShipping"]["Amount"];
		$amazon_fba_products[$productid]["pr_LowestPricePlusShipping"] = $v_member_arr["LowestPricePlusShipping"]["Amount"];
		$amazon_fba_products[$productid]["pr_PriceDifferenceToLowPrice"] = $v_member_arr["PriceDifferenceToLowPrice"]["Amount"];
		$amazon_fba_products[$productid]["pr_MedianPricePlusShipping"] = $v_member_arr["MedianPricePlusShipping"]["Amount"];
		$amazon_fba_products[$productid]["pr_LowestMerchantFulfilledOfferPrice"] = $v_member_arr["LowestMerchantFulfilledOfferPrice"]["Amount"];
		$amazon_fba_products[$productid]["pr_LowestAmazonFulfilledOfferPrice"] = $v_member_arr["LowestAmazonFulfilledOfferPrice"]["Amount"];
		$amazon_fba_products[$productid]["pr_NumberOfOffers"] = $v_member_arr["NumberOfOffers"];
		$amazon_fba_products[$productid]["pr_NumberOfMerchantFulfilledOffers"] = $v_member_arr["NumberOfMerchantFulfilledOffers"];
		$amazon_fba_products[$productid]["pr_NumberOfAmazonFulfilledOffers"] = $v_member_arr["NumberOfAmazonFulfilledOffers"];
	}
...
//func_print_r($amazon_fba_products);
...
#
		...
		if ($lowest_LandedPrice != "" && $use_lowest_k_LowestOfferListing != ""){
			...
			$amazon_fba_products[$v_lol_arr["productid"]]["lp_MultipleOfferListingsAtLowestPrice"] = $v_lol_arr["Product"]["LowestOfferListings"][$k_LowestOfferListing]["MultipleOffersAtLowestPrice"];
			...
			$amazon_fba_products[$v_lol_arr["productid"]]["lp_NumberOfOfferListingsConsidered"] = $v_lol_arr["Product"]["LowestOfferListings"][$k_LowestOfferListing]["NumberOfOfferListingsConsidered"];
			$amazon_fba_products[$v_lol_arr["productid"]]["lp_SellerFeedbackCount"] = $v_lol_arr["Product"]["LowestOfferListings"][$k_LowestOfferListing]["SellerFeedbackCount"];
		
			$FulfillmentChannel = "";
			if ($v_lol_arr["Product"]["LowestOfferListings"][$k_LowestOfferListing]["Qualifiers"]["FulfillmentChannel"] == "Merchant") $FulfillmentChannel = "MFN";
			if ($v_lol_arr["Product"]["LowestOfferListings"][$k_LowestOfferListing]["Qualifiers"]["FulfillmentChannel"] == "Amazon") $FulfillmentChannel = "AFN";
			$amazon_fba_products[$v_lol_arr["productid"]]["lp_FulfillmentChannel"] = $FulfillmentChannel;
			$amazon_fba_products[$v_lol_arr["productid"]]["lp_ShippingTime"] = $v_lol_arr["Product"]["LowestOfferListings"][$k_LowestOfferListing]["Qualifiers"]["ShippingTime"]["Max"];
			$amazon_fba_products[$v_lol_arr["productid"]]["lp_SellerPositiveFeedbackRating"] = $v_lol_arr["Product"]["LowestOfferListings"][$k_LowestOfferListing]["Qualifiers"]["SellerPositiveFeedbackRating"];
		} else {
