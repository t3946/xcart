...
###    func_print_r($dom_xml);
        ...
        if (!empty($dom_xml)) {
            ...
            if ($pos !== "false") {
                ...
                $dom_xml_orig = str_replace('https://mws.amazonservices.com/Orders/2013-09-01','',$dom_xml_orig);
                $docOrders->loadXML($dom_xml_orig);
                $xpath = new DOMXPath($docOrders);
                $queryOrders = '/ListOrdersByNextTokenResponse/ListOrdersByNextTokenResult/Orders/Order';
                $aOrdersEntries = $xpath->query($queryOrders);
...
//	foreach ($dom_xml_arr["ListOrdersResponse"]["ListOrdersResult"]["Orders"] as $k => $v
        foreach ($aOrdersEntries as $k => $v) {
            func_flush(".");
            func_flush();

            $oOrder = \Xcart\Order::model()->find(\Xcart\SQLBuilder::getInstance()->addCondition("amazonorderid='".$v->getElementsByTagName('AmazonOrderId')->item(0)->nodeValue."'"));

//func_print_r($orderid, $v);
//die("====");

            if (!$oOrder->getOrderId()) {
                $oOrder->setField('amazonorderid', $v->getElementsByTagName('AmazonOrderId')->item(0)->nodeValue);
                $request = new MarketplaceWebServiceOrders_Model_GetOrderRequest();
                $request->setSellerId(MERCHANT_ID);
                $request->setAmazonOrderId($oOrder->getField('amazonorderid'));
                // object or array of parameters
...
//                        if (!empty($GetOrder_xml["Caught_Exception"]) && $GetOrder_xml["Caught_Exception"] == "Request is throttled" && $GetOrder_xml["Response_Status_Code"] == "503")
                ...
                $log_text = "Processing order: " . $oOrder->getField('amazonorderid') . " - " . $oOrder->getOrderId() . "  status: " . $order_info["OrderStatus"];
                print($log_text . "\r\n");
                func_backprocess_log("amazon_orders", $log_text);
...
//die("111---===---111");
                ...
                if (!empty($OrderItems_arr) && is_array($OrderItems_arr["ListOrderItemsResponse"]["ListOrderItemsResult"]["OrderItems"]) && ($order_info['OrderStatus'] == 'Unshipped' || $order_info['OrderStatus'] == 'Shipped')) {
                    ...
                    setField('amazon_fulfillment_channel', $order_info["FulfillmentChannel"])->
                    setField('total', $order_info['OrderTotal']['Amount'])->
                    setField('date', $PurchaseDate)->
                    setField('cb_status', ($order_info['OrderStatus'] == 'Canceled' ? 'A' : 'P'))->
                    setField('dc_status', ($order_info['OrderStatus'] == 'Unshipped' ? 'T' : 'S'))->
                    setField('bd_status', 'W')->
                    setField('payment_method', 'Amazon Seller')->
                    setField('firstname', addslashes($order_info['BuyerName']))->
                    setField('s_firstname', addslashes($order_info['ShippingAddress']['Name']))->
                    setField('s_address', addslashes($Address))->
                    setField('s_city', addslashes($order_info['ShippingAddress']['City']))->
                    setField('s_state', addslashes($StateOrRegion))->
                    setField('s_country', addslashes($order_info['ShippingAddress']['CountryCode']))->
                    setField('s_zipcode', addslashes($PostalCode))->
                    setField('b_firstname', addslashes($order_info['BuyerName']))->
                    setField('b_address', addslashes($Address))->
                    setField('b_city', addslashes($order_info['ShippingAddress']['City']))->
                    setField('b_state', addslashes($StateOrRegion))->
                    setField('b_country', addslashes($order_info['ShippingAddress']['CountryCode']))->
                    setField('b_zipcode', addslashes($PostalCode))->
                    setField('phone', addslashes($order_info['ShippingAddress']['Phone']))->
                    setField('email', addslashes($order_info['BuyerEmail']))->
                    ...
                    setField('vn_status', ($order_info['FulfillmentChannel'] == 'AFN' ? 'PV' : 'NS'));
                    ...
                    foreach ($OrderItems_arr["ListOrderItemsResponse"]["ListOrderItemsResult"]["OrderItems"] as $k_item => $v_item) {
                        $oProduct = Xcart\Product::model()->getProductBySKU(addslashes($v_item['SellerSKU']));

                        if (!$oProduct->getProductId()) {
                            /*search product in fba_missing_sku*/
                            $oFbaMissing = \Xcart\FbaMissingSku::model(['missing_productcode' => addslashes($v_item['SellerSKU'])]);
                            if ($oFbaMissing->getProductId()) {
                                $oProduct = \Xcart\Product::model(['productid' => $oFbaMissing->getProductId()]);
                            } else
                                if ($oFbaMissing->getField('missing_productcode') == '') {
                                    $oFbaMissing->setField('missing_productcode', addslashes($v_item['SellerSKU']))->setField('productid', 0)->_insert();
                                }

                        }

                        if (!in_array($oProduct->getManufacturerId(), $manufacturerid_arr)) {
                            $oOrderGroup = Xcart\OrderGroup::model()->
                            setField('orderid', $oOrder->getOrderId())->
                            setField('manufacturerid', $oProduct->getManufacturerId())->
                            setField('shipping', addslashes($order_info['ShipmentServiceLevelCategory']))->
                            setField('cb_status', ($order_info['OrderStatus'] == 'Canceled' ? 'A' : 'P'))->
                            setField('dc_status', ($order_info['OrderStatus'] == 'Unshipped' ? 'T' : 'S'))->
                            setField('bd_status', 'W')->
                            setField('total_net', $order_info['OrderTotal']['Amount'])->
                            setField('total_gross', $order_info['OrderTotal']['Amount']);
                            $oOrderGroup->_insert();
                            $manufacturerid_arr[] = $oProduct->getManufacturerId();
                        }
                        ...
                        $oOrderDetail = Xcart\OrderDetail::model()->
                        setField('orderid', $oOrder->getOrderId())->
                        setField('productid', $oProduct->getProductId())->
                        setField('item_cost_to_us', $oProduct->getProductCostToUs())->
                        setField('price', $v_item['ItemPrice']['Amount'] / $v_item['QuantityOrdered'])->
                        setField('amount', $v_item['QuantityOrdered'])->
                        setField('productcode', addslashes($v_item['SellerSKU']))->
                        setField('AmazonOrderItemCode', addslashes($v_item['OrderItemId']))->
                        setField('product', addslashes($oProduct->getProductName()))->
                        setField('extra_data', serialize($extra_data));
                        if (!empty($order_info['item_cost_to_us'])) {
                            $oOrderDetail->setField('item_cost_to_us', $order_info['item_cost_to_us']);
                        }
                    ...
                    $oOrder->updateFields(['extra' => serialize($extra), 'subtotal' => $product_total]);

                    $oOrder->updateVerificationStatus()->reCalculateTotals();

                    $id = func_query_first_cell("SELECT id FROM $sql_tbl[cidev_amazon_order_raw] WHERE orderid='" . $oOrder->getOrderId() . "'");
                    if (!empty($id)) {
                        db_query("DELETE FROM $sql_tbl[cidev_amazon_order_raw] WHERE id='$id'");
                    }

                    db_query("INSERT INTO $sql_tbl[cidev_amazon_order_raw] (orderid, order_info, orderitems_info) VALUES ('" . $oOrder->getOrderId() . "', '" . addslashes(serialize($v)) . "', '" . addslashes(serialize($OrderItems_arr) . $GetOrder_xml) . "')");
                    ...
                    func_log_order($oOrder->getOrderId(), 'S', $log, "Amazon");
