...
###
//$file_is_found = true;
//$local_file = $xcart_dir . "/files/product_feeds_v2/feed198p.txt";
###
					...
					if (!empty($not_xcart_products_fields["attributes"]) && is_array($not_xcart_products_fields["attributes"])){
						...
						foreach ($not_xcart_products_fields["attributes"] as $f_name => $fv_name_arr){
							if (!empty($fv_name_arr) && is_array($fv_name_arr)){
								...
								$f_id = func_query_first_cell("SELECT f_id FROM $sql_tbl[cidev_filters] WHERE f_name='$f_name' AND storefrontid='$v[storefrontid]'");
								if (empty($f_id)){
									db_query("INSERT INTO $sql_tbl[cidev_filters] (f_name, storefrontid) VALUES ('$f_name', '$v[storefrontid]')");
									$f_id = db_insert_id();
								}
					...
					if (!empty($not_xcart_products_fields["brand_name"])){
						...
						$BRAND = $not_xcart_products_fields["brand_name"];
	                                        ...
	                                        if (empty($brandid)){
                                	                ...
                                	                db_query("INSERT INTO $sql_tbl[brands_sf] (brandid, sfid) VALUES('$brandid', '$v[storefrontid]')");
                                        ...
                                        if (!empty($not_xcart_products_fields["supplier_categories"][0])){
                                                ...
                                                if (!empty($cats_arr) && is_array($cats_arr)){
                                                        ...
                                                        foreach ($cats_arr as $v_cat){
                                                                ...
                                                                if (empty($categoryid)){
                                                                        db_query("INSERT INTO $sql_tbl[categories] (parentid, storefrontid, category, is_bold, order_by) VALUES ('$parentid', '$v[storefrontid]', '".addslashes($v_cat)."', 'N', '10')");
