...
#
# $Id: zones.php,v 1.18.2.2 2006/10/03 10:20:57 max Exp $
#
...
if ($REQUEST_METHOD == "POST") {
	...
	#
		...
		if (!empty($zone_name)) {
			if (empty($zoneid)) {
				db_query("INSERT INTO $sql_tbl[zones] (zone_name, provider) VALUES ('$zone_name', '$login')");
				$zoneid = db_insert_id();
				$new_zone = true;
				...
				for ($i = 0; $i < count($zone_cache_array); $i++) {
					if (preg_match("/^([\w])([0-9]+)$/", $zone_cache_array[$i], $match)) {
						...
						if ($match[2] == 1) {
							...
							$_element = func_query_first_cell("SELECT field FROM $sql_tbl[zone_element] WHERE zoneid='$zone[zoneid]' AND field_type='$match[1]' LIMIT 1");
							if ($match[1] == "C")
