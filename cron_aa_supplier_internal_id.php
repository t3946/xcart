...
if ($config["cron_aa_supplier_internal_id"] == "Y"){
        die("Already launched"); // ################################
}
db_query("UPDATE $sql_tbl[config] SET value='Y' WHERE name='cron_aa_supplier_internal_id'");
...
###
	...
	curl_setopt($ch, CURLOPT_TIMEOUT_MS, 5000);
		...
		$productid = func_query_first_cell("SELECT $sql_tbl[products].productid FROM $sql_tbl[products] LEFT JOIN $sql_tbl[products_sf] ON $sql_tbl[products].productid=$sql_tbl[products_sf].productid WHERE $sql_tbl[products_sf].sfid='12' AND $sql_tbl[products].supplier_internal_id='$i'");

		if (!empty($productid)){
			db_query("UPDATE $sql_tbl[products] SET forsale='N', supplier_internal_id='', supplier_internal_option='' WHERE productid='$productid' ");
		}
	...
	$product_info_arr = func_query_first("SELECT productid, productcode, forsale, avail, r_avail, eta_date_mm_dd_yyyy, cost_to_us, update_search_index, discount_table, new_map_price, min_amount, list_price FROM $sql_tbl[products] WHERE productcode='".addslashes($productcode)."'");
	...
	$productid = $product_info_arr["productid"];
        ...
        $current_min_amount = $product_info_arr["min_amount"];
        $current_mult = $product_info_arr["mult_order_quantity"];
...
//      $current_avail = $product_info_arr["avail"];
        ...
        $current_discount_table = $product_info_arr["discount_table"];

        $new_forsale = "Y";
        $new_r_avail = $current_r_avail;
        $new_min_amount = $current_min_amount;
...
##########################################
	...
	if (empty($opConfig)){
		...
		$opConfig_arr = json_decode($opConfig, true);

		$opConfig_arr_keys = array_keys($opConfig_arr);
    		
    		unset($loadproducts);
    		$loadproducts = array();

		foreach ($opConfig_arr_keys as $k_k => $v_k){
			$loadproducts[$k_k] = func_GetAAJ_product_info($i, $v_k);
		}
		func_print_r($loadproducts);

		$min_amount = "";
		if (!empty($loadproducts) && is_array($loadproducts)){
			foreach ($loadproducts as $kl => $vl){
				if ($min_amount == "" && !empty($vl["min_amount"])){
					$min_amount = $vl["min_amount"];
					$key_kl = $kl;
					$t_uom = $vl["unit"];
					$rd = $vl["rawdata"];
					$rd = str_replace(array("'"), '', $rd)
					db_query("replace into xcart_k.cidev_aaj_units SET uom='$t_uom', supplierid='$i', sample='$rd'  ");
					continue;
				}

				if (!empty($vl["min_amount"]) && $min_amount > $vl["min_amount"]){
					$min_amount = $vl["min_amount"];
					$key_kl = $kl;
					$t_uom = $vl["unit"];
					$rd = $vl["rawdata"];
		...
		$loadproduct = $loadproducts[$key_kl];
			...
			if (!empty($loadproduct["discount_table"])){
				$new_discount_table = $loadproduct["discount_table"];
                        ...
                        if (isset($loadproduct["cost_to_us"])){
                                $new_cost_to_us = $loadproduct["cost_to_us"] / $min_amount;
                        }
		}
	}

//func_print_r($output, $opConfig, $opConfig_arr, $opConfig_arr_keys, $loadproduct, $loadproducts);
//die("test");
...
//	$parsed_cost_to_us = func_parse_cost_to_us($output);
		...
		if (!empty($data_arr["product"]["discount"]) && is_array($data_arr["product"]["discount"])){
			...
			if (!empty($new_discount_table)){
				$new_discount_table = implode(",", $new_discount_table_arr);
			} else {
				$new_discount_table = "2,3,4,6,8,12";
			}
		}

//func_print_r($data_arr, $new_discount_table, $productid);
//die();
...
//func_print_r($data_arr, $parsed_cost_to_us);
//die();
		...
		unset($post);
//              func_print_r($parsed_cost_to_us, $i);
###
...
*/
	...
	if ($new_forsale != $current_forsale || $new_mult != $current_mult || $new_eta_date_mm_dd_yyyy != $current_eta_date_mm_dd_yyyy || $current_r_avail != $new_r_avail || $current_cost_to_us != $new_cost_to_us || $current_discount_table != $new_discount_table || $new_new_map_price != $current_new_map_price || $new_min_amount != $current_min_amount || $new_list_price != $current_list_price){
...
*/
		...
		db_query("UPDATE $sql_tbl[products] SET supplier_internal_id='$i', r_avail='$new_r_avail', eta_date_mm_dd_yyyy='$new_eta_date_mm_dd_yyyy', forsale='$new_forsale', supplier_internal_id_last_parsed_update='".time()."', supplier_internal_id_last_parsed='".time()."', cost_to_us='$new_cost_to_us', discount_table='$new_discount_table', new_map_price='$new_new_map_price', supplier_internal_option='$loadproduct[supplier_internal_option]', min_amount='$new_min_amount', mult_order_quantity='$new_mult', list_price='$new_list_price' WHERE productid='$productid'");
		$product_is_updated = true;

		if ($current_discount_table != $new_discount_table){
...
//die("asd");
	...
	if (!empty($filters) && is_array($filters)){
		foreach ($filters as $filter){
			...
			if (empty($f_id)){
				db_query("REPLACE INTO $sql_tbl[cidev_filters] (f_name, storefrontid) VALUES ('".addslashes($f_name)."', '12')");
				$f_id = db_insert_id();
			}
			...
			foreach ($filter["fv_name_arr"] as $fv_name){
				...
				if (empty($fv_id)){
					db_query("REPLACE INTO $sql_tbl[cidev_filter_values] (f_id, fv_name) VALUES ('$f_id', '".addslashes($fv_name)."')");
					$fv_id = db_insert_id();
				}

				$fp_id = func_query_first_cell("SELECT fp_id FROM $sql_tbl[cidev_filter_products] WHERE fv_id='$fv_id' AND productid='$productid'");
				if (empty($fp_id)){
					db_query("REPLACE INTO $sql_tbl[cidev_filter_products] (fv_id, productid) VALUES ('$fv_id', '$productid')");
				}
			}
		}
	}
...
db_query("UPDATE $sql_tbl[config] SET value='N' WHERE name='cron_aa_supplier_internal_id'");
