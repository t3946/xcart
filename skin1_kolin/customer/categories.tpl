...
<td></td>
<td width="100%" valign="middle"><div class="seo_brands_link"><a href="#"><font class="VertMenuTitle" style="color: #0033cc;">Brands</font></a> <img src="{$ImagesDir}/br_down.png" alt="" /></div></td>
</tr></table>
</td>
</tr>
<tr id="seo_brands_1">
...
{foreach from=$menu_brands item=v key=k}

{if $k lt $show_count_before_see_more}
<a href="brands.php?brandid={$v.brandid}">{$v.brand}</a><br />
{/if}
...
{if $k gte $show_count_before_see_more}
<a href="brands.php?brandid={$v.brandid}">{$v.brand}</a><br />
{/if}
...
{/capture}
{include file="menu.tpl" menu_title="Brands" menu_content=$smarty.capture.menu cellpadding=$fc_cellpadding id_expand="brands_menu" link_href="javascript: void();" content_hide="Y"}
*}
         ...
         {if $aBrandFilters|@count > 0}
             {if $filter_name ne ""}
                 ...
                 {foreach from=$aBrandFilters item=oBrand key=k name=brandloop}
                     ...
                     {/if}
                 {/foreach}
                 {if count($aBrandFilters|@count) gt 5}
                 <tr>
                     <td colspan="2" align="right">
                         <a class="simple-button" target="_blank" title="Show more"
