...
{/if}
	<td class="SubcatTitle"{if $current_category.main_order_by gt 500} style="padding-left: 5px"{/if}><a href="home.php?cat={ $subcat.categoryid }&amp;path=alt"> <font class="{if ($subcat.parentid eq $cat && $subcat.is_bold eq 'Y') || ($subcat.parentid ne $cat && $subcat.add_is_bold eq 'Y')}ItemsList{else}ItemsList1{/if}"{if $current_category.main_order_by gt 500} face="Verdana"{/if}>{ $subcat.category|escape }</font></a><br /></td>
...
{if $products}
{if $sort_fields}
<div align="right">{include file="main/search_sort_by.tpl" sort_fields=$sort_fields selected=$search_prefilled.sort_field direction=$search_prefilled.sort_direction url="home.php?cat=`$cat`&amp;path=alt&amp;"}</div>
{/if}
{if $total_pages gt 2}
