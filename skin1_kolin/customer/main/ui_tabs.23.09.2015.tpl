...
{if $product_wholesale.0.price ne ""}
	{assign var="current_price" value=$product_wholesale.0.price}
{/if}

{* {if $product.min_amount gt 1 && $product.mult_order_quantity eq "Y"} *}
{if $product.min_amount gt 1 && $product.product_availability eq "in stock"}
	{math assign="itemprop_price" equation="y*x" y=$product.min_amount x=$current_price}
{*	<input type="hidden" name="product_min_amount" value="{$product.min_amount}" id="product_min_amount" /> *}
{else}
	{if $product.min_amount gt 1 && $product.mult_order_quantity eq "Y" && $product.product_availability ne "in stock"}
		{math assign="itemprop_price" equation="y*x" y=$product.min_amount x=$current_price}
	{else}
