...
$(document).ready(function() {
 ...
 {foreach from=$shipping_groups item=v key=k}
  ...
  {literal}tmp_k = {/literal}{$k}{literal};{/literal}

  {foreach from=$shippings[$k] item=s}
  {literal}

	$('#use_my_account_number_'+tmp_k).focusout(function() {
		$('#use_my_account_number_'+tmp_k).attr('placeholder', '{/literal}{$lng.lbl_use_my_account_number}{literal}');
	});

        $('#use_my_account_number_'+tmp_k).focus(function() {
                $('#use_my_account_number_'+tmp_k).attr('placeholder', '');
        });

        $('#ship_by_shipping_method_'+tmp_k).focusout(function() {
                $('#ship_by_shipping_method_'+tmp_k).attr('placeholder', '{/literal}{$lng.lbl_ship_by_shipping_method}{literal}');
        });

        $('#ship_by_shipping_method_'+tmp_k).focus(function() {
                $('#ship_by_shipping_method_'+tmp_k).attr('placeholder', '');
        });


        $('#t_use_my_account_number_'+tmp_k).focusout(function() {
                $('#t_use_my_account_number_'+tmp_k).attr('placeholder', '123456');
        });

        $('#t_use_my_account_number_'+tmp_k).focus(function() {
                $('#t_use_my_account_number_'+tmp_k).attr('placeholder', '');
        });

        $('#t_ship_by_shipping_method_'+tmp_k).focusout(function() {
                $('#t_ship_by_shipping_method_'+tmp_k).attr('placeholder', 'AFB Freight');
        });

        $('#t_ship_by_shipping_method_'+tmp_k).focus(function() {
                $('#t_ship_by_shipping_method_'+tmp_k).attr('placeholder', '');
        });
...
<br />
{if $variant_id eq "0"}
	<img src="{$ImagesDir}/free_shipping_competitor.png" alt="" />
{else}
{*	Here will NOT be any Image. Because variant_id != 0. In order image to be loaded variant_id should be equal "0". *}
{/if}
