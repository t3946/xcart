...
<a href="product.php?productid={$products[product].productid}&amp;cat={$cat}&amp;page={$navigation_page}" class="ProductTitle{if $flag eq "related"}Related{/if}"><font color=#0033CC>{$products[product].product}</font></a><br />
...
<a href="product.php?productid={$products[product].productid}&amp;cat={$cat}&amp;page={$navigation_page}" class="VertMenuItems">{$lng.lbl_see_details}</a>
...
{/if}
{/if}
<font class="ProductPrice{if $flag eq "related"}Related{/if}">{$lng.lbl_our_price}: {include file="currency.tpl" value=$current_price}</font><br /><font class="MarketPrice">{include file="customer/main/alter_currency_value.tpl" alter_currency_value=$current_price}</font>{if $discount gt 0}{if $config.General.alter_currency_symbol ne ""},{/if} <font color=#000000 size=2>{$lng.lbl_save_price} {$discount}%</font>{/if}
{if $products[product].map_price gt $products[product].taxed_price}
