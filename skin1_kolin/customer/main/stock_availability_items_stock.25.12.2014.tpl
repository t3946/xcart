...
{if $item.manufacturerid eq $m}
                <tr>
                    ...
                    <td class="survey_form_right">
                        ...
                        {/literal}">
                                <option value="all_in_stock" {if $admin_area_uses eq "Y" && $item.forsale eq "Y" && $item.back eq "0" && $item.eta_date_mm_dd_yyyy eq ""}selected="selected"{/if}>{if $item.amount eq "1"}In stock{else}All items are in stock{/if}</option>
				{if $item.amount gt 1}
