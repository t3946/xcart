...
{if $current_category.SEO_category_name ne ""}
    {assign var="capture_title" value=$current_category.SEO_category_name}
{elseif $keyphrase eq ''}
...
{if $current_category.main_order_by gt 500 || $active_modules.CIDEV_Best_Search_Filter eq ""}
...
{if $cidev_orig_dispatched_request ne ""}
{include file="main/search_sort_by.tpl" sort_fields=$sort_fields selected=$search_prefilled.sort_field direction=$search_prefilled.sort_direction url="`$cidev_orig_dispatched_request`/?`$fv_ids_navigation_url_amp`&amp;path=alt&amp;page=`$navigation_page`&amp;"}
{else}
{include file="main/search_sort_by.tpl" sort_fields=$sort_fields selected=$search_prefilled.sort_field direction=$search_prefilled.sort_direction url="home.php?cat=`$cat``$fv_ids_navigation_url_amp``$b_ids_navigation_url_amp`&amp;path=alt&amp;page=`$navigation_page`&amp;"}
{/if}
...
{$lng.lbl_nothing_found_cat_page}
{/if}
{* ----------- *}
...
{if ($navigation_page eq "")||($navigation_page eq "1")}<p><span class="SPItems-description">{$current_category.description}</span><p />{/if}

{if $current_storefront eq "34"}

{assign var="show_SEO_related_categories_title" value="Y"}
...
{foreach from=$linked_out_category_indexes item=v key=k}
        ...
        {if $current_category.$linked_out_category_name ne ""}
			...
			{if $show_SEO_related_categories_title eq "Y"}
				<B>{$lng.lbl_SEO_related_categories_title}</B>
				<br />  
				{assign var="show_SEO_related_categories_title" value="N"}
			{/if}
...
*}
<div style="float: left; {if $k gt 0}padding-left: 15px;{/if}">
                        <a target="_blank" style="color: blue;" href="/home.php?cat={$current_category.$linked_out_category_id}">{$current_category.$linked_out_category_keyphrase_selected}</a>
</div>
                {/if}
        {/if}
{/foreach}
