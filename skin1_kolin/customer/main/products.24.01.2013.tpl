...
<a href="{if $search_all_website eq 'Y'}http://{$products[product].domain}/{/if}product.php?productid={$products[product].productid}&amp;cat={$cat}&amp;path=alt&amp;page={$navigation_page}{if $featured eq 'Y'}&amp;featured{/if}"  {if $search_all_website eq 'Y'}target="_blank"{/if}>{include file="product_thumbnail.tpl" productid=$products[product].productid image_x=$config.Appearance.thumbnail_width product=$products[product].product tmbn_url=$products[product].tmbn_url}</a>
{if $active_modules.Special_Offers ne "" and $products[product].have_offers}
{include file="modules/Special_Offers/customer/product_offer_thumb.tpl" product=$products[product]}
{/if}
</div>
<a href="{if $search_all_website eq 'Y'}http://{$products[product].domain}/{/if}product.php?productid={$products[product].productid}&amp;cat={$cat}&amp;path=alt&amp;page={$navigation_page}{if $featured eq 'Y'}&amp;featured{/if}" class="SeeDetails" {if $search_all_website eq 'Y'}target="_blank"{/if}>{$lng.lbl_see_details}</a>
...
<td valign="top">
<a href="{if $search_all_website eq 'Y'}http://{$products[product].domain}/{/if}product.php?productid={$products[product].productid}&amp;cat={$cat}&amp;path=alt&amp;page={$navigation_page}{if $featured eq 'Y'}&amp;featured{/if}" {if $search_all_website eq 'Y'}target="_blank"{/if}><font class="ProductTitle">{$products[product].product}</font></a>
...
<div style="max-height: 44px; overflow: hidden; line-height: 14px">
	{$products[product].descr|truncate:225:"...":true}
</div>
</font>
{*<hr class="PListLine" size="1" />*}
<br>
{if $products[product].product_type eq "C"}
{include file="buttons/details.tpl" href="product.php?productid=`$products[product].productid`&amp;cat=`$cat`&amp;path=alt&amp;page=`$navigation_page`"}
{else}
