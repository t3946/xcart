	...
	<td height="25">
		...
		<tr>
			<td>
				...
				{include file="buttons/buy_now.tpl" style="button" href="product.php?productid=`$product.productid`&cat=`$cat`&page=`$button_href`" b=1}
			</td>
			{if $featured eq 'Y'}
				{assign var="button_href" value="product.php?productid=`$products[product].productid`&cat=`$cat`&path=alt&page=`$navigation_page`&featured"}
			{else}
				{assign var="button_href" value="product.php?productid=`$products[product].productid`&cat=`$cat`&path=alt&page=`$navigation_page`"}
			{/if}
