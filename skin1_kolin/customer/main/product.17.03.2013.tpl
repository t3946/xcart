...
<tr>
	<td class="PImgBox" rowspan="2" width="300" style="border: 1px dashed #cccccc;">
...
{/if}
	...
	<td valign="top" width="170" style="padding-left: 20px;">
...
{if $discount gte 1}
<TABLE border="0" cellpadding="0" cellspacing="0" id="save_percent_box"{if $product.taxed_price >= $product.list_price} style="display: none;"{/if}>
<TR>
<TD nowrap="nowrap">
<font style="font-size: 12px; color: #CC3333;">and</font>&nbsp;
</TD>
<TD nowrap height="20" style="background-color: #cc3333; color: white; font-size: 12px; font-weight: normal;" align="center">
&nbsp;SAVE&nbsp;<SPAN id="save_percent">{$discount|string_format:"%3.0f"}</SPAN>%&nbsp;
</TD>
<TD nowrap="nowrap">
&nbsp;<font style="font-size: 12px; color: #CC3333;">off List!</font>
</TD>
</TR>
</TABLE>
{/if}
...
<span style="font-size: 13px; color: #000000;">{if $product.fulldescr ne ""}{$product.fulldescr}{else}{$product.descr}{/if}</span>
...
<tr>
        <td width="10%" class="BlackT">{$product.upc_ean_isbn.type}:</td>
        <td nowrap="nowrap">{$product.upc_ean_isbn.value}</td>
</tr>
{/if}
{if $show_dimensions}
<tr>
        <td width="10%">{$lng.lbl_shipping_dimensions}:</td>
