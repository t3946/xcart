    ...
    <div class="header">
        ...
        <div class="search">
            <table class="search-table">
              <tr>
{*                <td><img alt="{$config.Search_All.search_all_website_name}" class="logo" src="{$ImagesDir}/front-page/S3-Logo-Small-v1.gif" /></td> *}
                <td><img alt="{$config.Search_All.search_all_website_name}" class="logo" src="{$ImagesDir}/S3-Stores-Logo-S2.png" /></td>
...
{*
            <table cellspacing="0" cellpadding="0" class="sf-links">
                <tr>
                    <td style="vertical-align: top;" width="{$sf_column_percent}%">
                        <span>{$lng.lbl_sf_links_on_search_page}:</span><br />
                        {assign var=cell_counter value=1}
                        {foreach item=v from=$sf_links}
                            {assign var=cell_counter value=$cell_counter+1}
                            <a href="http://{$v.domain}" target="_blank">{$v.name}</a> 
                            {if $cell_counter eq $items_per_column}
