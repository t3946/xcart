...
/* ------------------------------------- */
	if (obj.name == 's_country'){

		if ($('#s_country').val() == "US"){
			$('#s_zipcode').attr('maxlength', '5');
		} else if ($('#s_country').val() == "CA"){
			$('#s_zipcode').attr('maxlength', '6');
		} else {
			$('#s_zipcode').attr('maxlength', '20');
		}

		if (geo_litecity_location_region != "" && ($('#s_country').val() == "US" || $('#s_country').val() == "CA" || $('#b_country').val() == "AT" || $('#b_country').val() == "DE" || $('#b_country').val() == "AU" || $('#b_country').val() == "BE" || $('#b_country').val() == "ES" || $('#b_country').val() == "FR" || $('#b_country').val() == "IT" || $('#b_country').val() == "LU" || $('#b_country').val() == "NL")){
			$('#s_state').val(geo_litecity_location_region);
		}

		$('#s_state').attr('placeholder', cidev_placeholder_state);
		cidev_check_onSelectChange_load();
	}

        if (obj.name == 'b_country'){

                if ($('#b_country').val() == "US"){
                        $('#b_zipcode').attr('maxlength', '5');
                } else if ($('#b_country').val() == "CA"){
                        $('#b_zipcode').attr('maxlength', '6');
                } else {
                        $('#b_zipcode').attr('maxlength', '20');
                }

                if (geo_litecity_location_region != "" && ($('#b_country').val() == "US" || $('#b_country').val() == "CA" || $('#b_country').val() == "AT" || $('#b_country').val() == "DE" || $('#b_country').val() == "AU" || $('#b_country').val() == "BE" || $('#b_country').val() == "ES" || $('#b_country').val() == "FR" || $('#b_country').val() == "IT" || $('#b_country').val() == "LU" || $('#b_country').val() == "NL")){
                        $('#b_state').val(geo_litecity_location_region);
                }

		$('#b_state').attr('placeholder', cidev_placeholder_state);
                cidev_check_onSelectChange_load_b();
        }
/* ------------------------------------- */
...
function cidev_check_onSelectChange_load() {
...
function cidev_check_onSelectChange_load_b() {
	...
	if (countries[code].states === false) {
		...
		if (type != 'S' || this.lastCode != code) {
			...
			if (states_sort_override) {
				...
				for (var i in countries[code].states) {
					this.states.options[this.states.options.length] = new Option(countries[code].states[i].name + ' (' + countries[code].states[i].code + ')', countries[code].states[i].code);
...
/* -------------- ***************** -------------- */
	...
	if (this.name == 's_country'){
		this.states.onchange = cidev_check_onSelectChange;
		$('#s_state').attr('placeholder', cidev_placeholder_state);
        ...
        if (this.name == 'b_country'){
                this.states.onchange = cidev_check_onSelectChange_b;
		$('#b_state').attr('placeholder', cidev_placeholder_state);
...
/* -------------- ***************** -------------- */
function cidev_check_onSelectChange(){
	onSelectChange2();
}
function onSelectChange2() {
	...
	var clear_city_in_Change_states_js = $('#clear_city_in_Change_states_js').val();

        if (stateSelected.val() && clear_city_in_Change_states_js=='Y'){
...
function cidev_check_onSelectChange_b(){
        onSelectChange_b2();
}
function onSelectChange_b2() {
