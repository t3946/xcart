...
{* {assign var=set_width value="33%"} *} {* for good formatting *}
{assign var=count_in_cell value="4"}
<table cellspacing="0" cellpadding="0" width="100%" style="margin-left: 13px;" border="0">
<tr>
<td align="left" style="vertical-align: top;" {if $set_width ne ""}width="{$set_width}"{/if}>
<span class="ProductPrice">{$lng.lbl_help}:</span><br />
<a href="{$config.Company.company_website}/sitemap.php" class="VertMenuItems">{$lng.lbl_sitemap}</a><br />
<a href="retrieve_orders.php" class="VertMenuItems">{$lng.lbl_retrieve_orders}</a><br />
{assign var=cell_counter value=3}

{if $cell_counter gte $count_in_cell}
{assign var=cell_counter value=0}
</td><td align="left" style="vertical-align: top;" {if $set_width ne ""}width="{$set_width}"{/if}>
{/if}
...
{section name=pg loop=$pages_menu}
        ...
        {if $cell_counter eq $count_in_cell}
                {assign var=cell_counter value=0}
                </td>
                {if !%pg.last%}
                        <td align="left" style="vertical-align: top;" {if $set_width ne ""}width="{$set_width}"{/if}>
...
{if $cell_counter ne $count_in_cell}
        </td>
{/if}
</tr>
</table>
...
<TR>
<TD class="Bottom" align="left" colspan="4" style="padding-left: 20px;" height="30">
{/if}
