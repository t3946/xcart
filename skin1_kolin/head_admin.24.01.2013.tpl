...
<tr> 
	<td class="HeadLogo"><a href="{$http_location}/{if $usertype eq "P"}provider{else}admin{/if}/"><img src="{$ImagesDir}/admin_xlogo.gif" width="244" height="67" alt="" /></a></td>
{if $login ne ""}
...
<td class="HeadLine" align="right" height="22">
	...
	<input type="hidden" name="mode" value="change_storefront" />
		<select name="cur_sf" onchange="javascript: document.storefrontsform.submit();">
			<option value="0"{if $current_storefront eq '0'} selected="selected"{/if}>{$main_storefront}</option>
...
<td class="HeadLine" align="right" height="22" style="padding-right: 12px">
    {if $active_modules.Multiple_Storefronts && $usertype eq "A" && $login && $current_membership_flag ne 'FS'}
       <a href="configuration.php?option=Multiple_Storefronts" title="">{$lng.lbl_sf_properties}</a>
    {else}
        &nbsp;
    {/if}
</td>
