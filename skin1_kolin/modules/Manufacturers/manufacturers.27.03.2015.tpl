...
<td>&nbsp;</td>
<td><textarea class="new_editor" name="d_dispatch_instructions" rows="10" cols="60" style="width: 80%;">{$manufacturer.d_dispatch_instructions}</textarea></td>
</tr>
...
<td>Shipping options (use comma to separate)</td>
<td>&nbsp;</td>
...
<tr id="tr_d_message_body_14" {if $manufacturer.d_availability_must_be_checked ne 'Y'}style="display: none;"{/if}>
	...
	<td>&nbsp;</td>
	<td>
	<textarea class="new_editor" name="d_message_body_14" rows="20" cols="60" style="width:80%">{$manufacturer.d_message_body_14|default:$lng.txt_distributor_section_14_email_body}</textarea>
...
<table cellpadding="3" cellspacing="1" width="100%" id="distributor_section_id_20" {if $distributor_section ne "20"}style="display: none;" {/if}>

<tr>
        <td colspan="3" class="FormButton">Quantity in stock behavior on the SF product page:</td>
</tr>

<tr>
	<td colspan="3" class="FormButton"><input type="radio" name="products_quantity_behavior" value="R"{if $manufacturer.products_quantity_behavior eq "R"} checked="checked"{/if} /> display real quantity
	</td>
</tr>
        <td colspan="3" class="FormButton"><input type="radio" name="products_quantity_behavior" value="D"{if $manufacturer.products_quantity_behavior eq "D"} checked="checked"{/if} /> display quantity of

        <input type="text" size="5" name="display_quantity_of" value="{$manufacturer.display_quantity_of}" /> if product is in stock

<br />

	<input type="checkbox" name="allow_pre_orders" value="Y"{if $manufacturer.allow_pre_orders eq 'Y'} checked="checked"{/if} /> Allow pre-orders
