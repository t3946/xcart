{* $Id: orders_export_qb.tpl,v 1.22.2.1 2006/06/23 07:59:08 max Exp $ *}
...
{if $new_order ne ""}
{* make qb happy *}
SPL{$delimiter}INVOICE{$delimiter}{$orders[oid].date|date_format:"%m/%d/%Y"}{$delimiter}{$config.QuickBooks.acct_tax}{$delimiter}{$orders[oid].b_lastname}, {$orders[oid].b_firstname} - {$orders[oid].login}{$delimiter}{$config.QuickBooks.trans_class}{$delimiter}0{$delimiter}{$orders[oid].order_prefix}{$orders[oid].orderid}{$delimiter}TAX{$delimiter}{$delimiter}{$delimiter}{$delimiter}N{$delimiter}AUTOSTAX
ENDTRNS
{/if}
{assign var="new_order" value=$orders[oid].orderid}
TRNS{$delimiter}INVOICE{$delimiter}{$orders[oid].date|date_format:"%m/%d/%Y"}{$delimiter}Accounts Receivable{$delimiter}{$orders[oid].b_lastname}, {$orders[oid].b_firstname} - {$orders[oid].login}{$delimiter}{$config.QuickBooks.trans_class}{$delimiter}{$order_total}{$delimiter}{$orders[oid].order_prefix}{$orders[oid].orderid}{$delimiter}Website Order: {$orders[oid].details|replace:"\r":" "|replace:"\n":" "|replace:"\t":" "}{$delimiter}{$orders[oid].b_firstname} {$orders[oid].b_lastname}{$delimiter}{$orders[oid].b_address} {$orders[oid].b_address_2}{$delimiter}"{$orders[oid].b_city}, {$orders[oid].b_state} {$orders[oid].b_zipcode}"{$delimiter}{$orders[oid].b_countryname}{$delimiter}{$delimiter}{$delimiter}{$orders[oid].s_firstname} {$orders[oid].s_lastname}{$delimiter}{$orders[oid].s_address} {$orders[oid].s_address_2}{$delimiter}"{$orders[oid].s_city}, {$orders[oid].s_state} {$orders[oid].s_zipcode}"{$delimiter}{$orders[oid].s_countryname}{$delimiter}{$delimiter}Y

SPL{$delimiter}INVOICE{$delimiter}{$orders[oid].date|date_format:"%m/%d/%Y"}{$delimiter}{$config.QuickBooks.acct_shipping}{$delimiter}{$orders[oid].b_lastname}, {$orders[oid].b_firstname} - {$orders[oid].login}{$delimiter}{$config.QuickBooks.trans_class}{$delimiter}-{$orders[oid].shipping_cost}{$delimiter}{$orders[oid].order_prefix}{$orders[oid].orderid}{$delimiter}{$orders[oid].shipping|trademark}{$delimiter}{$orders[oid].shipping_cost}{$delimiter}-1{$delimiter}SHIPPING{$delimiter}N{$delimiter}

{if $orders[oid].coupon_discount gt 0}
SPL{$delimiter}INVOICE{$delimiter}{$orders[oid].date|date_format:"%m/%d/%Y"}{$delimiter}{$config.QuickBooks.acct_coupon_discount}{$delimiter}{$orders[oid].b_lastname}, {$orders[oid].b_firstname} - {$orders[oid].login}{$delimiter}{$config.QuickBooks.trans_class}{$delimiter}{$orders[oid].coupon_discount}{$delimiter}{$orders[oid].order_prefix}{$orders[oid].orderid}{$delimiter}{$orders[oid].coupon}{$delimiter}-{$orders[oid].coupon_discount}{$delimiter}-1{$delimiter}COUPON DISCOUNT{$delimiter}N{$delimiter}
{/if}

{if $orders[oid].discount gt 0}
SPL{$delimiter}INVOICE{$delimiter}{$orders[oid].date|date_format:"%m/%d/%Y"}{$delimiter}{$config.QuickBooks.acct_discount}{$delimiter}{$orders[oid].b_lastname}, {$orders[oid].b_firstname} - {$orders[oid].login}{$delimiter}{$config.QuickBooks.trans_class}{$delimiter}{$orders[oid].discount}{$delimiter}{$orders[oid].order_prefix}{$orders[oid].orderid}{$delimiter}DISCOUNT{$delimiter}-{$orders[oid].discount}{$delimiter}-1{$delimiter}DISCOUNT{$delimiter}N{$delimiter}
{/if}

{foreach key=tax_name item=tax from=$orders[oid].tax_values}
{if $tax.tax_cost gt 0}
SPL{$delimiter}INVOICE{$delimiter}{$orders[oid].date|date_format:"%m/%d/%Y"}{$delimiter}{$config.QuickBooks.acct_tax}{$delimiter}{$orders[oid].b_lastname}, {$orders[oid].b_firstname} - {$orders[oid].login}{$delimiter}{$config.QuickBooks.trans_class}{$delimiter}-{$tax.tax_cost}{$delimiter}{$orders[oid].order_prefix}{$orders[oid].orderid}{$delimiter}{$tax.tax_display_name} {$tax.rate_type}{$tax.rate_value}{$delimiter}{$vat_value}{$delimiter}-1{$delimiter}TAX{$delimiter}N{$delimiter}
{/if}
{/foreach}

{/if}{* start new order *}
{if $orders[oid].gcid}{* INVOICE:GIFT CERTIFICATE *}
SPL{$delimiter}INVOICE{$delimiter}{$orders[oid].date|date_format:"%m/%d/%Y"}{$delimiter}{$config.QuickBooks.acct_gc}{$delimiter}{$orders[oid].b_lastname}, {$orders[oid].b_firstname} - {$orders[oid].login}{$delimiter}{$config.QuickBooks.trans_class}{$delimiter}-{$orders[oid].amount}{$delimiter}{$orders[oid].order_prefix}{$orders[oid].orderid}{$delimiter}GC#{$orders[oid].gcid}{$delimiter}{$orders[oid].amount}{$delimiter}-1{$delimiter}GIFT CERTIFICATE{$delimiter}N{$delimiter}
{else}{* INVOICE:PRODUCT *}
{if $orders[oid].productcode ne ""}{assign var="invitem_name" value=$orders[oid].productcode}
{else}{assign var="invitem_name" value=$orders[oid].productid}
{/if}
SPL{$delimiter}INVOICE{$delimiter}{$orders[oid].date|date_format:"%m/%d/%Y"}{$delimiter}{$config.QuickBooks.acct_product}{$delimiter}{$orders[oid].b_lastname}, {$orders[oid].b_firstname} - {$orders[oid].login}{$delimiter}{$config.QuickBooks.trans_class}{$delimiter}-{$orders[oid].cost}{$delimiter}{$orders[oid].order_prefix}{$orders[oid].orderid}{$delimiter}{$orders[oid].product}{if $orders[oid].product_options ne ""}\n{$orders[oid].product_options}{/if}{$delimiter}{$orders[oid].price}{$delimiter}-{$orders[oid].amount}{$delimiter}{$invitem_name}{$delimiter}N{$delimiter}
{/if}

{if $orders[oid].total gt 0 and $orders[oid].giftcert_discount gt 0}
SPL{$delimiter}INVOICE{$delimiter}{$orders[oid].date|date_format:"%m/%d/%Y"}{$delimiter}{$config.QuickBooks.acct_gc_discount}{$delimiter}{$orders[oid].b_lastname}, {$orders[oid].b_firstname} - {$orders[oid].login}{$delimiter}{$config.QuickBooks.trans_class}{$delimiter}{$orders[oid].giftcert_discount}{$delimiter}{$orders[oid].order_prefix}{$orders[oid].orderid}{$delimiter}{$orders[oid].applied_giftcerts}{$delimiter}-{$orders[oid].giftcert_discount}{$delimiter}-1{$delimiter}GIFT CERTIFICATE(s){$delimiter}N{$delimiter}
{/if}
...
{assign var="new_order" value=""}
{section name=oid loop=$orders}
{if ($orders[oid].cb_status eq "P" || $orders[oid].dc_status eq "C") and $new_order ne $orders[oid].orderid}
{if $orders[oid].total gt 0}
{assign var="order_total" value=$orders[oid].total}
{else}
{math equation="x+y" x=$orders[oid].total y=$orders[oid].giftcert_discount assign="order_total"}
{/if}
{assign var="new_order" value=$orders[oid].orderid}
TRNS{$delimiter}PAYMENT{$delimiter}{$orders[oid].date|date_format:"%m/%d/%Y"}{$delimiter}Undeposited Funds{$delimiter}{$orders[oid].b_lastname}, {$orders[oid].b_firstname} - {$orders[oid].login}{$delimiter}{$order_total}{$delimiter}{$orders[oid].payment_method}{$delimiter}{$orders[oid].order_prefix}{$orders[oid].orderid}
SPL{$delimiter}PAYMENT{$delimiter}{$orders[oid].date|date_format:"%m/%d/%Y"}{$delimiter}Accounts Receivable{$delimiter}{$orders[oid].b_lastname}, {$orders[oid].b_firstname} - {$orders[oid].login}{$delimiter}-{$order_total}{$delimiter}{$orders[oid].order_prefix}{$orders[oid].orderid}
ENDTRNS
{/if}
{/section}
{/if}{* $config.QuickBooks.qb_exportpayments eq "Y" *}
