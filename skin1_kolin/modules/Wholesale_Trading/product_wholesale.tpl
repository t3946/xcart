{* $Id: product_wholesale.tpl,v 1.24.2.4 2006/10/11 11:01:43 max Exp $ *}
...
<tr>
    ...
    <td class="TableSubHead" colspan="4"><b>* {$lng.lbl_note}:</b> {$lng.txt_edit_product_group}</td>
</tr>
{/if}
<tr class="TableHead">
	{if $geid ne ''}<td width="15" class="TableSubHead">&nbsp;</td>{/if}
	<td width="15" class="DataTable"><img src="{$ImagesDir}/spacer.gif" width="15" height="1" border="0" /></td>
	<td width="25%" class="DataTable">{$lng.lbl_quantity}</td>
	<td width="70%" class="DataTable">{$lng.lbl_price_per_item} ({$config.General.currency_symbol})</td>
</tr>
...
<tr>
	{if $geid ne ''}<td width="15" class="TableSubHead">&nbsp;</td>{/if}
	<td colspan="4"><br /><input type="submit" value="{$lng.lbl_add_update|strip_tags:false|escape}" />
{if $pricing ne ''} 
&nbsp;&nbsp;&nbsp;
	<input type="button" value="{$lng.lbl_delete_selected|strip_tags:false|escape}" onclick="javascript: document.pricing_form.mode.value='wholesales_delete'; document.pricing_form.submit();" />
{/if}
&nbsp;&nbsp;&nbsp;
	<input type="button" value="{$lng.lbl_generate_discounts|strip_tags:false|escape}" onclick="javascript: document.pricing_form.mode.value='generate_discounts'; document.pricing_form.submit();" />
	</td>
</tr>
