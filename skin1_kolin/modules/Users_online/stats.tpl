{* $Id: stats.tpl,v 1.8 2005/11/28 14:19:35 max Exp $ *}
{if $statistics}
...
<tr>
	...
	<td bgcolor="#FFFFFF" valign="top"><a href="{$v.current_url_page}" target="_blank">{$v.display_url_page}</a></td>
	...
	<tr{cycle values=", class=TableSubHead" name=$v.last_date}>
		...
		<tr>
			<td>{$p.amount} x {include file="currency.tpl" value=$p.price} = {if $p.price > 0}{math assign="total" equation="x*y" x=$p.amount y=$p.price}{else}{assign var="total" value=0}{/if}{include file="currency.tpl" value=$total}</td>
		</tr>
		</table>
	{if $p.product_options ne ''}
		<table cellpadding="0" cellspacing="1">
		<tr>
			<td width="15"><img src="{$ImagesDir}/spacer.gif" width="15" height="1" border="0" /></td>
