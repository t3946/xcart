...
{if $geid ne ''}<tr><td class="TableSubHead" height="20">&nbsp;&nbsp;<b>* {$lng.lbl_note}:</b> {$lng.txt_edit_product_group}</td></tr>{/if}
{foreach from=$cidev_filters_tree item=filter key=filter_key}
 <tr>
   ...
   <td valign="top">
	...
	<tr>
	{if $geid ne ''}<td width="25" class="TableSubHead">&nbsp;</td>{/if}
	<td>&nbsp;&nbsp;&nbsp;<B>{$lng.lbl_cidev_filter}: {$filter.f_name}</B> {if $filter.f_active ne "Y"}({$lng.lbl_cidev_f_is_disabled}){/if}</td>
	</tr>
	</table>
...
{if $item.fv_id eq $filter_values.fv_id}
	...
	{if $geid ne ''}
	  <td width="25" class="TableSubHead"><input type="checkbox" value="Y" name="fv_id_fields[del_fv_ids][{$filter_values.fv_id}]" /></td>
	{/if}
...
{/if}
 ...
 <tr><td height="10" {if $geid ne ''}class="TableSubHead"{/if}>&nbsp;</td></tr>
 {/if}
