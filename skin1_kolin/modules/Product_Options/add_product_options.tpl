{* $Id: add_product_options.tpl,v 1.10.2.3 2006/07/11 08:39:33 svowl Exp $ *}
...
{/if}
	...
	<td><input type="text" size="50" maxlength="128" id="add_class" name="add[class]" value="{$product_option.class}" /></td>
</tr>
<tr>
	{if $geid ne '' && $product_option ne ''}<td width="15" class="TableSubHead">&nbsp;</td>{/if}
	<td colspan="3" class="DataField">{$lng.txt_option_group_name_note}</td>
</tr>
<tr>
	{if $geid ne '' && $product_option ne ''}<td width="15" class="TableSubHead"><input type="checkbox" value="Y" name="fields[classtext]" /></td>{/if}
    <td><b>{$lng.lbl_option_text}:</b></td>
	<td><font class="Star">*</font></td>
    <td><input type="text" size="50" maxlength="255" id="add_classtext" name="add[classtext]" value="{$product_option.classtext}" /></td> 
</tr>
<tr>
	{if $geid ne '' && $product_option ne ''}<td width="15" class="TableSubHead">&nbsp;</td>{/if}
...
<tr>
	{if $geid ne '' && $product_option ne ''}<td width="15" class="TableSubHead"><input type="checkbox" value="Y" name="fields[is_modifier]" /></td>{/if}
	...
	<option value=''{if $product_option.is_modifier eq '' && $product_option.classid > 0} selected="selected"{/if}>{$lng.lbl_variant}</option>
	<option value='T'{if $product_option.is_modifier eq 'T' && $product_option.classid > 0} selected="selected"{/if}>{$lng.lbl_text_field}</option>
	</select></td> 
</tr>
<tr>
	{if $geid ne '' && $product_option ne ''}<td width="15" class="TableSubHead"><input type="checkbox" value="Y" name="fields[orderby]" /></td>{/if}
...
<tr>
	{if $geid ne '' && $product_option ne ''}<td width="15" class="TableSubHead"><input type="checkbox" value="Y" name="fields[avail]" /></td>{/if}
    <td class="DataField"><b>{$lng.lbl_availability}:</b></td>
	<td>&nbsp;</td>
    <td class="DataField"><select name="add[avail]">
		<option value="Y"{if $product_option.avail eq 'Y' || $product_option.classid eq ''} selected="selected"{/if}>{$lng.lbl_enabled}</option>
		<option value=""{if $product_option.avail ne 'Y' && $product_option ne ''} selected="selected"{/if}>{$lng.lbl_disabled}</option>
	</select></td>
</tr>
<tr>
	{if $geid ne '' && $product_option ne ''}<td width="15" class="TableSubHead"><input type="checkbox" value="Y" name="fields[options]" /></td>{/if}
...
{/if}
	...
	<tr{cycle name="options" values=', class="TableSubHead"'}>
		...
		<td><input type="text" name="list[{$o.optionid}][option_name]" value="{$o.option_name}" /></td>
...
{/if}
	...
	{/foreach}
	<tr>
		<td colspan="{if $product_option.is_modifier eq 'Y'}6{else}4{/if}"><input type="button" value="{$lng.lbl_delete_selected|strip_tags:false|escape}" onclick="javascript: document.optionform.mode.value='product_option_delete'; document.optionform.submit();" /></td>
	</tr>
	<tr>
		<td>&nbsp;</td>
	</tr>
