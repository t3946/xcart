...
//		td.innerHTML = '<input type="button" value="{$lng.lbl_copy_prod_name|strip_tags:false|escape}" onclick="javascript: copy_product_title(' + row_max_index + ')" />'
	...
	function cidev_show_wait_message () {ldelim}
	    document.getElementById('cidev_popup_poptions1').style.display = 'none';
	    document.getElementById('cidev_popup_poptions2').style.display = 'block';
...
<form action="product_modify.php" method="post" name="uploadform" enctype="multipart/form-data" onsubmit="javascript: return cidev_show_wait_message();">
...
{*
{if $images[image].image_x lt "620" || $images[image].image_y lt "800"}
                <input type="button" value=" Re-generate Detailed image " onclick="javascript: $('#det_thumb_field').val($('#field_thumb').val()); document.uploadform.thumbid.value = {$images[image].imageid}; submitForm(this, 're_gen_det_img');" />
{/if}
*}
...
<div id="cidev_popup_poptions2" style="display: none; background: url(../skin1_kolin/images/progress.gif); width: 120px; height: 11px;">
<B>Please wait...</B>
</div>
