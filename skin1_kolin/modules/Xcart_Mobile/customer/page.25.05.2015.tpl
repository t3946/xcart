...
{else}
  ...
  <div data-role="{$data_role|default:'page'}" {*data-url="{$php_url.url}{if $php_url.query_string}?{$php_url.query_string}{/if}"*} class="page-holder{if $page_container_class} {$page_container_class}{/if}{if $main eq 'product'} product-details-page{/if}{if $main eq 'cart' and not $cart_empty} cart-page{/if}{if $container_classes} {foreach from=$container_classes item=c}{$c} {/foreach}{/if}" data-dom-cache="true" data-add-back-btn="true" data-url="{$data_url}">
    ...
    {if !$no_header}
      <div data-role="header" data-inline="true">
        {if !$no_nav}
          <a href="#" class="custom-arrow back-title-button" data-role="button" data-rel="back" data-icon="custom-arrow-l" data-iconpos="notext" data-theme="a" data-shadow="false" data-corners="false">{$lng.lbl_back}</a>
        {/if}
