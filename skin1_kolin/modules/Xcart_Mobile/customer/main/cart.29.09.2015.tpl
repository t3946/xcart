...
<td>
<input type="number" {if $main eq "fast_lane_checkout"} autocomplete="off" style="background: #ffffff; width: 50px;" id="productindex_{$products[product].cartid}" onkeyup="cidev_update_product_amount('{$products[product].cartid}', '{$products[product].manufacturerid}')"{/if}  size="3"{/if} name="productindexes[{$products[product].cartid}]" value="{$products[product].amount}" />
</td>
<td>
 = 
</td>
...
*}
	<td class="ButtonsRow">{include file="customer/buttons/button.tpl" href="cart.php?mode=delete&amp;productindex=`$products[product].cartid`" button_title="Delete item"}</td>
	<td class="ButtonsRow">
