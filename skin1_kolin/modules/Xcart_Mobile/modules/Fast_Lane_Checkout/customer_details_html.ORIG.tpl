{*
$Id: customer_details_html.tpl 78 2012-12-28 13:59:37Z skot $
...
<div class="ui-grid-a">
  ...
  <div class="ui-block-b">
    ...
    <table cellspacing="0" class="flc-checkout-address" summary="{$lng.lbl_billing_address|escape}">
      ...
      {/foreach}
      {if $login ne ''}
        {assign var=modify_url value="`$current_location`/popup_address.php?mode=select&for=cart&type=B"}
        {assign var=link_href value="`$current_location`/popup_address.php?mode=select&for=cart&type=B"}
      {/if}
...
<div class="ui-grid-a">
  <div class="ui-block-a">
    {capture name=addfields}
      ...
      {/foreach}
    {/capture}
    {if $smarty.capture.addfields|@trim ne ""}
  ...
  {if $config.Shipping.enable_shipping eq "Y" && $config.Shipping.need_shipping_section eq "Y"}
    <div class="ui-block-b">
      ...
      <table cellspacing="0" class="flc-checkout-address" summary="{$lng.lbl_shipping_address|escape}">
        ...
        {/foreach}
        {if $login ne ''}
          {assign var=modify_url value="`$current_location`/popup_address.php?mode=select&for=cart&type=S"}
          {assign var=link_href value="`$current_location`/popup_address.php?mode=select&for=cart&type=S"}
        {/if}
        <tr><td colspan="2">{include file="customer/buttons/modify.tpl" href=$modify_url link_href=$link_href|default:$modify_url style="link"}</td></tr>
      </table>
    </div>
