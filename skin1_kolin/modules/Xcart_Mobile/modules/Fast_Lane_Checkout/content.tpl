...
{if $main eq 'cart' or $main eq "fast_lane_checkout"}
  {if !$std_checkout_disabled}
    {capture name="checkout_button"}
      ...
      {else}
        {include file="customer/buttons/button.tpl" button_title=$checkout_button_title style="div_button" href="cart.php?mode=checkout&l=y" data_theme="b" data_icon="arrow-r" data_iconpos="right" data_inline="false"}
      {/if}
    {/capture}
  {/if}
