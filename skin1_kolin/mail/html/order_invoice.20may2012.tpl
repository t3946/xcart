...
<br /><br />
<b>{$lng.lbl_date}:</b> {$order.date|date_format:$config.Appearance.datetime_format}<br /><b>{$lng.lbl_order_id}:</b> {if $type eq 'A'}<a href="{$catalogs.admin}/order.php?orderid={$order.orderid}">{$order.order_prefix}{$order.orderid}</a>{else}{$order.order_prefix}{$order.orderid}{/if}<br /><b>{$lng.lbl_order_status}:</b> {$lng.lbl_please_see_below}<br />
<b>{$lng.lbl_payment_method}:</b><br />{$order.payment_method}<br /><b>{$lng.lbl_delivery}s:</b><br />{if $order.shipping_groups ne ''}{foreach from=$order.shipping_groups item=v}{if $v.shipping}{$v.shipping|trademark:''}<br />{/if}{/foreach}{/if}
{if $giftcerts}
...
{/if}
	...
	<tr>
		<td colspan='2'> {* <img height="2" src="{$ImagesDir}/spacer_black.gif" width="100%" alt="" style="height: 2px; max-height: 2px;" />*} <hr style="width:100%;margin: 0px;" /></td>
	</tr>
	</table>
	<br /> 
	<table cellspacing="0" cellpadding="0" width="100%" border="0">
...
{/foreach}
	...
	<table cellspacing="0" cellpadding="0" width="100%" border="0">
	<tr>
		<td width="45%" height="25"><b>{$lng.lbl_billing_address}</b><hr style="width:100%;margin: 0px;" /></td>
		<td width="10%">&nbsp;</td>
		<td width="45%" height="25"><b>{$lng.lbl_shipping_address}</b><hr style="width:100%;margin: 0px;" /></td>
	</tr>
{*	<tr>
		<td height="2">  <img height="2" src="{$ImagesDir}/spacer_black.gif" width="100%" alt="" style="height: 2px; max-height: 2px;" />  <hr style="width:100%;margin: 0px;" /></td>
