...
<br /><br />
<b>{$lng.lbl_date}:</b> {$order.date|date_format:$config.Appearance.datetime_format}<br /><b>{$lng.lbl_order_id}:</b> #{$order.orderid}<br />{if $show_shipping eq 'Y'}<b>{$lng.lbl_delivery}:</b><br />{foreach from=$order.shipping_groups item=v key=k}{if $k eq $manufacturerid}{$v.shipping|trademark:''}{/if}{/foreach}{else}<br />{/if}<br /><br /><br /><br /><br />
			</td>
