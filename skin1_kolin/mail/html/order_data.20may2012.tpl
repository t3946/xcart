...
<th align="center" bgcolor="#cccccc">{$lng.lbl_product}</th>
...
<th bgcolor="#cccccc" align="center">{$lng.lbl_qty_ord|capitalize}</th>
{if $order.has_backordered_status}
    <th bgcolor="#cccccc" align="center">{$lng.lbl_qty_ship|capitalize}</th>
    <th bgcolor="#cccccc" align="center">{$lng.lbl_qty_back|capitalize}</th>
{/if}
<th width="60" bgcolor="#cccccc" align="center">{$lng.lbl_extended}<br /> {* <img height="1" src="{$ImagesDir}/spacer.gif" width="50" border="0" alt="" style="height: 2px;max-height: 2px;" /> *} </th>
</tr>
...
<tr>
<td align="center">{if $type eq 'A' || $type eq 'P'}<a href="{if $provider_notification eq 'Y'}{$catalogs.provider}{else}{$catalogs.admin}{/if}/product_modify.php?productid={$product.productid}">{$product.productcode}</a>{else}{$product.productcode}{/if}</td>
<td><font style="FONT-SIZE: 11px"><a href="{$catalogs.customer}/product.php?productid={$product.productid}">{$product.product}</a></font>
        ...
        {foreach from=$v.tracking item=tr}
            <tr>
                <td colspan="{$colspan}" style="padding: 10px;">
                    {if $tr.tracknum ne ""}
                        {$lng.eml_order_shipped|substitute:"shipper":$tracking_links[$tr.linkid].shipping|substitute:"distributor":$v.group_name}<br />
                        {$lng.lbl_tracking_number_is} {$tr.tracknum}<br />
                    ...
                    {else}
                        {$lng.eml_order_shipped_nolink|substitute:"shipper":$tracking_links[$tr.linkid].shipping|substitute:"distributor":$v.group_name}<br />
                        {$tracking_links[$tr.linkid].link}
                    {/if}
                </td>
            </tr>
        {/foreach}
...
<tr>
    <td colspan="6">
        <b>{$v.group_name} {$lng.lbl_items} ({$lng.lbl_delivery_by} {if $gc.send_via eq "E"}{$lng.lbl_email}{else}{$lng.lbl_gc_postal_mail}{/if}, {include file="currency.tpl" value=$v.shipping_cost.gross|default:"0"}):</b>
    </td>
</tr>
<tr>
	<td>&nbsp;</td>
...
<tr>
<td align="right" height="20"><b>{$lng.lbl_total_shipping_cost}:</b>&nbsp;</td>
...
<tr>
<td colspan="2"> {* <img height="2" src="{$ImagesDir}/spacer_black.gif" width="100%" alt="" style="height: 2px;max-height: 2px;" /> *} <hr style="width: 100%;"></td>
</tr>

<tr>
<td align="right" bgcolor="#cccccc" height="25"><b>{$lng.lbl_grand_total|capitalize}:</b>&nbsp;</td>
<td align="right" bgcolor="#cccccc" height="25"><b>{include file="currency.tpl" value=$order.total}</b></td>
</tr>
