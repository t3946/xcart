...
{/if}
{if $main eq "help" && $usertype eq "C" && $smarty.get.section eq "contactus"}<font class="VertMenuItems">{else}<a href="help.php?section=contactus&amp;mode=update" class="VertMenuItems">{/if}{$lng.lbl_contact_us}{if $main eq "help" && $usertype eq "C" && $smarty.mode.section eq "contactus"}</font>{else}</a>{/if}<br />
