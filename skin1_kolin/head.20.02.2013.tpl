...
{* {if ($smarty.get.mode ne "checkout" && $main ne "fast_lane_checkout" ) || ($main eq "fast_lane_checkout" && $smarty.get.mode eq "") } *}
...
<TD height="20" valign="bottom">
        ...
        <tr>
        <td width="350" nowrap="nowrap" valign="bottom" >{$config.Company.cidev_top_header_code}</td>
...
<td valign="bottom">{if $smarty.get.pageid ne $top_pages_menu[top_page].pageid}<a href="pages.php?pageid={$top_pages_menu[top_page].pageid}">{/if}<img src="{$xcart_web_dir}/image.php?id={$top_pages_menu[top_page].image.id}&amp;type=A" alt="" {if $top_pages_menu[top_page].image.image_x gt "16"}width="16"{/if} />{if $smarty.get.pageid ne $top_pages_menu[top_page].pageid}</a>{/if}</td>
{/if}
<td valign="bottom">{if $smarty.get.pageid ne $top_pages_menu[top_page].pageid}<a href="pages.php?pageid={$top_pages_menu[top_page].pageid}">{/if}{$top_pages_menu[top_page].title}{if $smarty.get.pageid ne $top_pages_menu[top_page].pageid}</a>{/if}</td>
{* {if !%page.last%} *}
...
<td valign="bottom">{if $main ne "help" && $smarty.get.section ne "contactus"}<a href="help.php?section=contactus&mode=update">{/if}<IMG src="{$ImagesDir}/contact_us.png" width="16" border="0" alt="" />{if $main ne "help" && $smarty.get.section ne "contactus"}</a>{/if}</td>
<td valign="bottom">{if $main ne "help" && $smarty.get.section ne "contactus"}<a href="help.php?section=contactus&mode=update">{/if}Contact us{if $main ne "help" && $smarty.get.section ne "contactus"}</a>{/if}</td>

<td valign="bottom">{$lng.lbl_top_header_nbsp}</td>
</tr>
</table>
        </td>
        </tr>
...
<TD {* height="130" *} valign="top">
        ...
        <td width="250" {* style="background: url({$xcart_web_dir}/image.php?id={$current_storefront_info.storefrontid}&amp;type=S) no-repeat;" *} >
        {if !($main eq "catalog" && $current_category.category eq "") || $smarty.get.page ne ""}<a href="/">{/if}<img src="{$xcart_web_dir}/image.php?id={$current_storefront_info.storefrontid}&amp;type=S" {if $current_storefront_info.image.image_x gt "250"} width="250" {/if}>{if !($main eq "catalog" && $current_category.category eq "")}</a>{/if}
        </td>

        <td width="*" {* height="100%" *} valign="bottom">
                <table border="0" cellpadding="0" cellspacing="0" width="100%" {* height="100%" *} >
                        <tr><td {* height="90" *} align="center">{$config.Company.cidev_header_code}</td></tr>
                        <tr>
                            <td height="40">{include file="customer/search.tpl"}
                            </td>
                        </tr>
                </table>
        </td>
        ...
        <td align="right" valign="bottom" width="240">
                <table border="0" cellpadding="0" cellspacing="0"><tr><td width="240" height="102" style="background-color: #FFFFFF;" valign="middle" align="center">
...
<TR>
<TD align="right" valign="bottom" style="padding-bottom: 3px; padding-right: 60px; { * font-size: 14px; font-weight: bold; * }">
