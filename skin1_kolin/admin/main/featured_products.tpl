{* $Id: featured_products.tpl,v 1.24.2.3 2006/10/25 06:39:32 max Exp $ *}
{if $usertype eq "A"}
...
{*	<td><b><a href="{$catalogs.customer}/product.php?productid={$products[prod_num].productid}" target="_blank">{$products[prod_num].product}</a></b></td> *}
        <td><b><a href="{if $current_storefront_info.domain ne ""}http://{$current_storefront_info.domain}{else}{$catalogs.customer}{/if}/product.php?productid={$products[prod_num].productid}" target="_blank">{$products[prod_num].product}</a></b></td>
...
<tr>
	...
	<input type="button" value="{$lng.lbl_delete_selected|strip_tags:false|escape}" onclick="javascript: document.featuredproductsform.mode.value = 'delete'; document.featuredproductsform.submit();" />
...
<tr>
	...
	*}
	{$lng.lbl_sku}:&nbsp;<input type="text" name="newproductcodes" value="" size="55"/>
	</td>
