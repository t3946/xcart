...
<table width="100%" class="filter_preset_edit">
    <tr>
        ...
        <td width="85%"><input type="text" name="title" value="{$filter.title}" class="text" /></td>
    </tr>
    <tr>
    ...
    <tr>
        ...
        <td width="85%">
            ...
            <td width="350px">
                <input style="position: absolute;" type="radio" name="time_from_mode" value="H"{if $filter.time_from_mode eq "H"} checked="checked"{/if}/>
...
<tr>
    {foreach from=$row item=filter}
        <td width="20%"{if $filter.enabled ne 'Y'} class="filter_disabled"{/if}><a href="configuration.php?option=Filter_Presets&fid={$filter.fid}">{if $filter.title ne ''}{$filter.title}{else}{$lng.lbl_filter_empty|escape}{/if}</a></td>
    {/foreach}
<tr>
{/foreach}
</table>
