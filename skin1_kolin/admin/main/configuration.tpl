...
{if $option ne "User_Profiles" && $option ne "Contact_Us" && $option ne "Search_products" && $option ne "Templates_OrderRelatedMessages" && $option ne "Request_availability_options" && $option ne "Fraud_check" && $option ne "Reconciliation" && $option ne "currently_assigned_to_statuses" && $option ne "Attention_tag_options" && $option ne "Product_classification" && $option ne "OTRS_options" && $option ne "Order_page_permissions" && $option ne "Inquiries_options" && $option ne "Supplier_feeds" && $option ne "PBX_options" && $option ne "Attention_tags_invoices" && $option ne "Related_objects_collector_cron" && $option ne "Checks_deposited_options"}
...
{elseif $option eq "Related_objects_collector_cron"}

{include file="admin/main/related_objects_collector_cron.tpl"}

{elseif $option eq "Checks_deposited_options"}

{include file="admin/main/checks_deposited_options.tpl"}
...
{include file="admin/main/external_marketplaces.tpl"}
...
{if $configuration[cat_num].name eq "RMA_options_sep1"}
<span style="font-weight: normal;">{$lng.lbl_gen_set_RMA_email}</span>
<br />
<br />
{/if}
...
{elseif $configuration[cat_num].name eq "Checks_deposited_Attention_tag" || $configuration[cat_num].name eq "RMA_Attention_tag"}

        <select name="{$configuration[cat_num].name}">
        <option value=""></option>
        {foreach from=$attention_tags_values item=v_tag}
                <option value="{$v_tag.status_id}" {if $configuration[cat_num].value eq $v_tag.status_id}selected="selected"{/if}>{$v_tag.status}</option>
        {/foreach}
        </select>
