...
<table>
<tr><td colspan="3"><B>{if $tab eq "accounts_payable" || $tab eq "receivables"}Order dates{else}Transaction dates{/if}</B><br /><hr /></td></tr>
<tr>
...
{*
{$v.total_order_amounts} - {$v.amount_csv_abs} - {$v.orders.0.accounting.1.gross} - {$v.orders.0.accounting.2.gross}
*}
...
*}
  ...
  ($v.orders ne "" &&
	...
	($v.total_order_amounts eq $v.amount_csv_abs)
	||
	(
	$v.d_bulk_or_individual_order_payments eq "distributor_charges_for_each_order_twice_one_charge_for_products_and_one_charge_for_shipping" && 
		($v.amount_csv_abs eq $v.orders.0.accounting.1.gross || $v.amount_csv_abs eq $v.orders.0.accounting.2.gross)
	)
    )
  )
  }
...
//]]>
    ...
    {foreach from=$all_manufacturers_orders item=item key=key}
     {if $item.orders ne "" }
        ...
        <td width="*" valign="top" align="center">{$v.order_prefix}{$v.orderid}</td>
        <td width="*" valign="top" align="center">{$item.distr_code}</td>
