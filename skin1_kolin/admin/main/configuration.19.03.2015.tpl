...
{if $option ne "User_Profiles" && $option ne "Contact_Us" && $option ne "Search_products" && $option ne "Templates_OrderRelatedMessages" && $option ne "Request_availability_options" && $option ne "Fraud_check" && $option ne "Reconciliation" && $option ne "currently_assigned_to_statuses" && $option ne "Attention_tag_options" && $option ne "Product_classification" && $option ne "OTRS_options" && $option ne "Order_page_permissions" && $option ne "Inquiries_options" && $option ne "Supplier_feeds" && $option ne "PBX_options"}
