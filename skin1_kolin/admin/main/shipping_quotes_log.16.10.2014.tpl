 ...
 {if $v ne ""}
  ...
  {foreach from=$v item=vv key=kk}
   ...
   <tr {if $tmp_counter eq 1}class='TableSubHead'{/if}>
	...
	<td>{$vv.ups_ground_quote}</td>
	<td>{$vv.approx_ground_quote}</td>
{*	<td>{$vv.customer_id}</td> *}
{*	<td>{$vv.session_id}</td> *}
	<td>{$vv.product_cost|price_format}</td>
	<td nowrap="nowrap">
	{if $vv.products ne ""}
		{foreach from=$vv.products item=vvv key=kkk}
			<a target="_blank" href="{$vvv.url}">{$vvv.product} ({$vvv.qty})</a><br />
		{/foreach}
	{/if}
	</td>
