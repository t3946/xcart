...
{if $tab ne "accounts_payable"}<a href="reconciliation.php?tab=accounts_payable">{else}<B>{/if}Accounts payable{if $tab ne "accounts_payable"}</a>{else}</B>{/if}&nbsp;&nbsp;&nbsp;
...
{if $tab eq "unreconciled" || $tab eq "reconciled" || $tab eq "dropped" || $tab eq "expense_report"}
...
<table>
<tr><td colspan="3"><B>Transaction dates</B><br /><hr /></td></tr>
<tr>
        <td class="FormButton" nowrap="nowrap">Start date:</td>
        <td width="10">&nbsp;</td>
        <td>
...
<tr>
        <td class="FormButton" nowrap="nowrap">End date:</td>
        <td width="10">&nbsp;</td>
        <td>
...
<INPUT type="submit" value="{if $tab eq "expense_report"}Generate expense report{else}Show transactions{/if}">
...
{if $tab eq "unreconciled"}
	<td style="background-color: #D9EAD3;" width="10">Untie<br />orders</td>
{/if}
...
<td style="background-color: #F4CCCC;" width="100">Amount</td>
<td style="background-color: #F4CCCC;" width="100">Distr</td>
<td style="background-color: #F4CCCC;" width="100">Order #</td>
<td style="background-color: #F4CCCC;" width="100">B2D status</td>
<td style="background-color: #F4CCCC;" width="100">Date</td>
</tr>
...
{if $tab eq "unreconciled"}
<td align="center">
	<input type="checkbox" name="clear_orders[{$v.id}]" value="Y" />
</td>
{/if}

<td width="100" valign="top" align="center">{$v.date_csv|date_format:'%d-%b-%Y'}</td>
<td width="200" valign="top">{$v.description_csv}</td>
...
<td width="100" valign="top" align="center">
  ...
  {if $v.orders ne "" || $v.action ne "" || $v.config_search_keyphrase_found eq "Y"}
	...
	{else}
		...
		<option value="R"{if ($v.action eq "R") || ($v.total_order_amounts eq $v.amount_csv_abs) || ($v.amount_csv_abs eq $v.orders.0.accounting.1.gross)} selected="selected"{/if}>Reconcile</option>
		{/if}
...
{* <td align="center">{$v.distr_code}</td> *}
...
{*   {assign var="current_orders_variant" value=""} *}

   {foreach from=$v.orders item=vo key=ko}
{*
	{if $vo.orders_variant ne $current_orders_variant}
		{if $current_orders_variant ne ""}
		<tr><td colspan="5"><hr /></td></tr>
		{/if}
		{assign var="current_orders_variant" value=$vo.orders_variant}
	{/if}
*}
   <tr>
	<td width="100" align="center" nowrap="nowrap">
	    ...
	    {if $vo.ref_to_us eq "Y"}
            ...
            {else}
		({$vo.accounting.1.gross}){if $vo.accounting.2.gross gt 0}+({$vo.accounting.2.gross}){/if}
   ...
   {if $v.total_order_amounts gt 0 && $v.total_order_amounts ne $v.amount_csv_abs}
...
{if $tab eq "unreconciled"}
<INPUT type="button" value="Untie orders for selected" onclick="document.r_form.mode.value='clear_orders'; document.r_form.submit();"></TD>
{/if}
...
{* {elseif $tab eq "reconciled" || $tab eq "dropped"} *}
...
<tr {cycle values=", class='TableSubHead'" name="cycle_totals"}>
<td nowrap="nowrap">{$v.orig_file_name}<br /><span style="font-size: 8px;">({$v.local_file})</span></td>
...
<tr>
        ...
        <td>
{*
        {html_select_date prefix="date_csv_Start" time=$search_prefilled1.date_csv.start_date start_year=$config.Company.start_year end_year=$config.Company.end_year}
*}
...
<input id="id_date_csv_Start1" type="text" size="11" name="date_csv_Start" value="{$search_prefilled1.date_csv.start_date_str}" />
...
<tr>
        ...
        <td>
{*
        {html_select_date prefix="date_csv_End" time=$search_prefilled1.date_csv.end_date start_year=$config.Company.start_year end_year=$config.Company.end_year display_days=yes}
*}
...
<input id="id_date_csv_End1" type="text" size="11" name="date_csv_End" value="{$search_prefilled1.date_csv.end_date_str}" />
...
<tr><td colspan="3"><B>Order dates</B><br /><hr /></td></tr>
<tr>
        <td class="FormButton" nowrap="nowrap">Start date:</td>
        <td width="10">&nbsp;</td>
        <td>
{*
        {html_select_date prefix="date_Start" time=$search_prefilled1.date.start_date start_year=$config.Company.start_year end_year=$config.Company.end_year}
*}
...
<input id="id_date_Start1" type="text" size="11" name="date_Start" value="{$search_prefilled1.date.start_date_str}" />
...
<tr>
        ...
        <td>
{*
        {html_select_date prefix="date_End" time=$search_prefilled1.date.end_date start_year=$config.Company.start_year end_year=$config.Company.end_year display_days=yes}
*}
...
<input id="id_date_End1" type="text" size="11" name="date_End" value="{$search_prefilled1.date.end_date_str}" />
