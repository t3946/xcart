...
<tr{cycle values=", class='TableSubHead'"}>
	...
	<td align="center"><input type="text" size="8" name="data[{$s.shippingid}][service_code]" value="{$s.service_code}" /></td>
...
<tr>
	...
	<td>{if $config.Shipping.realtime_shipping eq "Y" and $config.Shipping.use_intershipper ne "Y" and $active_modules.UPS_OnLineTools eq "" and ($car.code eq "CPC" or $car.code eq "FDX" or $car.code eq "USPS" or $car.code eq "ARB" or $car.code eq "APOST" or $car.code eq "DHL")}<a href="shipping_options.php?carrier={$car.code}">{$lng.lbl_options} &gt;&gt;</a>{elseif $config.Shipping.realtime_shipping eq "Y" and $active_modules.UPS_OnLine_Tools and $config.Shipping.use_intershipper ne "Y" and $car.code eq "UPS"}<a href="ups.php">{$lng.lbl_ups_online_tools_configure} &gt;&gt;</a>{/if}</td>
</tr>
</table>
	</td>
</tr>
...
<tr{cycle values=", class='TableSubHead'"}>
	...
	<td align="center"><input type="text" size="8" name="data[{$s.shippingid}][vol_threshold]" value="{$s.vol_threshold|formatprice}" /></td>
	<td align="center"><input type="text" size="8" name="data[{$s.shippingid}][dim_factor]" value="{$s.dim_factor|formatprice}" /></td>
	<td align="center"><input type="text" name="data[{$s.shippingid}][orderby]" size="4" value="{$s.orderby}" /></td>
	<td align="center"><input type="checkbox" name="data[{$s.shippingid}][active]" value="Y"{if $s.active eq "Y"} checked="checked"{/if} /></td>
	<td align="center"><input type="checkbox" name="data[{$s.shippingid}][is_cod]" value="Y"{if $s.is_cod eq "Y"} checked="checked"{/if} /></td>
</tr>
...
<tr>
	...
	<td align="center"><select name="data[{$s.shippingid}][destination]">
		...
		<option value="L" {if $s.destination eq "L"}selected{/if}>{$lng.lbl_national}</option>
	</select></td>
...
<tr>
	<td colspan="6" class="SubmitBox"><input type="submit" value="{$lng.lbl_update|strip_tags:false|escape}" /></td>
</tr>
