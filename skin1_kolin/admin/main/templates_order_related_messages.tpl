...
<br />Add the following Attention tag
                ...
                <select name="templates_for_communication[{$key}][status_id]">
...
{*                              {if $item_v.code eq $template_item.ca_status} *}
                              {if $item_v.status_id eq $template_item.status_id} 
                                selected="selected"
                              {/if}
{*                            >{$item_v.name}</option> *}
...
<br />
	...
	<input type="checkbox" name="templates_for_communication[{$key}][attach_pdf_invoice]" value="Y" {if $template_item.attach_pdf_invoice eq "Y"}checked="checked"{/if} /> Attach order invoice in pdf format
