...
{capture name=dialog}
    ...
    {if $registered eq ""}
        ...
        {if $error ne ""}
            ...
            <br/>
        {/if}
        <form action="{$register_script_name}?{$smarty.server.QUERY_STRING|amp}" method="post"
              name="registerform"{if $js_enabled} onsubmit="javascript: if (check_zip_code(){if $default_fields.email.required eq 'Y'} && checkEmailAddress(document.registerform.email){/if} && checkRequired(requiredFields)) return true; else return false;"{/if}>
            ...
            <table cellspacing="1" cellpadding="2" width="100%">
                ...
                <tr>
                    <td align="right"><label for="s_events">Show events</label></td>
                    <td></td>
                    <td>
                        <input type="hidden" value="0" name="show_events">
                        <input type="checkbox" value="1" name="show_events" id="s_events"
                               {if $userinfo.show_events}checked{/if}>
                    </td>
                </tr>
                <tr>
