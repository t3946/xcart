...
{if $option ne "User_Profiles" && $option ne "Contact_Us" && $option ne "Search_products" && $option ne "Templates_OrderRelatedMessages" && $option ne "Request_availability_options" && $option ne "Fraud_check" && $option ne "Reconciliation" && $option ne "currently_assigned_to_statuses" && $option ne "Attention_tag_options" && $option ne "Product_classification" && $option ne "OTRS_options" && $option ne "Order_page_permissions" && $option ne "Inquiries_options"}
...
<tr><td colspan = "2">
Highlight ETA date on the order list pages in pink if<br />
ETA date - <input type="text" size="5" name="backorder_eta_date_x" value="{$config.backorder_decision_request.backorder_eta_date_x}" /> days &lt; Current date &lt; ETA date + <input type="text" size="5" name="backorder_eta_date_y" value="{$config.backorder_decision_request.backorder_eta_date_y}" /> days</td></tr>
...
{if $configuration[cat_num].name eq "thank_you_message_body" || $configuration[cat_num].name eq "po_instructions" || $configuration[cat_num].name eq "po_missing_instructions" || $configuration[cat_num].name eq "reference_text" || $configuration[cat_num].name eq "signature"}
