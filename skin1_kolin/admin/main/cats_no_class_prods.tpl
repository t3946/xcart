...
{if $kk eq ($v.categoryid_path_arr_count - 1)}<a href="http://{$current_storefront_info.domain}/home.php?cat={$v.categoryid}" target="_blank" style="color: blue;">{/if}{$vv}{if $kk eq ($v.categoryid_path_arr_count - 1)}</a>{/if}{if $kk < ($v.categoryid_path_arr_count - 1)} <B>></B> {/if}
{/foreach}
{/if}
</td>
