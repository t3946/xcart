...
*}
<td nowrap="nowrap" valign="middle">{if $main ne "help" && $smarty.get.section ne "contactus"}<a class="top_links" href="help.php?section=contactus&mode=update">{/if}Contact us{if $main ne "help" && $smarty.get.section ne "contactus"}</a>{/if}</td>
...
<TD {* height="130" *} valign="bottom">
        ...
        {if !($main eq "catalog" && $current_category.category eq "") || $smarty.get.page ne ""}<a href="/">{/if}<img src="{$xcart_web_dir}/image.php?id={$current_storefront_info.storefrontid}&amp;type=S" {if $current_storefront_info.image.image_x gt "250"} width="250" {/if}  alt="Home page" >{if !($main eq "catalog" && $current_category.category eq "")}</a>{/if}
        </td>
...
{*                        <tr><td align="center" colspan="3">{$config.Company.cidev_header_code}</td></tr> *}
			    ...
			    {/if}
                            ...
                            {if $main ne "fast_lane_checkout" }<td width="40">&nbsp;</td>{/if}
                            <td {if $main ne "fast_lane_checkout" }width="204" valign="middle" align="right" {else} width="150" valign="top" align="right"  {/if}>
                                        {if $main ne "fast_lane_checkout" }
                                                { include file="customer/menu_cart.tpl" }

					{else}
<img src="{$ImagesDir}/S3-Stores-Logo-S2.png" alt="" />
                                        {/if}
                            </td>
                        </tr>
                </table>
