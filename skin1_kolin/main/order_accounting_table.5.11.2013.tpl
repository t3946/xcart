...
<tr class="OrderSheetCell OrderSheetFirst" style="font-weight: bold;">
  ...
  <td>{include file="currency2.tpl" value=$data.total_accounting[$smarty.section.acc.index].net}</td>
  {/section}
...
<tr class="OrderSheetCell{$cycle_class}">
  <td width="5">{if $static eq 'Y' || $static eq 'O'}<a href="order.php?orderid={$order.orderid}">{/if}<b>{$order.order_prefix}{$order.orderid}</b>{if $static}</a>{/if}</td>
...
<tr class="OrderSheetCell{$cycle_class}">
  <td width="5">{if $static eq 'Y' || $static eq 'O'}<a href="order.php?orderid={$order.orderid}">{/if}{$v.code}{if $static}</a>{/if}</td>
...
<td colspan="12" align="left">
        ...
        <!--
                ...
                {if $count_links_to_distributor_invoices}
                        var row_max_index = {$count_links_to_distributor_invoices};
                {else}
                        var row_max_index = 1;
                {/if}
        -->
...
{include file="main/include_js.tpl" src="main/manage_distributor_links.js"}
        ...
        {if $distributor_links}
                {foreach from=$distributor_links item="distributor_link" key=key name="depforeach"}

                <tr id="distributor_link_row{$key}">
                        <td style="padding: 2px 0px;">

Link to distributor invoice&nbsp;<input type="text" size="40" name="links_to_distributor_invoices[{$key}][link_to_distributor_invoice]" value="{$distributor_link.link_to_distributor_invoice|escape}" />

&nbsp;<a href="javascript: void(0);" onclick="javascript: add_distributor_link_row('{$key}');"><img src="{$ImagesDir}/plus.gif" alt="{$lng.lbl_add|escape}" /></a>
&nbsp;<a href="javascript: void(0);" {if $distributor_link.frozen ne "Y"} onclick="javascript: remove_distributor_link_row('{$key}');"{else}onclick="javascript: alert('You cannot delete it.');"{/if}><img src="{$ImagesDir}/minus.gif" alt="{$lng.lbl_remove_row|escape:'javascript'}" /></a>
...
&nbsp;&nbsp;<a style="color: #3A3AFF; font-weight: normal;" href='{$distributor_link.link_to_distributor_invoice}' target="_blank">View invoice</a>
                ...
                {/foreach}
        {else}
                <tr id="distributor_link_row1">
                        <td style="padding: 2px 0px;">
                                Link to distributor invoice&nbsp;<input type="text" size="40" name="links_to_distributor_invoices[1][link_to_distributor_invoice]" value="" />
&nbsp;<a href="javascript: void(0);" onclick="javascript: add_distributor_link_row(1);"><img src="{$ImagesDir}/plus.gif" alt="{$lng.lbl_add|escape}" /></a>
