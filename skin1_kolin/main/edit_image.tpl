{* $Id: edit_image.tpl,v 1.7.2.3 2006/07/11 08:39:27 svowl Exp $ *}
...
<tr>
	<td id="{$idtag}_btns_td">
        {if $source eq 'PD'}
            <input type="button" id="{$type}_{$idtag}" value="{$lng.lbl_plus|strip_tags:false|escape}" onclick='javascript: popup_image_selection("{$type}", "{$id}", "{$idtag}");' />
            <input type="file" size="25" name="{$idtag}" onchange="javascript: $('#{$type}_{$idtag}').attr('disabled', 'disabled');" />
        {else}
