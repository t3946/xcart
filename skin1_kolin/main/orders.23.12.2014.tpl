...
<tr>
    {foreach from=$row item=filter}
        ...
        <td width="20%">&nbsp;</td>
        {else}
        <td width="20%"{if $filter.bold eq 'Y'} class="filter_bold"{/if}>{if $filter.count gt 0}<a target="_blank" href="orders.php?fid={$filter.fid}">{/if}{$filter.title} ({$filter.count|default:"0"}){if $filter.count gt 0}</a>{/if}</td>
        {/if}
    {/foreach}
<tr>
    {/if}
