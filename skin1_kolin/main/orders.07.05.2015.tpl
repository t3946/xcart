...
{if $show_no_orders_test_checkout_message eq "Y"}
    <br />
    <table width="100%">
    <tr>
    <td align="center" style="border: solid 1px #000000; background: #F4CCCC;">
        <form action="orders.php?page_name=dashboard" method="post" name="hide_no_orders_test_checkoutform">
        <input type="hidden" name="mode" value="" id="id_mode_hide_no_orders_test_checkout_message" />
        {$lng.lbl_no_orders_test_checkout_message}<input type="button" value="Done!" onclick="javascript: $('#id_mode_hide_no_orders_test_checkout_message').val('hide_no_orders_test_checkout_message'); this.form.submit();" />
        </form>
    </td>
    </tr>
    </table>
...
{if $order_page_title ne ""}
	...
	{/if}
{else}
	{if $page_name eq "dashboard"}
		{assign var="order_page_title" value="Customer Care dashboard"}
	{elseif $page_name eq "search"}
		{assign var="order_page_title" value="Order search"}
        {elseif $page_name eq "export"}
                {assign var="order_page_title" value="Export orders"}
        {else}
		{assign var="order_page_title" value=`$lng.lbl_orders_management`}
	{/if}
{/if}
...
{*
 {if $page_name eq "dashboard"}
 <td width="33%" style="font-size: 15px;"><a target="_blank" style="color: #140BFC;" href="orders.php?mode=search&date=7">Most recent orders</a></td>
 {/if}
*}
 <td align="center" width="*" nowrap="nowrap">
	{include file="page_title.tpl" title=$order_page_title}
 </td>
{*
 {if $page_name eq "dashboard"}
 <td align="right" style="font-size: 15px;" width="33%" >
	<a target="_blank" style="color: #140BFC;" href="create_new_inquiry.php">Create new inquiry</a>
 </td>
 {/if}
*}
 </tr>
...
{if $page_name eq "dashboard" || $page_name eq ""}
{capture name=dialog}
<table width="100%" class="preset_table">
{foreach from=$filters item=row}

    {assign var="count_disabled" value=0}
    {foreach from=$row item=filter}
        {if $filter.title eq '' || $filter.enabled eq "N"}
		{math equation="x+1" x=$count_disabled assign="count_disabled"}
        {/if}
    {/foreach}


    {if $count_disabled lt 5}
<tr>
    {foreach from=$row item=filter}
        {if $filter.title eq '' || $filter.enabled eq "N"}
        <td width="20%">&nbsp;</td>
        {else}
        <td width="20%"{if $filter.bold eq 'Y'} class="filter_bold"{/if}>
{if $filter.marker_arr ne "" && $filter.count gt 0}
	{foreach from=$filter.marker_arr item=vm key=km}
<span style="background: #{$vm.code}; color: #FFFFFF; font-weight: bold;">&nbsp;{$vm.title}&nbsp;</span>&nbsp;
	{/foreach}
{/if}
...
{/capture}
{include file="dialog.tpl" title="Order dashboard" content=$smarty.capture.dialog extra='width="100%"' extra_link='<a target="_blank" style="color: #140BFC;" href="orders.php?mode=search&date=7">Most recent orders</a>'}
<br />
...
<table cellpadding="3" cellspacing="1" width="100%">
 <tr>
  <td>
{$config.Purchase_Order.po_entry_dashboard_text}
  </td>
</tr>
</table>
{/capture}
{include file="dialog.tpl" title="Pending PO entry dashboard" content=$smarty.capture.dialog extra='width="100%"'}
<br />
...
{/foreach}
 ...
 {if $tmp_counter > 0}
        {math equation="5-x" x=$tmp_counter assign="tmp_counter"}
        <td width="*" {if $tmp_counter gt "1"}colspan="{$tmp_counter}"{/if}>&nbsp;</td>
  </tr>
 {/if}
...
{/capture}
{include file="dialog.tpl" title="Product questions dashboard" content=$smarty.capture.dialog extra='width="100%"'}
<br />
...
<a target="_blank" href="inquiries.php?inq_tag_id={$v.inq_tag_id}">{$v.inquiry_attn_tag} ({$v.count})</a><br />
{/foreach}
{/if}
  </td>
...
{if $page_name eq "search" || $page_name eq ""}
{capture name=dialog}
...
<tr>
	<td width="5"><input type="radio" id="date_period_null" name="posted_data[date_period]" value=""{if ($search_prefilled eq "" or $search_prefilled.date_period eq "") && $pre_date_period eq ""} checked="checked"{/if} onclick="javascript:managedate('date',true)" /></td>
...
<tr>
	<td colspan="2"></td>
        ...
        <td width="5"><input type="radio" id="date_period_31" name="posted_data[date_period]" value="31"{if $search_prefilled.date_period eq "31"} checked="checked"{/if} onclick="javascript:managedate('date',true)" /></td>
        <td class="OptionLabel"><label for="date_period_31">Last 31 days</label></td>

        <td width="5"><input type="radio" id="date_period_7" name="posted_data[date_period]" value="7"{if $search_prefilled.date_period eq "7" || $search_prefilled eq "" || $pre_date_period eq "7"} checked="checked"{/if} onclick="javascript:managedate('date',true)" /></td>
        <td class="OptionLabel"><label for="date_period_7">Last 7 days</label></td>
...
<tr>
        <td class="FormButton" nowrap="nowrap">Fraud check status:</td>
        <td width="10">&nbsp;</td>
        <td>
          <select name="posted_data[fraud_statuses][]" multiple="multiple" size="7">
          {foreach from=$fraud_statuses_filter item=fraud_s key=fraud_code}
                <option value="{$fraud_code}"{if $fraud_s.selected} selected="selected"{/if}>{$fraud_s.name}</option>
          {/foreach}
          </select>
        </td>
</tr>
...
<tr>
        <td class="FormButton" nowrap="nowrap">Attention tag:</td>
        <td width="10">&nbsp;</td>
        <td>
{*          <select name="posted_data[attention_tags_values][]"> *}
          <select name="posted_data[attention_tags_values]">
	  <option value=""></option>
          {foreach from=$attention_tags_values_filter item=tag_s key=fraud_code}
                <option value="{$fraud_code}"{if $tag_s.selected} selected="selected"{/if}>{$tag_s.name}</option>
          {/foreach}
          </select>
        </td>
</tr>
...
<input type="hidden" name="mode" value="" />
<input type="hidden" name="page_name" value="export" />
