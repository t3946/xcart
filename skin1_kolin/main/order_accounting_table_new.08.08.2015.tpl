...
<tr>
<td colspan="6">
<input type="checkbox" value="Y" name="manufacturer_invoices_data[{$m_id}][{$invoice_number}][items_shipped_to_wrong_address]" {if $invoice.items_shipped_to_wrong_address eq "Y"}checked{/if} /> Items are shipped to an address that is different from <a onclick="javascript: $('#customers_shipping_address_{$m_id}_{$invoice_number}').toggle();" style="color: blue; border-bottom:1px dotted; text-decoration: none;" href="javascript: void(0);">the customer's shipping address</a>.

<div id="customers_shipping_address_{$m_id}_{$invoice_number}" class="cidev_NoteBox" style="display: none; margin-left: 275px; color: #550000; text-align: left; border: 1px solid #ff6600;">
 <table>
...
{if $v_a.section eq 'S'}
  <tr>
    <td>{if $v_a.title ne "Company"}<b>{/if}{$v_a.title}:{if $v_a.title ne "Company"}</b>{/if}</td><td>&nbsp;</td>
        <td width="100%" nowrap="nowrap">{$v_a.value}</td>
  </tr>
{/if}
{/foreach}
