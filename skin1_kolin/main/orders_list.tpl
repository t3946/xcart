...
<table cellpadding="2" cellspacing="1" width="100%" class="orders-list">

{assign var="colspan" value=6}

<tr class="TableHead">
	<th width="5">&nbsp;</td>
	<th width="5%" nowrap="nowrap">{if $search_prefilled.sort_field eq "orderid"}{include file="buttons/sort_pointer.tpl" dir=$search_prefilled.sort_direction}&nbsp;{/if}<a href="orders.php?mode=search&amp;sort=orderid">#</a></td>
	<th nowrap="nowrap">{if $search_prefilled.sort_field eq "status"}{include file="buttons/sort_pointer.tpl" dir=$search_prefilled.sort_direction}&nbsp;{/if}<a href="orders.php?mode=search&amp;sort=status">{$lng.lbl_status}</a></td>
	<th width="30%" nowrap="nowrap">{if $search_prefilled.sort_field eq "customer"}{include file="buttons/sort_pointer.tpl" dir=$search_prefilled.sort_direction}&nbsp;{/if}<a href="orders.php?mode=search&amp;sort=customer">{$lng.lbl_customer}</a></td>
{if $usertype eq "A" and $single_mode eq ""}
{assign var="colspan" value=7}
	<th width="20%" nowrap="nowrap">{if $search_prefilled.sort_field eq "provider"}{include file="buttons/sort_pointer.tpl" dir=$search_prefilled.sort_direction}&nbsp;{/if}<a href="orders.php?mode=search&amp;sort=provider">{$lng.lbl_provider}</a></td>
{/if}
	<th width="20%" nowrap="nowrap">{if $search_prefilled.sort_field eq "date"}{include file="buttons/sort_pointer.tpl" dir=$search_prefilled.sort_direction}&nbsp;{/if}<a href="orders.php?mode=search&amp;sort=date">{$lng.lbl_date}</a></td>
	<th width="20%" align="right" nowrap="nowrap">{if $search_prefilled.sort_field eq "total"}{include file="buttons/sort_pointer.tpl" dir=$search_prefilled.sort_direction}&nbsp;{/if}<a href="orders.php?mode=search&amp;sort=total">{$lng.lbl_total}</a></td>
</tr>

{section name=oid loop=$orders}

{math equation="x + ordertotal" x=$total ordertotal=$orders[oid].total assign="total"}
{if $orders[oid].cb_status eq "P" or $orders[oid].dc_status eq "C" or $orders[oid].dc_status eq "S"}
...
<tr{cycle values=", class='TableSubHead'"}>
	...
	<td><a href="order.php?orderid={$orders[oid].orderid}">{$orders[oid].order_prefix}{$orders[oid].orderid}</a></td>
	<td nowrap="nowrap">
{if $usertype eq "A" or ($usertype eq "P" and $active_modules.Simple_Mode ne "")}
<input type="hidden" name="order_status_old[{$orders[oid].orderid}]" value="{$orders[oid].cb_status}" />
<input type="hidden" name="order_status_old[{$orders[oid].orderid}]" value="{$orders[oid].dc_status}" />
<input type="hidden" name="order_status_old[{$orders[oid].orderid}]" value="{$orders[oid].bd_status}" />
{if $orders[oid].goid ne ""}
{assign var="is_gcheckout_orders" value="1"}
{include file="main/order_status.tpl" status=$orders[oid].cb_status mode="select" name="order_status[`$orders[oid].orderid`]" extra="disabled='disabled'"}
{else}
{include file="main/order_status.tpl" status=$orders[oid].cb_status mode="select" name="order_status[`$orders[oid].orderid`]"}
{/if}
{else}
<a href="order.php?orderid={$orders[oid].orderid}"><b>{include file="main/order_status.tpl" status=$orders[oid].cb_status mode="static"}</b></a>
{/if}
