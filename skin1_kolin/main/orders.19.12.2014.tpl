...
<table align="center">
 ...
 <td style="font-size: 15px; {* font-weight: bold; *}">
        / <a target="_blank" style="color: #140BFC;{* text-decoration: none;*}" href="orders.php?mode=search&date=7">Most recent orders</a>
 </td>
 {/if}
 </tr>
</table>
...
<tr>
    {foreach from=$row item=filter}
        ...
        <td width="20%">&nbsp;</td>
        {else}
        <td width="20%"{if $filter.bold eq 'Y'} class="filter_bold"{/if}>{if $filter.count gt 0}<a href="orders.php?fid={$filter.fid}">{/if}{$filter.title} ({$filter.count|default:"0"}){if $filter.count gt 0}</a>{/if}</td>
        {/if}
    {/foreach}
<tr>
    {/if}
...
<form name="inquiry_typeform1" action="orders.php" method="POST">
    <input type="hidden" name="mode" value="add_inquiry">

<table cellpadding="3" cellspacing="1" width="100%">
 <tr class='TableSubHead'>
  <td width="20%" nowrap="nowrap">Inquiry type:</td>
  <td width="*" nowrap="nowrap">Base subject:</td>
 </tr>
 <tr>
  <td>
  ...
  <td align="center">
	<textarea name="add_inq_subject" value="" style="width: 98%;" rows="4" cols="20" maxlength="500" /></textarea>
  </td>
</tr>
<tr>
<td colspan="2" align="right">
<input type="submit" name="Submit new inquiry" value="Submit new inquiry" />
</td>
</tr>
</table>
</form>
...
<a href="inquiries.php?inq_type_id={$v.inq_type_id}">{$v.inquiry_type} ({$v.count})</a><br />
{/foreach}
{/if}
  </td>
  <td valign="top">
{if $inquiry_attn_tags ne ""}
<B>By attention tag</B></br />
{foreach from=$inquiry_attn_tags item=v key=k}
<a href="inquiries.php?inq_tag_id={$v.inq_tag_id}">{$v.inquiry_attn_tag} ({$v.count})</a><br />
{/foreach}
{/if}
  </td>
</tr>
