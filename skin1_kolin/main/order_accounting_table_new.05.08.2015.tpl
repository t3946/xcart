      ...
      {foreach from=$v.tracking item=t}
       ...
       {if ($t.invoice_number eq $invoice_number) || ($t.invoice_number eq "" && $invoice_number eq "1")}
        ...
        {if $t.tracknum ne ""}
          ...
          <a href="javascript: void(0);" onclick="javascript: $('#acc_tracknum_val_{$m_id}_{$invoice_number}_{$row_conter}').val(''); $('#acc_tracknum_link_{$m_id}_{$invoice_number}_{$row_conter}').val(''); $('#acc_tracknum_{$m_id}_{$invoice_number}_{$row_conter}').hide();"><img src="{$ImagesDir}/minus.gif" /></a>
          ...
          <input type="hidden" name="tracknums[{$m_id}][{$invoice_number}][{$row_conter}][linkid]" value="{$t.linkid}" id="acc_tracknum_link_{$m_id}_{$invoice_number}_{$row_conter}" />
{*          <input type="hidden" name="tracknums[{$m_id}][{$invoice_number}][{$row_conter}][invoice_number]" value="{$t.invoice_number}" id="acc_tracknum_link_{$m_id}_{$invoice_number}_{$row_conter}" /> *}
          <br />
          </div>
...
<tr id="acc_track_{$m_id}_{$invoice_number}_tr">
  ...
  <select name="groups[{$m_id}][tracking_shipper][{$invoice_number}][0]" {* {if $v.allow_dispatch_off_working_hours_functionality_enabled eq "Y"}disabled="disabled"{/if} *}>
  <option value=""></option>
