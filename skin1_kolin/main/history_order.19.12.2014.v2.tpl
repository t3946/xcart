...
{/if}
 ...
 {if $v.additional_shipping_status ne "W"}
...
{if $order.po_details && ($order.orig_po eq "" || ($order.po_issued_to eq 'A' || $order.po_issued_to eq "") || $order.total_shipping_charge_on_orig_po lte 0 || $order.po_details.name_of_purchaser eq 'unknown' || $order.po_details.purchase_manager_phone eq '(000) 000-0000' || $order.po_details.po_fax eq '(000) 000-0000' || $order.po_details.purchase_manager_email eq 'unknown@unknown.com' || $order.po_details.accounts_payable_full_name eq 'unknown' || $order.po_details.accounts_payable_phone eq '(000) 000-0000' || $order.po_details.accounts_payable_fax eq '(000) 000-0000' || $order.po_details.accounts_payable_email eq 'unknown@unknown.com')}
...
{$lng.lbl_send_dispatch_to_distributor}<br />
  ...
  <td>
  <input name="send_email_button" type="button" value="Send (Dispatch to distributor)" onclick="javascript: tinyMCE.triggerSave(); func_set_value_to_field(document.manuf_notifyform_{$mnf_id}, 'dispatch_to_distributor_', 'mnf_body', {$mnf_id});" {if $allowed_elements.send_dispatch_to_distributor_btn eq "N"}disabled="disabled"{/if} />{if $allowed_elements.send_dispatch_to_distributor_btn eq "N"}You are NOT authorized to click this{/if}
  </td>
  </tr>
  </table>
  <br /><br />
...
<div id="main_order_tabs-container">
  ...
  {foreach from=$main_order_tabs item=tab key=ind}
    <li
	{if $tab.anchor eq "reference" || $tab.anchor eq "ground_map"}
		style="float: right"
	{/if}
    >
{if $tab.anchor eq "ground_map"}
<div style="padding-top: 7px; padding-right: 10px; cursor: pointer;" onclick="javascript: window.open('ground_map.php?orderid={$orderid}');">
{$tab.title}
</div>
{else}
	<a {if $tab.anchor eq "order_details"}style="color: #580505; font-weight: bold;"{elseif $tab.anchor eq "customer_info" && $order.note_is_taken_care_of eq "N" && $order.customer_notes ne ""}style="font-weight: bold;"{/if} href="#main_order_tabs-{$tab.anchor}">{$tab.title}</a>
{/if}
    </li>
  {/foreach}
  </ul>
