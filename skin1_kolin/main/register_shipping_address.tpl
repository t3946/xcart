...
{section name=country_idx loop=$countries}
<option value="{$countries[country_idx].country_code}" {if $userinfo.s_country eq "" && ($new_login_type eq "P" || $new_login_type eq "A") && $main eq "user_add"} {if $countries[country_idx].country_code eq $config.Company.location_country} selected="selected"{/if} {else}{if $geo_litecity_location.country ne ""}{if $geo_litecity_location.country eq $countries[country_idx].country_code} selected="selected"{/if}{else}{if $userinfo.s_country eq $countries[country_idx].country_code} selected="selected"{elseif $countries[country_idx].country_code eq $config.General.default_country and $userinfo.s_country eq ""} selected="selected"{/if}{/if}{/if}>{$countries[country_idx].country|amp}</option>
{/section}
</select>
{if $reg_error ne "" and $userinfo.s_country eq "" and $default_fields.s_country.required eq 'Y'}<font class="Star">&lt;&lt;</font>{/if}
</td>
...
<td valign="top">{if $default_fields.s_state.required eq 'Y'}<font class="Star">*</font>{else}&nbsp;{/if}</td>
<td valign="top" nowrap="nowrap">
  {if $userinfo.s_state eq "" && ($new_login_type eq "P" || $new_login_type eq "A") && $main eq "user_add"}
	{include file="main/states.tpl" states=$states name="s_state" default=$config.Company.location_state default_country=$config.Company.location_country country_name="s_country"}
  {else}
...
<tr style="display: none;">
	<td valign="top">
  {if $userinfo.s_state eq "" && ($new_login_type eq "P" || $new_login_type eq "A") && $main eq "user_add"}
	{include file="main/register_states.tpl" state_name="s_state" country_name="s_country" county_name="s_county" state_value=$config.Company.location_state county_value=$userinfo.s_county}
  {else}
