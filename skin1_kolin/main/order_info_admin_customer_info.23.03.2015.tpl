...
{if !$static}
<input type="submit" value="{$lng.lbl_apply_changes|escape}" {if $order.amazonorderid ne ""}disabled="disabled" style="border: 1px solid #ff0000" {/if} />
{if $current_membership_flag ne 'FS'}
  &nbsp;&nbsp;&nbsp;&nbsp;
  <input type="button" value="{$lng.lbl_apply_changes_send_email|escape}" onclick="javascript: $('#send_email2').val('Y'); this.form.submit();" {if $order.amazonorderid ne ""}disabled="disabled" style="border: 1px solid #ff0000" {/if} />
{/if}
{/if}
