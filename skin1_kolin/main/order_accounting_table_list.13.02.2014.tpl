...
<tr class="OrderSheetCell{$cycle_class}" style="font-weight: bold;">
  ...
  <td>{include file="main/order_status.tpl" status=$order.cb_status mode="static" status_type="CB"}</td>
  <td nowrap="nowrap" class="OrderSheetCommonCell">{$order.firstname}</td>
  <td></td>
  <td>
  {foreach from=$all_processors item=ps key=pid}
      {if $pid eq $order.shipping_groups[$order.last_manufacturer_id].acc_paymentid}{$ps.payment_method}{/if}
  {/foreach}
  </td>
...
<tr class="OrderSheetCell{$cycle_class}">
  ...
  <td class="OrderSheetGreenCell"><b>{include file="main/order_status.tpl" status=$order.dc_status mode="static" status_type="DC"}</b></td>
...
<tr class="OrderSheetCell{$cycle_class}">
  <td width="5">
    {foreach from=$order.all_codes item=v_all_code key=k_all_code}
      <a href="order.php?orderid={$order.orderid}" target="_blank">{$v_all_code}</a><br />
    {/foreach}
  </td>
  <td class="OrderSheetGreenCell"><b>{include file="main/order_status.tpl" status=$order.bd_status mode="static" status_type="BD"}</b></td>
...
<tr class="OrderSheetCell{$cycle_class} OrderSheetLast">
  ...
  <td>&nbsp;</td>
  <td>{include file="currency2.tpl" value=$order.total}</td>
