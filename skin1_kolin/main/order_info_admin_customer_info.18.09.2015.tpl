...
Completed: {$count_Completed}, Fraud: {$count_Fraud}, Open: {$count_Open}
...
<tr>
<td>
<a href="order.php?orderid={$v_o.orderid}" target="_blank"># {$v_o.order_prefix}{$v_o.orderid}</a>
{assign var="v_o_status_found" value=""}
{foreach from=$v_o.statuses item=v_o_status key=k_o_status}
{if $v_o_status eq "Y"}{if $v_o_status_found eq "Y"}, {else}: {/if}<span style="background: {if $k_o_status eq 'Completed'}#F4CCCC{elseif $k_o_status eq 'Fraud'}Red{elseif $k_o_status eq 'Open'}#D9EAD3{/if};">{$k_o_status}</span>{assign var="v_o_status_found" value="Y"}{/if}
{/foreach}
</td>
</tr>
{/if}
