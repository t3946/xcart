...
<tr class="refund-distr-totals-line">
  <td>{$lng.lbl_refund_for} {$group.group_name} {$lng.lbl_items}</td>
  <td>{$group.code}</td>
  <td>&nbsp;</td>
  <td>&nbsp;</td>
  <td>&nbsp;</td>
...
<tr>
  <td colspan="{$colspan}">
    <input type="button" value="{$lng.lbl_send_refund_notification|strip_tags:false|escape}" onclick="javascript: $('#ordereditform_mode').val('ref_notify'); $('#ordereditform_mid').val('{$mid}'); this.form.submit();" />&nbsp;&nbsp;
