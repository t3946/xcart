...
<tr class="distributor-totals-line">
  <td>
    ...
    <tr>
      ...
      <td align="right"><a target="_blank" onmouseout="javascript: $('#d_specific_instructions_note_{$m_id}').delay(3000).hide(3000);" onmouseover="javascript: cidev_showNote('d_specific_instructions_note_{$m_id}', this);" style="color: #FF0000;" href="manufacturers.php?manufacturerid={$m_id}&distributor_section=1">?</a></td>
      {/if}
    </tr>
    </table>
...
{*  <td align="right">{include file="currency2.tpl" value=$v.total.pst hide_zero='Y'}</td> *}
  ...
  <td> 
{if $v.empty_products_list eq "Y"}<input type="checkbox" value="Y" name="distributors_to_delete[{$m_id}][delete]" {if $order.amazonorderid ne ""}disabled="disabled"{/if} />{else}&nbsp;{/if}
  </td>
</tr>
{foreach from=$v.products item=product key=prod_num}
...
{* --------------------- *}
    {if $product.orig_product_classes ne ""}
      ...
      {foreach from=$product.orig_product_classes item=item key=key}
        {if $item.options ne ""}
          <br /> {$item.classtext}
          <select name="items[{$product.itemid}][classid_optionid][{$item.classid}]" {if $refunded_option_found eq "Y" || $order.amazonorderid ne ""}disabled="disabled"{/if}>
        ...
        {elseif $item.is_modifier eq "T"}
          <br /> {$item.classtext}
          <input type="text" name="items[{$product.itemid}][classid_optionid][{$item.classid}]" {if $refunded_option_found eq "Y" || $order.amazonorderid ne ""}readonly="readonly"{/if} value="{$product.product_options[$item.classid].option_name}">
...
{/if}
  ...
  <td align="right">{if !$static}<input type="text" size="8" name="items[{$product.itemid}][price]" value="{$product.price|price_format}" {if $order.amazonorderid ne ""}readonly="readonly"{/if} />{else}{include file="currency2.tpl" value=$product.price|price_format}{/if}
...
{* --- *}
  ...
  <td align="right" {* valign="top" *}>{if !$static}<input type="text" size="5" id="items_amount_{$m_id}_{$product.itemid}" name="items[{$product.itemid}][amount]" value="{$product.amount}" {* {if $v.dc_status eq 'C' || $v.dc_status eq 'L' || $v.dc_status eq 'S'}readonly="readonly"{/if} *} {if $order.amazonorderid ne ""}readonly="readonly"{/if} />{else}{$product.amount}{/if}</td>
  <td align="right" {* valign="top" *}>
{if !$static}
  {if $v.dc_status eq 'K' || $v.dc_status eq 'E'}
    {$product.back}
    <input type="hidden" name="items[{$product.itemid}][back]" value="{$product.back}" {if $order.amazonorderid ne ""}readonly="readonly"{/if} />
  {else}
    <input type="text" size="5" name="items[{$product.itemid}][back]" value="{$product.back}" {if $order.amazonorderid ne ""}readonly="readonly"{/if} />
  {/if}
{else}
  {$product.back}
{/if}
...
{if !$static}
  ...
  {else}
  <input id="eta_date_mm_dd_yyyy_{$product.itemid}" type="text" size="9" style="width: 98%;" name="items[{$product.itemid}][eta_date_mm_dd_yyyy]" value="{$product.eta_date_mm_dd_yyyy}" {if $order.amazonorderid ne ""}readonly="readonly"{/if} />
  {/if}
{else}
  {$product.eta_date_mm_dd_yyyy}
{/if}
...
{* --- *}
  ...
  <td align="center">{if !$static}<input type="checkbox" value="Y" name="items[{$product.itemid}][delete]" {if $order.amazonorderid ne ""}disabled="disabled"{/if} />{else}&nbsp;{/if}</td>
</tr>
{/foreach}
<tr{cycle values=", class='TableSubHead'" name="cycle_`$m_id`"}>
  <td>{if !$static}<input type="text" maxlength="255" name="groups[{$m_id}][shipping]" value="{$v.shipping|trademark:''}" style="width: 99%;" {if $order.amazonorderid ne ""}readonly="readonly"{/if} />{else}{$v.shipping}{/if}</td>
  <td colspan="5">
    {if $v.tracking}
      ...
      {foreach from=$v.tracking item=t}
        ...
        {if $t.tracknum ne ""}
          ...
          <a href="javascript: void(0);" onclick="javascript: $('#tracknum_val_{$m_id}_{$row_conter}').val(''); $('#tracknum_link_{$m_id}_{$row_conter}').val(''); $('#tracknum_{$m_id}_{$row_conter}').hide();"><img src="{$ImagesDir}/minus.gif" /></a>
  ...
  <td align="right">
    {if !$static}
      <input type="hidden" name="groups[{$m_id}][shipping_cost_net_orig]" value="{$v.shipping_cost.net|price_format}" {if $order.amazonorderid ne ""}readonly="readonly"{/if} />
      <input id="groups_shipping_cost_net_{$m_id}" type="text" size="8" name="groups[{$m_id}][shipping_cost_net]" value="{$v.shipping_cost.net|price_format}" {* {if $v.cb_status eq 'P' || $v.cb_status eq '3' || $v.cb_status eq 'V' || $v.cb_status eq 'H' || $v.cb_status eq 'R'}readonly="readonly"{/if} *}  {if $order.amazonorderid ne ""}readonly="readonly"{/if} />
    {else}
...
<input type="hidden" name="groups[{$m_id}][shipping_value_selectbox]" id="shipping_value_selectbox_{$m_id}" value="{$v.shipping_value_selectbox}" />
  {/if}
  <select name="groups[{$m_id}][shipping_value_selectbox]" id="shipping_value_selectbox_{$m_id}" {if $order.amazonorderid ne ""}disabled="disabled"{/if} 
...
{/if}
  ...
  <td align="right">
      <input id="actual_shipping_cost_net_{$m_id}" type="text" size="8" name="groups[{$m_id}][actual_shipping_cost_net]" value="{$v.actual_shipping_cost.net|price_format}" {* {if $order.amazonorderid ne ""}readonly="readonly"{/if} *} />
  </td>
...
<td align="right"><B>{if $order_manufacturers[$m_id].estimated_profit_abs ne ""}<span style="color: #FF0000;">(${$order_manufacturers[$m_id].estimated_profit_abs})</span>{else}${$order_manufacturers[$m_id].estimated_profit}{/if}</B></td>
...
<input type="hidden" name="groups[{$m_id}][additional_shipping_status]" id='additional_shipping_status_{$m_id}' value="{$v.additional_shipping_status}" />
  {/if}
<select name="groups[{$m_id}][additional_shipping_status]" id='additional_shipping_status_{$m_id}' {if $order.amazonorderid ne ""}disabled="disabled"{/if}>
...
<input type="hidden" name="groups[{$m_id}][additional_vt_paymentid]" id="additional_vt_paymentid_{$m_id}" value="{$v.additional_vt_paymentid}" />
  {/if}
         <select name="groups[{$m_id}][additional_vt_paymentid]" id="additional_vt_paymentid_{$m_id}" {if $order.amazonorderid ne ""}disabled="disabled"{/if}>
       ...
       <td>
           ...
           <input type="text" name="groups[{$m_id}][additional_transaction_id_link]" id="additional_transaction_id_link_{$m_id}" value="{$v.additional_transaction_id_link}" size="40" {if $order.amazonorderid ne ""}readonly="readonly"{/if} />
       </td>
       <td width="20">&nbsp;</td>
       <td>
           <b>AVS code:</b><br />
           <input type="text" name="groups[{$m_id}][additional_avs_code]" id="additional_avs_code_{$m_id}" value="{$v.additional_avs_code}" size="1" maxlength="1" {if $order.amazonorderid ne ""}readonly="readonly"{/if} />
       </td>
     </tr>
   </table>
 </td>
...
Tracking number is put on the invoice
</td>
</tr>
...
<td colspan="10">
  <table>
    <tr>
      <td>
        <b>Payment method:</b><br />
        <select name="vt_paymentid" id="vt_paymentid" {if $order.amazonorderid ne ""}readonly="readonly"{/if}>
        <option value="0"></option>
        {foreach from=$all_vt_processors item=item_vt key=key_vt}
      ...
      <td>
          <b>Virtual terminal transaction ID:</b><br />
          <input type="text" name="transaction_id_link" id="transaction_id_link" value="{$order.transaction_id_link}" size="40" {if $order.amazonorderid ne ""}readonly="readonly"{/if} />
      </td>
      <td width="20">&nbsp;</td>
      <td>
          <b>AVS code:</b><br />
          <input type="text" name="avs_code" id="avs_code" value="{$order.avs_code}" size="1" maxlength="1" {if $order.amazonorderid ne ""}readonly="readonly"{/if} />
      </td>
    </tr>
  </table>
</td>
