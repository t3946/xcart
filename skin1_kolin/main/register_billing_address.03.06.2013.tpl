...
<td valign="top" nowrap="nowrap">
<input type="text" id="b_countryname" name="b_countryname" size="32" maxlength="64" value="{if $geo_litecity_location.country ne ""}{section name=country_idx loop=$countries}{if $geo_litecity_location.country eq $countries[country_idx].country_code}{$countries[country_idx].country|amp}{/if}{/section}{else}{$userinfo.b_countryname}{/if}" 
...
<td valign="top">{if $default_fields.b_country.required eq 'Y'}<font class="Star">*</font>{else}&nbsp;{/if}</td>
<td valign="top" nowrap="nowrap">
<select name="b_country" id="b_country" size="1" onchange="cidev_new_check_zip_code(); {if $usertype ne "P" && $usertype ne "A"} check_zip_code_ship('b_zipcode', 'b_countryname'); cidev_set_check_state_field(this.value, 'b_state'); {/if}">
...
<td valign="top" nowrap="nowrap">
<input type="text" id="b_zipcode" name="b_zipcode" size="32" maxlength="32" value="{$userinfo.b_zipcode}" onchange="if ($('#ship2diff').attr('checked') )cidev_new_check_zip_code(); {if $usertype ne "P" && $usertype ne "A"} check_zip_code_ship('b_zipcode', 'b_countryname'); {/if}" {if $usertype ne "P" && $usertype ne "A"} onkeyup="cidev_check_field('b_zipcode'); cidev_check_address_b();" {/if} autocomplete="off" placeholder="{if $geo_litecity_location.postalCode ne ""}{$geo_litecity_location.postalCode}{else}{$lng.lbl_fill_in_examples_zip}{/if}" />
