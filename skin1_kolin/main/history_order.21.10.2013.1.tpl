...
<tr>
<td>{$lng.lbl_date}: {$order.date|date_format:$config.Appearance.datetime_format}</td>
<td>Order source: <a href="{$customer.referer}" target="_blank">Referral link</a></td>
<td colspan="2"></td>
</tr>
</table>
</div>
...
*}
 ...
 {if $show_request_additional_shipping_charge eq "Y"} 
  ...
  Estimated profit = {if $v.estimated_profit_abs ne ""}<span style="color: #FF0000;">({$v.estimated_profit_abs})</span>{else}{$v.estimated_profit}{/if} =  {if $v.estimated_profit_margin_percent_abs ne ""}<span style="color: #FF0000;">({$v.estimated_profit_margin_percent_abs}%)</span>{else}{$v.estimated_profit_margin_percent}%{/if}
  <br />
  Estimated profit after additional payment = {if $v.estimated_profit_after_additional_payment_abs ne ""}<span style="color: #FF0000;">({$v.estimated_profit_after_additional_payment_abs})</span>{else}{$v.estimated_profit_after_additional_payment}{/if} = {if $v.estimated_profit_margin_after_additional_payment_percent_abs ne ""}<span style="color: #FF0000;">({$v.estimated_profit_margin_after_additional_payment_percent_abs}%)</span>{else}{$v.estimated_profit_margin_after_additional_payment_percent}%{/if}
  <br /><br />
  <B>{$lng.lbl_from}:</B><br />
  <input type="text" name="mnf_from" value="{$config.Company.orders_department}" readonly="readonly" style="width: 80%;" /><br /><br />
  <B>{$lng.lbl_to}:</B><br />
 ...
 {if $show_stock_availability_form eq "Y"}
	<div class="ProductTitle" align="center">{$v.manufacturer}: Info Request Survey</div>
	{include file="customer/main/stock_availability.tpl" o=$order.orderid m=$mnf_id products=$products admin_area_uses="Y"}
	<br /><br />
	<hr /><br />
 {/if}
