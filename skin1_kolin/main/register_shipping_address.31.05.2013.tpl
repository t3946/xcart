...
//		document.forms["registerform"].s_zipcode.value = $.trim(document.forms["registerform"].s_zipcode.value);
			...
			/* -- additional checking (when change country and enter zip) -- */
			var cityFilePath = '';
...
<td nowrap="nowrap">
<select name="s_country" id="s_country" size="1" onchange="check_zip_code(); {if $usertype ne "P" && $usertype ne "A"} check_zip_code_ship(); cidev_set_check_state_field(this.value, 's_state'); {/if}">
...
<td nowrap="nowrap">
<input type="text" id="s_city" name="s_city" size="32" maxlength="64" value="{$userinfo.s_city}" {if $usertype ne "P" && $usertype ne "A"} onkeyup="cidev_check_field('s_city'); cidev_check_zip();" {/if} placeholder="{if $geo_litecity_location.city ne ""}{$geo_litecity_location.city}{else}{$lng.lbl_fill_in_examples_city}{/if}" />
