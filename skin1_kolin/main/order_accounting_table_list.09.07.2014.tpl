...
{else}
        {assign var="cycle_class" value="class='TableSubHead'"}
{/if}
...
<tr {$cycle_class} style="font-weight: bold;">
  ...
  <td>{$fraud_statuses[$order.fraud_status]} ({$order.overall_fraud_score}) {*{include file="main/order_status.tpl" status=$v.cb_status mode="static" status_type="CB"}*}</td>
  <td nowrap="nowrap" class="OrderSheetCommonCell">{if $order.otrs_ticket ne ""}<a href="{$order.otrs_ticket}" target="_blank">OTRS ticket</a>{/if}{*{$order.firstname}*}</td>
  <td></td>
  <td>
...
<tr {$cycle_class}>
  ...
  <td class="OrderSheetGreenCell"><b>{include file="main/order_status.tpl" status=$v.cb_status mode="static" status_type="CB"}</b>{*<b>{include file="main/order_status.tpl" status=$v.dc_status mode="static" status_type="DC"}</b>*}</td>
  <td>{if $order.ca_status ne ""}<b>{include file="main/order_status.tpl" status=$order.ca_status mode="static" status_type="CA"}</b>{/if}{*{$order.lastname}*}</td>
  <td></td>
...
<tr {$cycle_class}>
  ...
  <td class="OrderSheetGreenCell"><b>{include file="main/order_status.tpl" status=$v.dc_status mode="static" status_type="DC"}</b>{*<b>{include file="main/order_status.tpl" status=$v.bd_status mode="static" status_type="BD"}</b>*}</td>
  <td>{$order.firstname}{*{$order.s_countryname}*}</td>
  <td></td>
...
<tr {$cycle_class}>
  ...
  <td class="OrderSheetGreenCell"><b>{include file="main/order_status.tpl" status=$v.bd_status mode="static" status_type="BD"}</b>{*{$fraud_statuses[$order.fraud_status]} / {if $order.ca_status ne ""}{include file="main/order_status.tpl" status=$order.ca_status mode="static" status_type="CA"}{/if}*}</td>
