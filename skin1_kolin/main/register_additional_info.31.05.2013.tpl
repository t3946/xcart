...
<tr>
<td align="right">{$v.title|default:$v.field} {if $v.required ne 'Y'}<font style="font-size: 11px; font-family: italic; color: #C0C0C0;"><I>(Optional)</I></font>{/if}</td>
<td>{if $v.required eq 'Y'}<font class="Star">*</font>{else}&nbsp;{/if}</td>
<td nowrap="nowrap">
{if $v.type eq 'T'}
<input type="text" name="additional_values[{$v.fieldid}]" id="additional_values_{$v.fieldid}" size="32" value="{$v.value|escape}" {if $v.title eq "Company"} placeholder="{$lng.lbl_fill_in_examples_Company_name}" {/if} onkeyup="cidev_check_field_if_empty('additional_values_{$v.fieldid}')" />
{elseif $v.type eq 'C'}
