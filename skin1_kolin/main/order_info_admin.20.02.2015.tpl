...
{*  <td align="right">{include file="currency2.tpl" value=$v.total.pst hide_zero='Y'}</td> *}
  ...
  <td> 
{if $v.empty_products_list eq "Y"}<input type="checkbox" value="Y" name="distributors_to_delete[{$m_id}][delete]" />{else}&nbsp;{/if}
  </td>
</tr>
{foreach from=$v.products item=product key=prod_num}
...
{* --------------------- *}
    {if $product.orig_product_classes ne ""}
      ...
      {foreach from=$product.orig_product_classes item=item key=key}
        {if $item.options ne ""}
          <br /> {$item.classtext}
          <select name="items[{$product.itemid}][classid_optionid][{$item.classid}]" {if $refunded_option_found eq "Y"}disabled="disabled"{/if}>
        ...
        {elseif $item.is_modifier eq "T"}
          <br /> {$item.classtext}
          <input type="text" name="items[{$product.itemid}][classid_optionid][{$item.classid}]" {if $refunded_option_found eq "Y"}readonly="readonly"{/if} value="{$product.product_options[$item.classid].option_name}">
...
{if !$static}
  ...
  {else}
  <input id="eta_date_mm_dd_yyyy_{$product.itemid}" type="text" size="9" style="width: 98%;" name="items[{$product.itemid}][eta_date_mm_dd_yyyy]" value="{$product.eta_date_mm_dd_yyyy}" />
  {/if}
{else}
  {$product.eta_date_mm_dd_yyyy}
{/if}
...
<tr{cycle values=", class='TableSubHead'" name="cycle_`$m_id`"}>
  ...
  <td align="right">
    {if !$static}
      ...
      <input id="groups_shipping_cost_net_{$m_id}" type="text" size="8" name="groups[{$m_id}][shipping_cost_net]" value="{$v.shipping_cost.net|price_format}" {* {if $v.cb_status eq 'P' || $v.cb_status eq '3' || $v.cb_status eq 'V' || $v.cb_status eq 'H' || $v.cb_status eq 'R'}readonly="readonly"{/if} *} />
    {else}
...
<tr id="add_to_order_tr">
  ...
  <td align="center" id="add_to_order_box_1"><input type="text" name="add_productcode[0]" value="" style="width: 96%;" id="sku_add_to_order_box_0" {* {if $all_cb_status_eq_P eq "Y" || $all_cb_status_eq_3 eq "Y" || $all_cb_status_eq_V eq "Y" || $all_cb_status_eq_H eq "Y" || $all_cb_status_eq_R eq "Y"}disabled="disabled" {/if} *} /></td>
...
<input type="submit" value="{$lng.lbl_apply_changes|escape}" {if $order.amazonorderid ne ""}disabled="disabled" {/if} />
{if $current_membership_flag ne 'FS'}
  &nbsp;&nbsp;&nbsp;&nbsp;
  <input type="button" value="{$lng.lbl_apply_changes_send_email|escape}" onclick="javascript: $('#send_email1').val('Y'); this.form.submit();" {if $order.amazonorderid ne ""}disabled="disabled" {/if} />
{/if}
