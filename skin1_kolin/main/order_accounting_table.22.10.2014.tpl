...
*}
    ...
    {if $v.profit_margin lt 0 || $v.accounting[5].net lt 0}({/if}{$v.profit_margin|price_format|replace:"-":""|replace:"0.00":"&infin;"}%{if $v.profit_margin lt 0 || $v.accounting[5].net lt 0}){/if}
  </td>
  {else}
...
<tr class="OrderSheetCell{$cycle_class}">
  ...
  {if !$static || $static eq 'O'}
    ...
    {else}
  <input name="groups{if $static eq 'O'}[{$order.orderid}]{/if}[{$m_id}][acc][{$smarty.section.acc.index}][gst]" size="8" value="{$v.accounting[$smarty.section.acc.index].gst|price_format}" {if ($smarty.section.acc.index eq "1" || $smarty.section.acc.index eq "2") && $v.full_reconciliation_info ne ""}readonly="readonly" style="background: #cccccc;"{/if} />
    {/if}
  {else}
  {include file="currency2.tpl" value=$v.accounting[$smarty.section.acc.index].gst hide_zero='Y'}
  {/if}
...
<tr class="OrderSheetCell{$cycle_class}">
  ...
  {if !$static || $static eq 'O'}
    ...
    {else}
  <input name="groups{if $static eq 'O'}[{$order.orderid}]{/if}[{$m_id}][acc][{$smarty.section.acc.index}][pst]" size="8" value="{$v.accounting[$smarty.section.acc.index].pst|price_format}" {if ($smarty.section.acc.index eq "1" || $smarty.section.acc.index eq "2") && $v.full_reconciliation_info ne ""}readonly="readonly" style="background: #cccccc;"{/if} />
    {/if}
  {else}
  {include file="currency2.tpl" value=$v.accounting[$smarty.section.acc.index].pst hide_zero='Y'}
  {/if}
