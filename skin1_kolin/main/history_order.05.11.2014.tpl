...
{else}
	...
	{if $count_locked_orders gt "1"}
		<br /><input type="button" value="Unlock all orders locked by me" onclick="javascript: $('#id_mode_unlock').val('unlock_orders'); this.form.submit();" />
	{/if}
...
//<![CDATA[
$(function() {ldelim}
  $('#main_order_tabs-container').tabs({if $smarty.get.tab ne "y"}{if $order.note_is_taken_care_of eq "N" && $order.customer_notes ne ""}{literal}{selected: 1}{/literal}{else}{literal}{selected: 0}{/literal}{/if}{/if});
{rdelim});
//]]>
</script>
...
<div id="main_order_tabs-container">
  ...
  {foreach from=$main_order_tabs item=tab key=ind}
    <li><a href="#main_order_tabs-{$tab.anchor}">{$tab.title}</a></li>
  {/foreach}
  </ul>
