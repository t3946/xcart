...
<br />
<span style="font-size: 12px; color: #FF0000; font-weight: bold;">{$warning_message}</span><br />
{else}
    {if $order_unlocked eq "Y"}
	<span style="font-size: 12px; color: #FF0000; font-weight: bold;">{$unlock_message}</span><br />
    {else}
	<form action="order.php?orderid={$orderid}" method="post" name="unlockorderform">
	<input type="hidden" name="mode" value="unlock_order" />
	<span style="font-size: 12px; color: #FF0000; font-weight: bold;">{$lock_message}</span><input type="submit" value="UNLOCK NOW" />
	</form>
    {/if}
{/if}
<br /><br />
