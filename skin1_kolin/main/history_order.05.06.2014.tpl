...
//<![CDATA[
{literal}
        tinymce.init({selector:'textarea.new_editor'});
{/literal}
//]]>
</script>
*}
...
{else}
	...
	{$lock_message}<input type="submit" value="Unlock it now" />.
	</form>
    {/if}
    </td>
    </tr>
...
{/capture}
{include file="dialog.tpl" title="Customer notes" content=$smarty.capture.dialog extra='width="100%"'}
<br />
{/if}
   ...
   <td width="*" valign="top">
	...
	<td>
            {if $ca_statuses ne ""}
                 <table cellspacing="0" cellpadding="0" border="0">
                 <tr>
                 <td nowrap="nowrap" valign="top"><B>Currently assigned to</B></td>
                 <td valign="top">
	...
	<tr>
	<td nowrap="nowrap">
	<a target="_blank" style="color: #140BFC" href="{$ticket_resolver_link}">OTRS ticket for this order{if $ticket_resolver_messages ne ""} ({$ticket_resolver_messages}){/if}</a>
        </td>
        </tr>
	{/if}
        <tr>
...
{if $found_show_stock_availability_form eq "Y"}
 {capture name=dialog}
  ...
  {/foreach}
 {/capture}
{include file="dialog.tpl" title="Information request" content=$smarty.capture.dialog extra='width="100%"'}
<br />
{/if}
{/if}
...
*}
  <textarea rows="30" cols="60" name="mnf_body" style="width: 80%;" class="new_editor">{$backorder_decision_request_message|replace:"\n":"<br />"}</textarea>
...
*}
  <INPUT type="button" value="Send (Backorder decision request)" onclick="document.decision_request_message_form.submit();">
