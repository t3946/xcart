...
{capture name=dialog}
    ...
    <form action="product_modify.php" method="post"
          name="modifyform" {* {if $config.SEO.clean_urls_enabled eq "Y"}onsubmit="javascript: return checkCleanUrl(document.modifyform.clean_url)"{/if} *} >
        <input type="hidden" name="productid" value="{$product.productid}"/>
        ...
        <table cellpadding="4" cellspacing="0" width="100%">
            ...
            <tr>
                ...
                <td class="FormButton" width="20%"
                    nowrap="nowrap">{if $usertype eq "A" and $new_product eq 1}{$lng.lbl_provider}{else}Last modified by{/if}
                    :
                </td>
                    ...
                    {if $usertype eq "A" and $new_product eq 1}
                        <select name="provider" class="InputWidth">
                            {section name=prov loop=$providers}
                                <option value="{$providers[prov].login}">{$providers[prov].login}
            ...
            <tr>
                {if $geid ne ''}
                    <td width="15" class="TableSubHead"><input type="checkbox" value="Y" name="fields[forsale]"/></td>
                {/if}
                ...
                <tr>
                    {if $geid ne ''}
                        <td width="15" class="TableSubHead"><input type="checkbox" value="Y"
                                                                   name="fields[lock_forsale]"/></td>
                    {/if}
                    <td class="FormButton" nowrap="nowrap">Lock 'Availability' status:</td>
                    <td class="ProductDetails">
                        <select name="lock_forsale">
                            <option value="N"{if $product.lock_forsale eq "N"} selected="selected"{/if}>Unlocked
                            </option>
                            <option value="Y"{if $product.lock_forsale eq "Y"} selected="selected"{/if}>Locked forever
                            </option>
                        </select>
                    </td>
                </tr>
            ...
            <tr>
                {if $geid ne ''}
                    <td width="15" class="TableSubHead"><input type="checkbox" value="Y"
                                                               name="fields[eta_date_mm_dd_yyyy]"/></td>
                {/if}
                <td class="FormButton" nowrap="nowrap">ETA date (mm/dd/yyyy):</td>
                <td class="ProductDetails">
                    <input type="text" name="eta_date_mm_dd_yyyy" id="eta_date_mm_dd_yyyy" size="18"
                           value="{if $product.eta_date_mm_dd_yyyy} {$product.eta_date_mm_dd_yyyy|date_format:'%m/%d/%Y'} {/if}"
                           {if $manufacturer_feed_fields.eta_date_mm_dd_yyyy.disable eq "Y"}readonly="readonly"{/if} />
                    {if $manufacturer_feed_fields.eta_date_mm_dd_yyyy.disable eq "Y"}
            ...
            <tr>
                {if $geid ne ''}
                    <td width="15" class="TableSubHead"><input type="checkbox" value="Y" name="fields[pc_mc_operator]"/>
                    </td>
                {/if}
            ...
            <tr>
                {if $geid ne ''}
                    <td width="15" class="TableSubHead"><input type="checkbox" value="Y"
                                                               name="fields[pc_acc_operator]"/></td>
                {/if}
                ...
                <td class="ProductDetails">
                    <select name="pc_acc_operator" disabled="disabled">
                        ...
                        {section name=prov loop=$providers}
                            <option value="{$providers[prov].login}"
                                    {if $product.pc_acc_operator eq $providers[prov].login}selected="selected"{/if}>{$providers[prov].login}
                                ({if $providers[prov].title ne "" }{$providers[prov].title} {/if}{$providers[prov].lastname} {$providers[prov].firstname}
                                )
                            </option>
                        {/section}
            ...
            <tr>
                {if $geid ne ''}
                    <td width="15" class="TableSubHead"><input type="checkbox" value="Y"
                                                               name="fields[pc_classify_status]"/></td>
                {/if}
                <td class="FormButton" nowrap="nowrap">Product classify status:</td>
                <td class="ProductDetails">
                    <select name="pc_classify_status" disabled="disabled">
                        <option value="NC"{if $product.pc_classify_status eq 'NC' || $product.pc_classify_status eq ''} selected="selected"{/if}>
                            not classified
                        </option>
                        <option value="MC"{if $product.pc_classify_status eq 'MC'} selected="selected"{/if}>manually
                            classified
                        </option>
                        <option value="AC"{if $product.pc_classify_status eq 'AC'} selected="selected"{/if}>
                            automatically classified
                        </option>
                        <option value="ACC"{if $product.pc_classify_status eq 'ACC'} selected="selected"{/if}>
                            automatically classified and confirmed by operator
                        </option>
                    </select>
                </td>
            </tr>
            ...
            <tr>
                {if $geid ne ''}
                    <td width="15" class="TableSubHead">{* &nbsp; *}
                    <input type="checkbox" value="Y" name="fields[categoryid]"/>
                    </td>{/if}
                <td nowrap="nowrap" class="FormButton">{$lng.lbl_main_category_id}:</td>
                <td class="ProductDetails">
                    <input type="text" name="categoryid" id="categoryid_input" size="8"
                           value="{$product.categoryid|default:$default_categoryid}"/>
                    {if $top_message.fillerror ne "" and ($product.categoryid eq "" || $category_exists eq 'N')}
            ...
            <tr>
                {if $geid ne ''}
                    <td width="15"
                        class="TableSubHead">{* <input type="checkbox" value="Y" name="fields[categoryids]" /> *}
                    &nbsp;</td>{/if}
                <td {* class="FormButton" *} nowrap="nowrap">{$lng.lbl_additional_categories}:</td>
                <td class="ProductDetails">
                    <select name="categoryids_text[]" id="categoryids_select" style="width: 100%;" multiple="multiple"
                            size="8" onchange="javascript: updateCategoryIds();">
            ...
            <tr>
                {if $geid ne ''}
                    <td width="15" class="TableSubHead">{* &nbsp; *}
                    <input type="checkbox" value="Y" name="fields[categoryids]"/>
                    </td>{/if}
...
{/strip}" style="width: 100%;"/>
            ...
            {if $product.forsale eq 'H'}
                <tr>
                    {if $geid ne ''}
                        <td width="15" class="TableSubHead"><input type="checkbox" value="Y"
                                                                   name="fields[categoryids]"/></td>
                    {/if}
                    <td class="FormButton" nowrap="nowrap">{$lng.lbl_product_url}:</td>
                    <td class="ProductDetails">{$catalogs.customer}/product.php?productid={$product.productid}
                        &cat={$product.categoryid}</td>
                </tr>
            {/if}
            ...
            {if $membership_code ne ""}
                ...
                <input type="hidden" name="seo_meta_descr"
                       value="{$product.prevent_search_indexing_this_product_page}"/>
            {else}
                <tr>
                    {if $geid ne ''}
                        <td width="15" class="TableSubHead">&nbsp;</td>
                ...
                <tr>
                    {if $geid ne ''}
                        <td width="15" class="TableSubHead"><input type="checkbox" value="Y"
                                                                   name="fields[prevent_search_indexing_this_product_page]"/>
                        </td>
                    {/if}
                    <td class="FormButton" nowrap="nowrap">Prevent search indexing this product page:</td>
                    <td class="ProductDetails"><input type="checkbox" name="prevent_search_indexing_this_product_page"
                                                      id="prevent_search_indexing_this_product_page" value="Y"
                                                      {if $product.prevent_search_indexing_this_product_page eq "Y"}checked="checked"{/if} />
                    </td>
                </tr>
                <tr>
                    {if $geid ne ''}
            ...
            <tr>
                ...
                <td class="ProductDetails"><input type="text" name="upc" size="20" value="{$product.upc}"
                                                  class="InputWidth"
            ...
            <tr>
                ...
                <td class="ProductDetails">
                    <input type="text" name="product" id="product_name" size="45" class="InputWidth"
                           value="{$product.product|escape}" {* {if $config.SEO.clean_urls_enabled eq "Y"}onchange="javascript: if (this.form.clean_url.value == '') copy_clean_url(this, this.form.clean_url)"{/if} *} {if $manufacturer_feed_fields.product.disable eq "Y"}readonly="readonly"{/if} />
            ...
            <tr>
                {if $geid ne ''}
                    <td width="15" class="TableSubHead"><input type="checkbox" value="Y"
                                                               name="fields[google_search_term]"/></td>
                {/if}
                <td class="FormButton" nowrap="nowrap">{$lng.lbl_google_prod_search_term}:</td>
                <td class="ProductDetails">
                    <input type="text" name="google_search_term" class="InputWidth"
                           value="{$product.google_search_term|escape:"html"}"
                           {if $manufacturer_feed_fields.google_search_term.disable eq "Y"}readonly="readonly"{/if} />
                    {if $config.Product_Page.google_prod_link_pattern && $product.google_search_term}
                        &nbsp;
                        <a href="{$config.Product_Page.google_prod_link_pattern|substitute:searchterm:$product.google_search_link}"
                           target="_blank" title="">{$lng.lbl_google_prod_link}</a>
                    {/if}
                </td>
            </tr>
            ...
            <tr>
                {if $geid ne ''}
                    <td width="15" class="TableSubHead"><input type="checkbox" value="Y" name="fields[fulldescr]"/></td>
                {/if}
            ...
            <tr {if $usertype eq "P"}style="display: none;"{/if}>
                {if $geid ne ''}
                    <td width="15" class="TableSubHead"><input type="checkbox" value="Y" name="fields[descr]"/></td>
                {/if}
            ...
            <tr>
                ...
                <td class="ProductDetails">
                    <input type="text" name="supplier_internal_id" id="supplier_internal_id" size="20"
                           value="{$product.supplier_internal_id}"
                           {if $manufacturer_feed_fields.supplier_internal_id.disable eq "Y"}readonly="readonly"{/if} />
            ...
            *}
            ...
            <tr>
                {if $geid ne ''}
                    <td width="15" class="TableSubHead">{if $product.is_variants eq 'Y'}&nbsp;{else}
                    <input type="checkbox" value="Y" name="fields[avail]"/>
                {/if}</td>{/if}
                ...
                <td class="ProductDetails">
                    {if $product.is_variants eq 'Y'}
                        ...
                        {$lng.txt_pvariant_edit_note|substitute:"href":$variant_href}
                    {else}
                        <input type="text" name="r_avail" size="18"
                               value="{if $product.productid eq ""}{$product.r_avail|default:1000000}{else}{$product.r_avail}{/if}"
                               {if $manufacturer_feed_fields.r_avail.disable eq "Y"}readonly="readonly"{/if} />
                        {if $product.productid eq ""}<input type="hidden" name="avail" value="1000000"/>{/if}

                        {if $top_message.fillerror ne "" and $product.avail eq ""}
                            <font class="Star">&lt;&lt;</font>
                        {/if}
                    {/if}
                </td>
            ...
            <tr {if $usertype eq "P"}style="display: none;"{/if}>
                {if $geid ne ''}
                    <td width="15" class="TableSubHead"><input type="checkbox" value="Y"
                                                               name="fields[low_avail_limit]"/></td>
                {/if}
            ...
            <tr>
                {if $geid ne ''}
                    <td width="15" class="TableSubHead"><input type="checkbox" value="Y" name="fields[min_amount]"/>
                    </td>
                {/if}
                <td class="FormButton" nowrap="nowrap">{$lng.lbl_min_order_amount}</td>
                <td class="ProductDetails"><input type="text" name="min_amount" size="18"
                                                  value="{if $product.productid eq ""}1{else}{$product.min_amount}{/if}"
                                                  id="min_amount" onBlur="javascript: cidev_change_discount_table();"
                                                  onKeyUp="javascript: cidev_change_discount_table();"
                                                  {if $manufacturer_feed_fields.min_amount.disable eq "Y"}readonly="readonly"{/if} />
                </td>
            </tr>
            ...
            <tr>
                {if $geid ne ''}
                    <td width="15" class="TableSubHead"><input type="checkbox" value="Y"
                                                               name="fields[mult_order_quantity]"/></td>
                {/if}
                <td class="FormButton" nowrap="nowrap">{$lng.lbl_mult_order_quantity}</td>
                <td class="ProductDetails">

                    <input type="checkbox" name="mult_order_quantity"
                           value="Y"{if $product.mult_order_quantity eq "Y"} checked="checked"{/if} {if $manufacturer_feed_fields.mult_order_quantity.disable eq "Y"}disabled="disabled"{/if} />
            ...
            {if $active_modules.RMA ne ''}
                <tr>
                    {if $geid ne ''}
                        <td width="15" class="TableSubHead"><input type="checkbox" value="Y"
                                                                   name="fields[return_time]"/></td>
                    {/if}
                    <td class="FormButton" nowrap="nowrap">{$lng.lbl_return_time}</td>
                    <td class="ProductDetails"><input type="text" name="return_time" size="18"
                                                      value="{$product.return_time}"
                                                      {if $manufacturer_feed_fields.return_time.disable eq "Y"}readonly="readonly"{/if} />
                    </td>
                </tr>
            {/if}
            ...
            *}
            ...
            <tr>
                {if $geid ne ''}
                    <td width="15" class="TableSubHead"><input type="checkbox" value="Y" name="fields[free_tax]"/></td>
                {/if}
            ...
            {if $taxes}
                <tr>
                    {if $geid ne ''}
                        <td width="15" class="TableSubHead"><input type="checkbox" value="Y" name="fields[taxes]"/></td>
                    {/if}
                    <td class="FormButton" nowrap="nowrap">{$lng.lbl_apply_taxes}</td>
                    <td class="ProductDetails">
                        <select name="taxes[]"
                                multiple="multiple"{if $product.free_tax eq "Y" || $manufacturer_feed_fields.taxes.disable eq "Y"} disabled="disabled"{/if}>
                        ...
                        {if $usertype eq "P" or $active_modules.Simple_Mode ne ""}
                            <br/>
                            <ahref="{$catalogs.provider}/taxes.php" class="SmallNote"
...
{$lng.lbl_we_dont_ship_to_Canada_product_page}
            ...
            <tr>
                ...
                <td class="ProductDetails">
                    ...
                    <div>
                        <label class="lock_product_weight_label" for="lock_product_weight">
                            <input style="position: relative;top: 3px;" type="checkbox" id = "lock_product_weight" name="lock_product_weight" {if $product.weight_lock == "Y"}checked="checked"{/if} />
                ...
                <td class="ProductDetails"><input type="text" name="dimensiony" size="18"
                    ...
                    <div>
                        <label class="lock_dimension_product_label" for="lock_product_dimension">
                            <input style="position: relative;top: 3px;" type="checkbox" id = "lock_product_dimension" name="lock_product_dimension" {if $product.dim_lock == "Y"}checked="checked"{/if} />
                         Locked by Product Manager</label>
                    </div>
                </td>
            ...
            <tr>
                ...
                <td class="ProductDetails"><input type="text" name="dimensionshipy" size="18"
                    ...
                    <div class="tableborderright"></div>
                    <div>
                        <label class="lock_shipping_dimension_label" for="lock_shipping_dimension">
                            <input style="position: relative;top: 3px;" type="checkbox" id = "lock_shipping_dimension" name="lock_shipping_dimension" {if $product.shipping_dim_lock == "Y"}checked="checked"{/if} />
                            Locked by Product Manager</label>
                    </div>
                </td>
            ...
            <tr {if $usertype eq "P"}style="display: none;"{/if}>
                {if $geid ne ''}
                    <td width="15" class="TableSubHead"><input type="checkbox" value="Y"
                                                               name="fields[shipping_freight]"/></td>
                {/if}
                <td class="FormButton" nowrap="nowrap">{$lng.lbl_shipping_freight} ({$config.General.currency_symbol})
                </td>
                <td class="ProductDetails">
                    <input type="text" name="shipping_freight" size="18"
                           value="{$product.shipping_freight|formatprice|default:0.01}"
                           {if $manufacturer_feed_fields.shipping_freight.disable eq "Y"}readonly="readonly"{/if} />
                </td>
            </tr>

            <tr>
                {if $geid ne ''}
                    <td width="15" class="TableSubHead"><input type="checkbox" value="Y" name="fields[free_ship_zone]"/>
                    </td>
                {/if}
            ...
            <tr>
                {if $geid ne ''}
                    <td width="15" class="TableSubHead"><input type="checkbox" value="Y" name="fields[free_ship_text]"/>
                    </td>
                {/if}
                <td class="FormButton" nowrap="nowrap">{$lng.lbl_free_ship_text}</td>
                <td class="ProductDetails"><input type="text" name="free_ship_text" size="45" class="InputWidth"
                                                  value="{$product.free_ship_text}"
            ...
            {if $gcheckout_enabled}
                ...
                <tr {if $usertype eq "P"}style="display: none;"{/if}>
                    {if $geid ne ''}
                        <td width="15" class="TableSubHead"><input type="checkbox" value="Y"
                                                                   name="fields[valid_for_gcheckout]"/></td>
                    {/if}
                    <td class="FormButton" nowrap="nowrap">{$lng.lbl_gcheckout_product_valid}</td>
                    <td class="ProductDetails">
                        <input type="checkbox" name="valid_for_gcheckout"
                               value="Y"{if $product.productid eq "" || $product.valid_for_gcheckout eq "Y"} checked="checked"{/if} {if $manufacturer_feed_fields.valid_for_gcheckout.disable eq "Y"}disabled="disabled"{/if} />
            ...
            <tr {if $usertype eq "P"}style="display: none;"{/if}>
                {if $geid ne ''}
                    <td width="15" class="TableSubHead"><input type="checkbox" value="Y" name="fields[discount_slope]"/>
                    </td>
                {/if}
                <td class="FormButton" nowrap="nowrap">{$lng.lbl_discount_slope}:</td>
                <td class="ProductDetails"><input type="text" name="discount_slope" size="18"
                                                  value="{$product.discount_slope|formatprice|default:'0.60'}"
                                                  {if $manufacturer_feed_fields.discount_slope.disable eq "Y"}readonly="readonly"{/if} />
                </td>
            </tr>

            <tr {if $usertype eq "P"}style="display: none;"{/if}>
                {if $geid ne ''}
                    <td width="15" class="TableSubHead"><input type="checkbox" value="Y" name="fields[discount_table]"/>
                    </td>
                {/if}
                <td class="FormButton" nowrap="nowrap">{$lng.lbl_discount_table}:</td>
                <td class="ProductDetails"><input type="text" name="discount_table" id="discount_table" size="45"
                                                  class="InputWidth"
                                                  value="{if $smarty.get.mode_add_product eq 'y'}{$product.discount_table|escape|default:'2,3,4,6,8,12'}{else}{$product.discount_table|escape}{/if}"
                                                  {if $manufacturer_feed_fields.discount_table.disable eq "Y"}readonly="readonly"{/if} />
                </td>
            </tr>

            <tr style="display: none;">
                {if $geid ne ''}
                    <td width="15" class="TableSubHead"><input type="checkbox" value="Y" name="fields[discount_avail]"/>
                    </td>
                {/if}
                <td class="FormButton" nowrap="nowrap">{$lng.lbl_apply_global_discounts}</td>
                <td class="ProductDetails">
                    <input type="checkbox" name="discount_avail"
                           value="Y"{if $product.productid eq "" || $product.discount_avail eq "Y"} checked="checked"{/if} {if $manufacturer_feed_fields.discount_avail.disable eq "Y"}disabled="disabled"{/if} />
            ...
            <tr {if $usertype eq "P"}style="display: none;"{/if}>
                {if $geid ne ''}
                    <td width="15" class="TableSubHead"><input type="checkbox" value="Y"
                                                               name="fields[generate_similar_products]"/></td>
                {/if}
                <td class="FormButton" nowrap="nowrap">Generate similar products:</td>
                <td class="ProductDetails">
                    <input type="checkbox" name="generate_similar_products"
                           value="Y"{if $product.productid eq "" || $product.generate_similar_products eq "Y"} checked="checked"{/if} {if $manufacturer_feed_fields.generate_similar_products.disable eq "Y"}disabled="disabled"{/if} />
                    {if $manufacturer_feed_fields.generate_similar_products.disable eq "Y"}
                        <input type="hidden" name="generate_similar_products"
                               value="{$product.generate_similar_products}"/>
                    {/if}
                </td>
            </tr>
            {if  $product.clone_parent_productid > 0}
                <tr>
                    <td colspan="2"><p
                                style="border: 1px solid threedlightshadow; background-color: #f4cccc; padding:4px;">
                            This product is cloned from {$product.parent_product.productcode}</p></td>
                </tr>
            {/if}
...
<script type="text/javascript" language="JavaScript 1.2">
    $( document ).ready(function() {
        var inputProductDim = $('input[name=dimensionx],input[name=dimensiony],input[name=dimensionz]');
        var inputShipingDim = $('input[name=dimensionshipx],input[name=dimensionshipy],input[name=dimensionshipz]');
        var inputProductWeigth = $('input[name=weight]');
        var inputShipingWeigth = $('input[name=shippingweight]');
