...
<tr {$cycle_class} style="font-weight: bold;">
  <td align="center" width="5" style="font-weight: normal;">{if $static eq 'Y' || $static eq 'O'}{if $smarty.foreach.groups.first}<input type="checkbox" name="orderids[{$order.orderid}]" />{/if}{else}&nbsp;{/if}</td>
...
<tr {$cycle_class}>
  ...
  <td align="center">{$order.firstname}</td>
  <td align="center"></td>
  <td align="center">{$order.payment_method}</td>
  <td colspan="2" align="left">{$order.order_age_str}{* {$order.order_age_arr.days} days, {$order.order_age_arr.hours}:{$order.order_age_arr.mins} hours *}</td>
  <td>
  {if $order.attention_tags ne ""}
        {foreach from=$order.attention_tags item=v_tag key=k_tag}
                <span style="background-color: #F4CCCC; color: #000000;">{$attention_tags_values[$v_tag.status_id]}</span><br />
        {/foreach}
  {/if}
  </td>
  <td></td>
...
<tr {$cycle_class}>
  <td align="center" width="5" {if $v.submit_to_operator eq "through_distributor_website"}style="background: #fff2cc"{/if}>{*<a href="order.php?orderid={$order.orderid}" target="_blank">*}{$v.code}{*</a>*}</td>
  ...
  <td align="center"><a style="color: blue;" href="javascript: void(0);" onclick="javascript: window.open('orders.php?fast_search=Y&posted_data[s_zipcode]={$order.s_zipcode}&mode=');">{$order.s_zipcode}</a></td>
  <td align="center"></td>
