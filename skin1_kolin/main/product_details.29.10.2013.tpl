...
<tr>
	<td align="left" class="TopLabel">
        ...
        <span class="detail-title" style="font-weight: normal;"><a href="{$product.customer_url}" title="" target="_blank">{$lng.lbl_current_product}: {$product.product}</a></span>
        {else}
        {$lng.lbl_current_product}: "{$product.product}"
        {/if}
	</td>

	{if $product.d_website_search_for_sku_url ne ""}
	<td align="right">
		{if $product.forsale neq "N"}<span class="detail-title" style="font-weight: normal;">{/if}
		<a href="{$product.d_website_search_for_sku_url}" title="" target="_blank">{$product.mpn}</a>
