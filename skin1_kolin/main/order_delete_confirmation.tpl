{* $Id: order_delete_confirmation.tpl,v 1.10 2006/02/03 14:27:18 max Exp $ *}
...
<li><span class="ProductPriceSmall">{$lng.lbl_order} {$orders[oid].order_prefix}{$orders[oid].orderid} - {include file="currency.tpl" value=$orders[oid].total}</span>
<dl>
<dd>{$lng.lbl_date}: {$orders[oid].date|date_format:$config.Appearance.date_format}</dd>
<dd>{$lng.lbl_status}: {include file="main/order_status.tpl" status=$orders[oid].cb_status mode="static"}</dd>
{if $orders[oid].provider}
<dd>{$lng.lbl_provider}: {$orders[oid].provider}</dd>
{/if}
</dl>
</li>
{/section}
