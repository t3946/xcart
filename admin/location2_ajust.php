...
if(!empty($current_category) and is_array($current_category["category_location"])) {
	...
	foreach($category_location as $key=>$cat_loc)
		$category_location[$key][1] = str_replace("home.php","info_category_modify.php",$category_location[$key][1]);
}

if (is_array($category_location)) {
	array_unshift($category_location, array(func_get_langvar_by_name("lbl_root_level"), "info_pages.php"));
}
