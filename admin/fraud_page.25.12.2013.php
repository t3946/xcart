		...
		foreach ($posted_data as $k => $v){
			...
			if ($auto == "Y"){
				...
				if (function_exists($func_name)) {
					$fraud_score = $func_name($order_data);
				}
			...
			$fraud_score *= $importance_factor;

			$overall_fraud_score += $fraud_score;

			db_query("INSERT INTO $sql_tbl[order_fraud_checks] (orderid, question_code, manual_action, fraud_score) VALUES ('$orderid', '$question_code', '$manual_action', '$fraud_score')");
		}
...
$google_phone =  str_replace(" ", "+", $phone);

$userinfo_site_arr = explode("@", $userinfo["email"]);
$userinfo_site = "http://www.".$userinfo_site_arr[1];

$orders_full_names = $userinfo["s_firstname"]."<br />".$userinfo["b_firstname"];
...
if (!empty($fraud_checks) && is_array($fraud_checks)){
	foreach ($fraud_checks as $k => $v){
		...
		$replace_with = '<a target="_blank" href="'.$userinfo_site.'" style="color: #1F08F8;">Website</a>';
		...
		$fraud_checks[$k]["fraud_score"] = func_query_first_cell("SELECT fraud_score FROM $sql_tbl[order_fraud_checks] WHERE orderid='$orderid' AND question_code='$v[question_code]'");
	}
}
