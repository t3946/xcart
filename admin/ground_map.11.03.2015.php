...
if (!empty($mnfs) && is_array($mnfs)){
    foreach ($mnfs as $k => $v){
	  ...
	  $full_server_url = "http://www.ups.com/maps/results?zip=$zip";
		...
		$output =  preg_replace('/ {2,}/',' ',$output);
	        ...
	        if (!empty($result[1])) {
					...
					db_query("INSERT INTO $sql_tbl[ground_map] (zipcode, map_url) VALUES ('$zip', '$map_url') ON DUPLICATE KEY UPDATE map_url = '$map_url'");
					$mnfs[$k]["map_url"] = $map_url;
        	                }
                	}
	        }
