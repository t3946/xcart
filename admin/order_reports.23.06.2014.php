...
//    $x_average = intval(($X0+($RW/$count) + ($RW/$count)/2) * $DATA[1][0]);

//$DATA[1][0] = 2;

    $x_average = intval( $X0 + ($RW/$count)/2 + ($second_hour_line_x - $first_hour_line_x)*$DATA[1][0]  );

//func_print_r($x_average, $second_hour_line_x, $first_hour_line_x, $DATA[1][0]);
//func_print_r($x_average, $po, $y, $DATA[1][0]);

    $y=$Y0-($RH/$max*$i);
    imageline($im,$x_average,$Y0,$x_average,$y,$bar[1]);
    $po=$y;
