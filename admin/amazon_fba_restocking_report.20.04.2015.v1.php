...
//					print("Ncp = ".$Ncp[$Pi]." j ".$j."\n");
				...
				$Extended = price_format(ceil($p["Re_stock_quantity"])*$p["cost_to_us"]);
				$p["Extended"] = $Extended;
...
*/
	...
	if ($download == "1"){
		...
		$header_arr[6] = "Stocking Days";
		$header_arr[7] = "Restocking period";
		$header_arr[8] = "N average";
		$header_arr[9] = "Sigma";
		$header_arr[10] = "Re-stock quantity";
		$header_arr[11] = "Cost to us";
		$header_arr[12] = "Extended";

		$header_line = implode("\t", $header_arr);
		print($header_line."\r\n");

		if (!empty($fba_report) && is_array($fba_report)){
			foreach ($fba_report as $k => $v){
				$line_arr = array();
				...
				$line_arr[6] = $v["StockingDays"];
				$line_arr[7] = $search_data["amazon_fba_restocking_report"]["Amazon_FBA_Restocking_period_days"];
				$line_arr[8] = $v["N_avg"];
				$line_arr[9] = $v["sigma"];
				$line_arr[10] = $v["Re_stock_quantity"];
				$line_arr[11] = $v["cost_to_us"];
				$line_arr[12] = $v["Extended"];
