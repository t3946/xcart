...
// 			'ca_status' => '',
 			...
 			'fraud_status' => $order_table['fraud_status'],
...
elseif ($mode == "add_additional_tag" && isset($additional_tag_status)) {
	...
	if (!$allowed_to_set_flag){
                ...
                $top_message["type"] = "W";
                func_header_location("order.php?orderid=".$orderid."#main_order_tabs-order_progress");
	}
...
//	        $section_name = "order_entry_".$mnf_id;
	        $section_name = "main_order_tabs-standard_emails";
	        x_session_save("section_name");
...
if ($find_one_IO_status){
	...
	$request_missing_information_message = str_replace("{{fullname}}", $userinfo["firstname"], $request_missing_information_message);

	$missing_information_replace_text = "";

	if ($userinfo["phone"] == "(000) 000-0000" || $userinfo["email"] == "unknown@unknown.com"){
		...
		if ($userinfo["email"] == "unknown@unknown.com"){
			 $missing_information_replace_text .= "Email address is missing";
		}
	}

	if ($order["po_details"]["name_of_purchaser"] == "unknown" || $order["po_details"]["purchase_manager_phone"] == "(000) 000-0000" || $order["po_details"]["po_fax"] == "(000) 000-0000" || $order["po_details"]["purchase_manager_email"] == "unknown@unknown.com"){
		...
		if ($order["po_details"]["name_of_purchaser"] == "unknown"){
		...
		if ($order["po_details"]["purchase_manager_email"] == "unknown@unknown.com"){
			$missing_information_replace_text .= "Email is missing<br />";
		}
	}

	 if ($order["po_details"]["accounts_payable_full_name"] == "unknown" || $order["po_details"]["accounts_payable_phone"] == "(000) 000-0000" || $order["po_details"]["accounts_payable_fax"] == "(000) 000-0000" || $order["po_details"]["accounts_payable_email"] == "unknown@unknown.com"){
                if (!empty($missing_information_replace_text)){
                        $missing_information_replace_text .= "<br />";
                }

		$missing_information_replace_text .= "<B>Accounts payable</B><br />";
                if ($order["po_details"]["accounts_payable_full_name"] == "unknown"){
                        $missing_information_replace_text .= "Full name is missing<br />";
                }
                ...
                if ($order["po_details"]["accounts_payable_email"] == "unknown@unknown.com"){
                        $missing_information_replace_text .= "Email is missing<br />";
                }
	}
...
$main_order_tabs[$tabs_key]["title"] = "Order details";
...
$main_order_tabs[$tabs_key]["title"] = "Standard emails";
$main_order_tabs[$tabs_key]["section"] = "standard_emails";
$main_order_tabs[$tabs_key]["anchor"] = $main_order_tabs[$tabs_key]["section"];
$tabs_key++;
...
if (!empty($attention_tags_values) && is_array($attention_tags_values)){
        foreach ($attention_tags_values as $k => $v){
                $attention_tags_values[$k]["operators"] = func_query("SELECT * FROM $sql_tbl[attention_tags_values_logins] WHERE status_id='$v[id]'");
        }
}
