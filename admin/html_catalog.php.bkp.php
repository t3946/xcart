...
# $Id: html_catalog.php,v 1.82.2.30 2007/06/18 05:42:34 max Exp $
...
$max_name_length = 64;
$php_scripts = array("search.php","giftcert.php","help.php", "cart.php", "product.php","register.php", "home.php", "pconf.php", "giftregs.php", "manufacturers.php","news.php","orders.php", "giftreg_manage.php","returns.php","survey.php","antibot_image.php","image.php");
...
$smarty->assign("name_styles", $name_styles);
...
function func_generate_catalog_arr($cat_arr = array()) {
	...
	foreach ($all_languages as $language) {
		if (substr(func_normalize_path($xcart_dir."/".$cat_arr[$language['code']]), 0, strlen($xcart_dir)) == $xcart_dir) {
			$cat_url = $http_location.$cat_arr[$language['code']]."/index.html";

		} else {
			$cat_url = $http_location.(substr($cat_arr[$language['code']], 0, 1) == '/' ? "" : "/").$cat_arr[$language['code']]."/index.html";
...
function category_filename($cat, $cat_name, $page = 1, $sort_field, $sort_direction){
	...
	if (empty($cat_name)) $cat_name = 'index';
...
function setNewLng() {
	...
	setCookie('store_language', '$current_lng', '/', d);
	setCookie('store_language', '$current_lng', '$xcart_web_dir/', d);
}
...
JS;
	...
	$page_src = preg_replace_callback("/(<a[^<>]+href[\s]*=[\s]*[\"']+)([^\"':]+)([\"'])/", "add_html_location", $page_src);
...
");
		...
		while ($page_data = db_fetch_array($pages_data)) {
			...
			if (empty($sorts)) {
				$sorts = array("orderby","productcode","title","price");
				$sorts[] = $config['Appearance']['products_order'];
				...
				if (!isset($hc_state["cat_done"]) || !@$hc_state["cat_done"]) {
					...
					for ($i = $first; $i <= $pages; $i++) {
						...
						foreach ($hc_state["last_sorts_defined"] as $sf) {
							...
							if (!isset($hc_state["last_sorts"][$sf]) || !in_array(1, $hc_state["last_sorts"][$sf])) {
								...
								convert_page('', $page_src, 'category', array($category_data["categoryid"], $category_data["category"], $i, $sf, 1));
							}

							if ($config['Appearance']['products_order'] == $sf)
								continue;

							if (!isset($hc_state["last_sorts"][$sf]) || !in_array(0, $hc_state["last_sorts"][$sf])) {
								...
								convert_page('', $page_src, 'category', array($category_data["categoryid"], $category_data["category"], $i, $sf, 0));
							}
						}
