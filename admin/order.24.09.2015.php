...
//                        $top_message["content"] = 'Order not saved! Order is locked by another operator.';
...
/*
							...
							$total_weight += $v_p["weight"] * $v_p["amount"];

							$total_shipping_freight += $v_p["shipping_freight"] * $v_p["amount"];
*/
...
/*
		$old_config = $config;
	        ...
	        $config['Company']['location_city'] = $order["shipping_groups"][$mid]["all_distributor_info"]['m_city'];
        	$config['Company']['location_state'] = $order["shipping_groups"][$mid]["all_distributor_info"]['m_state'];
	        $config['Company']['location_country'] = $order["shipping_groups"][$mid]["all_distributor_info"]['m_country'];
        	$config['Company']['location_zipcode'] = $order["shipping_groups"][$mid]["all_distributor_info"]['m_zipcode'];
		...
		$intershipper_rates = func_shipper($weight,$userinfo,"N");
*/
...
/*
		if (!empty($intershipper_rates) && is_array($intershipper_rates)){
			foreach ($intershipper_rates as $k => $v){
				$intershipper_rates[$k]["shipping"] = func_query_first_cell("SELECT shipping FROM $sql_tbl[shipping] WHERE shippingid='$v[methodid]'");
				$rate = $intershipper_rates[$k]["rate"] + $total_shipping_freight;
				$intershipper_rates[$k]["orig_rate"] = $intershipper_rates[$k]["rate"] = price_format($rate);
			}
		}
...
//		$show_intershipper_rates[$mid] = $intershipper_rates;
...
//func_print_r($all_distributor_links);
...
//func_print_r($all_distributor_memo_links);
...
/* Moved to auth.php for new customization and feature behavior too
#
...
##
###
$checks_deposited_order = func_query("SELECT * FROM $sql_tbl[checks_deposited_orders] WHERE orderid='$orderid'");
...
if (!empty($checks_deposited_order)){
	...
	foreach ($checks_deposited_order as $k => $v){
		$total_deposit_amount += $v["amount"];
	}
	$total_deposit_amount = price_format($total_deposit_amount);
	$smarty->assign("total_deposit_amount", $total_deposit_amount);

	$checks_deposited = func_query_first("SELECT * FROM $sql_tbl[checks_deposited] WHERE checks_deposited_id='$v[checks_deposited_id]'");
