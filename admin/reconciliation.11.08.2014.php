...
/*
if ($r_id == "2031"){
func_print_r($bd_status_not_invoiced, $bd_status_invoiced, $amount_csv_abs, $found_orders_for_several_orders_at_once_total_amount);
}
*/
...
//		                                                if (strpos($description_csv, $v["d_search_keyphrase_for_reconciliation"]) !== false){
		                                                if (strpos($description_csv, $vv_s_r) !== false){
        		                                                $manufacturerid = $k;
									break;
                	                                        }
		...
		if (!empty($check_reconciliations_for_mid)){
			foreach ($check_reconciliations_for_mid as $k_c => $v_c){
				    ...
				    if (!empty($v_c["description_csv"]) && !empty($v["d_search_keyphrase_for_reconciliation"])){
						...
						$vv_s_r = trim($vv_s_r);
	        		                if (strpos($v_c["description_csv"], $vv_s_r) !== false){
        	        		                $manufacturerid = $k;
							break;
                        	        	}
					}
...
if (!empty($reconciliations) && is_array($reconciliations)){
	foreach ($reconciliations as $k => $v){
                ...
                foreach ($manufacturerid_info as $kk => $vv){
					...
					$vv_s_r = trim($vv_s_r);
		      	                if (strpos($v["description_csv"], $vv_s_r) !== false){
	        	                        $manufacturerid = $kk;
						$reconciliations[$k]["description_csv"] = str_replace($vv_s_r, "<B>".$vv_s_r."</B>", $reconciliations[$k]["description_csv"]);
//						break;
					}
				}
			}
...
/*
								if (!isset($expense_report_sum_total_amount_with_abs))
									$expense_report_sum_total_amount_with_abs = 0;

								$expense_report_sum_total_amount_with_abs += $config_reconciliation_search_keyphrases[$kk]["total_amount_with_abs"];

func_print_r($expense_report_sum_total_amount_with_abs, $config_reconciliation_search_keyphrases[$kk]["total_amount_with_abs"]);
*/
							}
