...
//		$message_body .= func_eol2br(stripslashes($mnf_body));
		...
		$message_body .= "Login/username: $d_login <br />\r\n";
		$message_body .= "Password: $d_password <br />\r\n";
...
if (!empty($active_modules["Manufacturers"])) {
	if (!empty($order['shipping_groups'])) {
		...
		if (!empty($mnfs)) {
//            $mnfs = func_query_hash('SELECT manufacturerid, manufacturer, mess_body, email, submit_to_operator, d_order_entry_operator_email, d_instructions_to_order_entry_operator'
...
# END: random:20341 [2010 Jul 29 14:46] 
				...
				if (!empty($products) && is_array($products)){
					...
					$total_product_cost_to_us = 0;
				        ...
				        $cidev_items_table = "";
				        $cidev_items_table .= '<table width="250px" border="1" cellpadding="5" cellspacing="0" bordercolor="#414236" style="font-family: Arial, Helvetica, sans-serif; font-size: 14px; color: #000000; line-height: 18px;">';
				        $cidev_items_table .= '<tr><td width="150px" style="text-align: left; font-weight: bold;">Item number</td><td style="text-align: right; font-weight: bold;">Quantity</td></tr>';

				        foreach ($products as $k => $v){
						if ($v["manufacturerid"] == $m_id){
					                $tmp_sku = substr($v["productcode"], 4);
					                $cidev_items_table .= '<tr><td width="150px" style="text-align: left;">'.$tmp_sku.'</td><td style="text-align: right;">'.$v["amount"].'</td></tr>';
						}
...
### !!! ###
                                $webpagebutton = "<a href='$cidev_page_url1?$cidev_url_variables'><img src='http://www.artistsupplysource.com/skin1_kolin/images/webpage_button.png' alt='Click here to provide information for us' /></a>";
				...
				$mnfs[$m_id]["estimated_profit"] = price_format($estimated_profit);
				...
				$mnfs[$m_id]["estimated_profit_margin"] = price_format($estimated_profit_margin);

				$estimated_profit_margin_percent = $estimated_profit_margin*100;
				$mnfs[$m_id]["estimated_profit_margin_percent"] = price_format($estimated_profit_margin_percent);
				if ($estimated_profit_margin_percent < 0){
					$mnfs[$m_id]["estimated_profit_margin_percent_abs"] = abs($estimated_profit_margin_percent);
				}
/*
				if ($estimated_profit_margin < 0){
					$estimated_profit_margin_percent_txt = "(".abs($estimated_profit_margin_percent)."%)";
				} else {
					$estimated_profit_margin_percent_txt = $estimated_profit_margin_percent."%";
				}
*/
				$mnfs[$m_id]["estimated_profit_margin_percent_txt"] = $estimated_profit_margin_percent_txt;


//func_print_r($mnfs[$m_id], $order);
//die();
...
###
				...
				$mnfs[$m_id]["required_shipping_charge"] = price_format($required_shipping_charge);

				$additional_shipping_charge =  $required_shipping_charge - $order['shipping_groups'][$m_id]["shipping_cost"]["gross"]; 
				$mnfs[$m_id]["additional_shipping_charge"] = price_format($additional_shipping_charge);
				...
				$mnfs[$m_id]["estimated_profit_after_additional_payment"] = price_format($estimated_profit_after_additional_payment);
				...
				$mnfs[$m_id]["estimated_profit_margin_after_additional_payment"] = price_format($estimated_profit_margin_after_additional_payment);
				...
				$mnfs[$m_id]["estimated_profit_margin_after_additional_payment_percent"] = price_format($estimated_profit_margin_after_additional_payment_percent);
				if ($estimated_profit_margin_after_additional_payment < 0){
...
/*
                                if ($estimated_profit_margin_after_additional_payment < 0){
                                        $estimated_profit_margin_after_additional_payment_percent_txt = "(".abs($estimated_profit_margin_after_additional_payment_percent)."%)";
                                } else {
                                        $estimated_profit_margin_after_additional_payment_percent_txt = $estimated_profit_margin_after_additional_payment_percent."%";
                                }
                                $mnfs[$m_id]["estimated_profit_margin_after_additional_payment_percent_txt"] = $estimated_profit_margin_after_additional_payment_percent_txt;
*/
