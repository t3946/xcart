			...
			if ($line_number == "1"){
				...
				if (!$productcode_field_found){
					$err_message .= "!PRODUCTCODE field is mandatory. <br />";
                                ...
                                if ($avail_field_found){
                                        $err_message .= "!AVAIL field is found. You need to use r_avail. <br />";
                                }
			...
			if (!empty($row) && is_array($row)){
				...
				if (!empty($productid)){
					...
					if (!empty($field_and_value)){
						...
						$current_product_info = func_query_first("SELECT * FROM $sql_tbl[products] WHERE productid='$productid'");

        	                                if (
                	                                $current_product_info["update_search_index"] == "N" &&
                        	                        (
                                	                 ($new_product_info["product"] != $current_product_info["product"] && isset($new_product_info["product"])) ||
                                        	         ($new_product_info["productcode"] != $current_product_info["productcode"] && isset($new_product_info["productcode"])) ||
                                                	 ($new_product_info["brandid"] != $current_product_info["brandid"] && isset($new_product_info["brandid"])) ||
	                                                 ($new_product_info["fulldescr"] != $current_product_info["fulldescr"] && isset($new_product_info["fulldescr"])) ||                                           
        	                                         ($new_product_info["upc"] != $current_product_info["upc"] && isset($new_product_info["upc"]))
                	                                )
                        	                ){
							$field_and_value .= ", update_search_index='Y'";
	                                        }

                                        	if (isset($new_product_info["forsale"]) && $new_product_info["forsale"] == "N" && ($current_product_info["update_search_index"] == "N" || $new_product_info["update_search_index"] == "N")){
							$field_and_value .= ", update_search_index='D'";
	                                        }
