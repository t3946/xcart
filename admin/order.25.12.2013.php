...
if ($mode == "submit_message" && !empty($notes) && !empty($orderid)) {
	...
	$section_name = "order_logs";
        x_session_save("section_name");
	$log = "'Post message' at 'Important messages'";
	...
	$notes_arr = explode("\n", $notes);
	$subj = $order_prefix.$orderid." ".$notes_arr[0];
	$body = $notes."\n".$cidev_firstname." (".$login.")";

        func_send_simple_mail("custserv@s3stores.com", $subj, $body, "custserv@s3stores.com");
