...
$Telephone_area_codes = func_query_first_param(/** @lang MySQL */
    "SELECT * FROM xcart_Telephone_area_codes WHERE area_code=:userinfo_area_code", ['userinfo_area_code' => $userinfo_area_code]);
...
$cidev_order_details_TransID = func_query_first_cell_param(/** @lang MySQL */
    "SELECT transaction_id FROM xcart_transaction_logs WHERE transaction_id!='' AND orderid=:orderid", ['orderid' => $orderid]);
...
$fraud_checks = func_query_param(/** @lang MySQL */
    "SELECT id, question_code, question_template_body, importance_factor, auto FROM xcart_fraud_check ORDER BY orderby", []);
...
if (!empty($fraud_checks) && is_array($fraud_checks)) {
    foreach ($fraud_checks as $k => $v) {
        ...
        if (floatval($fraud_score) == 0 && $v["auto"] == "Y") {
            ...
            if (in_array($v["question_code"], [
                ...
                if ($oPaymentMethod && strpos('PayPal', $oPaymentMethod->payment_method) === false ) {
                    $fraud_checks[$k]["manual_action"] = "N";
                    $fraud_result = "negative";
                    $bare_fraud_score = $importance_factor_arr[0];
                    $fraud_score = $bare_fraud_score;
