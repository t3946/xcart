...
if ($find_one_DC_Pending_availability_check_OR_Pending_order_entry && $find_back){
	...
	$cidev_outofstock_items_table .= '<tr><td width="150px" style="text-align: left; font-weight: bold;">Item number</td><td width="250px" style="text-align: left; font-weight: bold;">Item name</td><td style="text-align: right; font-weight: bold;" nowrap="nowrap">Quantity out of stock</td></tr>';
...
foreach($department_arr as $department => $department_name){
	...
	if (!empty($department_info) && is_array($department_info)){
		foreach ($department_info as $k => $v){
			...
			$v["send_to_emai"] = urlencode($v["send_to_emai"]);
