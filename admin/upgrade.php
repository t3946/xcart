...
#
# $Id: upgrade.php,v 1.23 2006/03/29 13:39:03 mclap Exp $
#
...
# Prepare patch stage
#
if (is_file("$upgrade_repository/$patch_filename/patch_pre.php") ) {
	$prepatch_errorcode = 0;
	echo func_get_langvar_by_name("lbl_applying_pre_patch",false,false,true); flush();
	@include "$upgrade_repository/$patch_filename/patch_pre.php";
	if ($prepatch_errorcode == 1) {
		$patch_result[] = func_get_langvar_by_name("lbl_pre_patch_was_applied_successfuly");
...
if ($dir = @opendir($upgrade_repository."/".$target_version)) {
	#
	# Apply .sql patches
	#
...
if ($patch_errorcode == 1 && $sql_errorcode == 1) {
	if (is_file("$upgrade_repository/$patch_filename/patch_post.php")) {
		$postpatch_errorcode = 0;
		echo func_get_langvar_by_name("lbl_applying_after_patch",false,false,true); flush();
		@include "$upgrade_repository/$patch_filename/patch_post.php";

		if ($postpatch_errorcode == 1) {
			$patch_result[] = func_get_langvar_by_name("lbl_after_patch_was_applied_successfuly",false,false,true);
