...
//			db_query("INSERT INTO $sql_tbl[templates_for_communication] (template_name, pos, subject_line, send_to_email, message_body, department, active, ca_status) VALUES ('$v[template_name]', '$v[pos]', '$v[subject_line]', '$v[send_to_email]', '$v[message_body]', '$department', '$v[active]', '$v[ca_status]')");
			db_query("INSERT INTO $sql_tbl[templates_for_communication] (template_name, pos, subject_line, send_to_email, message_body, department, active, status_id) VALUES ('$v[template_name]', '$v[pos]', '$v[subject_line]', '$v[send_to_email]', '$v[message_body]', '$department', '$v[active]', '$v[status_id]')");
		}
	}
...
$ca_statuses = func_query("SELECT * FROM $sql_tbl[attention_tags_values] ORDER BY orderby");
...
//		$str_ca_statuses_options .= '<option value="'.$v["code"].'">'.$v["name"].'</option>';
		$str_ca_statuses_options .= '<option value="'.$v["status_id"].'">'.$v["status"].'</option>';
	}
