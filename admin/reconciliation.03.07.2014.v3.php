			...
			$amount_csv_abs = abs($amount_csv);
				...
				$bd_status_invoiced = 0;
						...
						$found_orders_for_several_orders_at_once_total_amount += $price_to_search;
	                        ...
	                        $action='';
        	                if (count($found_orders_for_several_orders_at_once) == $bd_status_invoiced && $bd_status_invoiced > 0 && $part_of_total_transaction_in_amount_of == $found_orders_for_several_orders_at_once_total_amount){
                	                $action='R';
                        	}
...
//func_print_r($orderids, $manufacturerid);
//die();
						...
						if ($current_bd_status != "Y"){
                	                                ...
                	                                $log = "(Reconciliation) <B>".$manufacturerid_info[$manufacturerid]["code"].":</B><br />";
                        	                        $log .= "REF TO US status: ".$current_bd_status_name." -> ".$new_bd_status_name."<br />";
                                	                func_log_order($vv["orderid"], 'X', $log, $login);
