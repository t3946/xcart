	...
	if ($mode == 'Update_Reconciliation'){
        	if (!empty($Reconciliations) && is_array($Reconciliations)){
                	foreach ($Reconciliations as $k => $v){
                        	db_query("UPDATE $sql_tbl[reconciliation_search_keyphrases] SET search_keyphrase='$v[search_keyphrase]' WHERE id='$k'");
	                }
        	}
...
$Reconciliations = func_query("SELECT * FROM $sql_tbl[reconciliation_search_keyphrases] ORDER BY id");
