...
*/
						...
						&& empty($is_such_orderid_in_reconciliation_orderid_table)
					      ...
					      ( // https://basecamp.com/2070980/projects/1577907/messages/44550708
                                                $amount_csv < 0 && price_format($vv["part_of_total_transaction_in_amount_of"] == "0.00") 
                                                && $price_to_search == $amount_csv_abs
                                                && $sum_total == $amount_csv_abs
                                                && !in_array($vv["orderid"], $found_orders_for_several_orders_at_once)
                                                && empty($current_reconciliation_id)
...
*/
                                        ...
                                        $is_such_orderid_in_reconciliation_orderid_table = func_query_first_cell("SELECT COUNT(*) FROM $sql_tbl[reconciliation_orderid] WHERE ref_reconciliation_id='$r_id' AND orderid='$vv[orderid]'");

                                        if (
                                                !empty($is_such_orderid_in_reconciliation_orderid_table) &&
//                                                $vv["part_of_total_transaction_in_amount_of"] != $vv["ref_to_us_part_of_transaction"] &&
                                                $vv["part_of_total_transaction_in_amount_of"] == $amount_csv_abs
                                        ){
                                                $is_such_orderid_in_reconciliation_orderid_table = 0;
                                        }

                                        if (
                                                $amount_csv < 0 && price_format($amount_csv_abs) == price_format($vv["ref_to_us_part_of_transaction"])
                                                && $price_to_search <= $amount_csv_abs
//                                                && !in_array($vv["orderid"], $found_orders_for_several_orders_at_once)
...
//                                                $found_orders_for_several_orders_at_once[] = $vv["orderid"];
                                                ...
                                                db_query("INSERT INTO $sql_tbl[reconciliation_orderid] (ref_reconciliation_id, orderid, time_to_db) VALUES ('$r_id', '$vv[orderid]', '$time_to_db')");
                                        }
#####
...
#
##
                                $orderids = func_query("SELECT orderid FROM $sql_tbl[reconciliation_orderid] WHERE ref_reconciliation_id='$k'");
...
//                                                        func_log_order($vv["orderid"], 'X', $log, $login);
                                                        ...
                                                        db_query("UPDATE $sql_tbl[order_groups] SET ref_reconciliation_id='$k', bd_status='Y' WHERE orderid='$vv[orderid]' AND manufacturerid='$manufacturerid'");
//                                                }
                                        }
                                }
...
//						break;
		...
		if ($tab == "unreconciled" || $tab == "reconciled"){
			...
			$orderids = func_query("SELECT orderid, orders_variant, ref_to_us FROM $sql_tbl[reconciliation_orderid] WHERE reconciliation_id='$v[id]' OR ref_reconciliation_id='$v[id]' ORDER BY orders_variant, orderid");
			if (!empty($orderids)){
