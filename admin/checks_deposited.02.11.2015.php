...
*/
						...
						$orders_marked_as_paid[] = $add_orderid;

						$order_data = func_order_data($add_orderid);
						...
						if (!empty($order["shipping_groups"]) && is_array($order["shipping_groups"])){
							foreach ($order["shipping_groups"] as $m_id => $group){
								...
								func_log_order_groups($update, $add_orderid, $m_id, 'X', $login);
								func_array2update("order_groups", $update ,"orderid='$add_orderid' AND manufacturerid='$m_id'");
								unset($update);
							}
						}
...
//func_print_r($add_orderid);
							...
							$orders_NOT_marked_as_paid[] = $add_orderid;
						}
	                                }
...
//			}
	...
	if (!empty($orders_marked_as_paid)){
		...
		foreach ($orders_marked_as_paid as $orderid){
			$top_message["content"] .= "<br />Order # $orderid has been marked as 'Paid'.";
		}
	}
	...
	if (!empty($orders_NOT_marked_as_paid)){
		...
		foreach ($orders_NOT_marked_as_paid as $orderid){
			$top_message["content"] .= "<br />Order # $orderid has NOT been marked as 'Paid'. No action needs to be taken. Our AR specialist will look into this situation.";
		}
	}
