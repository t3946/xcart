					...
					if (is_array($test_skus)) {
						foreach ($test_skus as $pid=>$sku) {
							if (!empty($active_modules['Multiple_Storefronts'])) {
								...
								$cidev_check_product_sfid = func_query_first_cell("SELECT sfid FROM $sql_tbl[products_sf] WHERE productid='$pid'");

								if ($cidev_check_product_sfid == $current_storefront){
									...
									$top_message["content"] = "Not added. Check SKU and storefront.";
								}
							} else {
