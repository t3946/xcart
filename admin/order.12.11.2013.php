		...
		foreach ($groups as $k => $v){
			...
			if (!empty($tracknums[$k]) && is_array($tracknums[$k])){
				...
				foreach ($tracknums[$k] as $kk => $vv){
					if (!empty($vv)){
						$tracknums_to_db[$tracknums_to_db_index]["linkid"] = $kk;
						$tracknums_to_db[$tracknums_to_db_index]["tracknum"] = $vv;
...
elseif ($mode == "accounting_apply") {
	...
	if (!empty($links_to_distributor_invoices) && is_array($links_to_distributor_invoices)){
		...
		foreach ($links_to_distributor_invoices as $k => $v){
			if (!empty($v["link_to_distributor_invoice"])){
				db_query("INSERT INTO $sql_tbl[links_to_distributor_invoices] (orderid, link_to_distributor_invoice) VALUES ('$orderid', '".addslashes($v["link_to_distributor_invoice"])."')");
			}
		}
	}
}
...
*/
	...
	$cidev_outofstock_items_table .= '<tr><td width="150px" style="text-align: left; font-weight: bold;">Item number</td><td width="250px" style="text-align: left; font-weight: bold;">Item name</td><td style="text-align: right; font-weight: bold;" nowrap="nowrap">Quantity out of stock</td><td style="text-align: right; font-weight: bold;" nowrap="nowrap">ETA date</td></tr>';
	...
	foreach ($products as $k => $v){
		...
		if ($v["back"] > 0){
			$is_back = "Y";
			$cidev_outofstock_items_table .= '<tr><td width="150px" style="text-align: left;">'.$tmp_sku.'</td><td width="250px" style="text-align: left;"><a href="'.$v["links"]["customer"].'">'.$v["product"].'</a></td><td style="text-align: right;">'.$v["back"].'</td><td style="text-align: right;">'.$v["eta_date_mm_dd_yyyy"].'</td></tr>';
		}
	}
...
##
###
$distributor_links = func_query("SELECT * FROM $sql_tbl[links_to_distributor_invoices] WHERE orderid='$orderid'");
if (!empty($distributor_links)) {
        $smarty->assign('distributor_links', $distributor_links);
        $smarty->assign('count_links_to_distributor_invoices', count($count_links_to_distributor_invoices));
}
###
##
#
...
//func_print_r($all__items_table__, $all__shipto_table__);
...
foreach($department_arr as $department => $department_name){
	...
	unset($department_info_sorted);

	if (!empty($department_info) && is_array($department_info)){
...
###
			...
			$message_body = $v["message_body"];
        		...
        		$subject_line = str_replace("{{orderid}}", $order["order_prefix"].$orderid, $subject_line);
        		$subject_line = str_replace("{{c-fullname}}", $userinfo["firstname"], $subject_line);

                        $message_body = str_replace("{{orderid}}", $order["order_prefix"].$orderid, $message_body);
                        $message_body = str_replace("{{c-fullname}}", $userinfo["firstname"], $message_body);

			if ($v["department"] == "our_customer_service" || $v["department"] == "customer"){


//func_print_r("message_body: start:", $message_body);

	                        $message_body = str_replace("{{items}}", $all__items_table__, $message_body);
//func_print_r($message_body);
//func_print_r("--end");
        	                $subject_line = str_replace("{{shipto}}", $all__shipto_table__, $subject_line);
			}

                        $department_info[$k]["subject_line"] = $subject_line;
                        $department_info[$k]["message_body"] = $message_body;
                }
###
##
#
                foreach ($department_info as $k => $v){
                        $v["subject_line"] = urlencode($v["subject_line"]);
                        $v["message_body"] = urlencode($v["message_body"]);
                        $v["send_to_email"] = urlencode($v["send_to_email"]);
                        $department_info_sorted[$v["id"]] = $v;
                }


//func_print_r($department_info_sorted);

	}

        $department_full_arr[$department] = $department_info_sorted;
}
