...
# END: random:20341 [2010 Jul 29 14:46] 
				...
				if (!empty($products) && is_array($products)){
					...
					$total_product_cost_to_us = 0;
				        ...
				        $cidev_items_table = "";
				        $cidev_items_table .= "<table border='0'>\r\n";
				        $cidev_items_table .= "<tr><th>Item number </th><th> Quantity</th></tr>\r\n";

				        foreach ($products as $k => $v){
						if ($v["manufacturerid"] == $m_id){
					                $tmp_sku = substr($v["productcode"], 4);
					                $cidev_items_table .= "<tr><td>".$tmp_sku."</td><td align='right'>".$v["amount"]."</td></tr>\r\n";
						}
				...
				$cidev_mess_body = "<a href='http://www.s3stores.com/index.php?pageid=42&s=$secure_check&o=$orderid&m=$m_id'><img src='http://www.artistsupplysource.com/skin1_kolin/images/received_img.png' alt='Please click to confirm that you received this order'/></a>";
				...
				if ($additional_shipping_charge > 0){
					...
					$additional_shipping_charge_message = str_replace("{{required}}", "$".$required_shipping_charge, $additional_shipping_charge_message);
					$additional_shipping_charge_message = str_replace("{{additional}}", "$".$additional_shipping_charge, $additional_shipping_charge_message);
