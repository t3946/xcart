		...
		foreach ($posted_data as $k => $v){
			...
			if ($auto == "Y"){
				...
				if (function_exists($func_name)) {
					$bare_fraud_score = $func_name($order_data);
				}
			...
			$fraud_score = $importance_factor * $bare_fraud_score;

			$overall_fraud_score += $fraud_score;

			db_query("INSERT INTO $sql_tbl[order_fraud_checks] (orderid, question_code, manual_action, fraud_score, bare_fraud_score) VALUES ('$orderid', '$question_code', '$manual_action', '$fraud_score', '$bare_fraud_score')");
		}
	...
	foreach ($fraud_checks as $k => $v){
		...
		if ($fraud_score == "" && $v["auto"] == "Y"){
			...
			if (function_exists($func_name)) {
				...
				$fraud_score = $bare_fraud_score*$v["importance_factor"];
			}
