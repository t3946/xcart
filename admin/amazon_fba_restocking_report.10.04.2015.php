...
//func_print_r($cur_time, $START_TIME_Amazon_FBA_Report_depth_months, $total_days_in_depth, $count_periods);
	...
	$fba_report = array();
		...
		$p["order_info"] = func_query("Select P.productid, OD.productid, O.orderid, FROM_UNIXTIME(O.date), OD.amount, 
                (SELECT count(F.id) 
                 FROM xcart_cidev_amazon_fba_products F
...
Where P.productid = '$p[productid]'");
		...
		if (!empty($p["order_info"])){

	                $p["total_amount"] = 0;
        	        $StockingDays = 0;

			foreach ($p["order_info"] as $k => $v){
				$p["total_amount"] += $v["amount"];
				$v["date_time"] = strtotime($v["FROM_UNIXTIME(O.date)"]);
				$p["order_info"][$k]["date_time"] = $v["date_time"];
				$StockingDays = $v["StockingDays"];
			}
...
//die();
	...
	if (!empty($fba_report) && is_array($fba_report)){
		...
		foreach ($fba_report as $productid => $p){
			...
			if (!empty($p["order_info"]) && is_array($p["order_info"])){
				...
				$days_tmp = $p["StockingDays"]/$Amazon_FBA_Restocking_period_days;
...
//func_print_r($sp_start_date, $sp_end_date);
//print"--";
